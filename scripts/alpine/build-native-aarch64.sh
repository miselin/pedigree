...
# Build the Alpine aarch64 image on Apple silicon without a Linux container.
set -euo pipefail

script_dir=$(cd -P -- "$(dirname -- "$0")" && pwd -P)
output_dir=${1:-"$script_dir/build/aarch64"}
release=3.22.1
...
work_dir=$(mktemp -d "$output_dir/.native-aarch64.XXXXXX")
...
curl -fLsS --retry 3 -o "$work_dir/minirootfs.tar.gz" \
    "$repository/releases/aarch64/alpine-minirootfs-$release-aarch64.tar.gz"
curl -fLsS --retry 3 -o "$work_dir/minirootfs.sha256" \
    "$repository/releases/aarch64/alpine-minirootfs-$release-aarch64.tar.gz.sha256"
(
    cd "$work_dir"
    mv minirootfs.tar.gz "alpine-minirootfs-$release-aarch64.tar.gz"
    shasum -a 256 -c minirootfs.sha256
)
bsdtar -xzf "$work_dir/alpine-minirootfs-$release-aarch64.tar.gz" -C "$root"

curl -fLsS --retry 3 -o "$work_dir/APKINDEX.tar.gz" \
    "$repository/main/aarch64/APKINDEX.tar.gz"
...
download_package() {
    ...
    curl -fLsS --retry 3 -o "$work_dir/$name.apk" \
        "$repository/main/aarch64/$name-$version.apk"
}
