...
`-DPEDIGREE_SAMPLED_TIME_ACCOUNTING=FALSE`, keep
`-DPEDIGREE_TIME_ACCOUNTING=TRUE`, and enable
`-DPEDIGREE_BENCHMARK_SYSCALL_TIMING=TRUE`. Add an empty `time-syscalls`
marker to the benchmark root. Per-syscall timing requires precise accounting;
the default sampled mode is incompatible with it. The driver enables attribution only in each
