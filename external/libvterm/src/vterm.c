...
{
  size_t len = vterm_vsnprintf(vt->tmpbuffer, vt->tmpbuffer_len,
      format, args);
...
INTERNAL void vterm_push_output_sprintf_ctrl(VTerm *vt, unsigned char ctrl, const char *fmt, ...)
{
  ...
  if(ctrl >= 0x80 && !vt->mode.ctrl8bit)
    cur = vterm_snprintf(vt->tmpbuffer, vt->tmpbuffer_len,
        ESC_S "%c", ctrl - 0x40);
  else
    cur = vterm_snprintf(vt->tmpbuffer, vt->tmpbuffer_len,
        "%c", ctrl);
  ...
  va_list args;
  va_start(args, fmt);
  cur += vterm_vsnprintf(vt->tmpbuffer + cur, vt->tmpbuffer_len - cur,
      fmt, args);
  va_end(args);
...
INTERNAL void vterm_push_output_sprintf_str(VTerm *vt, unsigned char ctrl, bool term, const char *fmt, ...)
{
  ...
  if(term) {
    cur += vterm_snprintf(vt->tmpbuffer + cur, vt->tmpbuffer_len - cur,
