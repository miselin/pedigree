...
class DelayTimerEvent : public Event {
 public:
  explicit DelayTimerEvent(Thread* owner, size_t nestingLevel = ~0UL);
...
void* addAlarm(Timestamp nanoseconds) {
  Thread* pThread = Processor::information().getCurrentThread();
  Event* pEvent = new DelayTimerEvent(pThread, pThread->getStateLevel());
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
...
void removeAlarm(void* handle) {
  ...
  DelayTimerEvent* pEvent = reinterpret_cast<DelayTimerEvent*>(handle);
...
DelayTimerEvent::DelayTimerEvent(Thread* owner, size_t nestingLevel)
    : Event(reinterpret_cast<uintptr_t>(&delayTimerFired), false, nestingLevel), m_pOwner(owner) {}
