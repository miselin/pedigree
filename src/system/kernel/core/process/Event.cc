...
Event::Event(
    ...
    : m_HandlerAddress(handlerAddress), m_bIsDeletable(isDeletable),
      ...
      m_Threads(), m_Lock(false), m_DeleteWhenUnused(false),
      m_DeliveriesClosed(false), m_DrainClaimed(false),
      m_SendersInFlight(0),
...
Event::Retirement::Retirement(Retirement &&other)
    : m_pEvent(other.m_pEvent)
{
    other.m_pEvent = nullptr;
}
...
Event::Delivery::Delivery()
    : m_pEvent(nullptr), m_pThread(nullptr), m_pPreviousActive(nullptr),
      m_pNextActive(nullptr), m_bActive(false)
{
}

Event::Delivery::Delivery(Event *event, Thread *thread)
    : m_pEvent(event), m_pThread(thread), m_pPreviousActive(nullptr),
      m_pNextActive(nullptr), m_bActive(false)
{
}

Event::Delivery::Delivery(Delivery &&other)
    : m_pEvent(other.m_pEvent), m_pThread(other.m_pThread),
      m_pPreviousActive(nullptr), m_pNextActive(nullptr), m_bActive(false)
{
    if (other.m_bActive)
    {
...
Event &Event::operator=(const Event &other)
{
    ...
    if (
        m_DrainClaimed || m_SendersInFlight ||
        m_pFirstActiveDelivery)
    {
        ...
        auto deliveryGuard = m_DeliveryWaiters.acquire();
        {
                 ...
                 it != m_Threads.end(); ++it)
            {
                if (*it == thread)
                {
                    m_Threads.erase(it);
                finalDelivery =
                    !m_Threads.count() && !m_SendersInFlight;
                    break;
                }
            }
...
void Event::endDispatch(Delivery *delivery)
{
    ...
    {
        delivery->m_pPreviousActive->m_pNextActive =
    ...
    {
        delivery->m_pNextActive->m_pPreviousActive =
...
void Event::waitForDeliveries()
{
    ...
    while (true)
    {
        auto guard = m_DeliveryWaiters.acquire();
        {
            ...
            for (
                Delivery *delivery = m_pFirstActiveDelivery; delivery;
