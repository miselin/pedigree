...
PerProcessorScheduler::PerProcessorScheduler()
    : m_pSchedulingAlgorithm(0),
      m_NewThreadDataLock(),
      m_NewThreadDataCondition(),
      m_NewThreadData(),
      m_DelayedNewThreadData(),
...
void PerProcessorScheduler::startNewThreadWorker(Process* pParent) {
  ...
  if (m_NewThreadWorker || m_NewThreadData.count() || m_DelayedNewThreadData.count()) {
  ...
  Thread* pAddThread =
      new Thread(pParent, processorAddThread, reinterpret_cast<void*>(this), 0, false, true);
...
void PerProcessorScheduler::stopNewThreadWorker() {
  ...
  m_NewThreadDataLock.acquire();
  const bool drained = !m_NewThreadData.count() && !m_DelayedNewThreadData.count();
  m_NewThreadDataLock.release();
  if (!drained) {
...
int PerProcessorScheduler::processorAddThread(void* instance) {
  ...
  while (true) {
    ...
    while (!pInstance->m_NewThreadData.count()) {
      ...
      pInstance->m_NewThreadDataCondition.waitForCompletion(pInstance->m_NewThreadDataLock);
    }
    ...
    const bool retireBeforeStart = pThread->getUnwindState() == Thread::TerminateThread;
    if (pThread->m_Status == Thread::Created && pThread->m_bStartRequested && !retireBeforeStart) {
    ...
    if (pData->useSyscallState) {
      ...
      pInstance->addThread(pThread, pData->pStartFunction, pData->pParam, pData->bUsermode,
                           pData->pStack);
    }
    delete pData;
  }
...
void PerProcessorScheduler::schedule(Thread::Status nextStatus, bool dispatchEvents) {
  ...
  if (nextStatus == Thread::Sleeping) {
    ...
    if (dispatchEvent) {
      ...
      pCurrentThread->interruptWaitUnlocked(WaitQueue::WakeReason::Event, readyScheduler);
    }
    if (!pCurrentThread->activeWaitPendingUnlocked() || dispatchEvent) {
      pCurrentThread->getLock().release();
      Processor::setInterrupts(bWasInterrupts);
      return;
    }
  ...
  EMIT_IF(SYSTEM_REQUIRES_ATOMIC_CONTEXT_SWITCH) {
    ...
    Processor::notifyHostedContextSwitchStage(
        ProcessorBase::HostedContextSwitchStage::SchedulerBookkeepingComplete);
    Processor::notifyHostedContextSwitchStage(
        ProcessorBase::HostedContextSwitchStage::SchedulerRestoringInterrupts);
#endif
    Processor::setInterrupts(bWasInterrupts);
...
void PerProcessorScheduler::checkEventState(uintptr_t userStack) {
  ...
  if (flags & VirtualAddressSpace::KernelMode) {
    ...
    callOnStack(reinterpret_cast<uintptr_t>(pThread->getKernelStack()), handlerAddress, addr);
#else
    ...
    EMIT_IF(SYSTEM_REQUIRES_ATOMIC_CONTEXT_SWITCH) {
      Processor::saveAndJumpUser(bWasInterrupts, *oldState, 0, Event::getTrampoline(), userStack,
                                 handlerAddress, addr);
      Processor::setInterrupts(bWasInterrupts);
    }
    else {
...
void PerProcessorScheduler::addThread(Thread* pThread, SyscallState& state) {
  ...
  MemoryCopy(reinterpret_cast<void*>(kStack), reinterpret_cast<void*>(&state),
             sizeof(SyscallState));
...
void PerProcessorScheduler::deleteThread(Thread* pThread) {
  ...
  Processor::switchAddressSpace(*replacement->getParent()->getAddressSpace());
#endif
...
void PerProcessorScheduler::serviceDeferredSubsystemException(InterruptState& state) {
  ...
  if (!thread->takeDeferredSubsystemException(rawType, faultAddress, errorCode)) {
    return;
  }
...
bool PerProcessorScheduler::currentIrqWorkDoorbellPendingForTest() {
  return Processor::information().getScheduler().m_IrqWorkDoorbell.value() != 0;
}
