...
size_t currentMutexOwner() {
  ...
  uintptr_t owner = thread ? reinterpret_cast<uintptr_t>(thread)
                           : reinterpret_cast<uintptr_t>(&Processor::information());
#endif
...
void observeMutexTransition(Semaphore::MutexTransitionWindow window) {
  Semaphore::MutexTransitionHook hook = __atomic_load_n(&g_MutexTransitionHook, __ATOMIC_ACQUIRE);
  if (hook) {
    hook(window);
  }
}
...
static void interruptSemaphore(uint8_t* pBuffer) {
  EMIT_IF(THREADS) {
    Processor::information().getCurrentThread()->markTimeoutInterruptedWait();
  }
}
...
Semaphore::Semaphore(size_t nInitialValue, bool canInterrupt)
    : magic(SemaphoreMagic), m_Counter(nInitialValue), m_Waiters(), m_bCanInterrupt(canInterrupt) {
...
Semaphore::SemaphoreResult Semaphore::acquireWithResult(size_t n, size_t timeoutSecs,
                                                        size_t timeoutUsecs, bool deferTerminal) {
...
bool Semaphore::acquireWithError(size_t n, size_t timeoutSecs, size_t timeoutUsecs,
...
bool Semaphore::acquireForCompletion(size_t n, size_t timeoutSecs, size_t timeoutUsecs) {
  EMIT_IF(!THREADS) {
    ...
    if (hasTimeout) {
      ...
      if (timeoutSecs > (maximum / Time::Multiplier::Second)) {
        ...
        timeout = timeoutSecs * Time::Multiplier::Second;
        ...
        timeout = microseconds > (maximum - timeout) ? maximum : timeout + microseconds;
      }
    }

    Thread::InterruptionReason retainedInterruption =
        thread ? thread->getInterruptionReason() : Thread::NotInterrupted;
    if (thread && retainedInterruption != Thread::NotInterrupted) {
    ...
    while (true) {
      SemaphoreResult result = acquireWithResult(n, remainingSecs, remainingUsecs, true);
      const bool acquired = result.hasValue() && result.value();
      const SemaphoreError error = result.hasError() ? result.error() : NoError;
      if (acquired) {
        if (thread && retainedInterruption != Thread::NotInterrupted) {
      ...
      if (error == TimedOut) {
        if (!hasTimeout) {
          ...
          retainedInterruption = Thread::InterruptedByTimeout;
          continue;
        }
        if (thread && retainedInterruption != Thread::NotInterrupted) {
          thread->setInterruptionReason(retainedInterruption);
...
bool Semaphore::tryAcquire(size_t n) {
  ...
  if (mutex) {
    ...
    if (!__sync_bool_compare_and_swap(&magic, MutexUnlocked, currentMutexOwner())) {
...
void Semaphore::release(size_t n) {
  EMIT_IF(!PEDIGREE_BENCHMARK) {
    if (!Processor::guardDeviceHardIrqOperation(DeviceHardIrqOperation::SemaphoreRelease)) {
      return;
    }
  }
  ...
  if (isMutexState(state)) {
...

    {
      ...
      if (!m_Counter.compareAndSwap(0, 1)) {
        ...
        // can recover.
        __sync_bool_compare_and_swap(&magic, MutexUnlocked, owner);
