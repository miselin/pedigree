...
MemoryPressureHandler::MemoryPressureHandler()
    : m_pPrevious(nullptr), m_pNext(nullptr), m_Priority(0),
      m_RegistrationSequence(0), m_bRegistered(false), m_bRemoving(false)
#if THREADS
      ,
      m_CallbacksInFlight(0), m_pCallbackOwner(nullptr), m_CallbackWaiters()
#endif
{
}
...
bool MemoryPressureManager::compact()
{
    ...
    while (true)
    {
        ...
        const WaitQueue::WakeReason compactWake =
            compactGuard.waitForCompletion(
    ...
    while (priority < MAX_MEMPRESSURE_PRIORITY)
    {
        ...
        {
            LockGuard<Spinlock> guard(m_Lock);
            for (MemoryPressureHandler *candidate = m_Handlers[priority];
                 candidate; candidate = candidate->m_pNext)
            {
        ...
        NOTICE(
            ...
            << (description ? description :
                              "<unnamed memory-pressure handler>"));
...
#if THREADS
        {
            ...
            callbackGuard.wakeAll(
                WaitQueue::WakeReason::Signalled,
                WaitQueue::Channel(handler));
        }
#endif
...
bool MemoryPressureManager::compactingForCurrentExecution() const
{
    return m_bCompacting &&
           __atomic_load_n(&m_pCompactOwner, __ATOMIC_ACQUIRE) ==
               currentCallbackOwner();
}

MemoryPressureManager::MemoryPressureManager()
    : m_Lock(false), m_Handlers(), m_HandlerTails(),
      m_NextRegistrationSequence(0), m_bCompacting(false),
    ...
    size_t prio, MemoryPressureHandler *pHandler)
{
    ...
    {
        FATAL(
            "Memory-pressure handler registered during callback removal.");
    }
#endif
...
void MemoryPressureManager::removeHandler(MemoryPressureHandler *pHandler)
{
    ...
    bool needsDrain = false;
    {
        ...
        auto callbackGuard = pHandler->m_CallbackWaiters.acquire();
        if (pHandler->m_CallbacksInFlight &&
            pHandler->m_pCallbackOwner == currentCallbackOwner())
        {
            FATAL(
    ...
    while (true)
    {
        ...
        const WaitQueue::WakeReason callbackWake =
            callbackGuard.waitForCompletion(
            WaitQueue::Channel(pHandler), Thread::CallbackDrain,
