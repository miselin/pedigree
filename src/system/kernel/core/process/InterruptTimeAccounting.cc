...
InterruptTimeAccounting::InterruptTimeAccounting(bool fromUserspace)
    : m_pThread(Processor::information().getCurrentThread()), m_bFromUserspace(fromUserspace) {
...
InterruptTimeAccounting::~InterruptTimeAccounting() {
  ...
  if (!m_bFromUserspace) {
    m_pThread->transitionTime(KernelTimeTransition::handler(),
                              KernelTimeTransition::resumed(false));
  }
}

void InterruptTimeAccounting::finishUserReturn(Thread* thread) {
  if (thread && thread->currentTimeAccountingMode() == CpuTimeMode::Kernel) {
    thread->transitionTimeAtInterruptReturn(CpuTimeMode::Kernel, CpuTimeMode::User);
  }
}
