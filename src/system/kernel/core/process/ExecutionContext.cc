...
ExecutionContextGuard::ExecutionContextGuard(ExecutionContext context)
    : m_Thread(Processor::information().getCurrentThread()), m_StateLevel(0),
      m_Previous(ExecutionContext::AtomicThread), m_Cleanup(), m_Active(false)
{
    if (!m_Thread)
    {
        return;
    ...
    m_Previous =
        m_Thread->m_StateLevels[m_StateLevel].m_ExecutionContext.current();
...
{
    ExecutionContextGuard *guard =
        reinterpret_cast<ExecutionContextGuard *>(context);
    if (guard)
    {
        guard->restore();
    }
...
void ExecutionContextGuard::restore()
{
    ...
    m_Thread->m_StateLevels[m_StateLevel].m_ExecutionContext.restore(
        m_Previous);
    m_Active = false;
}
