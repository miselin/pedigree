...
{
    demangle()
        : nSubstitutions(0), nTemplateParams(0), nParams(0), nLevel(0),
          nNameParseLevel(0)
    {
    }
...
#define DECLARE(id) \
    static int parse##id(LargeStaticString &, LargeStaticString &, demangle_t &)
#define DECLARE_LVAL(id)  \
    static int parse##id( \
        LargeStaticString &, LargeStaticString &, demangle_t &, int &)
#define DECLARE_SVAL(id)                                        \
    static int parse##id(                                       \
        LargeStaticString &, LargeStaticString &, demangle_t &, \
        LargeStaticString &)
DECLARE(MangledName);
DECLARE(Name);
...
*/
static int parseMangledName(
    LargeStaticString &src, LargeStaticString &dest, demangle_t &data)
{
    START("MangledName");
    ...
    // HACK:: Bit of a hack here - we add the entire function name (foo::bar) to
    // the substitution index, when it shouldn't be. Here we decrement the
    // number of substitutions so it is wiped.
    if (data.nSubstitutions)
        data.nSubstitutions--;

    // HACK:: OK here we go, hack again. We manually increase the
    // data.nNameParseLevel member so that our template parameters don't get
    // accidentally overwritten.
    data.nNameParseLevel = 5;

    // If we have some template params we should expect a type next (for return
    // type).
...
*/
static int
parseName(LargeStaticString &src, LargeStaticString &dest, demangle_t &data)
{
    START("Name");
...
<unscoped_template_name> ::= <unscoped_name>
  ::= <substitution>
*/
static int parseUnscopedTemplateName(
    LargeStaticString &src, LargeStaticString &dest, demangle_t &data)
{
...
<prefix> ::= <unqualified_name> <prefix>
  ...
  ::= <substitution>
*/
static int parsePrefix(
    LargeStaticString &src, LargeStaticString &dest, demangle_t &data,
    LargeStaticString &thisPrefix)
{
    START("Prefix");
...
<template_prefix> ::= <prefix> <unqualified_name>
  ...
  ::= <substitution>
*/
// static int parseTemplatePrefix(LargeStaticString &src, LargeStaticString
// &dest, demangle_t &data)
// {
// Unused.
// }
...
<unqualified_name> ::= <operator_name>
  ...
  ::= <source_name>
*/
static int parseUnqualifiedName(
    LargeStaticString &src, LargeStaticString &dest, demangle_t &data)
{
...
<ctor_dtor_name> ::= C {0,1,2}
  ::= D {0,1,2}
*/
static int parseCtorDtorName(
    LargeStaticString &src, LargeStaticString &dest, demangle_t &data)
{
    START("CtorDtorName");
    if (src[0] == 'C' && (src[1] == '0' || src[1] == '1' || src[1] == '2'))
    {
        src.stripFirst(2);
        dest += "`ctor'";
    }
    else if (src[0] == 'D' && (src[1] == '0' || src[1] == '1' || src[1] == '2'))
    {
        src.stripFirst(2);
        dest += "`dtor'";
    }
...
*/
static int parseSourceName(
    LargeStaticString &src, LargeStaticString &dest, demangle_t &data)
{
    START("SourceName");

    int lval;
    if (parseNumber(src, dest, data, lval) == SUCCESS && lval >= 0 &&
...
*/
static int
parseType(LargeStaticString &src, LargeStaticString &dest, demangle_t &data)
{
    START("Type");
    ...
    LargeStaticString &src, LargeStaticString &dest, demangle_t &data)
{
    ...
    switch (src[0])
    {
        ...
        case 'x':
            ...
            break;
        case 'y':
            dest += "unsigned __int64";
            break;
        case 'n':
            dest += "__int128";
            break;
        case 'o':
            dest += "unsigned __int128";
            break;
        case 'f':
            dest += "float";
            break;
...
*/
// static int parseBareFunctionType(LargeStaticString &src, LargeStaticString
// &dest, demangle_t &data)
// {
// Unused.
// }
...
*/
static int parsePointerToMemberType(
    LargeStaticString &src, LargeStaticString &dest, demangle_t &data)
{
    ...
    // If the next thing is a function, we cheat a little to get a decent
    // looking member member function.
    /// \note This piece of code will FAIL if the member function pointer
    /// returns a function
    ///       pointer!
    if (src[0] == 'F')
    {
...
<template_template_param> ::= <template-param> # first parameter
  ::= <substitution>
*/
static int parseTemplateTemplateParam(
    LargeStaticString &src, LargeStaticString &dest, demangle_t &data)
{
    ...
    LargeStaticString &src, LargeStaticString &dest, demangle_t &data)
{
    ...
    // HACK:: We don't want the full "unsigned int" nonsense, if it's a builtin
    // type we use our own handlers. Else we fall back on parseType.
    const char *cookie = 0;
    switch (src[0])
    {
        case 'i':
...
//
//   // HACK:: Bit of a hack here - we prepend "::" to every identifier. It
//   looks a bit ugly. if (dest[0] == ':' && dest[1] == ':')
//     dest.stripFirst(2);
//
