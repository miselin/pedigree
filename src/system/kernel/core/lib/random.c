...
static void random_cpuid(uint32_t leaf, uint32_t subleaf, uint32_t* eax, uint32_t* ebx,
...
static int random_hardware_word(uint64_t* value) {
  ...
  if (g_HasRdrand) {
    for (size_t attempt = 0; attempt < 10; ++attempt) {
      ...
      __asm__ volatile("rdrand %0; setc %1" : "=r"(candidate), "=qm"(ok) : : "cc");
      if (ok) {
        *value = candidate;
        return 1;
      }
  ...
  if (g_HasRdseed) {
    for (size_t attempt = 0; attempt < 64; ++attempt) {
      unsigned char ok = 0;
      uint64_t candidate = 0;
      __asm__ volatile("rdseed %0; setc %1" : "=r"(candidate), "=qm"(ok) : : "cc");
      if (ok) {
        *value = candidate;
        return 1;
      }
...
size_t hardware_random_bytes(void* buffer, size_t length) {
  ...
  while (produced < length) {
    ...
    for (size_t byte = 0; byte < sizeof(value) && produced < length; ++byte) {
