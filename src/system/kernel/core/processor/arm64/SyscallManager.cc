...
Arm64SyscallManager Arm64SyscallManager::m_Instance;
...
SyscallManager& SyscallManager::instance() {
  return Arm64SyscallManager::instance();
}

bool Arm64SyscallManager::registerSyscallHandler(Service_t service, SyscallHandler* handler,
                                                 Registration& registration, FastEntry entry) {
  return registerHandler(service, handler, registration, entry);
}

void Arm64SyscallManager::handle(SyscallState& state) {
  Arm64SyscallManager& manager = m_Instance;
      ...
      switch (action.kind) {
        ...
        case RestoreProcessorState:
          if (!userReturnTerminal) {
            for (size_t i = 0; i < 31; ++i) {
              state.x[i] = action.state.x[i];
            }
            state.sp = action.state.sp;
            state.pc = action.state.pc;
            state.pstate = action.state.pstate;
            userReturnTerminal =
        ...
        case JumpToUserspace:
          if (!userReturnTerminal) {
            ...
            state.pc = action.state.getInstructionPointer();
...
uintptr_t Arm64SyscallManager::syscall(Service_t service, uintptr_t function, uintptr_t p1,
                                       uintptr_t p2, uintptr_t p3, uintptr_t p4, uintptr_t p5) {
  register uintptr_t x0 asm("x0") = p1;
  register uintptr_t x1 asm("x1") = p2;
  register uintptr_t x2 asm("x2") = p3;
  register uintptr_t x3 asm("x3") = p4;
  register uintptr_t x4 asm("x4") = p5;
  register uintptr_t x8 asm("x8") = (static_cast<uintptr_t>(service) << 16) | function;
  asm volatile("svc #1" : "+r"(x0) : "r"(x1), "r"(x2), "r"(x3), "r"(x4), "r"(x8) : "memory");
  return x0;
}
