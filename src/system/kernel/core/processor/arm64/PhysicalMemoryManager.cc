...
namespace {
constexpr uint64_t MaximumPhysicalAddress = 64ULL << 30;
...
Arm64PhysicalMemoryManager& Arm64PhysicalMemoryManager::instance() {
  static Arm64PhysicalMemoryManager manager;
  return manager;
}

PhysicalMemoryManager& PhysicalMemoryManager::instance() {
  return Arm64PhysicalMemoryManager::instance();
}
...
void Arm64PhysicalMemoryManager::initialise(const BootstrapStruct_t& info) {
  ...
  if (!metadata) {
    panic("ARM64: cannot reserve page metadata");
  }

  m_References = reinterpret_cast<uint16_t*>(DirectMapBase + metadata);
...
void Arm64PhysicalMemoryManager::markPage(size_t page, bool available) {
...
void Arm64PhysicalMemoryManager::freePageUnlocked(physical_uintptr_t page) {
  ...
  if (!page || (page & (PAGE_SIZE - 1)) || index >= m_MaxPage || !m_References[index]) {
    panic("ARM64: invalid physical page free");
  }
...
bool Arm64PhysicalMemoryManager::copyPhysicalPageToBuffer(physical_uintptr_t page, void* buffer) {
  if (!page || page >= MaximumPhysicalAddress || (page & (PAGE_SIZE - 1)) || !buffer) {
    return false;
  }
  memcpy(buffer, reinterpret_cast<const void*>(DirectMapBase + page), PAGE_SIZE);
  return true;
}
...
bool Arm64PhysicalMemoryManager::copyPhysicalPageFromBuffer(physical_uintptr_t page,
  ...
  memcpy(reinterpret_cast<void*>(DirectMapBase + page), buffer, PAGE_SIZE);
  return true;
}
...
bool Arm64PhysicalMemoryManager::allocateRegion(MemoryRegion& region, size_t pages,
...
void Arm64PhysicalMemoryManager::unmapRegion(MemoryRegion* region) {
