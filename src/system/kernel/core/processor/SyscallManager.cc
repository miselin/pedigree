...
SyscallManager::HandlerSlot::HandlerSlot()
    : handler(nullptr), generation(0), inFlight(0), enabled(false),
      draining(false), dispatches(nullptr), drainWaiters()
{
}
...
SyscallManager::Registration::Registration()
    : m_pManager(nullptr), m_Service(serviceEnd),
      m_pHandler(nullptr), m_Generation(0)
{
}

SyscallManager::Registration::Registration(Registration &&other)
    : m_pManager(other.m_pManager), m_Service(other.m_Service),
      m_pHandler(other.m_pHandler), m_Generation(other.m_Generation)
{
...
SyscallManager::Registration &SyscallManager::Registration::operator=(
    Registration &&other)
{
    if (this != &other)
    {
...
SyscallManager::HandlerLease::HandlerLease()
    : m_pManager(nullptr), m_pSlot(nullptr), m_pHandler(nullptr),
      m_Generation(0), m_pThread(nullptr), m_Cleanup(),
...
SyscallManager::SyscallManager()
    : m_HandlerLock(), m_HandlerSlots(), m_NextDispatchSequence(0)
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
      ,
      m_HandlerPinHook(nullptr), m_PostSyscallHook(nullptr)
#endif
{
}
...
bool SyscallManager::registerHandler(
    Service_t service, SyscallHandler *pHandler,
    Registration &registration)
{
    if (
        UNLIKELY(service >= serviceEnd) || !pHandler ||
        registration)
    {
        return false;
    }
...
bool SyscallManager::unregisterHandler(Registration &registration)
{
    if (
        registration.m_pManager != this ||
        UNLIKELY(registration.m_Service >= serviceEnd) ||
    ...
    m_HandlerLock.acquire();
    HandlerSlot &slot =
        m_HandlerSlots[registration.m_Service];
    if (
        slot.handler != registration.m_pHandler ||
        slot.generation != registration.m_Generation)
    {
        m_HandlerLock.release();
        return false;
    }
    ...
    for (
        HandlerDispatch *dispatch = slot.dispatches; dispatch;
...
bool SyscallManager::acquireHandler(
    Service_t service, HandlerLease &lease, PostSyscallAction &action)
{
    ...
    if (thread)
    {
        thread->armStateCleanup(
            lease.m_Cleanup, abandonedHandlerCleanup, &lease);
    }
    ...
    lease.m_Dispatch.stateLevel =
        thread ? thread->getStateLevel() : 0;
...
void SyscallManager::releaseHandler(
    HandlerLease &lease, bool normalReturn)
{
...
{
    HandlerLease *lease =
        reinterpret_cast<HandlerLease *>(context);
...
bool SyscallManager::requestPostSyscallAction(
    PostSyscallActionKind kind, intptr_t value,
    const ProcessorState *state)
{
    ...
    {
        for (
            HandlerDispatch *dispatch = m_HandlerSlots[i].dispatches;
            dispatch; dispatch = dispatch->next)
        {
            if (
                dispatch->owner == owner &&
                (!target || dispatch->sequence > target->sequence))
            {
                target = dispatch;
            }
        }
    ...
    if (
        ...
        target->action->kind != NoPostSyscallAction)
    {
        m_HandlerLock.release();
        return false;
    }
    if (
        (kind == ReturnFromEvent || kind == PopEventState) &&
        !target->stateLevel)
    {
        m_HandlerLock.release();
        return false;
...
{
    return requestPostSyscallAction(
...
{
    return requestPostSyscallAction(
        ExitCurrentProcess, static_cast<intptr_t>(status));
}
...
bool SyscallManager::requestStateRestore(
    const ProcessorState &state)
{
    return requestPostSyscallAction(
        RestoreProcessorState, 0, &state);
}

bool SyscallManager::requestUserJump(
    uintptr_t instructionPointer, uintptr_t stackPointer)
{
    ProcessorState state;
    state.setInstructionPointer(instructionPointer);
    state.setStackPointer(stackPointer);
    return requestPostSyscallAction(
        JumpToUserspace, 0, &state);
}
...
bool SyscallManager::postSyscallHookHandled(
    const PostSyscallAction &action)
{
    PostSyscallHook hook =
        __atomic_load_n(&m_PostSyscallHook, __ATOMIC_ACQUIRE);
