...
void* VirtualAddressSpace::expandHeap(ssize_t incr, size_t flags) {
  ...
  m_HeapEnd = reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(m_HeapEnd) &
  ...
  if (newEnd >= getKernelHeapStart()) {
    ...
    if (newEnd >= getDynamicStart()) {
      ...
      // This is not allowed.
      ERROR(
          "Heap expansion no longer allowed; about to run into dynamic "
          "memory area.");
      return 0;
    }
    ...
    ERROR(
        "Heap expansion no longer allowed; have run over userspace "
  ...
  if (incr < 0) {
    ...
    while (reinterpret_cast<uintptr_t>(newHeapEnd) > reinterpret_cast<uintptr_t>(m_HeapEnd)) {
      ...
      if (map(m_ZeroPage, m_HeapEnd,
              (flags & ~VirtualAddressSpace::Write) | VirtualAddressSpace::CopyOnWrite) == false) {
