...
bool MIPS32InterruptManager::registerInterruptHandler(
...
bool MIPS32InterruptManager::registerExternalInterruptHandler(
    size_t interruptNumber, InterruptHandler *handler)
{
    // TODO: Needs locking
...
bool MIPS32InterruptManager::registerSyscallHandler(
...
void MIPS32InterruptManager::initialiseProcessor()
{
    ...
    pCode[0] =
        0x3c1a0000 | (reinterpret_cast<uint32_t>(&mips32_exception) >> 16);
    // ori $k0, $k0, <lower 16 bits of exception handler>
    pCode[1] = 0x375a0000 |
               (reinterpret_cast<uint32_t>(&mips32_exception) & 0x0000FFFF);
    // jr $k0
    pCode[2] = 0x03400008;
    // nop (delay slot)
    pCode[3] = 0x00000000;
    ...
    MemoryCopy(
        reinterpret_cast<void *>(KSEG1(0x80)), reinterpret_cast<void *>(pCode),
        32 * 4);
    MemoryCopy(
        reinterpret_cast<void *>(KSEG1(0x100)), reinterpret_cast<void *>(pCode),
        32 * 4);
    MemoryCopy(
        reinterpret_cast<void *>(KSEG1(0x180)), reinterpret_cast<void *>(pCode),
        32 * 4);
    MemoryCopy(
        reinterpret_cast<void *>(KSEG1(0x200)), reinterpret_cast<void *>(pCode),
        32 * 4);
    ...
    // lui $k0, <upper 16 bits of exception handler>
    pCode[0] = 0x3c1a0000 |
               (reinterpret_cast<uint32_t>(&TlbManager::interruptAsm) >> 16);
    // ori $k0, $k0, <lower 16 bits of exception handler>
    pCode[1] =
        0x375a0000 |
        (reinterpret_cast<uint32_t>(&TlbManager::interruptAsm) & 0x0000FFFF);
    // jr $k0
    pCode[2] = 0x03400008;
    // nop (delay slot)
    pCode[3] = 0x00000000;
    ...
    MemoryCopy(
        reinterpret_cast<void *>(KSEG1(0x0)), reinterpret_cast<void *>(pCode),
        32 * 4);

    // Invalidate the instruction cache - force a reload of the exception
    // handlers.
...
void MIPS32InterruptManager::interrupt(InterruptState &interruptState)
{
    ...
    // External interrupt, handled?
        ...
        m_Instance.m_ExternalHandler[externalInt]->interrupt(
            externalInt, interruptState);
