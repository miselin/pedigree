...
/**
 * The PPC hashed page table that is used almost as a secondary PTE cache by the
 * processor. It is a finite size, so not all PTEs may be able to fit in it
 * (hence why it is used as a secondary cache of sorts.
 *
 * The size is set at initialisation time, and is based on the amount of
 * available RAM.
 */
class HashedPageTable
{
  public:
    ...
    /** Adds a page table entry mapping effectiveAddress to physicalAddress for
       the given VSID, with mode 'mode'. \param effectiveAddress The EA (virtual
       address) to map. \param physicalAddress The physical address to map to.
        \param mode The mode of the mapping - in VirtualAddressSpace form
       (VirtualAddressSpace::WriteThrough etc) \param vsid The virtual space
       identifier. */
    void addMapping(
    ...
    /** Returns the mapping for effectiveAddress in VSID. If there is no such
