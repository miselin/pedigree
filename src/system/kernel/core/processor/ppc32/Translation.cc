...
Translations::Translations() : m_nTranslations(0)
{
    ...
    if (mmu.getProperty(
            "translations", m_pTranslations,
            sizeof(Translation) * NUM_TRANSLATIONS) == -1)
    {
    ...
    m_nTranslations =
        mmu.getPropertyLength("translations") / sizeof(Translation);
    for (unsigned int i = 0; i < m_nTranslations; i++)
    {
        NOTICE(
            "T: " << Hex << m_pTranslations[i].virt << ", "
                  << m_pTranslations[i].phys << ", " << m_pTranslations[i].size
...
void Translations::addTranslation(
    uint32_t virt, uint32_t phys, uint32_t size, uint32_t mode)
{
...
void Translations::removeRange(uintptr_t start, uintptr_t end)
{
    ...
    while (removed)
    {
        ...
        for (unsigned int i = 0; i < m_nTranslations; i++)
        {
            if (m_pTranslations[i].virt >= start &&
                m_pTranslations[i].virt < end)
            {
