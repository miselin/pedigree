...
#define HALF16(r, x) \
    ((x << 16) | (r & 0xFFFF))  // We deal with 32bit numbers, so we must keep
                                // the lower 16 bits.
    ...
    uintptr_t loadBase, SymbolTable::Policy policy)
{
    ...
    else if (ELF32_R_TYPE(rel.info) != R_PPC_RELATIVE)  // Relative doesn't need
                                                        // a symbol!
    {
    ...
    switch (ELF32_R_TYPE(rel.info))
    {
        ...
        default:
            ERROR(
                "Relocation not supported: " << Dec << ELF32_R_TYPE(rel.info));
    }
    ...
    *pResult = result;
    ...
    Processor::flushDCacheAndInvalidateICache(
        ...
        reinterpret_cast<uintptr_t>(pResult) + 4);

    return true;
}

bool Elf::applyRelocation(
    ElfRel_t rela, ElfSectionHeader_t *pSh, SymbolTable *pSymtab,
    uintptr_t loadBase, SymbolTable::Policy policy)
{
