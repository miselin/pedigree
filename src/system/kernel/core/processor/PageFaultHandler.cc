...
PageFaultHandler::HandlerPrePinHook PageFaultHandler::m_HandlerPrePinHook = nullptr;
PageFaultHandler::AtomicDrainHook PageFaultHandler::m_AtomicDrainHook = nullptr;
#endif

PageFaultHandler::PageFaultHandler()
    : m_Handlers(), m_ActiveDispatches(), m_DispatchWaiters(), m_HandlerLock(false) {}
...
bool PageFaultHandler::retireSlot(HandlerSlot& slot, size_t expectedPublication,
  ...
  MemoryTrapHandler* handler = __atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE);
  if (handler != expectedHandler) {
    size_t expectedRetiringPublication = retiringPublication;
    __atomic_compare_exchange_n(&slot.publication, &expectedRetiringPublication,
                                expectedPublication, false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
    return false;
  }
...
void PageFaultHandler::unpublishDispatch(void* token) {
  ...
  const size_t finalPublication = __atomic_load_n(&releasedSlot->publication, __ATOMIC_SEQ_CST);
  if (generationOf(finalPublication) == drainGeneration &&
      modeOf(finalPublication) == SlotMode::Deferred && !hasActiveDispatch(*releasedSlot)) {
    MemoryTrapHandler* handler = __atomic_load_n(&releasedSlot->handler, __ATOMIC_ACQUIRE);
    if (handler) {
      retireSlot(*releasedSlot, finalPublication, handler);
    }
  }

  guard.wakeAll(WaitQueue::WakeReason::Signalled,
...
bool PageFaultHandler::findCurrentDispatch(void* owner, HandlerSlot* target,
...
bool PageFaultHandler::registerHandler(MemoryTrapHandler* pHandler) {
  ...
  findCurrentDispatch(currentDispatchOwner(), nullptr, callbackContext);
  if (callbackContext) {
    return false;
  }
...
bool PageFaultHandler::unregisterHandler(MemoryTrapHandler* pHandler) {
  ...
  if (!canYield || callbackContext) {
    for (size_t i = 0; i < MaxMemoryTrapHandlers; ++i) {
      ...
      const bool selfUnregister = findCurrentDispatch(owner, &slot, currentTargetDispatch);
      if (selfUnregister) {
        while (true) {
          const SlotMode mode = modeOf(publication);
          if (mode == SlotMode::Deferred || mode == SlotMode::Empty || mode == SlotMode::Retiring) {
            return false;
          }
          ...
          const size_t deferredPublication =
              makePublication(generationOf(publication), SlotMode::Deferred);
          if (__atomic_compare_exchange_n(&slot.publication, &publication, deferredPublication,
                                          false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
            return false;
          }
          if (__atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE) != pHandler) {
      ...
      if (!__atomic_compare_exchange_n(&slot.publication, &publication, drainingPublication, false,
                                       __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
        return false;
      }

#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
      AtomicDrainHook drainHook = __atomic_load_n(&m_AtomicDrainHook, __ATOMIC_ACQUIRE);
      if (drainHook) {
        drainHook(pHandler);
      }
#endif
      ...
      if (hasActiveDispatch(slot)) {
        ...
        __atomic_compare_exchange_n(
            ...
            makePublication(generationOf(drainingPublication), SlotMode::Enabled), false,
            __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
        return false;
      }
  ...
  for (size_t i = 0; i < MaxMemoryTrapHandlers; ++i) {
    ...
    if (modeOf(candidatePublication) != SlotMode::Empty &&
        __atomic_load_n(&m_Handlers[i].handler, __ATOMIC_ACQUIRE) == pHandler) {
      slot = &m_Handlers[i];
  ...
  const size_t drainingPublication = makePublication(generationOf(publication), SlotMode::Draining);
  ...
  while (true) {
    ...
    MemoryTrapHandler* finalHandler = __atomic_load_n(&slot->handler, __ATOMIC_ACQUIRE);
    if (generationOf(finalPublication) != drainGeneration) {
      return true;
    }
...
bool PageFaultHandler::dispatchHandlers(InterruptState& state, uintptr_t address, bool bIsWrite,
  ...
  for (size_t i = 0; i < MaxMemoryTrapHandlers; ++i) {
    ...
    if (thread) {
      ...
      thread->armAtomicStateCleanup(dispatchCleanup.cleanup, abandonedHandlerCleanup,
                                    &dispatchCleanup);
    }
...
size_t PageFaultHandler::activeDispatchCountForTest(MemoryTrapHandler* pHandler) {
  ...
  for (size_t i = 0; i < MaxActiveDispatches; ++i) {
    ...
    const size_t generation = __atomic_load_n(&dispatch.generation, __ATOMIC_ACQUIRE);
    HandlerSlot* slot = __atomic_load_n(&dispatch.slot, __ATOMIC_SEQ_CST);
    MemoryTrapHandler* handler = slot ? __atomic_load_n(&slot->handler, __ATOMIC_ACQUIRE) : nullptr;
    if (handler == pHandler && __atomic_load_n(&dispatch.token, __ATOMIC_ACQUIRE) == token &&
        __atomic_load_n(&dispatch.generation, __ATOMIC_ACQUIRE) == generation) {
      ++count;
    }
  }
