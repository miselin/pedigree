...
bool X86InterruptManager::registerInterruptHandler(
    size_t nInterruptNumber, InterruptHandler *pHandler)
{
    LockGuard<Spinlock> lockGuard(m_Lock);

    if (UNLIKELY(
            nInterruptNumber >= 256 ||
            nInterruptNumber == SYSCALL_INTERRUPT_NUMBER))
        return false;
...
bool X86InterruptManager::registerInterruptHandlerDebugger(
    size_t nInterruptNumber, InterruptHandler *pHandler)
{
    LockGuard<Spinlock> lockGuard(m_Lock);
...
bool X86InterruptManager::registerSyscallHandler(
...
void X86InterruptManager::interrupt(InterruptState &interruptState)
{
    ...
    // Trigger the next value in the PRNG sequence, so interrupt jitter improves
    // randomness.
    random_next();
...
// Check for exceptions we can kill threads with
    ...
    if (pSubsystem)
    {
        if (UNLIKELY(nIntNumber == 0))
        {
            pSubsystem->threadException(
                pThread, Subsystem::DivideByZero, interruptState);
            return;
        }
        else if (UNLIKELY(nIntNumber == 6))
        {
            pSubsystem->threadException(
                pThread, Subsystem::InvalidOpcode, interruptState);
            return;
        }
        else if (UNLIKELY(nIntNumber == 13))
        {
            pSubsystem->threadException(
                pThread, Subsystem::GeneralProtectionFault, interruptState);
            return;
        }
        else if (UNLIKELY(nIntNumber == 16))
        {
            pSubsystem->threadException(
                pThread, Subsystem::FpuError, interruptState);
            return;
        }
        else if (UNLIKELY(nIntNumber == 19))
        {
            pSubsystem->threadException(
                pThread, Subsystem::SpecialFpuError, interruptState);
            return;
        }
    }
#endif
    ...
    if (LIKELY(nIntNumber < 32 && nIntNumber != 1 && nIntNumber != 3))
    {
        ...
        // over? for now just print out the exception name and number
        ...
        if (nIntNumber == 8)
        {
            X86TaskStateSegment *tss = reinterpret_cast<X86TaskStateSegment *>(
                Processor::information().getTss());
            ERROR_NOLOCK("");
            ERROR_NOLOCK("");
            ERROR_NOLOCK("");
            ...
            ERROR_NOLOCK("GPs:");
            ERROR_NOLOCK(
                "EAX: " << tss->eax << ", EBX: " << tss->ebx
                        << ", ECX: " << tss->ecx << ".");
            ERROR_NOLOCK(
                "EDX: " << tss->edx << ", ESI: " << tss->esi
                        << ", EDI: " << tss->edi << ".");
            ERROR_NOLOCK("");
            ERROR_NOLOCK("Segments:");
            ERROR_NOLOCK(
                "CS: " << tss->cs << ", DS: " << tss->ds << ", ES: " << tss->es
                       << ".");
            ERROR_NOLOCK(
                "FS: " << tss->fs << ", GS: " << tss->gs << ", SS: " << tss->ss
                       << ".");
...
uintptr_t X86InterruptManager::syscall(
    ...
    uintptr_t p3, uintptr_t p4, uintptr_t p5)
{
    ...
    asm volatile("int $255"
                 : "=a"(ret)
                 : "0"(eax), "b"(p1), "c"(p2), "d"(p3), "S"(p4), "D"(p5));
    return ret;
}
...
void X86InterruptManager::setInterruptGate(
    size_t nInterruptNumber, uintptr_t interruptHandler, bool bUserspace)
{
...
void X86InterruptManager::setTaskGate(size_t nInterruptNumber, uint16_t tssSeg)
{
...
X86InterruptManager::X86InterruptManager() : m_Lock(false, true)
{
    ...
    for (size_t i = 0; i < 256; i++)
        setInterruptGate(
            i, interrupt_handler_array[i],
            (i == SYSCALL_INTERRUPT_NUMBER || i == 3 /* Interrupt number */) ?
                true :
                false);
