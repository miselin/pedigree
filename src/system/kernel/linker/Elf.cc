...
static void resolveNeeded() {
  FATAL(
      "ELF: resolveNeeded() called but binary should have been fully "
      "relocated.");
}
...
template <typename T>
T* Elf::elfCopy(uint8_t* pBuffer, Elf::ElfProgramHeader_t* pProgramHeaders, size_t nProgramHeaders,
                T* pCurrent, size_t size) {
  for (size_t i = 0; i < nProgramHeaders; i++) {
    ...
    if ((ph.vaddr <= reinterpret_cast<uintptr_t>(pCurrent)) &&
      ...
      uintptr_t loc = (reinterpret_cast<uintptr_t>(pCurrent) - ph.vaddr) + ph.offset;
      pCurrent = new T[(size + sizeof(T) - 1) / sizeof(T)];
      MemoryCopy(reinterpret_cast<uint8_t*>(pCurrent), &pBuffer[loc], size);
      return pCurrent;
    }
  }
  return 0;
...
Elf::Elf(const Elf& elf)
  ...
  for (List<char*>::Iterator it = m_NeededLibraries.begin(); it != m_NeededLibraries.end(); it++) {
    *it = *it + diff;
  }
...
bool Elf::createNeededOnly(uint8_t* pBuffer, size_t length) {
  ...
  if (pHeader->phnum > 0) {
    ...
    MemoryCopy(reinterpret_cast<uint8_t*>(m_pProgramHeaders), &pBuffer[pHeader->phoff],
    ...
    if (m_pDynamicStringTable) {
      m_pDynamicStringTable = elfCopy(pBuffer, m_pProgramHeaders, m_nProgramHeaders,
                                      m_pDynamicStringTable, nDynamicStringTableSize);

      // Make sure the string references to needed libraries actually work
      // as pointers...
      for (List<char*>::Iterator it = m_NeededLibraries.begin(); it != m_NeededLibraries.end();
           it++) {
...
bool Elf::create(uint8_t* pBuffer, size_t length) {
  ...
  MemoryCopy(reinterpret_cast<uint8_t*>(m_pSectionHeaders), &pBuffer[pHeader->shoff],
             pHeader->shnum * sizeof(ElfSectionHeader_t));
  ...
  if (pSymbolTable == 0) {
    ...
    MemoryCopy(reinterpret_cast<uint8_t*>(m_pSymbolTable), &pBuffer[pSymbolTable->offset],
               pSymbolTable->size);
  }
  ...
  if (pStringTable == 0) {
    ...
    MemoryCopy(reinterpret_cast<uint8_t*>(m_pStringTable), &pBuffer[pStringTable->offset],
               m_nStringTableSize);
  }
  ...
  if (pHeader->phnum > 0) {
        ...
        while (pDyn->tag != DT_NULL) {
          switch (pDyn->tag) {
            ...
            case DT_SYMTAB:
              ...
              VERBOSE_NOTICE(" -> " << reinterpret_cast<void*>(m_pDynamicSymbolTable));
              break;
            case DT_STRTAB:
              VERBOSE_NOTICE("DT_STRTAB");
              m_pDynamicStringTable = reinterpret_cast<char*>(pDyn->un.ptr);
              VERBOSE_NOTICE(" -> " << reinterpret_cast<void*>(m_pDynamicStringTable));
              break;
            case DT_SYMENT:
    ...
    if (m_pDynamicSymbolTable) {
      m_pDynamicSymbolTable = elfCopy(pBuffer, m_pProgramHeaders, m_nProgramHeaders,
    ...
    if (m_pRelTable)
      m_pRelTable =
          elfCopy(pBuffer, m_pProgramHeaders, m_nProgramHeaders, m_pRelTable, m_nRelTableSize);
    if (m_pRelaTable)
      m_pRelaTable =
          elfCopy(pBuffer, m_pProgramHeaders, m_nProgramHeaders, m_pRelaTable, m_nRelaTableSize);
    if (m_pPltRelTable)
      m_pPltRelTable =
          elfCopy(pBuffer, m_pProgramHeaders, m_nProgramHeaders, m_pPltRelTable, m_nPltSize);
    if (m_pPltRelaTable)
      m_pPltRelaTable =
          elfCopy(pBuffer, m_pProgramHeaders, m_nProgramHeaders, m_pPltRelaTable, m_nPltSize);
  }
...
bool Elf::loadModule(uint8_t* pBuffer, size_t length, uintptr_t& loadBase, size_t& loadSize,
  ...
  for (size_t i = 0; i < m_nProgramHeaders; ++i) {
    if (m_pProgramHeaders[i].type == PT_LOAD) {
      ...
      MemoryCopy(reinterpret_cast<void*>(baseAddr), pBuffer + m_pProgramHeaders[i].offset,
                 m_pProgramHeaders[i].filesz);
      if (m_pProgramHeaders[i].memsz > m_pProgramHeaders[i].filesz) {
        ByteSet(reinterpret_cast<void*>(baseAddr + m_pProgramHeaders[i].filesz), 0,
...
bool Elf::allocate(uint8_t* pBuffer, size_t length, uintptr_t& loadBase, SymbolTable* pSymtab,
  ...
  if (m_pDynamicSymbolTable && m_pDynamicStringTable) {
    ...
    while (reinterpret_cast<uintptr_t>(pSymbol) <
           reinterpret_cast<uintptr_t>(m_pDynamicSymbolTable) + m_nDynamicSymbolTableSize) {
          ...
          if (pSymbol->shndx != 0) {
            ...
            if (binding == SymbolTable::Weak) {
              ...
              if (value == 0)
                value = ~0;
              if (*pStr != 0) {
                m_SymbolTable.insertMultiple(pSymtab, String(pStr), binding, this, value);
              }
            }
          }
        }
...
bool Elf::load(uint8_t* pBuffer, size_t length, uintptr_t loadBase, SymbolTable* pSymtab,
  ...
  if (m_pRelTable) {
    ...
    for (ElfRel_t* pRel = m_pRelTable; pRel < (m_pRelTable + (m_nRelTableSize / sizeof(ElfRel_t)));
         pRel++) {
...
bool Elf::extractInformation(uint8_t* pBuffer, size_t length, size_t& phdrCount,
...
bool Elf::relocate(uint8_t* pBuffer, uintptr_t length) {
  ...
  for (size_t i = 0; i < m_nSectionHeaders; i++) {
    ...
    ElfSectionHeader_t* pTarget = pSh->info ? &m_pSectionHeaders[pSh->info] : nullptr;

    if (pTarget) {
      const char* pStr = reinterpret_cast<const char*>(m_pShstrtab) + pTarget->name;
    ...
    if (pSh->type == SHT_REL) {
      ...
      for (ElfRel_t* pRel = reinterpret_cast<ElfRel_t*>(&pBuffer[pSh->offset]);
           pRel < reinterpret_cast<ElfRel_t*>(&pBuffer[pSh->offset + pSh->size]); pRel++) {
        bool applied = pTarget ? applyRelocation(*pRel, pTarget)
                               : applyRelocation(*pRel, nullptr, nullptr, m_LoadBase);
        if (!applied)
          return false;
      }
    }
    // How about a relocation with addend?
      ...
      for (ElfRela_t* pRel = reinterpret_cast<ElfRela_t*>(&pBuffer[pSh->offset]);
           pRel < reinterpret_cast<ElfRela_t*>(&pBuffer[pSh->offset + pSh->size]); pRel++) {
        bool applied = pTarget ? applyRelocation(*pRel, pTarget)
                               : applyRelocation(*pRel, nullptr, nullptr, m_LoadBase);
        if (!applied) {
          return false;
...
uintptr_t Elf::applySpecificRelocation(uintptr_t off, SymbolTable* pSymtab, uintptr_t loadBase,
...
void Elf::preallocateSymbols(SymbolTable* pSymtabOverride, SymbolTable* pAdditionalSymtab) {
  ...
  if (m_pDynamicSymbolTable) {
    // quick pass to preallocate for the symbol table
    for (size_t i = 0; i < m_nDynamicSymbolTableSize / sizeof(ElfSymbol_t); i++) {
...
template const char* Elf::lookupSymbol<Elf::ElfSymbol_t>(uintptr_t addr, uintptr_t* startAddr = 0,
                                                         ElfSymbol_t* symbolTable = 0);
#if BITS_64
template const char* Elf::lookupSymbol<Elf::Elf32Symbol_t>(uintptr_t addr, uintptr_t* startAddr = 0,
