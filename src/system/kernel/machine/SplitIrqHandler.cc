...
SplitIrqHandler::SplitIrqHandler(const String& name)
    : HardIrqHandler(),
      ...
      m_Dispatcher(name, 1, dispatchThreaded, this),
      m_StateLock(false),
      m_Quiescing(true),
      m_Stopping(1),
...
SplitIrqHandler::~SplitIrqHandler() {
  if (m_Started || m_RegistrationCount || m_LifecycleBusy || m_AcceptingRegistrations ||
      m_Dispatcher.isInitialised() || __atomic_load_n(&m_PendingWork, __ATOMIC_ACQUIRE)) {
    FATAL(
...
irq_id_t SplitIrqHandler::registerIsaSplitIrq(IrqManager& manager, uint8_t irq,
                                              const IrqPolicy& policy) {
  TerminationDeferral lifecycleTermination;
  SplitLifecycleGuard lifecycle(m_LifecycleBusy);
  if (!lifecycle.owned() || !m_AcceptingRegistrations || !m_Started || m_Stopping ||
      m_RegistrationCount >= MaxRegistrations) {
    return 0;
  }

  const irq_id_t id = manager.registerHardIsaIrqHandler(irq, this, policy);
  if (!id) {
    return 0;
  }
...
irq_id_t SplitIrqHandler::registerPciSplitIrq(IrqManager& manager, Device& device,
                                              const IrqPolicy& policy) {
  TerminationDeferral lifecycleTermination;
  SplitLifecycleGuard lifecycle(m_LifecycleBusy);
  if (!lifecycle.owned() || !m_AcceptingRegistrations || !m_Started || m_Stopping ||
  ...
  const irq_id_t id = manager.registerHardPciIrqHandler(this, &device, policy);
  if (!id) {
    return 0;
  }
...
bool SplitIrqHandler::shutdownSplitIrq() {
  ...
  if (!m_Started) {
    return m_RegistrationCount == 0 && !m_AcceptingRegistrations && !m_Dispatcher.isInitialised() &&
...
HardIrqDisposition SplitIrqHandler::irq(irq_id_t number, InterruptState& state) {
  size_t work = 0;
...
void SplitIrqHandler::dispatchThreaded(void* context, uint8_t, size_t) {
  reinterpret_cast<SplitIrqHandler*>(context)->dispatchThreaded();
}

void SplitIrqHandler::dispatchThreaded() {
  const size_t work = __atomic_exchange_n(&m_PendingWork, static_cast<size_t>(0), __ATOMIC_ACQ_REL);
  if (work) {
    threadedIrq(work);
    {
      LockGuard<Spinlock> guard(m_StateLock);
...
void SplitIrqHandler::setRegistrationPublishedHookForTest(RegistrationPublishedHook hook) {
