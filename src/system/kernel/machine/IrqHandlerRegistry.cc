...
IrqHandlerRegistry::IrqHandlerRegistry()
    : m_Handlers(),
      ...
      m_ThreadedActionMutationWriters(0),
      m_OccurrenceEpochs(),
      m_OccurrenceReaders(),
      m_OccurrenceBoundaryLocks(),
      m_HandlerLock(false),
      m_AdmissionEpoch(0),
      ...
      m_HandlerPrePinHook(nullptr),
      m_HandlerHazardHook(nullptr),
      m_DispatchAbandonHook(nullptr),
      m_OccurrenceCaptureHook(nullptr)
#endif
{
}

size_t IrqHandlerRegistry::makePublication(size_t generation, uint8_t irq, SlotMode mode,
                                           Delivery delivery) {
  return (generation << GenerationShift) | (static_cast<size_t>(irq) << IrqShift) |
         (static_cast<size_t>(delivery) << DeliveryShift) | static_cast<size_t>(mode);
}
...
bool IrqHandlerRegistry::threadedGenerationValid(uint8_t irq, size_t generation) const {
  if (!generation) {
    return false;
  }

  const size_t invalidThrough =
      __atomic_load_n(&m_ThreadedInvalidationGenerations[irq], __ATOMIC_ACQUIRE);
  return !invalidThrough || !generationReached(invalidThrough, generation);
}

size_t* IrqHandlerRegistry::quiescedLane(HandlerSlot& slot, QuiescedLane lane) {
  const size_t index = static_cast<size_t>(lane);
  assert(index < QuiescedLaneCount);
  return &slot.quiescedThreadedGenerations[index];
}

const size_t* IrqHandlerRegistry::quiescedLane(const HandlerSlot& slot, QuiescedLane lane) {
  const size_t index = static_cast<size_t>(lane);
  assert(index < QuiescedLaneCount);
  return &slot.quiescedThreadedGenerations[index];
}
...
bool IrqHandlerRegistry::hasQuiescedGeneration(const HandlerSlot& slot) {
  for (size_t i = 0; i < QuiescedLaneCount; ++i) {
    if (__atomic_load_n(&slot.quiescedThreadedGenerations[i], __ATOMIC_ACQUIRE)) {
      return true;
    }
  }
  return false;
...
void IrqHandlerRegistry::publishSlotQuiesced(HandlerSlot& slot, uint8_t irq,
                                             size_t dispatchGeneration, QuiescedLane lane) {
...
void IrqHandlerRegistry::publishSlotQuiescedValue(HandlerSlot& slot, uint8_t irq,
                                                  size_t dispatchGeneration, QuiescedLane lane) {
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
  HandlerHazardHook mutationHook = __atomic_load_n(&m_HandlerHazardHook, __ATOMIC_ACQUIRE);
  if (mutationHook) {
    mutationHook(__atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE),
                 HandlerHazardStage::BeforeQuiescedExchange);
  }
#endif

  size_t* publicationLane = quiescedLane(slot, lane);
  const size_t published = __atomic_load_n(publicationLane, __ATOMIC_ACQUIRE);
  if (published && generationReached(published, dispatchGeneration)) {
    return;
  }

  __atomic_exchange_n(publicationLane, dispatchGeneration, __ATOMIC_ACQ_REL);
  ...
  if (!threadedGenerationValid(irq, dispatchGeneration)) {
    ...
    __atomic_compare_exchange_n(publicationLane, &stale, static_cast<size_t>(0), false,
...
void IrqHandlerRegistry::invalidateThreadedGenerationFromInterrupt(uint8_t irq,
  ...
  __atomic_exchange_n(&m_ThreadedInvalidationGenerations[irq], throughGeneration, __ATOMIC_ACQ_REL);
}

void IrqHandlerRegistry::invalidateThreadedLine(uint8_t irq, size_t throughGeneration) {
  if (!throughGeneration) {
    return;
  }
  ...
  ThreadedActionMutationCleanup actionMutation(this);
  beginThreadedActionMutation(actionMutation);
  size_t invalidThrough =
      __atomic_load_n(&m_ThreadedInvalidationGenerations[irq], __ATOMIC_ACQUIRE);
  while (!invalidThrough || generationReached(throughGeneration, invalidThrough)) {
    if (__atomic_compare_exchange_n(&m_ThreadedInvalidationGenerations[irq], &invalidThrough,
                                    throughGeneration, false, __ATOMIC_RELEASE, __ATOMIC_ACQUIRE)) {
      break;
    }
  }
  if (invalidThrough && generationReached(invalidThrough, throughGeneration)) {
  ...
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    ...
    if (irqOf(publication) != irq || deliveryOf(publication) != Delivery::Threaded) {
      continue;
    }
    ...
    size_t* actions[] = {&slot.pendingThreadedGeneration, &slot.claimedThreadedGeneration,
    ...
    for (size_t action = 0; action < sizeof(actions) / sizeof(actions[0]); ++action) {
      ...
      while (generation && generationReached(throughGeneration, generation)) {
        if (__atomic_compare_exchange_n(actions[action], &generation, static_cast<size_t>(0), false,
                                        __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {
          break;
        }
      }
...
size_t IrqHandlerRegistry::encodePolicy(const IrqPolicy* policy) {
  ...
  return PolicyValid | (static_cast<size_t>(policy->trigger()) << PolicyTriggerShift) |
...
void IrqHandlerRegistry::decodePolicy(size_t policy, LineConfiguration& configuration) {
  configuration.policyConfigured = policy & PolicyValid;
  if (!configuration.policyConfigured) {
    return;
  }

  configuration.trigger = static_cast<IrqTrigger>((policy >> PolicyTriggerShift) & 3);
  configuration.controllerAck =
      static_cast<IrqControllerAck>((policy >> PolicyControllerAckShift) & 3);
  configuration.lineRelease = static_cast<IrqLineRelease>((policy >> PolicyLineReleaseShift) & 1);
}
...
bool IrqHandlerRegistry::mixedPoliciesCompatible(size_t first, size_t second) {
  ...
  return (first & PolicyMixedCompatibilityMask) == (second & PolicyMixedCompatibilityMask);
}

size_t IrqHandlerRegistry::effectiveMixedPolicy(size_t hard, size_t threaded) {
...
IrqHandlerRegistry::LineMode IrqHandlerRegistry::lineModeForDelivery(Delivery delivery) {
  return delivery == Delivery::Threaded ? LineMode::Threaded : LineMode::HardOnly;
}
...
void IrqHandlerRegistry::beginMutation() {
  ...
  __atomic_add_fetch(&m_MutationWriters, static_cast<size_t>(1), __ATOMIC_SEQ_CST);
}
...
void IrqHandlerRegistry::finishMutation() {
  ...
  __atomic_add_fetch(&m_MutationGeneration, static_cast<size_t>(1), __ATOMIC_SEQ_CST);
  __atomic_sub_fetch(&m_MutationWriters, static_cast<size_t>(1), __ATOMIC_SEQ_CST);
}

void IrqHandlerRegistry::beginThreadedActionMutation(ThreadedActionMutationCleanup& cleanup) {
  const bool interruptsWereEnabled = Processor::getInterrupts();
  Processor::setInterrupts(false);
  cleanup.thread = Processor::information().getCurrentThread();
  if (cleanup.thread) {
    cleanup.thread->armAtomicStateCleanup(cleanup.cleanup, abandonThreadedActionMutation, &cleanup);
  }
  __atomic_add_fetch(&m_ThreadedActionMutationWriters, static_cast<size_t>(1), __ATOMIC_SEQ_CST);
  Processor::setInterrupts(interruptsWereEnabled);
}

void IrqHandlerRegistry::finishThreadedActionMutation(ThreadedActionMutationCleanup& cleanup) {
  const bool interruptsWereEnabled = Processor::getInterrupts();
  Processor::setInterrupts(false);
...
void IrqHandlerRegistry::completeThreadedActionMutation() {
  __atomic_add_fetch(&m_ThreadedActionMutationGeneration, static_cast<size_t>(1), __ATOMIC_SEQ_CST);
  const size_t previous = __atomic_fetch_sub(&m_ThreadedActionMutationWriters,
                                             static_cast<size_t>(1), __ATOMIC_SEQ_CST);
  if (!previous) {
    __atomic_store_n(&m_ThreadedActionMutationWriters, static_cast<size_t>(0), __ATOMIC_SEQ_CST);
...
bool IrqHandlerRegistry::canWaitForActionFinalization() {
  ...
  bool canWait = current && Processor::getInterrupts() && !Processor::inDeviceHardIrq();
#if HOSTED
...
bool IrqHandlerRegistry::acquireFinalizationGate(HandlerSlot& slot, bool canWait) {
  bool contentionReported = false;
  while (true) {
    size_t expected = 0;
    if (__atomic_compare_exchange_n(&slot.finalizationGate, &expected, static_cast<size_t>(1),
                                    false, __ATOMIC_ACQUIRE, __ATOMIC_RELAXED)) {
      return true;
    }
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
    if (!contentionReported) {
      HandlerHazardHook hook = __atomic_load_n(&m_HandlerHazardHook, __ATOMIC_ACQUIRE);
      if (hook) {
        hook(__atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE),
...
void IrqHandlerRegistry::releaseFinalizationGate(HandlerSlot& slot) {
  __atomic_store_n(&slot.finalizationGate, static_cast<size_t>(0), __ATOMIC_RELEASE);
}

bool IrqHandlerRegistry::pinActionMutation(HandlerSlot& slot, size_t publication,
...
bool IrqHandlerRegistry::closeSlotAdmission(HandlerSlot& slot, size_t expectedPublication,
  ...
  if (!acquireFinalizationGate(slot, canWait)) {
    if (modeOf(originalPublication) == SlotMode::Draining) {
      ...
      __atomic_compare_exchange_n(
          ...
          makePublication(generationOf(originalPublication), irq, SlotMode::Enabled,
  ...
  while (!__atomic_compare_exchange_n(&m_OccurrenceBoundaryLocks[graceBucket], &boundaryExpected,
  ...
  const size_t occurrenceEpoch =
      __atomic_load_n(&m_OccurrenceEpochs[graceBucket], __ATOMIC_SEQ_CST);
  size_t boundary = occurrenceEpoch + 1;
  if (!boundary) {
    boundary = 1;
  }
  __atomic_store_n(&slot.retirementEpoch, boundary, __ATOMIC_SEQ_CST);
  const size_t cancellingPublication =
      makePublication(generationOf(expectedPublication), irq, SlotMode::Cancelling,
                      deliveryOf(expectedPublication));
  beginMutation();
  if (!__atomic_compare_exchange_n(&slot.publication, &expectedPublication, cancellingPublication,
                                   false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
    __atomic_store_n(&slot.retirementEpoch, static_cast<size_t>(0), __ATOMIC_SEQ_CST);
    finishMutation();
    __atomic_store_n(&m_OccurrenceBoundaryLocks[graceBucket], static_cast<size_t>(0),
                     __ATOMIC_RELEASE);
    releaseFinalizationGate(slot);
    return false;
  }
  ...
  __atomic_store_n(&m_OccurrenceEpochs[graceBucket], boundary, __ATOMIC_SEQ_CST);

#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
  HandlerHazardHook boundaryHook = __atomic_load_n(&m_HandlerHazardHook, __ATOMIC_ACQUIRE);
  if (boundaryHook) {
    boundaryHook(__atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE),
                 HandlerHazardStage::RetirementBoundaryPublished);
  }
#endif

  closedPublication = makePublication(generationOf(cancellingPublication), irq, SlotMode::Closed,
                                      deliveryOf(cancellingPublication));
  size_t expectedCancelling = cancellingPublication;
  const bool closed =
      __atomic_compare_exchange_n(&slot.publication, &expectedCancelling, closedPublication, false,
...
bool IrqHandlerRegistry::occurrencePrecedesRetirement(const HandlerSlot& slot,
                                                      AdmissionCutoff admissionCutoff) const {
  const size_t retirementEpoch = __atomic_load_n(&slot.retirementEpoch, __ATOMIC_SEQ_CST);
  return retirementEpoch && !generationReached(admissionCutoff.occurrenceEpoch, retirementEpoch);
}

void IrqHandlerRegistry::tryReclaimTombstones(uint8_t irq) {
  const size_t graceBucket = irq % GraceBucketCount;
  if (__atomic_load_n(&m_OccurrenceReaders[graceBucket][0], __ATOMIC_SEQ_CST) ||
      __atomic_load_n(&m_OccurrenceReaders[graceBucket][1], __ATOMIC_SEQ_CST)) {
    return;
  }

  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    HandlerSlot& slot = m_Handlers[i];
    size_t publication = __atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST);
    const uint8_t slotIrq = irqOf(publication);
    if (modeOf(publication) != SlotMode::Tombstone || slotIrq % GraceBucketCount != graceBucket ||
        __atomic_load_n(&slot.pendingThreadedGeneration, __ATOMIC_ACQUIRE) ||
        __atomic_load_n(&slot.claimedThreadedGeneration, __ATOMIC_ACQUIRE) ||
        hasQuiescedGeneration(slot)) {
      continue;
    }

    const size_t reclaimingPublication = makePublication(
        generationOf(publication), slotIrq, SlotMode::Retiring, deliveryOf(publication));
    if (!__atomic_compare_exchange_n(&slot.publication, &publication, reclaimingPublication, false,
                                     __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
      continue;
    }
    if (hasActiveDispatch(slot, publication)) {
      size_t reclaiming = reclaimingPublication;
      if (!__atomic_compare_exchange_n(&slot.publication, &reclaiming, publication, false,
                                       __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
        FATAL_NOLOCK("IRQ tombstone changed while action mutation was pinned.");
      }
      continue;
    }
    ...
    __atomic_store_n(&slot.admissionEpoch, static_cast<size_t>(0), __ATOMIC_RELEASE);
    __atomic_store_n(&slot.retirementEpoch, static_cast<size_t>(0), __ATOMIC_RELEASE);
    __atomic_store_n(&slot.publication,
                     makePublication(generationOf(reclaimingPublication), InvalidIrq,
                                     SlotMode::Empty, Delivery::Threaded),
                     __ATOMIC_SEQ_CST);
  }
}

bool IrqHandlerRegistry::retireSlot(HandlerSlot& slot, size_t expectedPublication,
                                    IrqHandlerBase* expectedHandler) {
  beginMutation();
  const size_t retiringPublication =
      makePublication(generationOf(expectedPublication), irqOf(expectedPublication),
                      SlotMode::Retiring, deliveryOf(expectedPublication));
  if (!__atomic_compare_exchange_n(&slot.publication, &expectedPublication, retiringPublication,
                                   false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
    finishMutation();
    return false;
  }
  ...
  if (hasActiveDispatch(slot, expectedPublication)) {
    ...
    if (!__atomic_compare_exchange_n(&slot.publication, &retiring, expectedPublication, false,
                                     __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
      FATAL_NOLOCK("IRQ slot changed while action mutation was pinned.");
    }
    finishMutation();
    return false;
  }
  ...
  if (deliveryOf(retiringPublication) == Delivery::Threaded) {
    ...
    for (size_t action = 0; action < 2; ++action) {
      ...
      while (generation) {
        ...
        publishSlotQuiesced(slot, irq, generation, QuiescedLane::Retirement);
        if (__atomic_compare_exchange_n(actions[action], &generation, static_cast<size_t>(0), false,
                                        __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {
          break;
        }
  ...
  const size_t hardHandoffState =
      __atomic_exchange_n(&slot.hardHandoffState, static_cast<size_t>(0), __ATOMIC_ACQ_REL);
  if (hardHandoffState & 1U) {
    __atomic_add_fetch(&m_HardHandoffEpochs[irq % GraceBucketCount], static_cast<size_t>(1),
                       __ATOMIC_ACQ_REL);
  }
  __atomic_store_n(&slot.handler, nullptr, __ATOMIC_RELEASE);
  __atomic_store_n(&slot.policy, static_cast<size_t>(0), __ATOMIC_RELEASE);
  __atomic_store_n(&slot.publication,
                   makePublication(generationOf(retiringPublication), irq, SlotMode::Tombstone,
...
bool IrqHandlerRegistry::retireSlotOrObserveClosed(HandlerSlot& slot, size_t expectedPublication,
                                                   IrqHandlerBase* expectedHandler) {
  ...
  const size_t publication = __atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST);
  return modeOf(publication) == SlotMode::Empty || modeOf(publication) == SlotMode::Closed ||
         modeOf(publication) == SlotMode::Retiring || modeOf(publication) == SlotMode::Tombstone ||
...
bool IrqHandlerRegistry::unpublishDispatch(void* token, HandlerSlot& slot,
  ...
  if (committed && callback && !required && controllerGeneration &&
    ...
    publishSlotQuiesced(slot, irqOf(admittedPublication), controllerGeneration,
                        QuiescedLane::Callback);
    size_t claimed = controllerGeneration;
    __atomic_compare_exchange_n(&slot.claimedThreadedGeneration, &claimed, static_cast<size_t>(0),
...
void IrqHandlerRegistry::abandonDispatch(void* context) {
  ...
  if (dispatch->callback) {
    DispatchAbandonHook hook =
        __atomic_load_n(&dispatch->registry->m_DispatchAbandonHook, __ATOMIC_ACQUIRE);
    if (hook) {
...
void IrqHandlerRegistry::restoreDispatchInterruptState(DispatchCleanup& dispatch) {
...
bool IrqHandlerRegistry::hasActiveDispatch(HandlerSlot& target, size_t admittedPublication) const {
  for (size_t i = 0; i < MaxActiveDispatches; ++i) {
    ...
    if (slot == &target && generationOf(dispatchPublication) == generationOf(admittedPublication) &&
...
bool IrqHandlerRegistry::findCurrentDispatch(void* owner, HandlerSlot* target,
                                             size_t targetPublication,
  ...
  for (size_t i = 0; i < MaxActiveDispatches; ++i) {
    ...
    const size_t generation = __atomic_load_n(&dispatch.generation, __ATOMIC_ACQUIRE);
    void* dispatchOwner = __atomic_load_n(&dispatch.owner, __ATOMIC_RELAXED);
    HandlerSlot* slot = __atomic_load_n(&dispatch.slot, __ATOMIC_SEQ_CST);
    const size_t dispatchPublication =
        __atomic_load_n(&dispatch.admittedPublication, __ATOMIC_RELAXED);
    if (__atomic_load_n(&dispatch.token, __ATOMIC_ACQUIRE) != token ||
    ...
    if (dispatchOwner == owner && slot && __atomic_load_n(&dispatch.callback, __ATOMIC_RELAXED)) {
      callbackContext = true;
      foundTarget |= slot == target && target &&
                     generationOf(dispatchPublication) == generationOf(targetPublication);
    }
  }
  return foundTarget;
}
...
bool IrqHandlerRegistry::registerHandler(uint8_t irq, IrqHandlerBase* handler, Delivery delivery,
                                         size_t policy) {
  if (!handler || (delivery != Delivery::Threaded && delivery != Delivery::HardOnly)) {
    return false;
  }
  ...
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    ...
    if (mode == SlotMode::Empty || mode == SlotMode::Cancelling || mode == SlotMode::Closed ||
        mode == SlotMode::Retiring || mode == SlotMode::Tombstone || irqOf(publication) != irq) {
      continue;
    }
    const Delivery existingDelivery = deliveryOf(publication);
    const size_t existingPolicy = __atomic_load_n(&slot.policy, __ATOMIC_ACQUIRE);
    if ((existingDelivery == delivery && existingPolicy != policy) ||
        (existingDelivery != delivery && !mixedPoliciesCompatible(existingPolicy, policy))) {
      return false;
    }
  ...
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    ...
    if (modeOf(publication) == SlotMode::Empty &&
      ...
      beginMutation();
      size_t admissionEpoch = __atomic_load_n(&m_AdmissionEpoch, __ATOMIC_RELAXED) + 1;
      if (!admissionEpoch) {
        ++admissionEpoch;
      }
      __atomic_store_n(&slot.pendingThreadedGeneration, static_cast<size_t>(0), __ATOMIC_RELEASE);
      __atomic_store_n(&slot.claimedThreadedGeneration, static_cast<size_t>(0), __ATOMIC_RELEASE);
      for (size_t lane = 0; lane < QuiescedLaneCount; ++lane) {
        __atomic_store_n(&slot.quiescedThreadedGenerations[lane], static_cast<size_t>(0),
                         __ATOMIC_RELEASE);
      }
      __atomic_store_n(&slot.retirementEpoch, static_cast<size_t>(0), __ATOMIC_RELEASE);
      __atomic_store_n(&slot.hardHandoffState, delivery == Delivery::HardOnly ? generation << 1 : 0,
                       __ATOMIC_RELEASE);
      ...
      __atomic_store_n(&slot.admissionEpoch, admissionEpoch, __ATOMIC_RELEASE);
      __atomic_store_n(&slot.publication,
      ...
      __atomic_store_n(&m_AdmissionEpoch, admissionEpoch, __ATOMIC_RELEASE);
      finishMutation();
      return true;
    }
  }
...
bool IrqHandlerRegistry::registerThreadedHandler(uint8_t irq, IrqHandler* handler) {
...
bool IrqHandlerRegistry::registerThreadedHandler(uint8_t irq, IrqHandler* handler,
  ...
  return policy.validForThreaded() &&
         registerHandler(irq, handler, Delivery::Threaded, encodePolicy(&policy));
}

bool IrqHandlerRegistry::registerHardHandler(uint8_t irq, HardIrqHandler* handler) {
...
bool IrqHandlerRegistry::registerHardHandler(uint8_t irq, HardIrqHandler* handler,
  ...
  return policy.validForHard() &&
         registerHandler(irq, handler, Delivery::HardOnly, encodePolicy(&policy));
}
...
IrqHandlerRegistry::UnregisterResult IrqHandlerRegistry::unregisterHandler(
  ...
  if (!canYield || callbackContext) {
    ...
    for (size_t i = 0; i < MaxHandlerSlots; ++i) {
      ...
      size_t publication = __atomic_load_n(&candidate.publication, __ATOMIC_SEQ_CST);
      if (modeOf(publication) != SlotMode::Enabled || irqOf(publication) != irq ||
          __atomic_load_n(&candidate.handler, __ATOMIC_ACQUIRE) != handler) {
        continue;
      }

      removedDelivery = lineModeForDelivery(deliveryOf(publication));
      bool currentTargetDispatch = false;
      if (findCurrentDispatch(owner, &candidate, publication, currentTargetDispatch)) {
        size_t closedPublication = 0;
        const bool deferred = closeSlotAdmission(candidate, publication, closedPublication);
        if (deferred) {
      ...
      const size_t drainingPublication = makePublication(
          generationOf(publication), irq, SlotMode::Draining, deliveryOf(publication));
      beginMutation();
      const bool draining =
          __atomic_compare_exchange_n(&candidate.publication, &publication, drainingPublication,
                                      false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
      finishMutation();
      if (!draining) {
        return UnregisterResult::Rejected;
      ...
      if (hasActiveDispatch(candidate, drainingPublication)) {
        ...
        __atomic_compare_exchange_n(
            ...
            makePublication(generationOf(drainingPublication), irq, SlotMode::Enabled,
                            deliveryOf(drainingPublication)),
            false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
        finishMutation();
        return UnregisterResult::Rejected;
      }
      ...
      size_t closedPublication = 0;
      if (!closeSlotAdmission(candidate, drainingPublication, closedPublication)) {
        return UnregisterResult::Rejected;
      }

      return retireSlotOrObserveClosed(candidate, closedPublication, handler)
  ...
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    size_t candidatePublication = __atomic_load_n(&m_Handlers[i].publication, __ATOMIC_SEQ_CST);
    if (modeOf(candidatePublication) != SlotMode::Empty && irqOf(candidatePublication) == irq &&
        __atomic_load_n(&m_Handlers[i].handler, __ATOMIC_ACQUIRE) == handler) {
      slot = &m_Handlers[i];
  ...
  removedDelivery = lineModeForDelivery(deliveryOf(publication));
  const bool selfUnregister = findCurrentDispatch(owner, slot, publication, callbackContext);
  if (selfUnregister) {
    ...
    size_t closedPublication = 0;
    const bool deferred = closeSlotAdmission(*slot, publication, closedPublication);
    m_HandlerLock.release();
    return deferred ? UnregisterResult::Deferred : UnregisterResult::Rejected;
  }
  ...
  if (!hasActiveDispatch(*slot, closedPublication)) {
    m_HandlerLock.acquire();
    const bool retired = retireSlotOrObserveClosed(*slot, closedPublication, handler);
    m_HandlerLock.release();
    return retired ? UnregisterResult::Completed : UnregisterResult::Rejected;
  }

  uintptr_t previousDebugAddress = 0;
  const Thread::DebugState previousDebugState = current->getDebugState(previousDebugAddress);
  current->setDebugState(Thread::CallbackDrain, reinterpret_cast<uintptr_t>(handler));
...
bool IrqHandlerRegistry::captureAdmissionCutoff(uint8_t irq, AdmissionCutoff& cutoff) {
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
  observeOccurrenceCaptureForTest(irq, OccurrenceCaptureStage::BankZeroClaimed, 0);
#endif
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
  observeOccurrenceCaptureForTest(irq, OccurrenceCaptureStage::BankOneClaimed, 0);
#endif
  ...
  const size_t occurrenceEpoch =
      __atomic_load_n(&m_OccurrenceEpochs[graceBucket], __ATOMIC_SEQ_CST);
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
  observeOccurrenceCaptureForTest(irq, OccurrenceCaptureStage::EpochSampled, occurrenceEpoch);
#endif
  const size_t readerBank = occurrenceEpoch & 1;
  releaseOccurrenceReaderLeases(irq, readerBank ^ 1, 1);
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
  observeOccurrenceCaptureForTest(irq, OccurrenceCaptureStage::UnusedBankReleased, occurrenceEpoch);
#endif
  const size_t admissionEpoch = __atomic_load_n(&m_AdmissionEpoch, __ATOMIC_ACQUIRE);
  cutoff = {admissionEpoch, occurrenceEpoch, (static_cast<size_t>(irq) * 2) + readerBank + 1};
  return true;
}

bool IrqHandlerRegistry::captureMixedAdmissionCutoffs(uint8_t irq, MixedAdmissionCutoffs& cutoffs) {
  cutoffs = {};
  const size_t graceBucket = irq % GraceBucketCount;
...
#endif
  const size_t admissionEpoch = __atomic_load_n(&m_AdmissionEpoch, __ATOMIC_ACQUIRE);
  const AdmissionCutoff cutoff = {admissionEpoch, occurrenceEpoch,
                                  (static_cast<size_t>(irq) * 2) + readerBank + 1};
  cutoffs = {cutoff, cutoff};
  return true;
}

bool IrqHandlerRegistry::acquireOccurrenceReaderLeases(uint8_t irq, size_t readerBank,
                                                       size_t count) {
  ...
  const size_t previous =
      __atomic_fetch_add(&m_OccurrenceReaders[graceBucket][readerBank], count, __ATOMIC_SEQ_CST);
  if (previous > ~static_cast<size_t>(0) - count) {
    __atomic_fetch_sub(&m_OccurrenceReaders[graceBucket][readerBank], count, __ATOMIC_SEQ_CST);
...
void IrqHandlerRegistry::releaseOccurrenceReaderLeases(uint8_t irq, size_t readerBank,
                                                       size_t count) {
  if (readerBank > 1 || !count) {
...
void IrqHandlerRegistry::beginAdmissionCutoffCleanup(AdmissionCutoffCleanup& cleanup) {
  const bool interruptsWereEnabled = Processor::getInterrupts();
  Processor::setInterrupts(false);
  cleanup.thread = Processor::information().getCurrentThread();
  ...
  if (cleanup.thread && cleanup.ownsCutoff) {
    ...
    // tombstone.
    cleanup.thread->armAtomicStateCleanup(cleanup.cleanup, abandonAdmissionCutoff, &cleanup);
  }
  Processor::setInterrupts(interruptsWereEnabled);
}

void IrqHandlerRegistry::finishAdmissionCutoffCleanup(AdmissionCutoffCleanup& cleanup) {
  const bool interruptsWereEnabled = Processor::getInterrupts();
  Processor::setInterrupts(false);
...
void IrqHandlerRegistry::abandonAdmissionCutoff(void* context) {
  AdmissionCutoffCleanup* cleanup = reinterpret_cast<AdmissionCutoffCleanup*>(context);
...
bool IrqHandlerRegistry::dispatchHard(uint8_t irq, InterruptState& state,
  ...
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    ...
    const size_t publication = __atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST);
    const SlotMode mode = modeOf(publication);
    if (irqOf(publication) != irq || deliveryOf(publication) != Delivery::HardOnly) {
      continue;
    }
    const size_t admissionEpoch = __atomic_load_n(&slot.admissionEpoch, __ATOMIC_ACQUIRE);
    if (!admissionEpoch ||
        (admissionEpoch != cutoffEpoch && generationReached(admissionEpoch, cutoffEpoch))) {
      continue;
    }

    if (mode == SlotMode::Cancelling || mode == SlotMode::Closed || mode == SlotMode::Retiring ||
        mode == SlotMode::Tombstone) {
    ...
    IrqHandlerBase* handler = __atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE);
    if (!handler || (onlyHandler && handler != static_cast<IrqHandlerBase*>(onlyHandler))) {
      continue;
    }

#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
    HandlerPrePinHook prePinHook = __atomic_load_n(&m_HandlerPrePinHook, __ATOMIC_ACQUIRE);
    if (prePinHook) {
      prePinHook(handler);
    }
#endif
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
    HandlerHazardHook hazardHook = __atomic_load_n(&m_HandlerHazardHook, __ATOMIC_ACQUIRE);
    if (hazardHook) {
    ...
    size_t currentPublication = __atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST);
    ...
    const bool sameLifetime =
        ...
        irqOf(currentPublication) == irq && deliveryOf(currentPublication) == Delivery::HardOnly &&
    ...
    if (sameLifetime && currentMode == SlotMode::Draining) {
      ...
      const size_t enabledPublication = makePublication(generationOf(currentPublication), irq,
                                                        SlotMode::Enabled, Delivery::HardOnly);
      beginMutation();
      __atomic_compare_exchange_n(&slot.publication, &expectedPublication, enabledPublication,
                                  false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
      finishMutation();
      currentPublication = __atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST);
...

    {
      ...
      dispatchCleanup.previousInterruptState = Processor::getInterrupts();
  ...
  finishAdmissionCutoffCleanup(cutoffCleanup);
  if (disposition == HardIrqDisposition::KeepMasked && !hardLineQuarantined(irq)) {
    disposition = HardIrqDisposition::Handled;
  }
  return admitted;
}
...
bool IrqHandlerRegistry::hardLineQuarantined(uint8_t irq) const {
  ...
  for (size_t attempt = 0; attempt < 2; ++attempt) {
    const size_t startEpoch = __atomic_load_n(&m_HardHandoffEpochs[epochBucket], __ATOMIC_ACQUIRE);
    for (size_t i = 0; i < MaxHandlerSlots; ++i) {
      const HandlerSlot& slot = m_Handlers[i];
      const size_t publication = __atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST);
      const SlotMode mode = modeOf(publication);
      if ((mode != SlotMode::Enabled && mode != SlotMode::Draining) || irqOf(publication) != irq ||
          deliveryOf(publication) != Delivery::HardOnly) {
        continue;
      }

      const size_t generation = generationOf(publication);
      if (__atomic_load_n(&slot.hardHandoffState, __ATOMIC_ACQUIRE) != ((generation << 1) | 1U)) {
        continue;
      }

      const size_t currentPublication = __atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST);
      const SlotMode currentMode = modeOf(currentPublication);
      if ((currentMode == SlotMode::Enabled || currentMode == SlotMode::Draining) &&
    ...
    if (startEpoch == __atomic_load_n(&m_HardHandoffEpochs[epochBucket], __ATOMIC_ACQUIRE)) {
      return false;
    }
  }
...
bool IrqHandlerRegistry::publishThreadedDispatch(uint8_t irq, size_t dispatchGeneration) {
  AdmissionCutoff admissionCutoff = {};
  if (!captureAdmissionCutoff(irq, admissionCutoff)) {
    return false;
  }
  return publishThreadedDispatch(irq, dispatchGeneration, admissionCutoff);
}
...
bool IrqHandlerRegistry::publishThreadedDispatch(uint8_t irq, size_t dispatchGeneration,
  ...
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    ...
    if (mode == SlotMode::Cancelling || mode == SlotMode::Closed || mode == SlotMode::Retiring ||
        mode == SlotMode::Tombstone) {
      if (occurrencePrecedesRetirement(slot, admissionCutoff)) {
        publishSlotQuiesced(slot, irq, dispatchGeneration, QuiescedLane::Controller);
        admitted = true;
      }
      continue;
    }
    ...
    IrqHandlerBase* handler = __atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE);
    if (handler) {
      candidates[candidateCount++] = {&slot, publication, handler, admissionEpoch};
    }
  }
  ...
  for (size_t i = 0; i < candidateCount; ++i) {
    ...
    void* owner = currentDispatchOwner();
    Thread* thread = Processor::information().getCurrentThread();
    DispatchCleanup dispatchCleanup(this, &slot, owner, publication, false);
    if (thread) {
    ...
    const bool sameLifetime =
        generationOf(currentPublication) == generationOf(publication) &&
        irqOf(currentPublication) == irq && deliveryOf(currentPublication) == Delivery::Threaded &&
        __atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE) == handler &&
        __atomic_load_n(&slot.admissionEpoch, __ATOMIC_ACQUIRE) == admissionEpoch;
    if (sameLifetime && (currentMode == SlotMode::Enabled || currentMode == SlotMode::Draining) &&
        threadedGenerationValid(irq, dispatchGeneration)) {
      if (currentMode == SlotMode::Draining) {
        size_t expectedPublication = currentPublication;
        const size_t enabledPublication = makePublication(generationOf(currentPublication), irq,
                                                          SlotMode::Enabled, Delivery::Threaded);
        beginMutation();
        __atomic_compare_exchange_n(&slot.publication, &expectedPublication, enabledPublication,
                                    false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
        finishMutation();
      ...
      if (currentMode == SlotMode::Enabled) {
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
        HandlerHazardHook publicationHook = __atomic_load_n(&m_HandlerHazardHook, __ATOMIC_ACQUIRE);
        if (publicationHook) {
          publicationHook(handler, HandlerHazardStage::BeforePendingExchange);
        }
#endif

        const size_t pending = __atomic_load_n(&slot.pendingThreadedGeneration, __ATOMIC_ACQUIRE);
        if (!pending || !generationReached(pending, dispatchGeneration)) {
          ...
          __atomic_exchange_n(&slot.pendingThreadedGeneration, dispatchGeneration,
                              __ATOMIC_ACQ_REL);
        }

#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
        publicationHook = __atomic_load_n(&m_HandlerHazardHook, __ATOMIC_ACQUIRE);
        if (publicationHook) {
          publicationHook(handler, HandlerHazardStage::PendingExchanged);
        }
#endif

        if (!threadedGenerationValid(irq, dispatchGeneration)) {
          size_t stale = dispatchGeneration;
          __atomic_compare_exchange_n(&slot.pendingThreadedGeneration, &stale,
                                      static_cast<size_t>(0), false, __ATOMIC_ACQ_REL,
                                      __ATOMIC_ACQUIRE);
        }
      }

      const size_t finalPublication = __atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST);
      ...
      if (generationOf(finalPublication) != generationOf(publication) ||
          ...
          (finalMode != SlotMode::Enabled && finalMode != SlotMode::Draining) ||
          __atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE) != handler ||
          __atomic_load_n(&slot.admissionEpoch, __ATOMIC_ACQUIRE) != admissionEpoch) {
        publishSlotQuiesced(slot, irq, dispatchGeneration, QuiescedLane::Controller);
      }
...
bool IrqHandlerRegistry::dispatchThreaded(uint8_t irq, size_t dispatchGeneration,
  ...
  while (true) {
    if (__atomic_load_n(&m_ThreadedActionMutationWriters, __ATOMIC_SEQ_CST)) {
      Scheduler::instance().yield();
      continue;
    }
    const size_t actionMutationGeneration =
        __atomic_load_n(&m_ThreadedActionMutationGeneration, __ATOMIC_SEQ_CST);
    if (__atomic_load_n(&m_ThreadedActionMutationWriters, __ATOMIC_SEQ_CST)) {
      Scheduler::instance().yield();
      continue;
    }
    ...
    for (size_t i = 0; i < MaxHandlerSlots; ++i) {
      ...
      for (size_t lane = 0; lane < QuiescedLaneCount; ++lane) {
        size_t* publicationLane = &slot.quiescedThreadedGenerations[lane];
        size_t quiesced = __atomic_load_n(publicationLane, __ATOMIC_ACQUIRE);
        if (quiesced && !threadedGenerationValid(irq, quiesced)) {
          __atomic_compare_exchange_n(publicationLane, &quiesced, static_cast<size_t>(0), false,
                                      __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE);
          continue;
        }
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
      HandlerHazardHook quiescedHook = __atomic_load_n(&m_HandlerHazardHook, __ATOMIC_ACQUIRE);
      if (quiescedHook) {
        quiescedHook(__atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE),
      ...
      if (!threadedGenerationValid(irq, pending)) {
        ...
        __atomic_compare_exchange_n(&slot.pendingThreadedGeneration, &stale, static_cast<size_t>(0),
                                    false, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE);
        continue;
      }
    ...
    for (size_t i = 0; i < candidateCount; ++i) {
      ...
      size_t pending = __atomic_load_n(&slot.pendingThreadedGeneration, __ATOMIC_ACQUIRE);
      bool claimed = false;
      size_t claimedGeneration = 0;
      while (pending && generationReached(dispatchGeneration, pending)) {
        size_t emptyClaim = 0;
        if (!__atomic_compare_exchange_n(&slot.claimedThreadedGeneration, &emptyClaim, pending,
                                         false, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {
          break;
        }
        ...
        if (__atomic_compare_exchange_n(&slot.pendingThreadedGeneration, &exactPending,
                                        static_cast<size_t>(0), false, __ATOMIC_ACQ_REL,
                                        __ATOMIC_ACQUIRE)) {
          claimed = true;
          break;
        ...
        __atomic_compare_exchange_n(&slot.claimedThreadedGeneration, &exactClaim,
                                    static_cast<size_t>(0), false, __ATOMIC_ACQ_REL,
                                    __ATOMIC_ACQUIRE);
      ...
      size_t finalPublication = __atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST);
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
      HandlerHazardHook finalizationHook = __atomic_load_n(&m_HandlerHazardHook, __ATOMIC_ACQUIRE);
      if (finalizationHook) {
        finalizationHook(handler, HandlerHazardStage::BeforeClaimFinalization);
      }
#endif
      if (generationOf(finalPublication) == generationOf(publication) &&
          irqOf(finalPublication) == irq && deliveryOf(finalPublication) == Delivery::Threaded &&
          modeOf(finalPublication) == SlotMode::Draining &&
          __atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE) == handler) {
        size_t expectedPublication = finalPublication;
        beginMutation();
        __atomic_compare_exchange_n(&slot.publication, &expectedPublication, publication, false,
                                    __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
        finishMutation();
        finalPublication = __atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST);
      }
      if (finalPublication == publication) {
        ...
        publishSlotQuiesced(slot, irq, claimedGeneration, QuiescedLane::Callback);
        size_t exactClaim = claimedGeneration;
        __atomic_compare_exchange_n(&slot.claimedThreadedGeneration, &exactClaim,
                                    static_cast<size_t>(0), false, __ATOMIC_ACQ_REL,
                                    __ATOMIC_ACQUIRE);
    ...
    for (size_t i = 0; i < MaxHandlerSlots; ++i) {
      ...
      size_t pending = __atomic_load_n(&slot.pendingThreadedGeneration, __ATOMIC_ACQUIRE);
      size_t claimed = __atomic_load_n(&slot.claimedThreadedGeneration, __ATOMIC_ACQUIRE);
      const bool pendingReached = pending && generationReached(dispatchGeneration, pending);
      const bool claimedReached = claimed && generationReached(dispatchGeneration, claimed);
      ...
      const SlotMode mode = modeOf(publication);
      if (mode == SlotMode::Draining || mode == SlotMode::Cancelling || mode == SlotMode::Closed ||
    ...
    const size_t finalActionMutationWriters =
        __atomic_load_n(&m_ThreadedActionMutationWriters, __ATOMIC_SEQ_CST);
    const size_t finalActionMutationGeneration =
        __atomic_load_n(&m_ThreadedActionMutationGeneration, __ATOMIC_SEQ_CST);
    if (finalActionMutationWriters || finalActionMutationGeneration != actionMutationGeneration) {
      continue;
    }
...
size_t IrqHandlerRegistry::handlerCount(uint8_t irq) {
  size_t count = 0;
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    const size_t publication = __atomic_load_n(&m_Handlers[i].publication, __ATOMIC_SEQ_CST);
    const SlotMode mode = modeOf(publication);
    if ((mode == SlotMode::Enabled || mode == SlotMode::Draining) && irqOf(publication) == irq) {
      ++count;
    }
...
IrqHandlerRegistry::LineMode IrqHandlerRegistry::lineMode(uint8_t irq) {
  ...
  return threaded ? LineMode::Threaded : hard ? LineMode::HardOnly : LineMode::Empty;
}

bool IrqHandlerRegistry::snapshotLineConfiguration(uint8_t irq,
                                                   LineConfiguration& configuration) const {
  for (size_t attempt = 0; attempt < LineSnapshotAttempts; ++attempt) {
    if (__atomic_load_n(&m_MutationWriters, __ATOMIC_SEQ_CST)) {
      continue;
    }
    const size_t generation = __atomic_load_n(&m_MutationGeneration, __ATOMIC_SEQ_CST);
    if (__atomic_load_n(&m_MutationWriters, __ATOMIC_SEQ_CST)) {
      continue;
    }
    ...
    for (size_t i = 0; i < MaxHandlerSlots; ++i) {
      ...
      const size_t policy = __atomic_load_n(&slot.policy, __ATOMIC_ACQUIRE);
      if (__atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST) != publication) {
        consistent = false;
        break;
      }
    ...
    const size_t finalWriters = __atomic_load_n(&m_MutationWriters, __ATOMIC_SEQ_CST);
    const size_t finalGeneration = __atomic_load_n(&m_MutationGeneration, __ATOMIC_SEQ_CST);
...
size_t IrqHandlerRegistry::hardDispatchState(uint8_t irq, size_t& exactGeneration) const {
  size_t count = 0;
  exactGeneration = 0;
  for (size_t i = 0; i < MaxActiveDispatches; ++i) {
    const ActiveDispatch& dispatch = m_ActiveDispatches[i];
    ...
    const size_t generation = __atomic_load_n(&dispatch.generation, __ATOMIC_ACQUIRE);
    HandlerSlot* slot = __atomic_load_n(&dispatch.slot, __ATOMIC_SEQ_CST);
    const size_t publication = __atomic_load_n(&dispatch.admittedPublication, __ATOMIC_RELAXED);
    const size_t controllerGeneration =
        __atomic_load_n(&dispatch.controllerGeneration, __ATOMIC_RELAXED);
    if (slot && irqOf(publication) == irq && deliveryOf(publication) == Delivery::HardOnly &&
        __atomic_load_n(&dispatch.token, __ATOMIC_ACQUIRE) == token &&
        __atomic_load_n(&dispatch.generation, __ATOMIC_ACQUIRE) == generation &&
...
size_t IrqHandlerRegistry::threadedDispatchState(uint8_t irq,
                                                 uintptr_t& exactHandlerIdentity) const {
  size_t count = 0;
  ...
  for (size_t i = 0; i < MaxActiveDispatches; ++i) {
    ...
    HandlerSlot* slot = __atomic_load_n(&dispatch.slot, __ATOMIC_SEQ_CST);
    const size_t publication = __atomic_load_n(&dispatch.admittedPublication, __ATOMIC_RELAXED);
    IrqHandlerBase* handler = slot ? __atomic_load_n(&slot->handler, __ATOMIC_ACQUIRE) : nullptr;
    if (slot && handler && irqOf(publication) == irq &&
        ...
        __atomic_load_n(&dispatch.slot, __ATOMIC_SEQ_CST) == slot) {
      ++count;
      exactHandlerIdentity = count == 1 ? reinterpret_cast<uintptr_t>(handler) : 0;
    }
  }
  return count;
}
...
void IrqHandlerRegistry::setOccurrenceCaptureHookForTest(OccurrenceCaptureHook hook) {
  __atomic_store_n(&m_OccurrenceCaptureHook, hook, __ATOMIC_RELEASE);
}

void IrqHandlerRegistry::observeOccurrenceCaptureForTest(uint8_t irq, OccurrenceCaptureStage stage,
                                                         size_t occurrenceEpoch) {
  OccurrenceCaptureHook hook = __atomic_load_n(&m_OccurrenceCaptureHook, __ATOMIC_ACQUIRE);
  if (hook) {
...
size_t IrqHandlerRegistry::activeDispatchCountForTest(IrqHandlerBase* handler) {
  ...
  for (size_t i = 0; i < MaxActiveDispatches; ++i) {
    ...
    if (activeHandler == handler && __atomic_load_n(&dispatch.callback, __ATOMIC_RELAXED) &&
        __atomic_load_n(&dispatch.token, __ATOMIC_ACQUIRE) == token &&
        __atomic_load_n(&dispatch.generation, __ATOMIC_ACQUIRE) == generation) {
      ++count;
    }
...
size_t IrqHandlerRegistry::claimedDispatchCountForOwnerForTest(void* owner) {
  ...
  for (size_t i = 0; i < MaxActiveDispatches; ++i) {
    ...
    const size_t generation = __atomic_load_n(&dispatch.generation, __ATOMIC_ACQUIRE);
    void* dispatchOwner = __atomic_load_n(&dispatch.owner, __ATOMIC_RELAXED);
    if (dispatchOwner == owner && __atomic_load_n(&dispatch.callback, __ATOMIC_RELAXED) &&
        __atomic_load_n(&dispatch.token, __ATOMIC_ACQUIRE) == token &&
        __atomic_load_n(&dispatch.generation, __ATOMIC_ACQUIRE) == generation) {
      ++count;
    }
...
bool IrqHandlerRegistry::containsHandlerForTest(uint8_t irq, IrqHandlerBase* handler) {
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    HandlerSlot& slot = m_Handlers[i];
    const size_t publication = __atomic_load_n(&slot.publication, __ATOMIC_SEQ_CST);
...
size_t IrqHandlerRegistry::tombstoneCountForTest(uint8_t irq) const {
  ...
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    const size_t publication = __atomic_load_n(&m_Handlers[i].publication, __ATOMIC_SEQ_CST);
    if (modeOf(publication) == SlotMode::Tombstone && irqOf(publication) == irq) {
      ++count;
    }
  }
  return count;
...
size_t IrqHandlerRegistry::threadedActionMutationWriterCountForTest() const {
  return __atomic_load_n(&m_ThreadedActionMutationWriters, __ATOMIC_SEQ_CST);
}

bool IrqHandlerRegistry::setThreadedActionLanesForTest(IrqHandlerBase* handler,
                                                       size_t pendingGeneration,
                                                       size_t claimedGeneration,
                                                       size_t quiescedGeneration) {
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    HandlerSlot& slot = m_Handlers[i];
    if (__atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE) != handler) {
      continue;
    ...
    __atomic_store_n(&slot.pendingThreadedGeneration, pendingGeneration, __ATOMIC_RELEASE);
    __atomic_store_n(&slot.claimedThreadedGeneration, claimedGeneration, __ATOMIC_RELEASE);
    for (size_t lane = 0; lane < QuiescedLaneCount; ++lane) {
      __atomic_store_n(&slot.quiescedThreadedGenerations[lane], static_cast<size_t>(0),
                       __ATOMIC_RELEASE);
    }
    __atomic_store_n(quiescedLane(slot, QuiescedLane::Controller), quiescedGeneration,
                     __ATOMIC_RELEASE);
    return true;
  }
  return false;
...
bool IrqHandlerRegistry::consumeThreadedQuiescedForTest(IrqHandlerBase* handler,
                                                        size_t generation) {
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    HandlerSlot& slot = m_Handlers[i];
    if (__atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE) != handler) {
    ...
    for (size_t lane = 0; lane < QuiescedLaneCount; ++lane) {
      ...
      if (__atomic_compare_exchange_n(&slot.quiescedThreadedGenerations[lane], &exact,
                                      static_cast<size_t>(0), false, __ATOMIC_ACQ_REL,
                                      __ATOMIC_ACQUIRE)) {
        return true;
      }
...
bool IrqHandlerRegistry::publishControllerQuiescedForTest(IrqHandlerBase* handler, uint8_t irq,
                                                          size_t generation) {
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    HandlerSlot& slot = m_Handlers[i];
    if (__atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE) != handler) {
    ...
    publishSlotQuiesced(slot, irq, generation, QuiescedLane::Controller);
    return true;
  }
  return false;
}
