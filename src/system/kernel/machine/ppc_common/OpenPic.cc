...
#error Problems here.
    ...
    // Set the task priority to 0, so all interrupts with priority >0 can be
    // received.
...
OpenPic::OpenPic() : m_pPort(0), m_nIrqs(0)
{
...
void OpenPic::enable(uint8_t irq, bool enable)
{
    ...
    if (enable)
        m_pPort->write32(
            HOST_TO_LITTLE32(receivedReg & ~OPENPIC_SOURCE_MASK), reg);
    else
        m_pPort->write32(
            HOST_TO_LITTLE32(receivedReg | OPENPIC_SOURCE_MASK), reg);
