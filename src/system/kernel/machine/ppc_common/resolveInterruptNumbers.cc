...
size_t resolveInterruptNumber(Device *pDev)
{
    ...
    uint32_t buf[2];
    int ret =
        ofDev.getProperty("interrupts", reinterpret_cast<OFParam>(buf), 8);
    ...
    uint32_t unit_address =
        reinterpret_cast<uint32_t>(ofDev.getProperty("reg"));
    ...
    while (this_node != 0)
    {
        ...
        if (dev.propertyExists("interrupt-controller"))
        {
            if (dev.propertyExists("interrupt-parent"))
            {
                ...
                unit_address =
                    reinterpret_cast<uint32_t>(dev.getProperty("reg"));

                interrupt_specifier =
                    reinterpret_cast<uint32_t>(dev.getProperty("interrupts"));
        ...
        else if (dev.propertyExists("interrupt-map"))
        {
            ...
            dev.getProperty(
                "interrupt-map", reinterpret_cast<OFParam>(map), mapLength);

            uint32_t address_cells = 0;
            if (dev.propertyExists("#address-cells"))
            {
                address_cells = reinterpret_cast<uint32_t>(
                    dev.getProperty("#address-cells"));
            }
            ...
            if (dev.propertyExists("interrupt-map-mask"))
            {
                ...
                dev.getProperty(
                    "interrupt-map-mask", reinterpret_cast<OFParam>(mask),
                    maskLength);
