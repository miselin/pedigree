...
// Routines for taking a 0-255 set of RGB values and converting them to a 8,16
// or 32 bpp specific form.
#ifdef BIG_ENDIAN
...
class PPCVga : public Vga
{
     ...
     */
    virtual bool
    isMode(size_t nCols, size_t nRows, bool bIsText, size_t nBpp = 0)
    {
        return false;
    }
     ...
     * framebuffer.
     *
     * The buffer is assumed to be in the correct format for directly copying
     * into video memory. This will obviously depend on the current mode
     * (text/graphical) as well as resolution and bits per pixel (graphics mode
     * only). \param pBuffer A pointer to the buffer to swap into video memory.
     ...
     *
     * The buffer is assumed to be in the correct format for directly copying
     * from video memory. This will obviously depend on the current mode
     * (text/graphical) as well as resolution and bits per pixel (graphics mode
     * only). \param A pointer to the buffer. \param The length of pBuffer.
     */
    ...
    /** We use a 16-bit-per-character text mode buffer to simplify things.
     * BootIO is a bit of a simpleton, really. */
    uint16_t
        m_pTextBuffer[(MAX_WIDTH / FONT_WIDTH) * (MAX_HEIGHT / FONT_HEIGHT)];

    MemoryRegion m_Framebuffer;

    /** Our real graphics framebuffer
        \todo: This should be a MemoryMappedIO - needs VirtualAddressSpace
       though. */
