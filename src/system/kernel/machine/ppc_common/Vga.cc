...
PPCVga::PPCVga()
    : m_Framebuffer("VGA framebuffer"), m_pFramebuffer(0), m_Width(0),
      m_Height(0), m_Depth(0), m_Stride(0)
{
}
...
void PPCVga::initialise()
{
    ...
    if (!physicalMemoryManager.allocateRegion(
            m_Framebuffer, 0x01000000 / 0x1000,
    ...
    m_pFramebuffer =
        reinterpret_cast<uint8_t *>(m_Framebuffer.virtualAddress());
    ...
    for (unsigned int i = 0;
         i < (m_Width / FONT_WIDTH) * (m_Height / FONT_HEIGHT); i++)
    {
...
void PPCVga::pokeBuffer(uint8_t *pBuffer, size_t nBufLen)
{
    ...
    for (unsigned int i = 0; i < (m_Width / FONT_WIDTH); i++)
    {
        for (unsigned int j = 0; j < (m_Height / FONT_HEIGHT); j++)
        {
            ...
            if ((pBuffer16 == m_pTextBuffer) ||
                (pBuffer16[idx] != m_pTextBuffer[idx]))
            {
...
void PPCVga::putChar(char c, int x, int y, unsigned int f, unsigned int b)
{
    ...
    for (int i = 0; i < FONT_HEIGHT; i++)
    {
        ...
        for (int j = 0; j < FONT_WIDTH; j++)
        {
            ...
            if (m_Depth == 8)
                m_pFramebuffer[y * m_Stride + i * m_Stride + x + j] =
                    col & 0xFF;
            else if (m_Depth == 16)
