...
Device::Device(Device* p)
    : m_Addresses(),
      ...
      m_ClassCode(p->m_ClassCode),
      m_SubclassCode(p->m_SubclassCode),
      m_VendorId(p->m_VendorId),
      m_DeviceId(p->m_DeviceId),
      m_ProgInterface(p->m_ProgInterface),
      m_PciBusPos(p->m_PciBusPos),
      m_PciDevicePos(p->m_PciDevicePos),
...
void Device::removeChild(size_t n) {
  unsigned int i = 0;
  for (Vector<Device*>::Iterator it = m_Children.begin(); it != m_Children.end(); it++, i++) {
    if (i == n) {
      m_Children.erase(it);
      break;
    }
...
void Device::searchByVendorIdAndDeviceId(uint16_t vendorId, uint16_t deviceId,
...
void Device::searchByClassAndSubclass(uint16_t classCode, uint16_t subclassCode,
                                      void (*callback)(Device*), Device* root) {
  RAII_LOCK();
...
void Device::searchByClassSubclassAndProgInterface(uint16_t classCode, uint16_t subclassCode,
                                                   uint8_t progInterface, void (*callback)(Device*),
                                                   Device* root) {
  RAII_LOCK();
  ...
  searchByClassSubclassAndProgInterfaceInternal(classCode, subclassCode, progInterface, callback,
                                                root);
}
...
void Device::searchByVendorIdAndDeviceIdInternal(uint16_t vendorId, uint16_t deviceId,
                                                 void (*callback)(Device*), Device* root) {
  for (unsigned int i = 0; i < root->getNumChildren(); i++) {
    Device* pChild = root->getChild(i);
...
void Device::searchByClassAndSubclassInternal(uint16_t classCode, uint16_t subclassCode,
                                              void (*callback)(Device*), Device* root) {
  for (unsigned int i = 0; i < root->getNumChildren(); i++) {
    Device* pChild = root->getChild(i);
    if ((pChild->getPciClassCode() == classCode) &&
...
void Device::searchByClassSubclassAndProgInterfaceInternal(uint16_t classCode,
...
void Device::Address::map(size_t forcedSize, bool bUser, bool bWriteCombine, bool bWriteThrough) {
  ...
  if (!physicalMemoryManager.allocateRegion(
          *static_cast<MemoryMappedIo*>(m_Io), numPages,
          ...
          (bUser ? 0 : VirtualAddressSpace::KernelMode) | VirtualAddressSpace::Write | cacheFlags,
          m_Address)) {
    ERROR("Device::Address - map for " << Hex << m_Address << " failed!");
  }

  NOTICE("Device::Address: mapped " << Hex << m_Address << " -> "
