...
namespace {
bool rangeContains(uint64_t rangeAddress, uint64_t rangeLength, uint64_t address, uint64_t length) {
  const uint64_t rangeEnd = rangeAddress + rangeLength;
  const uint64_t end = address + length;
  return rangeEnd >= rangeAddress && end >= address && address >= rangeAddress && end <= rangeEnd;
}

bool mappedRangeContains(const MemoryRegion& region, uint64_t address, uint64_t length) {
  return rangeContains(region.physicalAddress(), region.size(), address, length);
}
}  // namespace
...
void Acpi::initialise() {
  ...
  for (size_t i = 0; i < AcpiRanges.size(); ++i) {
    ...
    if (AcpiRanges.getRange(i, candidate) &&
        rangeContains(candidate.address, candidate.length, m_pRsdtPointer->rsdtAddress,
  ...
  if (!foundAcpiRange && g_pBootstrapInfo) {
    ...
    while (memoryMap) {
      const uint64_t address = g_pBootstrapInfo->getMemoryMapEntryAddress(memoryMap);
      const uint64_t length = g_pBootstrapInfo->getMemoryMapEntryLength(memoryMap);
      if (rangeContains(address, length, m_pRsdtPointer->rsdtAddress,
                        sizeof(SystemDescriptionTableHeader))) {
        AcpiRange.address = address;
        AcpiRange.length = length;
        foundAcpiRange = true;
        NOTICE(" ACPI tables are in firmware memory-map type "
  ...
  if (physicalMemoryManager.allocateRegion(
          ...
          VirtualAddressSpace::KernelMode | VirtualAddressSpace::Write, address) == false) {
    ERROR("Acpi: Could not allocate the MemoryRegion");
    return;
  }

  // Check the RSDT (Root System Description Table)
  m_pRsdt = m_AcpiMemoryRegion.convertPhysicalPointer<SystemDescriptionTableHeader>(
      m_pRsdtPointer->rsdtAddress);
  if (m_pRsdt->signature != 0x54445352 || m_pRsdt->length < sizeof(SystemDescriptionTableHeader) ||
      !mappedRangeContains(m_AcpiMemoryRegion, m_pRsdtPointer->rsdtAddress, m_pRsdt->length) ||
  ...
  for (size_t i = 0; i < sEntries; i++) {
    uint32_t* pTable = adjust_pointer(reinterpret_cast<uint32_t*>(m_pRsdt),
                                      sizeof(SystemDescriptionTableHeader) + 4 * i);

    SystemDescriptionTableHeader* pSystemDescTable =
        m_AcpiMemoryRegion.convertPhysicalPointer<SystemDescriptionTableHeader>(*pTable);

    if (!mappedRangeContains(m_AcpiMemoryRegion, *pTable, sizeof(SystemDescriptionTableHeader))) {
    ...
    if (pSystemDescTable->length < sizeof(SystemDescriptionTableHeader) ||
        !mappedRangeContains(m_AcpiMemoryRegion, *pTable, pSystemDescTable->length)) {
...
void Acpi::parseFixedACPIDescriptionTable() {
  ...
  NOTICE(" Interrupt Model: " << Dec
                              << ((m_pFacp->interruptModel == 0) ? "dual 8259 PICs"
  ...
  NOTICE("  Event 1A at " << Hex << m_pFacp->pm1aEventBlock << " - "
                          << (m_pFacp->pm1aEventBlock + m_pFacp->pm1EventLength));
  if (m_pFacp->pm1bEventBlock != 0) {
    NOTICE("  Event 1B at " << Hex << m_pFacp->pm1bEventBlock << " - "
                            << (m_pFacp->pm1bEventBlock + m_pFacp->pm1EventLength));
  }
  NOTICE("  Control 1A at " << Hex << m_pFacp->pm1aControlBlock << " - "
                            << (m_pFacp->pm1aControlBlock + m_pFacp->pm1ControlLength));
  if (m_pFacp->pm1bControlBlock != 0) {
    NOTICE("  Control 1B at " << Hex << m_pFacp->pm1bControlBlock << " - "
                              << (m_pFacp->pm1bControlBlock + m_pFacp->pm1ControlLength));
  }
  if (m_pFacp->pm2ControlBlock != 0) {
    NOTICE("  Control 2 at " << Hex << m_pFacp->pm2ControlBlock << " - "
                             << (m_pFacp->pm2ControlBlock + m_pFacp->pm2ControlLength));
  }
  NOTICE("  Timer at " << Hex << m_pFacp->pmTimerBlock << " - "
                       << (m_pFacp->pmTimerBlock + m_pFacp->pmTimerLength));
  if (m_pFacp->gpe0Block != 0) {
    NOTICE("  General Purpose Event 0 at " << Hex << m_pFacp->gpe0Block << " - "
                                           << (m_pFacp->gpe0Block + m_pFacp->gpe0BlockLength));
  }
  if (m_pFacp->gpe1Block != 0) {
    NOTICE("  General Purpose Event 1 at " << Hex << m_pFacp->gpe1Block << " - "
...
void Acpi::parseMultipleApicDescriptionTable() {
  ...
  for (; pType < reinterpret_cast<uint8_t*>(adjust_pointer(m_pApic, m_pApic->length));) {
    ...
    // Interrupt Source override
      ...
      ERROR(" Interrupt override: " << ((pInterruptSourceOverride->bus == 0) ? "ISA" : "unknown")
