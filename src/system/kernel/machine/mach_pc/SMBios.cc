...
void SMBios::initialise()
{
    ...
    NOTICE(
        "SMBIOS Specification " << Dec << m_pEntryPoint->majorVersion << "."
                                << m_pEntryPoint->minorVersion);
    NOTICE(
        " entry-point at " << Hex
                           << reinterpret_cast<uintptr_t>(m_pEntryPoint));
    NOTICE(
        " tables at " << Hex << m_pEntryPoint->tableAddress << " - "
                      << (m_pEntryPoint->tableAddress +
                          m_pEntryPoint->tableLength));

    // Go through the tables
    Header *pCurHeader =
        reinterpret_cast<Header *>(m_pEntryPoint->tableAddress);
    for (size_t i = 0; i < m_pEntryPoint->structureCount; i++)
    {
        switch (pCurHeader->type)
        {
            case 0:
            {
                BiosInformation *pBiosInfo =
                    reinterpret_cast<BiosInformation *>(
                        adjust_pointer(pCurHeader, sizeof(Header)));
                NOTICE("  BIOS Information:");
                NOTICE(
                    "   vendor \""
                    << getString(pCurHeader, pBiosInfo->vendorIndex) << "\"");
                NOTICE(
                    "   version \""
                    << getString(pCurHeader, pBiosInfo->biosVersionIndex)
                    << "\"");
                NOTICE(
                    "   release data \""
                    << getString(pCurHeader, pBiosInfo->biosReleaseDateIndex)
                    << "\"");
                NOTICE(
                    "   memory: "
                    << Hex
                    << (static_cast<size_t>(pBiosInfo->biosStartSegment) * 16)
                    << " - "
                    << (static_cast<size_t>(pBiosInfo->biosStartSegment) * 16 +
                        64 * 1024 *
                            (static_cast<size_t>(pBiosInfo->biosRomSize) + 1)));
                NOTICE(
                    "   characteristics: " << Hex
            ...
            {
                NOTICE(
                    "  unknown structure (" << Hex << pCurHeader->type << ")");
            }
        }
...
SMBios::Header *SMBios::next(Header *pHeader)
{
    uint16_t *stringTable =
        reinterpret_cast<uint16_t *>(adjust_pointer(pHeader, pHeader->length));
    while (*stringTable != 0)
...
{
    if (::checksum(
            reinterpret_cast<const uint8_t *>(pEntryPoint),
            pEntryPoint->length) == false)
        return false;

    return ::checksum(
        reinterpret_cast<const uint8_t *>(&pEntryPoint->signature2[0]), 0x0F);
}

SMBios::SMBios() : m_pEntryPoint(0)
{
}
SMBios::~SMBios()
{
