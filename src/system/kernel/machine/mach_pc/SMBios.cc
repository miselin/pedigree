...
void SMBios::initialise() {
  ...
  NOTICE("SMBIOS Specification " << Dec << m_pEntryPoint->majorVersion << "."
                                 << m_pEntryPoint->minorVersion);
  NOTICE(" entry-point at " << Hex << reinterpret_cast<uintptr_t>(m_pEntryPoint));
  NOTICE(" tables at " << Hex << m_pEntryPoint->tableAddress << " - "
                       << (m_pEntryPoint->tableAddress + m_pEntryPoint->tableLength));
    ...
    switch (pCurHeader->type) {
      case 0: {
        BiosInformation* pBiosInfo =
            reinterpret_cast<BiosInformation*>(adjust_pointer(pCurHeader, sizeof(Header)));
        ...
        NOTICE("   release data \"" << getString(pCurHeader, pBiosInfo->biosReleaseDateIndex)
                                    << "\"");
        NOTICE("   memory: " << Hex << (static_cast<size_t>(pBiosInfo->biosStartSegment) * 16)
                             << " - "
                             << (static_cast<size_t>(pBiosInfo->biosStartSegment) * 16 +
                                 64 * 1024 * (static_cast<size_t>(pBiosInfo->biosRomSize) + 1)));
...
void SMBios::find() {
  ...
  while (reinterpret_cast<uintptr_t>(m_pEntryPoint) < 0x100000) {
    if (m_pEntryPoint->signature == 0x5F4D535F && m_pEntryPoint->signature2[0] == 0x5F &&
        m_pEntryPoint->signature2[1] == 0x44 && m_pEntryPoint->signature2[2] == 0x4D &&
        m_pEntryPoint->signature2[3] == 0x49 && m_pEntryPoint->signature2[4] == 0x5F &&
