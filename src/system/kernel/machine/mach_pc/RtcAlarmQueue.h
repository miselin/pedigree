...
class RtcAlarmQueue {
 public:
    ...
    for (Record* record = m_pFirst; record; record = record->m_pNext) {
      ...
      if (record->m_bDispatching) {
        if (record->m_pDispatchOwner == owner) {
          ...
          return {RemovalDisposition::SelfDeferred, nullptr, record->m_Deadline};
        }
        return {RemovalDisposition::RemoteInFlight, nullptr, record->m_Deadline};
      }
  ...
  Record* removeAllQueued(void* event, void* owner, bool& remoteInFlight, bool& selfDeferred) {
