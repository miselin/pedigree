...
void Smp::initialise() {
  ...
  if (m_pFloatingPointer->features[0] != 0) {
    ERROR("smp: default configurations (#" << Dec << m_pFloatingPointer->features[0]
                                           << ") not supported");
    return;
  }
  ...
  if (m_pConfigTable->signature != 0x504D4350 || checksum(m_pConfigTable) != true ||
  ...
  uint8_t* pType =
      reinterpret_cast<uint8_t*>(adjust_pointer(m_pConfigTable, sizeof(ConfigTableHeader)));
  for (size_t i = 0; i < m_pConfigTable->entryCount; i++) {
    ...
    if (*pType == 0) {
      ...
      NOTICE("  Processor #" << Dec << pProcessor->localApicId
      ...
      if (bUsable) {
        ...
        Multiprocessor::ProcessorInformation* pProcessorInfo =
            new Multiprocessor::ProcessorInformation(pProcessor->localApicId,
    ...
    // I/O APIC entry?
      ...
      NOTICE("  I/O APIC #" << Dec << pIoApic->id << (bUsable ? " usable" : "") << " at " << Hex
                            << pIoApic->address);
      ...
      if (bUsable) {
        ...
        Multiprocessor::IoApicInformation* pIoApicInfo =
            new Multiprocessor::IoApicInformation(pIoApic->id, pIoApic->address);
...
Smp::FloatingPointer* Smp::find(void* pMemory, size_t sMemory) {
  ...
  while (reinterpret_cast<uintptr_t>(pFloatingPointer) <
    ...
    if (pFloatingPointer->signature == 0x5F504D5F && checksum(pFloatingPointer) == true)
...
bool Smp::checksum(const FloatingPointer* pFloatingPointer) {
  return ::checksum(reinterpret_cast<const uint8_t*>(pFloatingPointer),
                    pFloatingPointer->length * 16);
}

bool Smp::checksum(const ConfigTableHeader* pConfigTable) {
  // Base table checksum
  if (::checksum(reinterpret_cast<const uint8_t*>(pConfigTable), pConfigTable->baseTableLength) ==
      false)
    return false;
