...
constexpr uint8_t RtcInterruptEnableMask =
    RtcPeriodicInterruptEnable | RtcAlarmInterruptEnable | RtcUpdateEndedInterruptEnable;
...
// during initialise1().
constexpr Time::Timestamp RtcUpdateTimeout = 25 * Time::Multiplier::Millisecond;
...
static uint64_t addAlarmDuration(uint64_t deadline, size_t count, uint64_t multiplier) {
...
bool Rtc::CmosTransactionGuard::waitableContext() {
  return Processor::information().getCurrentThread() && Processor::getInterrupts() &&
         !Processor::inDeviceHardIrq();
}

Rtc::CmosTransactionGuard::CmosTransactionGuard(Mutex& lock)
    : m_Waitable(waitableContext()),
      m_TerminationDeferral(m_Waitable),
      m_Lock(lock),
      m_Owned(false) {
...
void Rtc::drainRemoteAlarmDispatch(Event* pEvent, void* owner) {
  ...
  while (true) {
    ...
    if (canYield) {
      if (!debuggingDrain) {
        previousDebugState = current->getDebugState(previousDebugAddress);
        current->setDebugState(Thread::CallbackDrain, reinterpret_cast<uintptr_t>(pEvent));
        debuggingDrain = true;
      }
...
size_t Rtc::removeAlarm(class Event* pEvent, bool bRetZero) {
  ...
  if (removal.disposition == RtcAlarmQueue::RemovalDisposition::RemoteInFlight) {
    drainRemoteAlarmDispatch(pEvent, owner);
    return 0;
  }
  if (removal.disposition != RtcAlarmQueue::RemovalDisposition::Removed || bRetZero ||
      removal.deadline < currTime) {
    return 0;
  }
...
uint64_t Rtc::getTickCountNano() {
  ...
  uint64_t candidate = PcTscClock::fromAnchor(tsc, anchorTsc, anchorNanoseconds, m_TscCalibration);
...
void Rtc::processElapsedTime(uint64_t observed) {
  ...
  while (true) {
    m_Lock.acquire();
    Alarm* claimed = m_AlarmQueue.claimDue(m_ProcessedTickCount, currentAlarmDispatchOwner());
    m_Lock.release();
  ...
  if (UNLIKELY(delta >= Time::Multiplier::Millisecond ||
...
bool Rtc::waitForUpdateCompletion(uint8_t index) {
  ...
  while (true) {
    ...
    if (++polls >= RtcUpdateMaximumPolls || (Time::getTicks() - started) >= RtcUpdateTimeout) {
      ERROR(
...
void Rtc::setPeriodicInterruptEnabled(bool enabled) {
  ...
  const uint8_t quiescentStatus = static_cast<uint8_t>(status & ~RtcInterruptEnableMask);
  writeLocked(0x0B, enabled ? static_cast<uint8_t>(quiescentStatus | RtcPeriodicInterruptEnable)
                            : quiescentStatus);
