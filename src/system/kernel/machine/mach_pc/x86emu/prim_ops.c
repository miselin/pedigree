...
u8 rol_byte(u8 d, u8 s)
{
    ...
    if ((cnt = s % 8) != 0)
    {
        ...
        CONDITIONAL_SET_FLAG(
            s == 1 && XOR2((res & 0x1) + ((res >> 6) & 0x2)), F_OF);
    }
    if (s != 0)
    {
        /* set the new carry flag, Note that it is the low order
...
u16 rol_word(u16 d, u8 s)
{
    ...
    if ((cnt = s % 16) != 0)
    {
        ...
        CONDITIONAL_SET_FLAG(res & 0x1, F_CF);
        CONDITIONAL_SET_FLAG(
            s == 1 && XOR2((res & 0x1) + ((res >> 14) & 0x2)), F_OF);
    }
    if (s != 0)
    {
        /* set the new carry flag, Note that it is the low order
...
u32 rol_long(u32 d, u8 s)
{
    ...
    if ((cnt = s % 32) != 0)
    {
        ...
        CONDITIONAL_SET_FLAG(res & 0x1, F_CF);
        CONDITIONAL_SET_FLAG(
            s == 1 && XOR2((res & 0x1) + ((res >> 30) & 0x2)), F_OF);
    }
    if (s != 0)
    {
        /* set the new carry flag, Note that it is the low order
...
u8 shl_byte(u8 d, u8 s)
{
    ...
    if (s < 8)
    {
        ...
        if (cnt == 1)
        {
            ...
            CONDITIONAL_SET_FLAG(
                (((res & 0x80) == 0x80) ^ (ACCESS_FLAG(F_CF) != 0)),
...
u16 shl_word(u16 d, u8 s)
{
    ...
    if (s < 16)
    {
        ...
        {
            CONDITIONAL_SET_FLAG(
                (((res & 0x8000) == 0x8000) ^ (ACCESS_FLAG(F_CF) != 0)), F_OF);
        }
        else
        {
            CLEAR_FLAG(F_OF);
...
u32 shl_long(u32 d, u8 s)
{
    ...
    if (s < 32)
    {
        ...
        {
            CONDITIONAL_SET_FLAG(
                (((res & 0x80000000) == 0x80000000) ^ (ACCESS_FLAG(F_CF) != 0)),
                F_OF);
        }
        else
        {
...
void idiv_byte(u8 s)
{
    ...
    div = dvd / (s8) s;
    mod = dvd % (s8) s;
    /* Exclaim modification - use __builtin_abs */
...
void idiv_long(u32 s)
{
    ...
    do
    {
        ...
        {
            abs_h_dvd -= (h_s + carry);
            l_dvd = carry ? ((0xFFFFFFFF - l_s) + l_dvd + 1) : (l_dvd - l_s);
            h_s >>= 1;
            l_s = abs_s << (--counter);
            div |= 1;
            continue;
...
void ins(int size)
{
    ...
    if (M.x86.mode & (SYSMODE_PREFIX_REPE | SYSMODE_PREFIX_REPNE))
    {
        ...
        /* in until CX is ZERO. */
        u32 count =
            ((M.x86.mode & SYSMODE_PREFIX_DATA) ? M.x86.R_ECX : M.x86.R_CX);
        switch (size)
        {
            case 1:
                while (count--)
                {
                    store_data_byte_abs(
                        M.x86.R_ES, M.x86.R_DI, (*sys_inb)(M.x86.R_DX));
                    M.x86.R_DI += inc;
                }
                break;
            ...
            case 2:
                ...
                {
                    store_data_word_abs(
                        M.x86.R_ES, M.x86.R_DI, (*sys_inw)(M.x86.R_DX));
                    M.x86.R_DI += inc;
                }
                break;
            case 4:
                ...
                {
                    store_data_long_abs(
                        M.x86.R_ES, M.x86.R_DI, (*sys_inl)(M.x86.R_DX));
                    M.x86.R_DI += inc;
                    break;
                }
        }
...
void outs(int size)
{
    ...
    else
    {
        switch (size)
        {
            case 1:
                (*sys_outb)(
                    M.x86.R_DX, fetch_data_byte_abs(M.x86.R_ES, M.x86.R_SI));
                break;
            case 2:
                (*sys_outw)(
                    M.x86.R_DX, fetch_data_word_abs(M.x86.R_ES, M.x86.R_SI));
                break;
            case 4:
                (*sys_outl)(
                    M.x86.R_DX, fetch_data_long_abs(M.x86.R_ES, M.x86.R_SI));
                break;
        }
        M.x86.R_SI += inc;
    }
...
{
    DB(if (CHECK_MEM_ACCESS()) x86emu_check_mem_access(addr);)
    return (*sys_rdw)(addr);
}
...
{
    DB(if (CHECK_SP_ACCESS()) x86emu_check_sp_access();)
    M.x86.R_SP -= 2;
