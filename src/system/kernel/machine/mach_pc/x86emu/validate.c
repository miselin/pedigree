...
#define VAL_FAIL_BYTE_BYTE_BINARY(name)                                      \
    if (failed)                                                              \
        printk("fail\n");                                                    \
    printk(                                                                  \
        "0x%02X = %-15s(0x%02X,0x%02X), flags = %s -> %s\n", r, #name, d, s, \
        print_flags(buf1, inflags), print_flags(buf2, M.x86.R_EFLG));        \
    printk(                                                                  \
        "0x%02X = %-15s(0x%02X,0x%02X), flags = %s -> %s\n", r_asm,          \
        #name "_asm", d, s, print_flags(buf1, inflags),                      \
        print_flags(buf2, flags));

#define VAL_FAIL_WORD_WORD_BINARY(name)                                      \
    if (failed)                                                              \
        printk("fail\n");                                                    \
    printk(                                                                  \
        "0x%04X = %-15s(0x%04X,0x%04X), flags = %s -> %s\n", r, #name, d, s, \
        print_flags(buf1, inflags), print_flags(buf2, M.x86.R_EFLG));        \
    printk(                                                                  \
        "0x%04X = %-15s(0x%04X,0x%04X), flags = %s -> %s\n", r_asm,          \
        #name "_asm", d, s, print_flags(buf1, inflags),                      \
...
#define VAL_FAIL_LONG_LONG_BINARY(name)                                      \
    if (failed)                                                              \
        printk("fail\n");                                                    \
    printk(                                                                  \
        "0x%08X = %-15s(0x%08X,0x%08X), flags = %s -> %s\n", r, #name, d, s, \
        print_flags(buf1, inflags), print_flags(buf2, M.x86.R_EFLG));        \
    printk(                                                                  \
        "0x%08X = %-15s(0x%08X,0x%08X), flags = %s -> %s\n", r_asm,          \
        #name "_asm", d, s, print_flags(buf1, inflags),                      \
...
#define VAL_START_TERNARY(                                      \
    parm_type, res_type, dmax, smax, dincr, sincr, maxshift)    \
    {                                                           \
        parm_type d, s;                                         \
        res_type r, r_asm;                                      \
        u8 shift;                                               \
...
#define VAL_FAIL_WORD_WORD_TERNARY(name)                                     \
    if (failed)                                                              \
        printk("fail\n");                                                    \
    printk(                                                                  \
        "0x%04X = %-15s(0x%04X,0x%04X,%d), flags = %s -> %s\n", r, #name, d, \
        s, shift, print_flags(buf1, inflags),                                \
        print_flags(buf2, M.x86.R_EFLG));                                    \
    printk(                                                                  \
        "0x%04X = %-15s(0x%04X,0x%04X,%d), flags = %s -> %s\n", r_asm,       \
        #name "_asm", d, s, shift, print_flags(buf1, inflags),               \
        print_flags(buf2, flags));
...
#define VAL_FAIL_LONG_LONG_TERNARY(name)                                     \
    if (failed)                                                              \
        printk("fail\n");                                                    \
    printk(                                                                  \
        "0x%08X = %-15s(0x%08X,0x%08X,%d), flags = %s -> %s\n", r, #name, d, \
        s, shift, print_flags(buf1, inflags),                                \
        print_flags(buf2, M.x86.R_EFLG));                                    \
    printk(                                                                  \
        "0x%08X = %-15s(0x%08X,0x%08X,%d), flags = %s -> %s\n", r_asm,       \
...
#define VAL_LONG_ROTATE_DBL(name)                                   \
    printk("Validating %s ... ", #name);                            \
    VAL_START_TERNARY(                                              \
        u32, u32, 0xFF000000, 0xFF000000, 0x1000000, 0x1000000, 32) \
    VAL_TEST_TERNARY(name)                                          \
...
#define VAL_FAIL_BYTE_UNARY(name)                                             \
    printk("fail\n");                                                         \
    printk(                                                                   \
        "0x%02X = %-15s(0x%02X), flags = %s -> %s\n", r, #name, d,            \
        print_flags(buf1, inflags), print_flags(buf2, M.x86.R_EFLG));         \
    printk(                                                                   \
        "0x%02X = %-15s(0x%02X), flags = %s -> %s\n", r_asm, #name "_asm", d, \
        print_flags(buf1, inflags), print_flags(buf2, flags));

#define VAL_FAIL_WORD_UNARY(name)                                             \
    printk("fail\n");                                                         \
    printk(                                                                   \
        "0x%04X = %-15s(0x%04X), flags = %s -> %s\n", r, #name, d,            \
        print_flags(buf1, inflags), print_flags(buf2, M.x86.R_EFLG));         \
    printk(                                                                   \
        "0x%04X = %-15s(0x%04X), flags = %s -> %s\n", r_asm, #name "_asm", d, \
        print_flags(buf1, inflags), print_flags(buf2, flags));
                ...
                {                                                           \
                    ...
                    {                                                       \
                        if (failed)                                         \
                            printk("fail\n");                               \
                        printk(                                             \
                            "0x%04X = %-15s(0x%02X,0x%02X), flags = %s -> " \
                            "%s\n",                                         \
                            r, #name, d, s, print_flags(buf1, inflags),     \
                            print_flags(buf2, M.x86.R_EFLG));               \
                        printk(                                             \
                            "0x%04X = %-15s(0x%02X,0x%02X), flags = %s -> " \
                            "%s\n",                                         \
        ...
        {                                                                    \
            ...
            {                                                                \
                ...
                {                                                            \
                    ...
                    r_lo = M.x86.R_AX;                                       \
                    r_hi = M.x86.R_DX;                                       \
                    if (r_lo != r_asm_lo || r_hi != r_asm_hi ||              \
                        M.x86.R_EFLG != flags)                               \
                        failed = true;                                       \
                    if (failed || trace)                                     \
                    {                                                        \
                        if (failed)                                          \
                            printk("fail\n");                                \
                        printk(                                              \
                            "0x%04X:0x%04X = %-15s(0x%04X,0x%04X), flags = " \
                            "%s -> %s\n",                                    \
                            r_hi, r_lo, #name, d, s,                         \
                            print_flags(buf1, inflags),                      \
                            print_flags(buf2, M.x86.R_EFLG));                \
                        printk(                                              \
                            "0x%04X:0x%04X = %-15s(0x%04X,0x%04X), flags = " \
                            "%s -> %s\n",                                    \
                            r_asm_hi, r_asm_lo, #name "_asm", d, s,          \
                            print_flags(buf1, inflags),                      \
                            print_flags(buf2, flags));                       \
                    }                                                        \
                    M.x86.R_EFLG = inflags = flags =                         \
                ...
                {                                                            \
                    ...
                    {                                                        \
                        if (failed)                                          \
                            printk("fail\n");                                \
                        printk(                                              \
                            "0x%08X:0x%08X = %-15s(0x%08X,0x%08X), flags = " \
                            "%s -> %s\n",                                    \
                            r_hi, r_lo, #name, d, s,                         \
                            print_flags(buf1, inflags),                      \
                            print_flags(buf2, M.x86.R_EFLG));                \
        ...
        {                                                                    \
            ...
            {                                                                \
                ...
                {                                                            \
                    ...
                    if (r_quot != r_asm_quot || r_rem != r_asm_rem ||        \
                        M.x86.R_EFLG != flags)                               \
                        failed = true;                                       \
                    if (failed || trace)                                     \
                    {                                                        \
                        if (failed)                                          \
                            printk("fail\n");                                \
                        printk(                                              \
                            "0x%02X:0x%02X = %-15s(0x%04X,0x%02X), flags = " \
                            "%s -> %s\n",                                    \
                            r_quot, r_rem, #name, d, s,                      \
                            print_flags(buf1, inflags),                      \
                            print_flags(buf2, M.x86.R_EFLG));                \
                        printk(                                              \
                            "0x%02X:0x%02X = %-15s(0x%04X,0x%02X), flags = " \
                            "%s -> %s\n",                                    \
                            r_asm_quot, r_asm_rem, #name "_asm", d, s,       \
                            print_flags(buf1, inflags),                      \
                            print_flags(buf2, flags));                       \
                    }                                                        \
                    M.x86.R_EFLG = inflags = flags =                         \
            ...
            {                                                                 \
                ...
                {                                                             \
                    ...
                    if (M.x86.intr & INTR_SYNCH)                              \
                        continue;                                             \
                    name##_asm(                                               \
                        &flags, &r_asm_quot, &r_asm_rem, d & 0xFFFF, d >> 16, \
                        s);                                                   \
                    if (r_quot != r_asm_quot || r_rem != r_asm_rem ||         \
                        M.x86.R_EFLG != flags)                                \
                        failed = true;                                        \
                    ...
                    {                                                         \
                        if (failed)                                           \
                            printk("fail\n");                                 \
                        printk(                                               \
                            "0x%04X:0x%04X = %-15s(0x%08X,0x%04X), flags = "  \
                            "%s -> %s\n",                                     \
                            r_quot, r_rem, #name, d, s,                       \
                            print_flags(buf1, inflags),                       \
                            print_flags(buf2, M.x86.R_EFLG));                 \
                ...
                {                                                           \
                    ...
                    {                                                       \
                        if (failed)                                         \
                            printk("fail\n");                               \
                        printk(                                             \
                            "0x%08X:0x%08X = %-15s(0x%08X:0x%08X,0x%08X), " \
                            "flags = %s -> %s\n",                           \
                            r_quot, r_rem, #name, 0, d, s,                  \
                            print_flags(buf1, inflags),                     \
                            print_flags(buf2, M.x86.R_EFLG));               \
                        printk(                                             \
                            "0x%08X:0x%08X = %-15s(0x%08X:0x%08X,0x%08X), " \
                            "flags = %s -> %s\n",                           \
                            r_asm_quot, r_asm_rem, #name "_asm", 0, d, s,   \
                            print_flags(buf1, inflags),                     \
                            print_flags(buf2, flags));                      \
                    }                                                       \
                    M.x86.R_EFLG = inflags = flags =                        \
