...
char X86Keyboard::getCharNonBlock() {
  if (m_pPs2Controller->getDebugState()) {
    ...
    ERROR(
        "Keyboard::getCharNonBlock should not be called outside debug "
        "mode");
    return 0;
  }
}
...
char X86Keyboard::scancodeToAscii(uint8_t scancode) {
  ...
  if (!KeymapManager::instance().handleHidModifier(keyCode, !(scancode & 0x80)) &&
      !(scancode & 0x80))
...
void X86Keyboard::readerThread() {
  while (true) {
    ...
    if (!m_pPs2Controller->readFirstPort(scancode)) {
      ...
      if (thread && thread->getUnwindState() != Thread::Continue) {
        return;
      }
      continue;
    }
    ...
    // Get the HID keycode corresponding to the scancode
    uint8_t keyCode =
        KeymapManager::instance().convertPc102ScancodeToHidKeycode(scancode, m_Escape);
    if (!keyCode) {
