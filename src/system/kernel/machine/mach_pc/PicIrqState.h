    ...
    void rebuildMask()
    {
        ...
        for (size_t i = 0; i < LineCount; ++i)
        {
            if (!m_RequestedEnabled[i] || m_AcknowledgementPending[i] ||
                m_ThreadedPending[i] || m_TransitionPending[i])
            {
                mask |= bit(i);
            }
        }
...
inline PicContentionLineResult resolvePicContentionLine(
    PicIrqState &state, size_t irq, size_t realEntries, size_t deferredTails,
    size_t deferredTailEois)
{
    ...
    result.invalidateThreaded = delivery == IrqDelivery::Threaded ||
...
class PicControllerStateGate
{
  public:
    ...
    static constexpr size_t CountMask =
        (static_cast<size_t>(1) << CountBits) - 1;
    static constexpr size_t LifetimeMask =
    ...
    struct PendingActions
    {
        ...
        bool empty() const
        {
            for (size_t i = 0; i < LineCount; ++i)
            {
                if (entry[i] || tail[i] || tailEoi[i] || staleEntry[i] ||
                    staleTail[i] || staleTailEoi[i])
                {
                    return false;
                }
            }
    ...
    bool tryAcquireClean()
    {
        ...
        return __atomic_compare_exchange_n(
            &m_State, &expected, Owner, false, __ATOMIC_ACQUIRE,
            __ATOMIC_RELAXED);
    }
    ...
    bool tryClaim()
    {
        ...
        return __atomic_compare_exchange_n(
            &m_State, &expected, state | Owner, false, __ATOMIC_ACQ_REL,
            __ATOMIC_RELAXED);
    }
    ...
    bool queueEntry(size_t irq, size_t lifetime)
    {
        assert(irq < LineCount);
        queueTagged(
            m_EntryCounts[irq], m_StaleEntryCounts[irq], lifetime);
    ...
    bool queueTail(size_t irq, size_t lifetime, bool owesEoi)
    {
        ...
        {
            queueTagged(
                m_TailEoiCounts[irq], m_StaleTailEoiCounts[irq], lifetime);
        }
        else
        {
            queueTagged(
                m_TailNoEoiCounts[irq], m_StaleTailNoEoiCounts[irq],
                lifetime);
        }
        publishPending(irq == 0);
        return tryClaim();
    ...
    bool takePending(PendingActions &actions)
    {
        ...
        do
        {
            ...
            &m_State, &expected, Owner, false, __ATOMIC_ACQ_REL,
            __ATOMIC_ACQUIRE));
        ...
        for (size_t i = 0; i < LineCount; ++i)
        {
            ...
            takeTagged(
                m_EntryCounts[i], m_StaleEntryCounts[i], lifetime,
                actions.entry[i], actions.staleEntry[i]);

            size_t tailNoEoi = 0;
            size_t staleTailNoEoi = 0;
            takeTagged(
                m_TailNoEoiCounts[i], m_StaleTailNoEoiCounts[i], lifetime,
                tailNoEoi, staleTailNoEoi);
            takeTagged(
                m_TailEoiCounts[i], m_StaleTailEoiCounts[i], lifetime,
                actions.tailEoi[i], actions.staleTailEoi[i]);
            actions.tail[i] = tailNoEoi + actions.tailEoi[i];
            actions.staleTail[i] =
                staleTailNoEoi + actions.staleTailEoi[i];
        }
        return true;
    }
    ...
    bool releaseIfIdle()
    {
        ...
        return __atomic_compare_exchange_n(
            &m_State, &expected, Clean, false, __ATOMIC_RELEASE,
            __ATOMIC_RELAXED);
    }
    ...
    bool relinquishOwnerForContinuation()
    {
        ...
        return __atomic_compare_exchange_n(
            &m_State, &expected, Pending, false, __ATOMIC_RELEASE,
            __ATOMIC_ACQUIRE);
    }
  ...
  private:
    ...
    {
        return ((lifetime & LifetimeMask) << CountBits) |
               (count & CountMask);
    }
    ...
    static void queueTagged(
        size_t &currentCounter, size_t &staleCounter, size_t lifetime)
    {
        ...
        size_t expected =
            __atomic_load_n(&currentCounter, __ATOMIC_ACQUIRE);
        const size_t count = taggedCount(expected);
        if (taggedLifetime(expected) != qualifiedLifetime ||
            count == CountMask ||
            !__atomic_compare_exchange_n(
                &currentCounter, &expected,
                tagged(qualifiedLifetime, count + 1), false,
    ...
    static void takeTagged(
        size_t &currentCounter, size_t &staleCounter, size_t lifetime,
        size_t &current, size_t &stale)
    {
        const size_t value = __atomic_exchange_n(
            &currentCounter, tagged(lifetime, 0), __ATOMIC_ACQ_REL);
        ...
        stale += __atomic_exchange_n(
            &staleCounter, static_cast<size_t>(0), __ATOMIC_ACQ_REL);
    }

    void replaceLifetime(size_t irq, size_t lifetime)
    {
        size_t moved = 0;
        size_t value = __atomic_exchange_n(
            &m_EntryCounts[irq], tagged(lifetime, 0), __ATOMIC_ACQ_REL);
        moved += taggedCount(value);
        addStale(m_StaleEntryCounts[irq], taggedCount(value));

        value = __atomic_exchange_n(
            &m_TailNoEoiCounts[irq], tagged(lifetime, 0), __ATOMIC_ACQ_REL);
        moved += taggedCount(value);
        addStale(m_StaleTailNoEoiCounts[irq], taggedCount(value));

        value = __atomic_exchange_n(
            &m_TailEoiCounts[irq], tagged(lifetime, 0), __ATOMIC_ACQ_REL);
        moved += taggedCount(value);
        addStale(m_StaleTailEoiCounts[irq], taggedCount(value));

        __atomic_store_n(
            &m_LineLifetimes[irq], lifetime, __ATOMIC_RELEASE);
        if (moved)
        {
            publishPending(irq == 0);
        }
    ...
    {
        __atomic_fetch_or(
            &m_State, Pending | (urgent ? Urgent : 0), __ATOMIC_RELEASE);
    }
...
inline PicHardTailPlan resolvePicHardTail(
    const PicHardTailRecord &record, const PicHardTailCurrentState &current)
{
    PicHardTailPlan plan = {};
    plan.controllerLifetimeCurrent =
        record.controllerLifetime == current.controllerLifetime;
    plan.hardStageLifetimeCurrent =
        record.hardStageGeneration == current.hardStageGeneration;
    plan.threadedLifetimeCurrent =
        record.hasThreadedStage &&
        record.threadedCookie == current.threadedCookie &&
    ...
    {
        plan.threadedAction = record.threadedPublished ?
                                  PicHardTailThreadedAction::Publish :
                                  PicHardTailThreadedAction::MissingPublication;
    }
    else if (
        record.dispatchGeneration == current.dispatchGeneration &&
        current.hardHandlerCount &&
        (current.delivery == IrqDelivery::Hard ||
         current.delivery == IrqDelivery::Mixed))
    {
...
inline PicHardTailDoorbellResult resolvePicHardTailDoorbell(
    bool staged, bool accepted)
{
...
{
  public:
    PicHardTailTerminalSequence(
        bool temporaryMaskRequired, IrqControllerAck controllerAck)
        : m_TemporaryMaskRequired(temporaryMaskRequired),
          m_TemporaryMaskApplied(false), m_ControllerAck(controllerAck),
  ...
  public:
    ...
    bool publish(uint8_t irq, const PicHardTailRecord &record)
    {
        ...
        if (!__atomic_compare_exchange_n(
                &m_States[irq], &expected, Claimed, false,
                __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE))
        {
            return false;
        }
    ...
    void complete(uint8_t irq)
    {
        ...
        const bool completed = __atomic_compare_exchange_n(
            &m_States[irq], &expected, Empty, false, __ATOMIC_RELEASE,
            __ATOMIC_RELAXED);
        assert(completed);
    }
    ...
    {
        return irq < LineCount &&
               __atomic_load_n(&m_States[irq], __ATOMIC_ACQUIRE) == Ready;
    }
...
inline uint16_t preserveUnsafePicUnmasks(
    uint16_t applied, uint16_t canonical, uint16_t temporary,
    uint16_t deferredTransitions, bool canPublishBeforeRelease)
{
    if (!canPublishBeforeRelease)
    {
        const uint16_t newlyUnmasked = static_cast<uint16_t>(
            applied & ~effectivePicMask(canonical, temporary));
        temporary |= static_cast<uint16_t>(
...
inline uint16_t restorablePicTemporaryMask(
    uint16_t temporary, uint16_t deferredTransitions)
{
    return static_cast<uint16_t>(temporary & ~deferredTransitions);
}

inline uint16_t cancelDeferredPicLineTransition(
    uint16_t deferred, size_t irq)
{
    assert(irq < PicIrqState::LineCount);
    ...
    const size_t (&spuriousEntries)[PicIrqState::LineCount])
{
    ...
    {
        if (realEntries[irq] || actions.tailEoi[irq] ||
            spuriousEntries[irq])
        {
...
{
    sink(
        PicControllerWriteTarget::MasterMask,
        static_cast<uint8_t>(mask & 0xFF));
    sink(
        PicControllerWriteTarget::SlaveMask,
        static_cast<uint8_t>(mask >> 8));
}

/** Emits one deferred batch in the only safe terminal hardware order. */
template <typename Sink>
void emitPicContentionWrites(
    const PicIrqState &state, bool writeMask,
    const size_t (&realEntries)[PicIrqState::LineCount],
    ...
    size_t spuriousCascadeEois, Sink &&sink)
{
    ...
    for (size_t irq = 0; irq < PicIrqState::LineCount; ++irq)
    {
        ...
        for (size_t occurrence = 0; occurrence < owedEois; ++occurrence)
        {
            ...
            {
                sink(
                    PicControllerWriteTarget::SlaveCommand,
                    static_cast<uint8_t>(0x60 + (irq - 8)));
                sink(PicControllerWriteTarget::MasterCommand, 0x62);
            }
            else
            {
                sink(
                    PicControllerWriteTarget::MasterCommand,
                    static_cast<uint8_t>(0x60 + irq));
            }
        }
    }

    for (size_t occurrence = 0; occurrence < spuriousCascadeEois;
         ++occurrence)
    {
        sink(PicControllerWriteTarget::MasterCommand, 0x62);
    }
