...
TimerHandlerRegistry::TimerHandlerRegistry()
    : m_Handlers(),
      ...
      m_HandlerHazardClaimHook(nullptr),
      m_HandlerAtomicDrainHook(nullptr)
#endif
{
}

size_t TimerHandlerRegistry::makePublication(size_t generation, SlotMode mode, bool selfRemoval,
                                             bool synchronousDrain) {
  return (generation << GenerationShift) | (selfRemoval ? SelfRemovalBit : 0) |
         (synchronousDrain ? SynchronousDrainBit : 0) | static_cast<size_t>(mode);
}
...
bool TimerHandlerRegistry::retireSlot(HandlerSlot& slot, size_t expectedPublication,
...
TimerHandlerRegistry::ActiveDispatch* TimerHandlerRegistry::publishDispatch(HandlerSlot& slot,
  ...
  for (size_t i = 0; i < MaxActiveDispatches; ++i) {
    ...
    if (__atomic_compare_exchange_n(&dispatch.token, &expectedToken, token, false, __ATOMIC_SEQ_CST,
      ...
      if (hazardClaimHook) {
        hazardClaimHook(__atomic_load_n(&slot.handler, __ATOMIC_ACQUIRE));
      }
#endif
...
bool TimerHandlerRegistry::unpublishDispatch(void* token, HandlerSlot& slot,
  ...
  if (generationOf(finalPublication) != drainGeneration || hasActiveDispatch(slot)) {
    return true;
  }
  ...
  if (synchronousDrain) {
    guard.wakeAll(WaitQueue::WakeReason::Signalled, WaitQueue::Channel(&slot, drainGeneration));
  }
  return true;
}
...
bool TimerHandlerRegistry::hasActiveDispatch(HandlerSlot& target) const {
  for (size_t i = 0; i < MaxActiveDispatches; ++i) {
    ...
    if (slot == &target && __atomic_load_n(&dispatch.token, __ATOMIC_ACQUIRE) == token &&
        __atomic_load_n(&dispatch.generation, __ATOMIC_ACQUIRE) == generation) {
      return true;
    }
  }
...
bool TimerHandlerRegistry::findCurrentDispatch(void* owner, HandlerSlot* target,
...
bool TimerHandlerRegistry::registerHandler(TimerHandler* handler) {
  ...
  for (size_t i = 0; i < MaxHandlerSlots; ++i) {
    ...
    while (true) {
      ...
      const bool abandonedAtomicDrain = mode == SlotMode::Draining && selfRemovalOf(publication) &&
                                        !synchronousDrainOf(publication);
      if (mode != SlotMode::Deferred &&
          !(mode == SlotMode::Enabled && selfRemovalOf(publication)) && !abandonedAtomicDrain) {
        return false;
      }
...
bool TimerHandlerRegistry::unregisterHandler(TimerHandler* handler) {
  ...
  if (!canYield || callbackContext) {
    for (size_t i = 0; i < MaxHandlerSlots; ++i) {
      ...
      while (true) {
        ...
        if (__atomic_load_n(&candidate.handler, __ATOMIC_ACQUIRE) != handler) {
          break;
        }

        const SlotMode mode = modeOf(publication);
        bool currentTargetDispatch = false;
        const bool selfUnregister = findCurrentDispatch(owner, &candidate, currentTargetDispatch);
        if (selfUnregister) {
          if (mode == SlotMode::Empty || mode == SlotMode::Retiring || selfRemovalOf(publication)) {
            return false;
          }

          const SlotMode deferredMode = mode == SlotMode::Enabled ? SlotMode::Deferred : mode;
          size_t expectedPublication = publication;
          if (__atomic_compare_exchange_n(&candidate.publication, &expectedPublication,
                                          makePublication(generationOf(publication), deferredMode,
                                                          true, synchronousDrainOf(publication)),
                                          false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
            return false;
          }
          continue;
        ...
        const size_t drainingPublication =
            makePublication(generationOf(publication), SlotMode::Draining);
        if (!__atomic_compare_exchange_n(&candidate.publication, &publication, drainingPublication,
                                         false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
          continue;
        }

#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
        HandlerAtomicDrainHook atomicDrainHook =
            __atomic_load_n(&m_HandlerAtomicDrainHook, __ATOMIC_ACQUIRE);
        if (atomicDrainHook) {
        ...
        if (hasActiveDispatch(candidate)) {
          ...
          const bool reopened = __atomic_compare_exchange_n(
              ...
              makePublication(generationOf(drainingPublication), SlotMode::Enabled,
                              selfRemovalOf(drainingPublication)),
              false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
          if (!reopened && modeOf(expectedPublication) == SlotMode::Draining &&
              selfRemovalOf(expectedPublication) && !synchronousDrainOf(expectedPublication)) {
            ...
            __atomic_compare_exchange_n(
                ...
                makePublication(generationOf(expectedPublication), SlotMode::Deferred, true), false,
  ...
  while (true) {
    ...
    if (mode == SlotMode::Empty) {
      const bool retired = __atomic_load_n(&slot->handler, __ATOMIC_ACQUIRE) != handler;
      m_HandlerLock.release();
      return retired;
    }
    ...
    drainingPublication = makePublication(generationOf(publication), SlotMode::Draining,
                                          selfRemovalOf(publication), true);
    size_t expectedPublication = publication;
    if (__atomic_compare_exchange_n(&slot->publication, &expectedPublication, drainingPublication,
                                    false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
      break;
    }
  ...
  while (true) {
    auto guard = m_DispatchWaiters.acquire();
    const size_t finalPublication = __atomic_load_n(&slot->publication, __ATOMIC_SEQ_CST);
    TimerHandler* finalHandler = __atomic_load_n(&slot->handler, __ATOMIC_ACQUIRE);
    ...
    if (finalHandler != handler || !synchronousDrainOf(finalPublication) ||
        (finalMode != SlotMode::Draining && finalMode != SlotMode::Deferred)) {
      return false;
    }
    ...
    const WaitQueue::WakeReason reason =
        guard.waitForCompletion(WaitQueue::Channel(slot, drainGeneration), Thread::CallbackDrain,
...
void TimerHandlerRegistry::reset() {
  ...
  for (size_t i = 0; i < MaxActiveDispatches; ++i) {
    assert(!__atomic_load_n(&m_ActiveDispatches[i].token, __ATOMIC_ACQUIRE));
    assert(!__atomic_load_n(&m_ActiveDispatches[i].owner, __ATOMIC_ACQUIRE));
...
void TimerHandlerRegistry::setHandlerHazardClaimHook(HandlerHazardClaimHook hook) {
  __atomic_store_n(&m_HandlerHazardClaimHook, hook, __ATOMIC_RELEASE);
}

void TimerHandlerRegistry::setHandlerAtomicDrainHook(HandlerAtomicDrainHook hook) {
...
size_t TimerHandlerRegistry::activeDispatchCountForTest(TimerHandler* handler) {
  ...
  for (size_t i = 0; i < MaxActiveDispatches; ++i) {
    ...
    if (activeHandler == handler && __atomic_load_n(&dispatch.token, __ATOMIC_ACQUIRE) == token &&
        __atomic_load_n(&dispatch.generation, __ATOMIC_ACQUIRE) == generation) {
      ++count;
    }
  }
