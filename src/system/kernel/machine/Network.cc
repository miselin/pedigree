...
StationInfo::StationInfo(const StationInfo& info)
    : ipv4(info.ipv4),
      ipv6(info.ipv6),
      nIpv6Addresses(info.nIpv6Addresses),
      subnetMask(info.subnetMask),
      broadcast(info.broadcast),
      gateway(info.gateway),
      gatewayIpv6(info.gatewayIpv6),
      dnsServers(info.dnsServers),
      nDnsServers(info.nDnsServers),
      mac(info.mac),
      nPackets(info.nPackets),
      nDropped(info.nDropped),
      nBad(info.nBad) {}
...
IpAddress Network::convertToIpv6(uint8_t a, uint8_t b, uint8_t c, uint8_t d, uint8_t e, uint8_t f,
