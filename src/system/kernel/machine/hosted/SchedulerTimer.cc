...
HostedSchedulerTimer::HardContextHook HostedSchedulerTimer::m_HardContextHook = nullptr;
...
bool HostedSchedulerTimer::initialise() {
  ...
  m_IrqId = irqManager.registerSchedulerIrqHandler(1, this, IrqPolicy::syntheticHard());
  if (m_IrqId == 0) {
...
void HostedSchedulerTimer::schedulerIrq(irq_id_t number, InterruptState& state) {
  ...
  if (result == HostedTickSource::TakeResult::Invalid) {
    FATAL_NOLOCK("HostedSchedulerTimer received invalid tick-source metadata");
    return;
  }
  if (!expirations) {
    return;
  ...
  if (LIKELY(m_Handler.beginDispatch(Processor::id(), dispatch))) {
    ...
    HardContextHook hook = __atomic_load_n(&m_HardContextHook, __ATOMIC_ACQUIRE);
    if (hook) {
      hook(delta, state);
    }
#endif
