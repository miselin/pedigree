...
struct DispatcherContext {
  DispatcherContext() : calls(0), lastCookie(0), contextFailures(0), driver(nullptr) {}
...
void dispatchThreaded(void* opaque, uint8_t line, size_t cookie) {
  ...
  if (line != 0 || !current || current == context->driver || !Processor::getInterrupts() ||
...
bool threadedDispatcherLifecycle() {
  ...
  ThreadedIrqDispatcher dispatcher(MakeConstantString("IRQ closure dispatcher"), 1,
                                   dispatchThreaded, &context);

  bool passed = check(dispatcher.initialise(), Test, "initialise failed");
  passed &= check(!dispatcher.publishFromInterrupt(1, 1) && !dispatcher.publishFromInterrupt(0, 0),
  ...
  passed &= check(firstPublished && newerPublished &&
                      waitUntil([&dispatcher] { return dispatcher.completedCookie(0) == 9; }),
  ...
  Processor::setInterrupts(interruptsWereEnabled);
  passed &= check(rejectionObserved && recoveryPublished &&
                      waitUntil([&dispatcher] { return dispatcher.completedCookie(0) == 12; }),
  ...
  passed &= check(shutdown && callsBeforeShutdown && !context.contextFailures &&
                      !dispatcher.isInitialised() && !dispatcher.publishFromInterrupt(0, 13),
...
class SplitLifecycleProbe final : public SplitIrqHandler {
 public:
  explicit SplitLifecycleProbe(Thread* driver)
      : SplitIrqHandler(MakeConstantString("IRQ closure split handler")),
        ...
        quiesceCalls(0),
        contextFailures(0),
        quiesceContextFailures(0),
        rearmContextFailures(0),
        workerCalls(0),
        m_Driver(driver),
        m_Active(false) {}
 ...
 private:
  HardStageDisposition hardIrq(irq_id_t, InterruptState&, size_t&) override {
    return HardStageDisposition::NotHandled;
  }

  void threadedIrq(size_t work) override {
    Thread* current = Processor::information().getCurrentThread();
    if (!current || !Processor::getInterrupts() || Processor::inDeviceHardIrq() ||
        Processor::executionContext() != ExecutionContext::WaitableThread ||
        current->getHostedSignalDepth()) {
      contextFailures += 1;
    }
...
bool splitHandlerLifecycle() {
  ...
  passed &= check(firstPublished && secondPublished && waitUntil([&probe] {
                    return probe.completedBatchesForTest() && !probe.pendingWorkForTest();
                  }) &&
                      probe.threadedWork.value() == 3 && probe.rearmedWork.value() == 3 &&
                      probe.workerCalls.value() && probe.deferredIrqsForTest() == 2 &&
  ...
  passed &= check(rejectionObserved && orphanRecorded && probe.publicationFailuresForTest() == 1,
                  Test, "the forced rejection did not leave one orphan batch");
  passed &= check(stopped, Test, "shutdown rejected an ordinary caller");
  passed &= check(!probe.pendingWorkForTest() && probe.threadedWork.value() == 7, Test,
                  "shutdown did not drain the orphan batch");
  passed &= check(probe.rearmedWork.value() == rearmsBeforeShutdown, Test,
                  "shutdown rearmed a quiesced source");
  passed &= check(probe.quiesceCalls.value() == 2, Test,
                  "shutdown did not reassert source quiescence after draining");
  passed &= check(!probe.contextFailures, Test,
                  "the threaded callback ran outside ordinary waitable context");
  passed &= check(!probe.quiesceContextFailures, Test,
                  "source quiescence ran outside ordinary waitable context");
  passed &=
      check(!probe.rearmContextFailures, Test, "source rearm did not run with interrupts disabled");
...
class TimerContextProbe final : public TimerHandler {
 public:
  ...
  void timer(uint64_t delta) override {
    Thread* current = Processor::information().getCurrentThread();
    if (!delta || !current || current == m_Driver || !Processor::getInterrupts() ||
        Processor::inDeviceHardIrq() ||
        Processor::executionContext() != ExecutionContext::WaitableThread ||
        current->getHostedSignalDepth()) {
      contextFailures += 1;
...
bool hostedTimerSplitDelivery() {
  ...
  bool passed =
      check(timer && timer->registerHandler(&probe), Test, "timer handler registration failed");
  passed &= check(waitUntil([&probe] { return probe.calls.value() >= 3; }), Test,
  ...
  const Time::Timestamp drainDeadline = Time::getTicks() + (10 * Time::Multiplier::Millisecond);
  while (Time::getTicks() < drainDeadline) {
    Scheduler::instance().yield();
  }
  passed &= check(removed && callsAfterRemoval >= 3 && probe.calls.value() == callsAfterRemoval &&
                      probe.elapsed.value() && !probe.contextFailures,
...
bool denyHardIrqOperation(DeviceHardIrqOperation operation) {
  GuardContext* context = __atomic_load_n(&g_GuardContext, __ATOMIC_ACQUIRE);
  if (!context) {
    return false;
  }
  const size_t call = __atomic_fetch_add(&context->calls, static_cast<size_t>(1), __ATOMIC_RELAXED);
...
bool hardIrqOperationGuards() {
  ...
  const size_t denialsBefore = Processor::deviceHardIrqOperationDenialsForTest();
  ...
  Processor::setInterrupts(false);
  {
    ...
    bool restorationArmed = false;
    DeviceHardIrqContext hardIrq(previousDepth, restorationArmed);
    marked = Processor::inDeviceHardIrq() && Processor::deviceHardIrqDepthForTest() == 1;
    hardContext = Processor::executionContext() == ExecutionContext::HardDeviceIrq;
    ...
    semaphoreAcquireDenied =
        !semaphore.acquireWithError(1, 0, 0, error) && error == Semaphore::Interrupted;
    semaphore.release(1);
    ...
    wakeDenied = waitQueue.wakeAll(WaitQueue::WakeReason::Signalled) == 0;
  ...
  __atomic_store_n(&g_GuardContext, static_cast<GuardContext*>(nullptr), __ATOMIC_RELEASE);
  const size_t denialsAfter = Processor::deviceHardIrqOperationDenialsForTest();

  const DeviceHardIrqOperation expected[ExpectedGuardDenials] = {
      DeviceHardIrqOperation::Schedule,         DeviceHardIrqOperation::SemaphoreAcquire,
      DeviceHardIrqOperation::SemaphoreRelease, DeviceHardIrqOperation::WaitQueueAccess,
      DeviceHardIrqOperation::WaitQueueAccess,  DeviceHardIrqOperation::HeapAllocate,
      DeviceHardIrqOperation::HeapFree,
  };
  bool operationsMatch = context.calls == ExpectedGuardDenials && !context.overflow;
  ...
  const bool passed = check(
      marked && hardContext && scheduleReturned && semaphoreAcquireDenied &&
          semaphoreReleaseDenied && waitDenied && wakeDenied && allocateDenied && operationsMatch &&
          denialsAfter - denialsBefore == ExpectedGuardDenials && !Processor::inDeviceHardIrq() &&
          Processor::executionContext() == ExecutionContext::WaitableThread &&
