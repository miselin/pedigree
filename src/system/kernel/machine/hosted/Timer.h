...
class HostedTimer : public Timer, private SplitIrqHandler {
 public:
  ...
  static EXPORTED_PUBLIC void setHandlerHazardClaimHook(HandlerHazardClaimHook hook);
  /** Installs an observer after an atomic caller closes admission. */
  static EXPORTED_PUBLIC void setHandlerAtomicDrainHook(HandlerAtomicDrainHook hook);
  /** Runs a test callback while the registry writer lock is held. */
  static EXPORTED_PUBLIC void withHandlerMutationLockForTest(HandlerMutationLockHook hook);
  /** Dispatches one handler through the production registry path. */
  static EXPORTED_PUBLIC bool dispatchHandlerForTest(TimerHandler* handler, uint64_t delta);
  /** Returns committed callback hazards for one handler. */
  static EXPORTED_PUBLIC size_t activeDispatchCountForTest(TimerHandler* handler);
  ...
  static EXPORTED_PUBLIC void setAlarmSendAdmissionHookForTest(AlarmSendAdmissionHook hook);
