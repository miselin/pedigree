...
HostedTimer::AlarmSendAdmissionHook HostedTimer::m_AlarmSendAdmissionHook = nullptr;
...
void HostedTimer::addAlarm(Event* pEvent, size_t alarmSecs, size_t alarmUsecs) {
  ...
  Alarm* pAlarm = new Alarm(pEvent, deadline, Processor::information().getCurrentThread());
...
void HostedTimer::setAlarmSendAdmissionHookForTest(AlarmSendAdmissionHook hook) {
...
bool HostedTimer::initialise3() {
  ...
  m_IrqId = registerIsaSplitIrq(irqManager, 0, IrqPolicy::syntheticHard());
  if (!m_IrqId) {
    if (!shutdownSplitIrq()) {
      FATAL(
...
HostedTimer::HostedTimer()
    : SplitIrqHandler(MakeConstantString("Hosted timer bottom half")),
      ...
      m_TickSource(),
      m_IrqId(0),
      m_PendingExpirations(),
      m_HandlerRegistry(),
      m_Alarms(),
      m_AlarmLock(false) {}

SplitIrqHandler::HardStageDisposition HostedTimer::hardIrq(irq_id_t number, InterruptState& state,
                                                           size_t& work) {
  const siginfo_t* signalInfo = reinterpret_cast<const siginfo_t*>(state.getRegister(1));
  if (number != 0 || state.getInterruptNumber() != SIGUSR1) {
    return HardStageDisposition::NotHandled;
  }

  size_t expirations = 0;
  const HostedTickSource::TakeResult result = m_TickSource.takeExpirations(signalInfo, expirations);
...
void HostedTimer::processTimerBatch(uint64_t delta) {
  const bool elapsedSecond =
      delta >= Time::Multiplier::Second || m_Nanosecond >= (Time::Multiplier::Second - delta);
  if (elapsedSecond) {
    synchronise();
  } else {
    m_Nanosecond += delta;
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
        AlarmSendAdmissionHook hook = __atomic_load_n(&m_AlarmSendAdmissionHook, __ATOMIC_ACQUIRE);
        if (hook) {
          hook(pA->m_pEvent);
        }
#endif
