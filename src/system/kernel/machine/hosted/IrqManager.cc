...
irq_id_t HostedIrqManager::registerIsaIrqHandler(
    uint8_t, IrqHandler *, const IrqPolicy &)
{
    return 0;
}

irq_id_t HostedIrqManager::registerPciIrqHandler(
    IrqHandler *, Device *, const IrqPolicy &)
{
    return 0;
}
    ...
    uint8_t irq, HardIrqHandler *handler, const IrqPolicy &policy)
{
    ...
    if (!__atomic_compare_exchange_n(
            &m_TimerHandler, &expected, handler, false, __ATOMIC_RELEASE,
            __ATOMIC_ACQUIRE))
    {
        return 0;
    }
...
irq_id_t HostedIrqManager::registerHardPciIrqHandler(
    HardIrqHandler *, Device *, const IrqPolicy &)
{
    return 0;
}
...
{
    if (irq != SchedulerIrq || !handler ||
        policy != IrqPolicy::syntheticHard())
    {
        return 0;
    }
    ...
    if (!__atomic_compare_exchange_n(
            &m_SchedulerHandler, &expected, handler, false, __ATOMIC_RELEASE,
            __ATOMIC_ACQUIRE))
    {
        return 0;
    }
    ...
    irq_id_t id, SchedulerIrqHandler *handler)
{
    ...
    return __atomic_compare_exchange_n(
        &m_SchedulerHandler, &expected,
        static_cast<SchedulerIrqHandler *>(nullptr), false, __ATOMIC_ACQ_REL,
        __ATOMIC_ACQUIRE);
}

bool HostedIrqManager::unregisterHandler(
    irq_id_t id, IrqHandlerBase *handler)
{
    ...
    HardIrqHandler *expected =
        __atomic_load_n(&m_TimerHandler, __ATOMIC_ACQUIRE);
    if (static_cast<IrqHandlerBase *>(expected) != handler)
    {
        return false;
    }
    return __atomic_compare_exchange_n(
        &m_TimerHandler, &expected, static_cast<HardIrqHandler *>(nullptr),
...
HostedIrqManager::HostedIrqManager()
    : m_TimerHandler(nullptr), m_SchedulerHandler(nullptr)
{
}

void HostedIrqManager::interrupt(
    size_t interruptNumber, InterruptState &state)
{
    if (interruptNumber == SIGUSR1)
    {
        ...
        {
            FATAL_NOLOCK(
                "Hosted timer hard stage failed to publish its deferred work");
        }
        return;
    }
    ...
    if (interruptNumber == SIGUSR2)
    {
        SchedulerIrqHandler *handler =
            __atomic_load_n(&m_SchedulerHandler, __ATOMIC_ACQUIRE);
        if (handler)
        {
