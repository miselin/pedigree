...
int waitFor(pthread_cond_t* condition, pthread_mutex_t* mutex, uint64_t nanoseconds) {
...
bool HostedTickSource::prepare(int signal, void* owner) {
  if (__atomic_load_n(&m_Prepared, __ATOMIC_ACQUIRE) || signal <= 0 || !owner) {
    return false;
  }
...
#if defined(__linux__)
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
  __atomic_store_n(&m_InjectedExpirations, static_cast<size_t>(0), __ATOMIC_RELEASE);
#endif
  ...
  event._sigev_un._tid = static_cast<decltype(event._sigev_un._tid)>(executionThreadId);
...
bool HostedTickSource::arm(uint64_t intervalNanoseconds) {
  if (!__atomic_load_n(&m_Prepared, __ATOMIC_ACQUIRE) || !intervalNanoseconds) {
    return false;
  }
...
bool HostedTickSource::disarm() {
  ...
  __atomic_store_n(&m_PendingExpirations, static_cast<size_t>(0), __ATOMIC_RELEASE);
...
HostedTickSource::TakeResult HostedTickSource::takeExpirations(const siginfo_t* info,
                                                               size_t& expirations) {
  expirations = 0;
  if (!__atomic_load_n(&m_Prepared, __ATOMIC_ACQUIRE) || !info || info->si_signo != m_Signal) {
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
    expirations =
        __atomic_exchange_n(&m_InjectedExpirations, static_cast<size_t>(0), __ATOMIC_ACQ_REL);
    if (expirations) {
  ...
  expirations =
      __atomic_exchange_n(&m_PendingExpirations, static_cast<size_t>(0), __ATOMIC_ACQ_REL);
#endif
  return TakeResult::Expirations;
}
...
#if defined(__linux__)
  size_t pending = __atomic_load_n(&m_InjectedExpirations, __ATOMIC_ACQUIRE);
  while (true) {
    if (pending == ~static_cast<size_t>(0)) {
      return false;
    }
    if (__atomic_compare_exchange_n(&m_InjectedExpirations, &pending, pending + 1, false,
...
bool HostedTickSource::recordExpirations(size_t expirations) {
  size_t pending = __atomic_load_n(&m_PendingExpirations, __ATOMIC_ACQUIRE);
  while (true) {
    if (expirations > (~static_cast<size_t>(0) - pending)) {
      return false;
    }
    if (__atomic_compare_exchange_n(&m_PendingExpirations, &pending, pending + expirations, false,
                                    __ATOMIC_RELEASE, __ATOMIC_ACQUIRE)) {
      return true;
    }
  }
...
void HostedTickSource::helper() {
  ...
  while (!m_Stop) {
    ...
    while (!m_Stop && m_Armed && generation == m_Generation) {
      ...
      const bool recorded = recordExpirations(static_cast<size_t>(count64));
      const int delivered = recorded ? pthread_kill(m_TargetThread, m_Signal) : EOVERFLOW;
