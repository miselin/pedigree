  ...
  if (m_pParent) {
    ...
    if (!bChild) {
      if (m_pParent->getFormat() == m_PixelFormat) {
        ...
        ERROR(
            "Child framebuffer has different pixel format to "
            "parent!");
      }
    }
...
void Framebuffer::blit(Graphics::Buffer* pBuffer, size_t srcx, size_t srcy, size_t destx,
...
void Framebuffer::draw(void* pBuffer, size_t srcx, size_t srcy, size_t destx, size_t desty,
                       size_t width, size_t height, Graphics::PixelFormat format,
                       bool bLowestCall) {
...
void Framebuffer::rect(size_t x, size_t y, size_t width, size_t height, uint32_t colour,
...
void Framebuffer::copy(size_t srcx, size_t srcy, size_t destx, size_t desty, size_t w, size_t h,
                       bool bLowestCall) {
  if (m_pParent)
...
void Framebuffer::line(size_t x1, size_t y1, size_t x2, size_t y2, uint32_t colour,
                       Graphics::PixelFormat format, bool bLowestCall) {
  if (m_pParent)
...
void Framebuffer::draw(Graphics::Buffer* pBuffer, size_t srcx, size_t srcy, size_t destx,
                       size_t desty, size_t width, size_t height, bool bLowestCall) {
  swDraw(pBuffer, srcx, srcy, destx, desty, width, height, bLowestCall);
}

Graphics::Buffer* Framebuffer::swCreateBuffer(const void* srcData, Graphics::PixelFormat srcFormat,
                                              size_t width, size_t height, uint32_t* pPalette) {
  ...
  if (((sourceBytesPerPixel == destBytesPerPixel) && (sourceBytesPerLine == destBytesPerLine)) &&
    ...
    for (y = 0; y < height; y++) {
      for (x = 0; x < width; x++) {
        ...
        const uint32_t* pSource =
            reinterpret_cast<const uint32_t*>(adjust_pointer(srcData, sourceOffset));
...
void Framebuffer::swBlit(Graphics::Buffer* pBuffer, size_t srcx, size_t srcy, size_t destx,
...
void Framebuffer::swRect(size_t x, size_t y, size_t width, size_t height, uint32_t colour,
  ...
  if (UNLIKELY((!x) && (width == m_nWidth))) {
    ...
    if (bytesPerPixel == 4)  /// \todo Handle 24-bit properly
    {
      ...
      if ((copySize % 8) == 0)  // QWORD boundary
      {
        uint64_t val = (static_cast<uint64_t>(transformColour) << 48) |
                       (static_cast<uint64_t>(transformColour) << 32) | (transformColour << 16) |
                       transformColour;
        QuadWordSet(dest, val, copySize / 8);
    ...
    for (size_t desty = y; desty < (y + height); desty++) {
      ...
      if (bytesPerPixel == 4)  /// \todo Handle 24-bit properly
      {
        ...
        for (size_t i = 0; i < (copySize / 3); i++) {
          uint32_t* p =
              reinterpret_cast<uint32_t*>(m_FramebufferBase + frameBufferOffset + (i * 3));
          *p = (*p & 0xFF000000) | transformColour;
        }
      } else if (bytesPerPixel == 2) {
        if ((copySize % 8) == 0)  // QWORD boundary
...
void Framebuffer::swLine(size_t x1, size_t y1, size_t x2, size_t y2, uint32_t colour,
                         Graphics::PixelFormat format) {
  if (UNLIKELY(!m_FramebufferBase))
    return;
...
void Framebuffer::swDraw(void* pBuffer, size_t srcx, size_t srcy, size_t destx, size_t desty,
                         size_t width, size_t height, Graphics::PixelFormat format,
                         bool bLowestCall) {
...
void Framebuffer::swDraw(Graphics::Buffer* pBuffer, size_t srcx, size_t srcy, size_t destx,
                         size_t desty, size_t width, size_t height, bool bLowestCall) {
