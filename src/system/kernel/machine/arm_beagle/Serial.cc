...
void ArmBeagleSerial::setBase(uintptr_t nBaseAddr)
{
    ...
    if (!PhysicalMemoryManager::instance().allocateRegion(
            m_BaseRegion, 1, PhysicalMemoryManager::continuous,
    ...
    m_Base =
        reinterpret_cast<volatile uint8_t *>(m_BaseRegion.virtualAddress());
    ...
    switch ((intStatus >> 1) & 0x1F)
    {
        ...
        case 2:
        {
            ...
            while (m_Base[LSR_REG] & 0x1)
                uint8_t c =
                    m_Base[RHR_REG];  /// \todo Queue somewhere for input
        }
        break;
        case 3:
        {
...
/// \note Page/section references are from the OMAP35xx Technical Reference
/// Manual
...
bool ArmBeagleSerial::setFifoDefaults()
{
    ...
    // mode (part 1 of 2)
    m_Base[FCR_REG] =
        1;  // TX and RX FIFO triggers at 8 characters, no DMA mode
...
bool ArmBeagleSerial::configureProtocol()
{
    ...
    m_Base[IER_REG] =
        0x0D;  // RHR interrupt. // No interrupts wanted at this stage
