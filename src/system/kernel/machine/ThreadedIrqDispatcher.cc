...
static_assert(__atomic_always_lock_free(sizeof(Thread::DebugState), nullptr),
...
ThreadedIrqDispatcher::Line::Line()
    : m_Owner(nullptr),
      ...
      m_CompletedCookie(0),
      m_PendingSinceTimestamp(0),
      m_ActiveCallbackStartedTimestamp(0),
      m_LastWakeLatency(0),
      m_MaximumWakeLatency(0),
      m_LastCallbackRuntime(0),
...
void ThreadedIrqDispatcher::Line::configure(ThreadedIrqDispatcher* owner, uint8_t line,
...
bool ThreadedIrqDispatcher::Line::start() {
  ...
  const size_t testPendingCookieCount =
      __atomic_load_n(&m_Owner->m_PendingSlotCountForTest, __ATOMIC_ACQUIRE);
  ...
  for (size_t i = 0; i < pendingCookieCount; ++i) {
    __atomic_store_n(&pendingCookies[i], static_cast<size_t>(0), __ATOMIC_RELAXED);
  }
  ...
  __atomic_store_n(&m_PublicationState, static_cast<size_t>(0), __ATOMIC_RELEASE);
  __atomic_store_n(&m_CompletedBatches, static_cast<size_t>(0), __ATOMIC_RELEASE);
  __atomic_store_n(&m_CompletedCookie, static_cast<size_t>(0), __ATOMIC_RELEASE);
  __atomic_store_n(&m_PendingSinceTimestamp, static_cast<size_t>(0), __ATOMIC_RELEASE);
  __atomic_store_n(&m_ActiveCallbackStartedTimestamp, static_cast<size_t>(0), __ATOMIC_RELEASE);
  __atomic_store_n(&m_LastWakeLatency, static_cast<size_t>(0), __ATOMIC_RELEASE);
  __atomic_store_n(&m_MaximumWakeLatency, static_cast<size_t>(0), __ATOMIC_RELEASE);
...
bool ThreadedIrqDispatcher::Line::publishFromInterrupt(size_t cookie) {
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
  const size_t processorSlot =
      __atomic_load_n(&m_Owner->m_PublicationSlotForTest, __ATOMIC_ACQUIRE);
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
    __atomic_add_fetch(&m_Owner->m_RemotePublicationRejectionsForTest, static_cast<size_t>(1),
                       __ATOMIC_RELAXED);
#endif
  ...
  const size_t admission =
      __atomic_fetch_add(&m_PublicationState, static_cast<size_t>(1), __ATOMIC_ACQ_REL);
  if (admission & PublicationClosed) {
    __atomic_fetch_sub(&m_PublicationState, static_cast<size_t>(1), __ATOMIC_RELEASE);
    return false;
  }
  ...
  const size_t pending =
      __atomic_exchange_n(&m_PendingCookies[processor], cookie, __ATOMIC_ACQ_REL);
  if (!pending) {
    __atomic_store_n(&m_PendingSinceTimestamp, static_cast<size_t>(Time::getTicks()),
                     __ATOMIC_RELEASE);
  }

#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
  PublicationObservedHook hook =
      __atomic_load_n(&m_Owner->m_PublicationObservedHook, __ATOMIC_ACQUIRE);
  if (hook) {
...
size_t ThreadedIrqDispatcher::Line::pendingCookie() const {
  ...
  PendingScanAdmittedHook hook =
      __atomic_load_n(&m_Owner->m_PendingScanAdmittedHook, __ATOMIC_ACQUIRE);
  if (hook) {
    hook(m_Owner, m_Line);
  }
#endif
...
size_t ThreadedIrqDispatcher::Line::pendingCookieForWorker() const {
  ...
  for (size_t i = 0; i < m_PendingCookieCount; ++i) {
    const size_t candidate = __atomic_load_n(&m_PendingCookies[i], __ATOMIC_ACQUIRE);
...
uintptr_t ThreadedIrqDispatcher::Line::workerIdentity() const {
  return reinterpret_cast<uintptr_t>(__atomic_load_n(&m_Thread, __ATOMIC_ACQUIRE));
}
...
bool ThreadedIrqDispatcher::Line::publicationClosed() const {
  return (__atomic_load_n(&m_PublicationState, __ATOMIC_ACQUIRE) & PublicationClosed) != 0;
}
...
void ThreadedIrqDispatcher::Line::snapshotDiagnostics(IrqLineDiagnosticSnapshot& snapshot) const {
  ...
  snapshot.pendingSinceTimestamp = __atomic_load_n(&m_PendingSinceTimestamp, __ATOMIC_ACQUIRE);
  snapshot.activeCallbackStartedTimestamp =
      __atomic_load_n(&m_ActiveCallbackStartedTimestamp, __ATOMIC_ACQUIRE);
  snapshot.lastWakeLatency = __atomic_load_n(&m_LastWakeLatency, __ATOMIC_ACQUIRE);
  snapshot.maximumWakeLatency = __atomic_load_n(&m_MaximumWakeLatency, __ATOMIC_ACQUIRE);
  snapshot.lastCallbackRuntime = __atomic_load_n(&m_LastCallbackRuntime, __ATOMIC_ACQUIRE);
  snapshot.maximumCallbackRuntime = __atomic_load_n(&m_MaximumCallbackRuntime, __ATOMIC_ACQUIRE);
  ...
  if (thread) {
    ...
    snapshot.workerDebugState = workerDebugState(thread->getDebugState(debugAddress));
    ...
    if (thread->getWaitDebugInfo(wait)) {
      ...
      snapshot.workerWaitChannelOwner = reinterpret_cast<uintptr_t>(wait.channelOwner);
...
int ThreadedIrqDispatcher::Line::run() {
  ...
  while (true) {
    ...
    if (cookie) {
      const size_t completedCookie = __atomic_load_n(&m_CompletedCookie, __ATOMIC_ACQUIRE);
      if (completedCookie && !generationReached(cookie, completedCookie)) {
      ...
      __atomic_store_n(&m_ActiveCallbackStartedTimestamp, started, __ATOMIC_RELEASE);
    ...
    __atomic_store_n(&m_CallbackActive, static_cast<size_t>(0), __ATOMIC_RELEASE);
    const size_t publicationState = __atomic_load_n(&m_PublicationState, __ATOMIC_ACQUIRE);
...
size_t ThreadedIrqDispatcher::Line::takePendingCookie() {
  ...
  for (size_t i = 0; i < m_PendingCookieCount; ++i) {
    const size_t candidate =
        __atomic_exchange_n(&m_PendingCookies[i], static_cast<size_t>(0), __ATOMIC_ACQ_REL);
    if (candidate && (!newest || generationReached(candidate, newest))) {
      newest = candidate;
    }
  }
...
bool ThreadedIrqDispatcher::Line::generationReached(size_t current, size_t target) {
...
ThreadedIrqDispatcher::ThreadedIrqDispatcher(const String& name, size_t lineCount,
    ...
    : m_Lines(),
      ...
      m_PendingScanAdmittedHook(nullptr),
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
void ThreadedIrqDispatcher::setPublicationObservedHookForTest(PublicationObservedHook hook) {
...
bool ThreadedIrqDispatcher::setPendingSlotCountForTest(size_t slotCount) {
  ...
  __atomic_store_n(&m_PendingSlotCountForTest, slotCount, __ATOMIC_RELEASE);
  return true;
}

bool ThreadedIrqDispatcher::publishFromSlotForTest(uint8_t line, size_t slot, size_t cookie) {
  if (!isInitialised() || line >= m_LineCount) {
    return false;
  }

  size_t expected = static_cast<size_t>(-1);
  if (!__atomic_compare_exchange_n(&m_PublicationSlotForTest, &expected, slot, false,
                                   __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {
    return false;
  }
  const bool published = m_Lines[line].publishFromInterrupt(cookie);
  __atomic_store_n(&m_PublicationSlotForTest, static_cast<size_t>(-1), __ATOMIC_RELEASE);
  return published;
}

void ThreadedIrqDispatcher::rejectNextPublicationForTest() {
  __atomic_store_n(&m_RejectNextPublicationForTest, static_cast<size_t>(1), __ATOMIC_RELEASE);
}

void ThreadedIrqDispatcher::setPendingScanAdmittedHookForTest(PendingScanAdmittedHook hook) {
...
bool ThreadedIrqDispatcher::initialise() {
  ...
  if (!__atomic_compare_exchange_n(&m_ConfigurationClosed, &configurationExpected,
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
  __atomic_store_n(&m_RejectNextPublicationForTest, static_cast<size_t>(0), __ATOMIC_RELEASE);
  ...
  for (size_t i = 0; i < m_LineCount; ++i) {
    ...
    if (!m_Lines[i].start()) {
      ...
      __atomic_store_n(&m_ConfigurationClosed, static_cast<size_t>(0), __ATOMIC_RELEASE);
      return false;
    }
  }
...
bool ThreadedIrqDispatcher::shutdown() {
  ...
  size_t expected = 0;
  if (!__atomic_compare_exchange_n(&m_ShutdownClaimed, &expected, static_cast<size_t>(1), false,
                                   __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {
  ...
  if (joined) {
    __atomic_store_n(&m_Initialised, static_cast<size_t>(0), __ATOMIC_RELEASE);
    __atomic_store_n(&m_ConfigurationClosed, static_cast<size_t>(0), __ATOMIC_RELEASE);
  } else {
...
bool ThreadedIrqDispatcher::canShutdown() const {
  ...
  bool safe = current && Processor::executionContext() == ExecutionContext::WaitableThread &&
              !isCurrentWorker();
#if HOSTED
...
#if HOSTED && PEDIGREE_HOSTED_SMOKE_TESTS
  if (__atomic_exchange_n(&m_RejectNextPublicationForTest, static_cast<size_t>(0),
                          __ATOMIC_ACQ_REL)) {
    return false;
  }
#endif
