...
LocalIO::LocalIO(Vga *pVga, Keyboard *pKeyboard)
    : m_nWidth(80), m_nHeight(25), m_UpperCliLimit(0), m_LowerCliLimit(0),
      m_CursorX(0), m_CursorY(0), m_pVga(pVga), m_pKeyboard(pKeyboard)
{
    initialise();
}
...
void LocalIO::initialise()
{
    ...
    for (size_t i = 0; i < MAX_CONSOLE_WIDTH * MAX_CONSOLE_HEIGHT; i++)
    {
        uint8_t attributeByte =
            (DebuggerIO::Black << 4) | (DebuggerIO::White & 0x0F);
    ...
    m_pVga->peekBuffer(
        reinterpret_cast<uint8_t *>(m_pOldFramebuffer),
...
void LocalIO::drawHorizontalLine(
...
{
    m_pVga->pokeBuffer(
        reinterpret_cast<uint8_t *>(m_pFramebuffer), m_nWidth * m_nHeight * 2);
    moveCursor();
}
...
void LocalIO::scroll()
{
    ...
    if (m_CursorY >= m_nHeight - m_LowerCliLimit)
    {
        for (size_t i = m_UpperCliLimit * m_nWidth;
             i < (m_nHeight - m_LowerCliLimit - 1) * m_nWidth; i++)
            m_pFramebuffer[i] = m_pFramebuffer[i + m_nWidth];

        for (size_t i = (m_nHeight - m_LowerCliLimit - 1) * m_nWidth;
             i < (m_nHeight - m_LowerCliLimit) * m_nWidth; i++)
            m_pFramebuffer[i] = blank;
    ...
    char c, DebuggerIO::Colour foreColour, DebuggerIO::Colour backColour)
{
    ...
    if (c == 0x08)
    {
        ...
        uint8_t attributeByte = (backColour << 4) | (foreColour & 0x0F);
        m_pFramebuffer[m_CursorY * m_nWidth + m_CursorX] =
            ' ' | (attributeByte << 8);
    }
