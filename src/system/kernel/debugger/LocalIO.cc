...
void LocalIO::drawHorizontalLine(char c, size_t row, size_t colStart, size_t colEnd,
...
void LocalIO::drawVerticalLine(char c, size_t col, size_t rowStart, size_t rowEnd,
                               DebuggerIO::Colour foreColour, DebuggerIO::Colour backColour) {
...
void LocalIO::drawString(const char* str, size_t row, size_t col, DebuggerIO::Colour foreColour,
...
void LocalIO::scroll() {
  ...
  uint16_t blank = ' ' | (attributeByte << 8);
  if (m_CursorY >= m_nHeight - m_LowerCliLimit) {
    for (size_t i = m_UpperCliLimit * m_nWidth; i < (m_nHeight - m_LowerCliLimit - 1) * m_nWidth;
         i++)
