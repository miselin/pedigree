...
template <class T>
void extractDwarfState(const DwarfState* endState, const DwarfState& startState, T& outState,
                       uint32_t nReturnAddressRegister);
...
void extractDwarfState(const DwarfState* endState, const DwarfState& startState,
                       X64ProcessorState& outState, uint32_t nReturnAddressRegister) {
...
template <>
void extractDwarfState(const DwarfState* endState, const DwarfState& startState,
                       HostedProcessorState& outState, uint32_t nReturnAddressRegister) {
  /// \todo
}
...
void fillDwarfState(DwarfState& outState, const Armv7ProcessorState& inState) {
  for (size_t i = 0; i < 13; ++i) {
    outState.m_R[i] = inState.r[i];
  }
  outState.m_R[13] = inState.sp;
  outState.m_R[14] = inState.lr;
  outState.m_R[15] = inState.pc;
  outState.m_R[16] = inState.cpsr;
}

template <>
void extractDwarfState(const DwarfState* endState, const DwarfState& startState,
                       Armv7ProcessorState& outState, uint32_t returnRegister) {
  for (size_t i = 0; i < 13; ++i) {
    outState.r[i] = endState->getRegister(i, startState);
  }
  outState.sp = endState->getCfa(startState);
  outState.lr = endState->getRegister(14, startState);
  outState.pc = endState->getRegister(returnRegister, startState);
  outState.cpsr = startState.m_R[16];
}
#endif
...
bool DwarfUnwinder::unwind(const ProcessorState& inState, ProcessorState& outState,
  ...
  while (nIndex < m_nLength) {
    ...
    size_t nInstructionLength = nLength - sizeof(uint32_t) - sizeof(uintptr_t) - sizeof(size_t);
    ...
    DwarfState* endState = automaton.execute(m_nData + nInstructionStart, nInstructionLength,
...
int32_t DwarfUnwinder::decodeSleb128(uint8_t* pBase, uint32_t& nOffset) {
  ...
  if ((shift < sizeof(int32_t) * 8) && (byte & 0x40)) /* If sign bit of byte is set */
