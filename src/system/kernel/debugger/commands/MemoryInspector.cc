    ...
    InterruptState &state, DebuggerIO *pScreen)
{
    ...
    pScreen->drawString(
        "Pedigree debugger - Memory inspector", 0, 0, DebuggerIO::White,
        DebuggerIO::Green);
...
const char *MemoryInspector::getLine1(
...
const char *MemoryInspector::getLine2(
...
void MemoryInspector::resetStatusLine(DebuggerIO *pScreen)
{
    ...
    pScreen->drawString(
        ...
        DebuggerIO::Green);
    pScreen->drawString(
        "/,?", pScreen->getHeight() - 1, 47, DebuggerIO::Yellow,
        DebuggerIO::Green);
    pScreen->drawString(
        "g", pScreen->getHeight() - 1, 68, DebuggerIO::Yellow,
        DebuggerIO::Green);
}
...
void MemoryInspector::doGoto(DebuggerIO *pScreen, InterruptState &state)
{
    ...
    while (1)
    {
        ...
        pScreen->drawHorizontalLine(
            ' ', pScreen->getHeight() - 2, 0, pScreen->getWidth() - 1,
            DebuggerIO::White, DebuggerIO::Black);
        // Print helper text.
        pScreen->drawString(
            "[Go to address or register value]", pScreen->getHeight() - 2, 0,
            DebuggerIO::White, DebuggerIO::Black);
        // Print current string contents.
        pScreen->drawString(
            str, pScreen->getHeight() - 2, 35, DebuggerIO::White,
            DebuggerIO::Black);
        // Get a character.
        char c;
        while ((c = pScreen->getChar()) == 0)
    ...
    {
        pScreen->drawString(
            "Bad address or register.", pScreen->getHeight() - 2, 0,
...
void MemoryInspector::doSearch(
    bool bForward, DebuggerIO *pScreen, InterruptState &state)
{
    static LargeStaticString str;
    ...
    while (1)
    {
        ...
        if (bForward)
            pScreen->drawString(
                "[Search] ", pScreen->getHeight() - 2, 0, DebuggerIO::White,
                DebuggerIO::Black);
        else
            pScreen->drawString(
                "[Reverse]", pScreen->getHeight() - 2, 0, DebuggerIO::White,
                DebuggerIO::Black);
        // Print current string contents.
        pScreen->drawString(
            str, pScreen->getHeight() - 2, 10, DebuggerIO::White,
            DebuggerIO::Black);
        // Get a character.
        char c;
        while ((c = pScreen->getChar()) == 0)
    ...
    if (str[0] == '"')
    {
        ...
        {
            pScreen->drawString(
                "Search string too long (> 8 characters)",
                pScreen->getHeight() - 2, 0, DebuggerIO::Red,
                DebuggerIO::Black);
            return;
        }
    ...
    else
    {
        ...
        {
            pScreen->drawString(
                "Hex string too long, 8 bytes max.", pScreen->getHeight() - 2,
                0, DebuggerIO::Red, DebuggerIO::Black);
            return;
        }
        for (size_t i = 0; i < nChars; i++)
        {
            ...
            {
                pScreen->drawString(
                    "Malformed hexadecimal number.", pScreen->getHeight() - 2,
                    0, DebuggerIO::Red, DebuggerIO::Black);
                return;
            }
    ...
    pScreen->drawString(
        str, pScreen->getHeight() - 2, 0, DebuggerIO::White, DebuggerIO::Black);
    ...
    while (1)
    {
        ...
        // Print helper text.
        pScreen->drawString(
            str, pScreen->getHeight() - 2, 0, DebuggerIO::White,
            DebuggerIO::Black);
        // Print current string contents.
        pScreen->drawString(
            str2, pScreen->getHeight() - 2, str.length() + 1, DebuggerIO::White,
            DebuggerIO::Black);
        // Get a character.
        char c;
        while ((c = pScreen->getChar()) == 0)
    ...
    {
        pScreen->drawString(
            "Malformed number.", pScreen->getHeight() - 2, 0, DebuggerIO::Red,
            DebuggerIO::Black);
        return;
    }
    ...
    pScreen->drawString(
        "Searching...", pScreen->getHeight() - 2, 0, DebuggerIO::Green,
        DebuggerIO::Black);

    // Do the search.
}

bool MemoryInspector::tryGoto(
    LargeStaticString &str, uintptr_t &result, InterruptState &state)
{
