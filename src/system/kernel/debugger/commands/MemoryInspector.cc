...
MemoryInspector::MemoryInspector() : DebuggerCommand(), Scrollable(), m_nCharsPerLine(8) {}
...
bool MemoryInspector::execute(const HugeStaticString& input, HugeStaticString& output,
  ...
  pScreen->drawHorizontalLine(' ', 0, 0, pScreen->getWidth() - 1, DebuggerIO::White,
                              DebuggerIO::Green);

  // Write the correct text in the upper status line.
  pScreen->drawString("Pedigree debugger - Memory inspector", 0, 0, DebuggerIO::White,
                      DebuggerIO::Green);
...
const char* MemoryInspector::getLine1(size_t index, DebuggerIO::Colour& colour,
...
const char* MemoryInspector::getLine2(size_t index, size_t& colOffset, DebuggerIO::Colour& colour,
                                      DebuggerIO::Colour& bgColour) {
...
void MemoryInspector::resetStatusLine(DebuggerIO* pScreen) {
  ...
  pScreen->drawString(
      "backspace: Page up. space: Page down. q: Quit. /,?: Fwd/Rev search. "
      "g: Goto.",
      pScreen->getHeight() - 1, 0, DebuggerIO::White, DebuggerIO::Green);
  pScreen->drawString("backspace", pScreen->getHeight() - 1, 0, DebuggerIO::Yellow,
                      DebuggerIO::Green);
...
void MemoryInspector::doGoto(DebuggerIO* pScreen, InterruptState& state) {
  ...
  while (1) {
    ...
    pScreen->drawHorizontalLine(' ', pScreen->getHeight() - 2, 0, pScreen->getWidth() - 1,
                                DebuggerIO::White, DebuggerIO::Black);
    // Print helper text.
    pScreen->drawString("[Go to address or register value]", pScreen->getHeight() - 2, 0,
                        DebuggerIO::White, DebuggerIO::Black);
  ...
  if (!tryGoto(str, nAddress, state)) {
    pScreen->drawString("Bad address or register.", pScreen->getHeight() - 2, 0, DebuggerIO::Red,
                        DebuggerIO::Black);
  } else {
    scrollTo(nAddress / m_nCharsPerLine);
  }
...
void MemoryInspector::doSearch(bool bForward, DebuggerIO* pScreen, InterruptState& state) {
  ...
  while (1) {
    ...
    // Print helper text.
    if (bForward)
      pScreen->drawString("[Search] ", pScreen->getHeight() - 2, 0, DebuggerIO::White,
                          DebuggerIO::Black);
    else
      pScreen->drawString("[Reverse]", pScreen->getHeight() - 2, 0, DebuggerIO::White,
                          DebuggerIO::Black);
    // Print current string contents.
  ...
  if (str[0] == '"') {
    ...
    if (str.length() > 8) {
      pScreen->drawString("Search string too long (> 8 characters)", pScreen->getHeight() - 2, 0,
                          DebuggerIO::Red, DebuggerIO::Black);
      return;
    }
    ...
    nChars = str.length() / 2;
    if (nChars >= 8) {
      pScreen->drawString("Hex string too long, 8 bytes max.", pScreen->getHeight() - 2, 0,
                          DebuggerIO::Red, DebuggerIO::Black);
      return;
    }
    for (size_t i = 0; i < nChars; i++) {
      ...
      // Failed?
      if (nConverted == -1) {
        pScreen->drawString("Malformed hexadecimal number.", pScreen->getHeight() - 2, 0,
                            DebuggerIO::Red, DebuggerIO::Black);
        return;
      }
  ...
  while (1) {
    ...
    pScreen->drawString(str, pScreen->getHeight() - 2, 0, DebuggerIO::White, DebuggerIO::Black);
    // Print current string contents.
    pScreen->drawString(str2, pScreen->getHeight() - 2, str.length() + 1, DebuggerIO::White,
                        DebuggerIO::Black);
    // Get a character.
    char c;
    while ((c = pScreen->getChar()) == 0)
  ...
  if (num == -1) {
    pScreen->drawString("Malformed number.", pScreen->getHeight() - 2, 0, DebuggerIO::Red,
                        DebuggerIO::Black);
    return;
  }
  ...
  pScreen->drawString("Searching...", pScreen->getHeight() - 2, 0, DebuggerIO::Green,
                      DebuggerIO::Black);
