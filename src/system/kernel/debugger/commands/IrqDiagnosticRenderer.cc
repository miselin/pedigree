...
namespace {
constexpr uint16_t KnownMaskReasons = IrqMaskNoHandler | IrqMaskAdministrativelyDisabled |
                                      IrqMaskAwaitingAcknowledgement |
                                      IrqMaskAwaitingThreadedCompletion | IrqMaskMitigated |
                                      IrqMaskShuttingDown | IrqMaskControllerContention;
...
void appendMaskReason(IrqDiagnosticString& line, bool& first, uint16_t reasons, uint16_t reason,
                      const char* name) {
  if (!(reasons & reason)) {
    return;
  }
...
void appendMaskReasons(IrqDiagnosticString& line, uint16_t reasons) {
  ...
  appendMaskReason(line, first, reasons, IrqMaskAdministrativelyDisabled, "admin-disabled");
  appendMaskReason(line, first, reasons, IrqMaskAwaitingAcknowledgement, "awaiting-ack");
  appendMaskReason(line, first, reasons, IrqMaskAwaitingThreadedCompletion, "awaiting-threaded");
  appendMaskReason(line, first, reasons, IrqMaskMitigated, "mitigated");
  appendMaskReason(line, first, reasons, IrqMaskShuttingDown, "shutting-down");
  appendMaskReason(line, first, reasons, IrqMaskControllerContention, "controller-contention");
...
void appendHardDispatchState(IrqDiagnosticString& line, const IrqLineDiagnosticSnapshot& snapshot) {
  line += " hard[pins=";
  line.append(snapshot.activeHardDispatchCount);
  line += " gen=";
  if (!snapshot.activeHardDispatchCount) {
    line += "none";
  } else if (snapshot.activeHardDispatchCount == 1 && snapshot.activeHardDispatchGeneration) {
...
void appendAge(IrqDiagnosticString& line, bool present, size_t observed, size_t started) {
...
void appendWorkerState(IrqDiagnosticString& line, const IrqLineDiagnosticSnapshot& snapshot) {
  line += "\n  worker=0x";
  line.append(snapshot.workerIdentity, 16);
  line += " handler=";
  if (!snapshot.activeThreadedDispatchCount) {
    line += "none";
  } else if (snapshot.activeThreadedDispatchCount == 1 && snapshot.activeThreadedHandlerIdentity) {
    line += "0x";
...
void appendTimingState(IrqDiagnosticString& line, const IrqLineDiagnosticSnapshot& snapshot) {
  line += "  ns[pending=";
  appendAge(line, snapshot.pendingCookie != 0, snapshot.observationTimestamp,
            snapshot.pendingSinceTimestamp);
  line += " active=";
  appendAge(line, snapshot.activeCookie != 0, snapshot.observationTimestamp,
...
void appendWorkerDebugState(IrqDiagnosticString& line, const IrqLineDiagnosticSnapshot& snapshot) {
  line += "  debug[";
...
void IrqDiagnosticRenderer::render(const IrqLineDiagnosticSnapshot& snapshot,
  ...
  if (!snapshot.snapshotGeneration) {
    ...
    line += " ack=";
    line += snapshot.configured ? controllerAckName(snapshot.controllerAck) : "n/a";
    line += " release=";
    line += snapshot.configured ? lineReleaseName(snapshot.lineRelease) : "n/a";
    line += " req=";
