...
bool AllocationCommand::execute(const HugeStaticString& input, HugeStaticString& output,
  ...
  for (Vector<Allocation*>::Iterator it = m_Allocations.begin(); it != m_Allocations.end(); it++) {
    Allocation* pA = *it;
  ...
  pScreen->drawString("Pedigree debugger - Page allocation resolver", 0, 0, DebuggerIO::White,
                      DebuggerIO::Green);
...
const char* AllocationCommand::getLine1(size_t index, DebuggerIO::Colour& colour,
...
const char* AllocationCommand::getLine2(size_t index, size_t& colOffset, DebuggerIO::Colour& colour,
                                        DebuggerIO::Colour& bgColour) {
...
void AllocationCommand::postProcess() {
  ...
  for (Vector<void*>::Iterator it = m_Frees.begin(); it != m_Frees.end(); it++) {
    physical_uintptr_t page = reinterpret_cast<physical_uintptr_t>(*it);

    // Look through the allocations vector for this address.
    for (Vector<Allocation*>::Iterator it2 = m_Allocations.begin(); it2 != m_Allocations.end();
         it2++) {
      if ((*it2)->page == page) {
        delete (*it2);
