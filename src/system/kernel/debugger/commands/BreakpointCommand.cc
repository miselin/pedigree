    ...
    InterruptState &state, DebuggerIO *pScreen)
{
    ...
    if (input == "breakpoint")
    {
        ...
        for (size_t i = 0; i < Processor::getDebugBreakpointCount(); i++)
        {
            ...
            bool bEnabled;
            uintptr_t nAddress =
                Processor::getDebugBreakpoint(i, nFt, nLen, bEnabled);
            ...
            output += i;
            output += ": 0x";
            output.append(nAddress, 16, sizeof(uintptr_t) * 2, '0');
            output += " \t";
            output.append(pFaultType);
            output += " \t";
            output.append(nLen);
            output += " \t";
            output.append(pEnabled);
            output += "\n";
        }
    }
    else
    {
        ...
        if (nBp < 0 ||
            nBp >= static_cast<int32_t>(Processor::getDebugBreakpointCount()))
        {
        ...
        bool bEnabled;
        uintptr_t address =
            Processor::getDebugBreakpoint(nBp, nFaultType, nLength, bEnabled);
        ...
        else if (command == "enabled")
        {
            ...
            {
                Processor::enableDebugBreakpoint(
                    nBp, address, nFaultType, nLength);
            }
            else
            {
