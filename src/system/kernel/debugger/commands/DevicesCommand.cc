...
bool DevicesCommand::execute(const HugeStaticString& input, HugeStaticString& output,
  ...
  while (bContinue) {
    ...
    if (c == 'q')
      ...
      pScreen->drawString("Querying device - press any key to exit. ", 1, 0, DebuggerIO::White,
                          DebuggerIO::Black);
      pScreen->drawString(str.cstr(), 2, 0, DebuggerIO::LightGrey, DebuggerIO::Black);
      while ((c = pScreen->getChar()) == 0)
        ;
      drawBackground(nLines, pScreen);
      tree.refresh(pScreen);
...
void DevicesCommand::drawBackground(size_t nLines, DebuggerIO* pScreen) {
  ...
  pScreen->drawString("Pedigree debugger - Device tree", 0, 0, DebuggerIO::White,
                      DebuggerIO::Green);
  // Write some helper text in the lower status line.
  pScreen->drawString("", pScreen->getHeight() - 1, 0, DebuggerIO::White, DebuggerIO::Green);

  pScreen->drawHorizontalLine('-', nLines + 1, 0, pScreen->getWidth(), DebuggerIO::DarkGrey,
                              DebuggerIO::Black);
}

DevicesCommand::DeviceTree::DeviceTree() : m_Line(0), m_LinearTree() {
  // Create the (flattened) device tree, so we can look up the index of any
  // particular device.
...
const char* DevicesCommand::DeviceTree::getLine1(size_t index, DebuggerIO::Colour& colour,
  ...
  // How many parents does this node have? that will determine how many |'s to
  // draw.
...
const char* DevicesCommand::DeviceTree::getLine2(size_t index, size_t& colOffset,
...
const char* DevicesCommand::DeviceInfo::getLine1(size_t index, DebuggerIO::Colour& colour,
                                                 DebuggerIO::Colour& bgColour) {
  colour = DebuggerIO::Yellow;
  switch (index) {
    case 0:
...
const char* DevicesCommand::DeviceInfo::getLine2(size_t index, size_t& colOffset,
                                                 DebuggerIO::Colour& colour,
                                                 DebuggerIO::Colour& bgColour) {
  if (!m_pDev)
    return "";
