...
bool LocksCommand::execute(const HugeStaticString& input, HugeStaticString& output,
  ...
  pScreen->drawString("Pedigree debugger - Lock tracker", 0, 0, DebuggerIO::White,
                      DebuggerIO::Green);

  pScreen->drawString("backspace: Page up. space: Page down. q: Quit.", pScreen->getHeight() - 1, 0,
                      DebuggerIO::White, DebuggerIO::Green);
...
const char* LocksCommand::getLine1(size_t index, DebuggerIO::Colour& colour,
...
const char* LocksCommand::getLine2(size_t index, size_t& colOffset, DebuggerIO::Colour& colour,
                                   DebuggerIO::Colour& bgColour) {
  ...
  for (size_t i = 0; i < LOCKS_COMMAND_NUM_CPU; ++i) {
    ...
    for (size_t j = 0; j < m_NextPosition[nCpu]; ++j) {
      ...
      if (pMyD->pLock == pD->pLock && pMyD->state == Acquired) {
        ...
        // Deadlock.
        ERROR_OR_FATAL("Detected lock dependency inversion (deadlock) between "
