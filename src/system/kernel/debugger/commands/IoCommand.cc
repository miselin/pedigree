...
bool IoCommand::execute(const HugeStaticString& input, HugeStaticString& output,
...

  {
    ...
    for (; i != end; i++) {
      ...
      output += (*i)->pName;
      output += ": virtual 0x";
      output.append(reinterpret_cast<uintptr_t>((*i)->pVirtualAddress), 16,
                    sizeof(processor_register_t) * 2, '0');
      output += " - 0x";
      output.append(reinterpret_cast<uintptr_t>((*i)->pVirtualAddress) + (*i)->sVirtualAddress, 16,
                    sizeof(processor_register_t) * 2, '0');
      if ((*i)->physicalAddress != 0) {
        output += '\n';
        output.append(' ', StringLength((*i)->pName) + 3, ' ');
        output += "physical 0x";
        output.append((*i)->physicalAddress, 16, sizeof(physical_uintptr_t) * 2, '0');
        output += " - 0x";
        output.append((*i)->physicalAddress + (*i)->sVirtualAddress, 16,
