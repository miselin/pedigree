...
void DumpCommand::autocomplete(
    const HugeStaticString &input, HugeStaticString &output)
{
}

bool DumpCommand::execute(
    const HugeStaticString &input, HugeStaticString &output,
    InterruptState &state, DebuggerIO *pScreen)
{
    ...
    for (size_t i = 0; i < state.getRegisterCount(); i++)
    {
        ...
        output += ": 0x";
        output.append(
            state.getRegister(i), 16, state.getRegisterSize(i) * 2, '0');

        if (i == (state.getRegisterCount() - 1))
            output += '\n';
        else
        {
            size_t addLength = StringLength(state.getRegisterName(i + 1)) + 4 +
                               state.getRegisterSize(i + 1) * 2;
