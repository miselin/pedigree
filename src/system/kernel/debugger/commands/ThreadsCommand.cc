...
static void appendWaitState(Thread* thread, HugeStaticString& line) {
  ...
  diagnostic.channelOwner = reinterpret_cast<uintptr_t>(wait.channelOwner);
  ...
  if (thread->getDebugState(debugAddress) == Thread::SemWait && wait.channelOwner) {
    const Semaphore* semaphore = reinterpret_cast<const Semaphore*>(wait.channelOwner);
...
static void selectLine(size_t index, Scheduler::ProcessLease& selectedProcess,
  ...
  for (size_t i = 0; i < Scheduler::instance().getNumProcesses(); ++i) {
    ...
    for (size_t j = 0; j < process->getNumThreads(); ++j) {
      ...
      const bool threadAcquired = process->acquireThread(thread, j);
      if (index == line) {
        if (threadAcquired) {
          selectedProcess = pedigree_std::move(process);
...
bool ThreadsCommand::execute(const HugeStaticString& input, HugeStaticString& output,
  ...
  pScreen->drawString("Pedigree debugger - Thread selector", 0, 0, DebuggerIO::White,
                      DebuggerIO::Green);
...
const char* ThreadsCommand::getLine1(size_t index, DebuggerIO::Colour& colour,
...
const char* ThreadsCommand::getLine2(size_t index, size_t& colOffset, DebuggerIO::Colour& colour,
                                     DebuggerIO::Colour& bgColour) {
