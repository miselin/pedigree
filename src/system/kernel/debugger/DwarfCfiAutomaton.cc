...
void DwarfCfiAutomaton::initialise(const DwarfState& startingState, uintptr_t nCodeLocation,
...
DwarfState* DwarfCfiAutomaton::execute(uintptr_t nCodeLocation, size_t nCodeLen,
                                       uintptr_t nBreakAt) {
  uintptr_t nCurrentCodeLocation = nCodeLocation;
  uintptr_t nProgramCounter = m_nStartingPc;
  while ((nCurrentCodeLocation < nCodeLocation + nCodeLen) && (nProgramCounter <= nBreakAt)) {
...
void DwarfCfiAutomaton::executeInstruction(uintptr_t& nLocation, uintptr_t& nPc) {
  ...
  if ((pLocation[0] & 0xc0) == DW_CFA_advance_loc) {
    ...
    int32_t nOffset =
        static_cast<int32_t>(DwarfUnwinder::decodeUleb128(pLocation, nLocationOffset));
    ...
    switch (pLocation[0]) {
      ...
      case DW_CFA_def_cfa: {
        ...
        m_CurrentState.m_CfaOffset =
            static_cast<ssize_t>(DwarfUnwinder::decodeUleb128(pLocation, nOffset));
        nLocation += nOffset;
