...
/// none found.
static int getCommandMatchingPrefix(char* prefix, DebuggerCommand** pCommands, size_t nCmds,
                                    size_t start) {
...
static bool matchesCommand(char* pStr, DebuggerCommand* pCommand) {
  ...
  if (inputLength > commandLength && pStr[commandLength] != ' ' && pStr[commandLength] != '\t') {
    return false;
  }
...
// current thread, in case we decide to switch.
  ...
  if (!nInterfaces) {
    ...
    ERROR_NOLOCK(
        "This machine/CPU combination doesn't support any output "
  ...
  if (n == -1) {
    ...
    for (int i = 0; i < nInterfaces; i++) {
      ...
      pInterfaces[i]->drawString("Press any key to enter the debugger...", 0, 0,
  ...
  for (int i = 0; i < nInterfaces; i++)
    if (pIo != pInterfaces[i])
      pInterfaces[i]->drawString("Locked by another device.", 1, 0, DebuggerIO::LightRed,
                                 DebuggerIO::Black);
  ...
  description.append(
      reinterpret_cast<uintptr_t>(VirtualAddressSpace::getKernelAddressSpace().m_HeapEnd), 16);
  description += "\n";
  pIo->writeCli(description, DebuggerIO::Yellow, DebuggerIO::Black);
  ...
  do {
    ...
    pIo->drawHorizontalLine(' ', pIo->getHeight() - 1, 0, pIo->getWidth() - 1, DebuggerIO::White,
                            DebuggerIO::Green);
    ...
    while (1) {
      ...
      if (!matchedCommand) {
        int i = -1;
        while ((i = getCommandMatchingPrefix(const_cast<char*>(static_cast<const char*>(command)),
      ...
      pIo->drawString(str, pIo->getHeight() - 1, str2.length(), DebuggerIO::White,
                      DebuggerIO::Green);
    }
...
#if MULTIPROCESSOR
  if (processorsQuiesced && !Machine::instance().resumeAllOtherProcessors()) {
