...
void Scrollable::scroll(ssize_t lines)
{
    ...
    if (m_line >
        (static_cast<ssize_t>(getLineCount()) - static_cast<ssize_t>(m_height)))
        m_line = static_cast<ssize_t>(getLineCount()) -
                 static_cast<ssize_t>(m_height);
...
void Scrollable::refresh(DebuggerIO *pScreen)
{
    ...
    for (size_t i = 0; i < m_height; i++)
    {
        pScreen->drawHorizontalLine(
            ' ', m_y + i, m_x, m_x + m_width - 1, DebuggerIO::White,
            DebuggerIO::Black);
        ...
        if (line >= 0 && line < static_cast<ssize_t>(getLineCount()))
        {
            ...
            if (Line)
                pScreen->drawString(
                    Line, m_y + i, m_x + colOffset, colour, bgColour);
        }
        line++;
    }
    ...
    if (bCanScrollUp)
        pScreen->drawString(
            str, m_y, m_x + m_width - 3, DebuggerIO::White, DebuggerIO::Blue);
    str[0] = 'v';
    str[2] = m_ScrollDown;
    if (bCanScrollDown)
        pScreen->drawString(
            str, m_y + m_height - 1, m_x + m_width - 3, DebuggerIO::White,
            DebuggerIO::Blue);
