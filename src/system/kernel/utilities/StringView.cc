...
StringView::StringView()
    : m_String(nullptr), m_Length(0), m_HashingEnabled(HASH_STRINGVIEWS_BY_DEFAULT) {
  m_Hash = 0;
}
...
StringView::StringView(const char* s)
    : m_String(s),
      ...
      m_Hash(0),
      m_HashingEnabled(HASH_STRINGVIEWS_BY_DEFAULT) {
...
StringView StringView::substring(size_t start, size_t end, bool hashed) const {
  if (start == 0 && end == m_Length) {
    return StringView(m_String, m_Length, hashed ? hash() : 0, hashed);
  }
