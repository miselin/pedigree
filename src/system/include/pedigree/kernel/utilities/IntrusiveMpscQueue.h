...
{
    static_assert(
        __atomic_always_lock_free(sizeof(Node *), nullptr),
    ...
    static_assert(
        NextMember != nullptr,
        "IntrusiveMpscQueue requires a valid intrusive link member");
  ...
  public:
     ...
     */
    explicit IntrusiveMpscQueue(Node &stub)
        : m_Head(&stub), m_Tail(&stub), m_Stub(&stub)
    {
