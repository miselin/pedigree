...
class EXPORTED_PUBLIC RadixTree {
 ...
 public:
  ...
  typename pedigree_std::enable_if<pedigree_std::is_same<U, T>::value &&
...
template <class T>
RadixTree<T>::RadixTree() : m_nItems(0), m_pRoot(0), m_bCaseSensitive(true), m_NodePool() {}
    ...
    switch (pNode->matchKey(cpKey, partialOffset)) {
      ...
      case Node::PartialMatch: {
        ...
        if (cpKey[partialOffset] != 0) {
          ...
          pChild->setKey(&cpKey[partialOffset],
...
void RadixTree<T>::Node::dump(void (*emit_line)(const char* s)) const {
  for (auto it : m_Children) {
    ...
    String s;
    s.Format("  \"Node<%p: %s>\" -> \"Node<%p: %s>\";", static_cast<const void*>(it), it->getKey(),
