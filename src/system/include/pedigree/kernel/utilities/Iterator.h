 ...
 *previous structure in the datastructure and a "value" member. This template
 *provides a bidirectional, a constant bidirectional, a reverse bidirectional
 *and a constant reverse bidirectional iterator. \brief An iterator applicable
 *for many data structures \param[in] originalT the original element type of the
 *iterator \param[in] Struct the datastructure that provides functions for the
 *next/previous datastructure and a "value" member \param[in] previous pointer
 *to the member function used to iterate forward \param[in] next pointer to the
 *member function used to iterate backwards \param[in] T the real element type
 *of the iterator */
template <typename originalT, class Struct, Struct* (Struct::*FunctionPrev)() = &Struct::previous,
...
class Iterator {
   ...
   * iterator types */
  template <typename _originalT, class _Struct, _Struct* (_Struct::*_FunctionPrev)(),
  ...
  friend bool operator==(
      const Iterator<_originalT, _Struct, _FunctionPrev, _FunctionNext, _T1>& x1,
 ...
 *previous structure in the datastructure and a "value" member. This template
 *provides a bidirectional, a constant bidirectional, a reverse bidirectional
 *and a constant reverse bidirectional iterator. This specific Tree-style
 *implementation provides access to a key and value stored in the Struct
...
class TreeIterator {
  ...
  /** The assignment operator is extern */
  template <typename _originalT, class _Struct, _Struct* (_Struct::*_FunctionPrev)(),
            _Struct* (_Struct::*_FunctionNext)(), typename _K1, typename _T1, typename _K2,
            typename _T2>
  friend bool operator==(
      const TreeIterator<_originalT, _Struct, _FunctionPrev, _FunctionNext, _K1, _T1>& x1,
...
template <typename originalT, class Struct, Struct* (Struct::*FunctionPrev)(),
