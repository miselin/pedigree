...
enum Error { HashTableEmpty, NotFound, IterationComplete };
}
 ...
 */
template <class K, class V, class SiblingK = K, size_t InitialBuckets = 4,
...
class HashTable {
 private:
  ...
  static_assert(InitialBuckets > 0, "At least one initial bucket must be available.");
 ...
 public:
  ...
  typename pedigree_std::enable_if<!pedigree_std::is_same<K, SK>::value, LookupResult>::type lookup(
      const SK& k) const {
    HashTableError::Error err;
