...
class EXPORTED_PUBLIC Process {
 ...
 public:
  ...
  using OrphanPublicationHook = void (*)(Process*, OrphanPublicationPhase, bool interruptsEnabled,
  ...
  void trackHeap(ssize_t nBytes) {
    __atomic_fetch_add(&m_Metadata.heapUsage, nBytes, __ATOMIC_RELAXED);
  }

  void trackPages(ssize_t nVirtual, ssize_t nPhysical, ssize_t nShared) {
    __atomic_fetch_add(&m_Metadata.virtualPages, nVirtual, __ATOMIC_RELAXED);
    __atomic_fetch_add(&m_Metadata.physicalPages, nPhysical, __ATOMIC_RELAXED);
    __atomic_fetch_add(&m_Metadata.sharedPages, nShared, __ATOMIC_RELAXED);
  }

  void resetCounts() {
    __atomic_store_n(&m_Metadata.virtualPages, static_cast<ssize_t>(0), __ATOMIC_RELEASE);
    __atomic_store_n(&m_Metadata.physicalPages, static_cast<ssize_t>(0), __ATOMIC_RELEASE);
    __atomic_store_n(&m_Metadata.sharedPages, static_cast<ssize_t>(0), __ATOMIC_RELEASE);
    __atomic_store_n(&m_Metadata.startTime, Time::getTimeNanoseconds(), __ATOMIC_RELEASE);
  }
  ...
  ssize_t getVirtualPageCount() const {
    return __atomic_load_n(&m_Metadata.virtualPages, __ATOMIC_ACQUIRE);
  }
  ssize_t getPhysicalPageCount() const {
    return __atomic_load_n(&m_Metadata.physicalPages, __ATOMIC_ACQUIRE);
  }
