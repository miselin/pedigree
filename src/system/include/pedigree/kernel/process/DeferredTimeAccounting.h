...
class ThreadTimeAccounting
{
  public:
    ...
    void record(
        CpuTimeMode mode, Time::Timestamp now, size_t processor = 0)
    {
        ...
        Time::Timestamp previous =
            __atomic_load_n(baseline, __ATOMIC_ACQUIRE);
        if (now <= previous)
        {
            return;
        }
        ...
        __atomic_compare_exchange_n(
            baseline, &previous, now, false, __ATOMIC_ACQ_REL,
            __ATOMIC_ACQUIRE);
    }
  ...
  private:
    ...
    static bool installProcessorBaseline(
        Entry *state, size_t processor, Time::Timestamp now)
    {
        if (
            __atomic_load_n(&state->processor, __ATOMIC_ACQUIRE) ==
            processor)
        {
            return false;
        }
  ...
  public:
    ...
    bool publish(Time::Timestamp elapsed)
    {
        ...
        return __atomic_exchange_n(
                   &m_Pending, static_cast<size_t>(1), __ATOMIC_ACQ_REL) == 0;
    }
    ...
    {
        return __atomic_exchange_n(
                   &m_Pending, static_cast<size_t>(0), __ATOMIC_ACQ_REL) != 0;
    }
...
class DeferredTimeAccountingWorkerState
{
  public:
    DeferredTimeAccountingWorkerState()
        : m_Published(0), m_Completed(0), m_Active(0)
    {
    }
    ...
    {
        __atomic_add_fetch(
            &m_Published, static_cast<size_t>(1), __ATOMIC_RELEASE);
    }
    ...
    {
        __atomic_store_n(
            &m_Active, static_cast<size_t>(1), __ATOMIC_RELEASE);
    ...
    void finishBatch(size_t generation)
    {
        ...
        __atomic_store_n(
            &m_Active, static_cast<size_t>(0), __ATOMIC_RELEASE);
    }

    bool ready(bool stopping = false) const
    {
        return stopping ||
               __atomic_load_n(&m_Active, __ATOMIC_ACQUIRE) ||
               !caughtUp();
    }
  ...
  private:
    ...
    DeferredTimeAccountingWorkerState(
        const DeferredTimeAccountingWorkerState &) = delete;
    DeferredTimeAccountingWorkerState &operator=(
        const DeferredTimeAccountingWorkerState &) = delete;
};
