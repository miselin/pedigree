...
class EXPORTED_PUBLIC WaitQueue
{
  ...
  public:
    ...
    class EXPORTED_PUBLIC Guard
    {
      public:
         ...
         */
        MUST_USE_RESULT WakeReason wait(
            const Channel &channel = Channel(), size_t debugState = 0,
         ...
         */
        MUST_USE_RESULT WakeReason waitForCompletion(
            const Channel &channel = Channel(), size_t debugState = 0,
            uintptr_t debugAddress = 0);
         ...
         * wait.
         */
        MUST_USE_RESULT WakeReason waitAndUnlock(
            Mutex &mutex, const Channel &channel = Channel(),
         ...
         */
        MUST_USE_RESULT WakeReason waitAndUnlockForCompletion(
            Mutex &mutex, const Channel &channel = Channel(),
            size_t debugState = 0, uintptr_t debugAddress = 0);

        bool wakeOne(
            WakeReason reason = WakeReason::Signalled,
    ...
    using BeforeBlockHook = void (*)(
        WaitQueue *queue, Thread *thread, const Channel &channel,
        size_t debugState);
    using BeforeCancelAbandonHook = void (*)(Thread *thread);

    static void setBeforeBlockHook(BeforeBlockHook hook);
    static void setBeforeCancelAbandonHook(
  ...
  private:
    ...
    struct Waiter
    {
        ...
        {
            __atomic_store_n(
                &reason, static_cast<size_t>(value), __ATOMIC_RELEASE);
        }
    ...
    WakeReason wait(
        ...
        void *abandonContext, bool deferTerminal);
    bool wakeOneLocked(
        Guard &guard, WakeReason reason, const Channel &channel);
    size_t wakeAllLocked(
        Guard &guard, WakeReason reason, const Channel &channel);
    bool completeWaiter(
