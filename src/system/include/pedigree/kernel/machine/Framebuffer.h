...
class EXPORTED_PUBLIC Framebuffer {
 public:
   ...
   */
  void redraw(size_t x = ~0UL, size_t y = ~0UL, size_t w = ~0UL, size_t h = ~0UL,
              bool bChild = false);
   ...
   */
  virtual void blit(Graphics::Buffer* pBuffer, size_t srcx, size_t srcy, size_t destx, size_t desty,
   ...
   *  pixel buffers. */
  virtual void draw(void* pBuffer, size_t srcx, size_t srcy, size_t destx, size_t desty,
                    size_t width, size_t height,
                    Graphics::PixelFormat format = Graphics::Bits32_Argb, bool bLowestCall = true);

  /** Draws a single rectangle to the screen with the given colour. */
  virtual void rect(size_t x, size_t y, size_t width, size_t height, uint32_t colour,
                    Graphics::PixelFormat format = Graphics::Bits32_Argb, bool bLowestCall = true);

  /** Copies a rectangle already on the framebuffer to a new location */
  virtual void copy(size_t srcx, size_t srcy, size_t destx, size_t desty, size_t w, size_t h,
                    bool bLowestCall = true);

  /** Draws a line one pixel wide between two points on the screen */
  virtual void line(size_t x1, size_t y1, size_t x2, size_t y2, uint32_t colour,
                    Graphics::PixelFormat format = Graphics::Bits32_Argb, bool bLowestCall = true);
 ...
 protected:
  ...
  virtual void draw(Graphics::Buffer* pBuffer, size_t srcx, size_t srcy, size_t destx, size_t desty,
                    size_t width, size_t height, bool bLowestCall = true);

  void swBlit(Graphics::Buffer* pBuffer, size_t srcx, size_t srcy, size_t destx, size_t desty,
              size_t width, size_t height);

  void swRect(size_t x, size_t y, size_t width, size_t height, uint32_t colour,
              Graphics::PixelFormat format);

  void swCopy(size_t srcx, size_t srcy, size_t destx, size_t desty, size_t w, size_t h);

  void swLine(size_t x1, size_t y1, size_t x2, size_t y2, uint32_t colour,
              Graphics::PixelFormat format);

  void swDraw(void* pBuffer, size_t srcx, size_t srcy, size_t destx, size_t desty, size_t width,
              size_t height, Graphics::PixelFormat format = Graphics::Bits32_Argb,
              bool bLowestCall = true);

  void swDraw(Graphics::Buffer* pBuffer, size_t srcx, size_t srcy, size_t destx, size_t desty,
              size_t width, size_t height, bool bLowestCall = true);
