...
class IrqDiagnosticSnapshotStore
{
  public:
    IrqDiagnosticSnapshotStore()
        : m_Banks(), m_Publications(), m_BankClaims(), m_WriterAdmissions(),
          m_MissedPublications(), m_Dirty()
    {
    ...
    IrqLineDiagnosticSnapshot *beginPublication(size_t line, size_t &targetBank)
    {
        ...
        if (!__atomic_compare_exchange_n(
                &m_WriterAdmissions[line], &expectedAdmission,
        ...
        const size_t publication =
            __atomic_load_n(&m_Publications[line], __ATOMIC_ACQUIRE);
        ...
        for (size_t offset = 1; offset < BankCount; ++offset)
        {
            ...
            if (__atomic_compare_exchange_n(
                    &m_BankClaims[line][candidate], &expectedClaim, WriterClaim,
        ...
        __atomic_store_n(
            &m_WriterAdmissions[line], static_cast<size_t>(0),
            __ATOMIC_RELEASE);
        missed(line);
        return nullptr;
    }
    ...
    void finishPublication(size_t line, size_t targetBank)
    {
        ...
        const size_t current =
            __atomic_load_n(&m_Publications[line], __ATOMIC_RELAXED);
        ...
        __atomic_store_n(
            &m_BankClaims[line][targetBank], static_cast<size_t>(0),
            __ATOMIC_RELEASE);
        __atomic_store_n(
            &m_Publications[line], (generation << BankBits) | targetBank,
            __ATOMIC_RELEASE);
        __atomic_store_n(
            &m_WriterAdmissions[line], static_cast<size_t>(0),
            __ATOMIC_RELEASE);
    ...
    bool snapshot(size_t line, IrqLineDiagnosticSnapshot &out) const
    {
        ...
        for (size_t attempt = 0; attempt < SnapshotAttempts; ++attempt)
        {
            ...
            size_t claim =
                __atomic_load_n(&m_BankClaims[line][bank], __ATOMIC_ACQUIRE);
            if (claim & WriterClaim)
            {
                continue;
            }
            if (!__atomic_compare_exchange_n(
                    &m_BankClaims[line][bank], &claim, claim + 1, false,
                    __ATOMIC_ACQUIRE, __ATOMIC_RELAXED))
            {
                continue;
            }

            if (__atomic_load_n(&m_Publications[line], __ATOMIC_ACQUIRE) !=
                publication)
            {
                __atomic_fetch_sub(
                    &m_BankClaims[line][bank], static_cast<size_t>(1),
                    __ATOMIC_RELEASE);
                continue;
            }
    ...
    size_t missedPublications(size_t line) const
    {
        return line < LineCount ?
                   __atomic_load_n(
                       &m_MissedPublications[line], __ATOMIC_RELAXED) :
                   0;
    }
    ...
    {
        return line < LineCount && __atomic_exchange_n(
                                       &m_Dirty[line], static_cast<size_t>(0),
                                       __ATOMIC_ACQ_REL) != 0;
    }
  ...
  private:
    ...
    static constexpr size_t WriterClaim = static_cast<size_t>(1)
                                          << ((sizeof(size_t) * 8) - 1);

    void missed(size_t line)
    {
        __atomic_store_n(
            &m_Dirty[line], static_cast<size_t>(1), __ATOMIC_RELEASE);
        __atomic_add_fetch(
            &m_MissedPublications[line], static_cast<size_t>(1),
            __ATOMIC_RELAXED);
    }
