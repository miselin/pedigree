...
class SchedulerTimerHandlerSlot
{
  public:
    ...
    SchedulerTimerHandlerSlot()
        : m_Handler(nullptr), m_Owner(NoOwner), m_State(Empty)
    {
    }
    ...
    bool publish(size_t owner, SchedulerTimerHandler *handler)
    {
        ...
        if (!__atomic_compare_exchange_n(
                &m_State, &expected, Publishing, false, __ATOMIC_ACQ_REL,
                __ATOMIC_ACQUIRE))
        {
            return false;
        }
    ...
    {
        if (!handler || owner == NoOwner ||
            __atomic_load_n(&m_Owner, __ATOMIC_ACQUIRE) != owner)
            return false;
        ...
        if (!__atomic_compare_exchange_n(
                &m_State, &expected, Removing, false, __ATOMIC_ACQ_REL,
                __ATOMIC_ACQUIRE))
        {
            return false;
        }
        ...
        __atomic_store_n(
            &m_Handler, static_cast<SchedulerTimerHandler *>(nullptr),
            __ATOMIC_RELEASE);
    ...
    bool beginDispatch(size_t owner, DispatchGuard &guard)
    {
        ...
        size_t observed =
            __atomic_load_n(&m_State, __ATOMIC_ACQUIRE);
        while ((observed & ModeMask) == Published)
        {
            ...
            if (__atomic_compare_exchange_n(
                    &m_State, &observed, admitted, false,
        ...
        SchedulerTimerHandler *handler =
            __atomic_load_n(&m_Handler, __ATOMIC_ACQUIRE);
    ...
    size_t activeDispatches() const
    {
        const size_t state =
            __atomic_load_n(&m_State, __ATOMIC_ACQUIRE);
        return (state & ModeMask) == Published ?
                   (state & DispatchCountMask) / DispatchIncrement :
                   0;
    }
  ...
  private:
    ...
    void completeDispatch()
    {
        ...
        // admission is live.
        __atomic_fetch_sub(
            &m_State, DispatchIncrement, __ATOMIC_ACQ_REL);
    }
