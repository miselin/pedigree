...
class IrqPolicy {
 public:
  constexpr IrqPolicy(IrqTrigger trigger, IrqControllerAck controllerAck,
                      IrqLineRelease lineRelease)
      : m_Trigger(trigger), m_ControllerAck(controllerAck), m_LineRelease(lineRelease) {}

  static constexpr IrqPolicy edgeHard() {
    return IrqPolicy(IrqTrigger::Edge, IrqControllerAck::BeforeHardStage,
                     IrqLineRelease::AfterHardStage);
  }

  static constexpr IrqPolicy edgeThreaded() {
    return IrqPolicy(IrqTrigger::Edge, IrqControllerAck::AfterHardStage,
                     IrqLineRelease::AfterHardStage);
  }

  static constexpr IrqPolicy levelHard() {
    return IrqPolicy(IrqTrigger::Level, IrqControllerAck::AfterHardStage,
                     IrqLineRelease::AfterHardStage);
  }
  ...
  static constexpr IrqPolicy syntheticHard() {
    return IrqPolicy(IrqTrigger::Synthetic, IrqControllerAck::None, IrqLineRelease::AfterHardStage);
  }
  ...
  constexpr bool validForThreaded() const {
    return valid() &&
           ((m_Trigger == IrqTrigger::Level &&
             m_LineRelease == IrqLineRelease::AfterThreadedCompletion) ||
            (m_Trigger != IrqTrigger::Level && m_LineRelease == IrqLineRelease::AfterHardStage));
  }

  constexpr bool operator==(const IrqPolicy& other) const {
    return m_Trigger == other.m_Trigger && m_ControllerAck == other.m_ControllerAck &&
