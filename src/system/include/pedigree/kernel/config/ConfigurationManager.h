...
enum ConfigValueType { Invalid, Number, Str, Bool };
struct ConfigValue {
  ConfigValue() : str(""), num(0), b(false), type(Invalid) {
    for (int i = 0; i < MAX_WATCHERS; i++)
      watchers[i] = 0;
  }
  ConfigValue(const ConfigValue& cv) : str(cv.str), num(cv.num), b(cv.b), type(cv.type) {
    for (int i = 0; i < MAX_WATCHERS; i++)
...
class ConfigurationManager {
 public:
   ...
   */
  void insert(const String& configStore, const String& table, const String& key,
  ...
  void watch(const String& configStore, const String& table, const String& key,
             ConfigurationWatcher watcher);
  /** Remove a watcher from a table entry. */
  void unwatch(const String& configStore, const String& table, const String& key,
               ConfigurationWatcher watcher);
