...
/**
 * Defines an interface for a console for the debugger to interact with the
 * user.
 *
 * The console can be in "raw" or "cli" mode - In CLI mode it is expected that
 * the writeCli and readCli functions are heavily used - scrolling and
 * backspacing is provided. In "raw" mode, any function (except writeCli and
 * readCli) may be used.
 *
 * The CLI interface has support for contracting the available window used for
 * the CLI. The setCli{Upper|Lower}Limit functions define how many lines the CLI
 * should not touch from the top and bottom of the screen, respectively.
 *
 * Note that columns are numbered starting at 0 from the left, and rows are
 * numbered starting at 0 from the top.
 */
class DebuggerIO
{
  public:
     ...
     */
    DebuggerIO() : m_bReady(true), m_pCommand(), m_bRefreshesEnabled(true)
    {
    }
    virtual ~DebuggerIO()
    {
    ...
    /**
     * Writes the given text out to the CLI, in the given colour and background
     * colour.
     */
    virtual void
    writeCli(const char *str, Colour foreColour, Colour backColour);
    virtual void writeCli(
        char ch, DebuggerIO::Colour foreColour, DebuggerIO::Colour backColour);
    /**
     * Reads a command from the interface. Blocks until a character is pressed,
     * and then the current buffer is returned in *str, and the return value is
     * true if the command is complete (if enter has been pressed). *str will
     * never exceed maxLen.
     */
    ...
    virtual void drawHorizontalLine(
        char c, size_t row, size_t colStart, size_t colEnd, Colour foreColour,
        Colour backColour) = 0;
    virtual void drawVerticalLine(
        char c, size_t col, size_t rowStart, size_t rowEnd, Colour foreColour,
        Colour backColour) = 0;
    ...
    virtual void drawString(
        const char *str, size_t row, size_t col, Colour foreColour,
        Colour backColour) = 0;
