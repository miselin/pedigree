...
struct ThreadWaitDiagnostic
{
    ThreadWaitDiagnostic()
        : sleeping(false), hasWait(false), queue(0), channelOwner(0),
          channelValue(0), reason(WaitQueue::WakeReason::Waiting),
          stateLevel(0), mutexOwner(0), queued(false)
    {
    }
...
inline void appendThreadWaitDiagnostic(
    const ThreadWaitDiagnostic &wait, HugeStaticString &line)
{
    if (!wait.hasWait)
    {
        if (wait.sleeping)
