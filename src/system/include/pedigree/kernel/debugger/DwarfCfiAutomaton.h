...
// These three are different - they have a delta/register number stored in the
// lower 6 bits.
 ...
 * This table has a column for each register and a row for each possible value
 * of the program counter. Obviously, this table would be huge, so instead they
 * encode it using instructions for an imaginary machine.
 *
 * For each function, a new table is made and initialised. Opcodes cause
 * sequential creation of the table. An extra column is added for the CFA
