...
class EXPORTED_PUBLIC PhysicalMemoryManager {
 ...
 public:
  ...
  /** If this flag is set we try to remove the range from the
     PhysicalMemoryManager, but if that fails, we still map the physical
     memory region */
   ...
   *
   * Every time a pinned page is passed to freePage, its refcount is
   * decreased, and the page is only freed once the refcount hits zero.
   *
   ...
   *object \param[in] pageConstraints the constraints the pages have to
   *fullfill \param[in] Flags flags from the VirtualAddressSpace class
   *namespace \param[in] start the physical address of the beginning of the
   *region (optional) \return true, if a valid MemoryRegion object is created,
   *false otherwise */
  virtual bool allocateRegion(MemoryRegion& Region, size_t cPages, size_t pageConstraints,
                              size_t Flags, physical_uintptr_t start = -1) = 0;

  /** Structure containing information about one memory region. */
  struct MemoryRegionInfo {
    /** Constructor initialises all structure members
     *\param[in] pVirtualAddress virtual address of the beginning of the
     *memory region \param[in] PhysicalAddress physical address of the
     *beginning of the memory region (or 0) \param[in] sAddress size (in
     *bytes) of the memory region \param[in] Name user-visible name of the
     *memory region */
    inline MemoryRegionInfo(void* VirtualAddress, physical_uintptr_t PhysicalAddress, size_t size,
                            const char* name)
        : pVirtualAddress(VirtualAddress),
          physicalAddress(PhysicalAddress),
          sVirtualAddress(size),
          pName(name) {}
