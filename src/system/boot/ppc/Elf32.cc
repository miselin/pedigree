...
bool Elf32::writeSections()
{
    ...
    for (int i = 0; i < m_pHeader->shnum; i++)
    {
        if (m_pSectionHeaders[i].flags & SHF_ALLOC)
        {
            ...
            {
                prom_map(
                    physAddress, m_pSectionHeaders[i].addr,
                    m_pSectionHeaders[i].size + 0x1000);
            ...
            for (unsigned int j = m_pSectionHeaders[i].addr;
                 j < m_pSectionHeaders[i].addr + m_pSectionHeaders[i].size;
                 j += 4)
            {
