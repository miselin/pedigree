...
void *call_prom(
    const char *service, int nargs, int nret, void *a1, void *a2, void *a3,
...
void prom_init(prom_entry pe)
{
    ...
    if (prom_chosen == reinterpret_cast<void *>(-1))
        prom_exit();
    if (prom_get_chosen(
            const_cast<char *>("stdout"), &prom_stdout, sizeof(prom_stdout)) <=
        0)
        prom_exit();
    if (prom_get_chosen(
            const_cast<char *>("mmu"), &prom_mmu, sizeof(prom_mmu)) <= 0)
        prom_exit();
...
{
    return call_prom(
        "finddevice", 1, 1, reinterpret_cast<void *>(const_cast<char *>(dev)));
}
...
{
    return reinterpret_cast<int>(call_prom(
        "getprop", 4, 1, dev, reinterpret_cast<void *>(name), buf,
...
void prom_putchar(char c)
{
    if (c == '\n')
        call_prom(
            "write", 3, 1, prom_stdout,
            reinterpret_cast<void *>(const_cast<char *>("\r\n")),
            reinterpret_cast<void *>(2));
    else
        call_prom(
            "write", 3, 1, prom_stdout, reinterpret_cast<void *>(&c),
...
void prom_map(unsigned int phys, unsigned int virt, unsigned int size)
{
    call_prom(
        "call-method", 6, 1,
        reinterpret_cast<void *>(const_cast<char *>("map")),
