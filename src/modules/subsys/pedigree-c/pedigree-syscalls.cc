...
void pedigree_config_getstr(size_t resultIdx, size_t row, const char* col, char* buf,
                            size_t bufsz) {
  if (resultIdx >= MAX_RESULTS || g_Results[resultIdx] == 0)
    return;
...
void pedigree_input_remove_callback(void* p) {
  // First parameter is now obsolete, gotta remove it some time...
  InputManager::instance().removeCallback(reinterpret_cast<InputManager::callback_t>(p), 0,
...
int pedigree_gfx_get_provider(void* p) {
  ...
  if (pFeatures && pService && pFeatures->provides(ServiceFeatures::probe)) {
    if (!pService->serve(ServiceFeatures::probe, reinterpret_cast<void*>(&params),
                         sizeof(params)) ||
...
int pedigree_gfx_create_buffer(void* p, void** b, void* args) {
  ...
  *b = pProvider->pFramebuffer->createBuffer(reinterpret_cast<void*>(pArgs->a),
                                             static_cast<Graphics::PixelFormat>(pArgs->b), pArgs->c,
                                             pArgs->d);
...
void pedigree_gfx_blit(void* p, void* args) {
  ...
  pProvider->pFramebuffer->blit(pArgs->pBuffer, pArgs->srcx, pArgs->srcy, pArgs->destx,
                                pArgs->desty, pArgs->width, pArgs->height);
}
...
void pedigree_gfx_rect(void* p, void* args) {
  ...
  pProvider->pFramebuffer->rect(pArgs->a, pArgs->b, pArgs->c, pArgs->d, pArgs->e,
...
void pedigree_gfx_line(void* p, void* args) {
  ...
  pProvider->pFramebuffer->line(pArgs->a, pArgs->b, pArgs->c, pArgs->d, pArgs->e,
                                static_cast<Graphics::PixelFormat>(pArgs->f));
}
...
void pedigree_gfx_draw(void* p, void* args) {
  ...
  pProvider->pFramebuffer->draw(pArgs->a, pArgs->b, pArgs->c, pArgs->d, pArgs->e, pArgs->f,
                                pArgs->g, static_cast<Graphics::PixelFormat>(pArgs->h));
}
...
int pedigree_gfx_create_fbuffer(void* p, void* args) {
  ...
  GraphicsService::GraphicsProvider* pReturnProvider =
      reinterpret_cast<GraphicsService::GraphicsProvider*>(pArgs->pReturnProvider);

  pReturnProvider->pFramebuffer = Graphics::createFramebuffer(
      pProvider->pFramebuffer, pArgs->x, pArgs->y, pArgs->w, pArgs->h, pArgs->pFramebuffer);
