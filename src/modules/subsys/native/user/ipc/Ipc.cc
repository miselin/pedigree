...
PedigreeIpc::StandardIpcMessage::StandardIpcMessage(void* pKernelMessage) {
  m_vAddr = reinterpret_cast<void*>(syscall2(IPC_RECV_PHASE2, reinterpret_cast<uintptr_t>(this),
                                             reinterpret_cast<uintptr_t>(pKernelMessage)));
}

bool PedigreeIpc::StandardIpcMessage::initialise() {
  m_vAddr = reinterpret_cast<void*>(
      syscall1(IPC_CREATE_STANDARD_MESSAGE, reinterpret_cast<uintptr_t>(this)));
...
bool PedigreeIpc::SharedIpcMessage::initialise() {
  m_vAddr =
      reinterpret_cast<void*>(syscall3(IPC_CREATE_SHARED_MESSAGE, reinterpret_cast<uintptr_t>(this),
                                       m_nBytes, reinterpret_cast<uintptr_t>(m_pHandle)));
  m_pHandle =
      reinterpret_cast<void*>(syscall1(IPC_GET_SHARED_REGION, reinterpret_cast<uintptr_t>(this)));

  return (m_vAddr != 0) && (m_pHandle != 0);
}

bool PedigreeIpc::send(IpcEndpoint pEndpoint, IpcMessage* pMessage, bool bAsync) {
  return static_cast<bool>(syscall3(IPC_SEND_IPC, reinterpret_cast<uintptr_t>(pEndpoint),
                                    reinterpret_cast<uintptr_t>(pMessage),
...
bool PedigreeIpc::recv(IpcEndpoint pEndpoint, IpcMessage** pMessage, bool bAsync) {
  void* kernelPointer = reinterpret_cast<void*>(syscall2(
      IPC_RECV_PHASE1, reinterpret_cast<uintptr_t>(pEndpoint), static_cast<uintptr_t>(bAsync)));
  if (!kernelPointer)
    return false;
