...
static_assert(
    sizeof(Sigcontext) == 256,
    "Linux amd64 sigcontext must be 256 bytes");
static_assert(
    sizeof(Ucontext) == 304, "Linux amd64 ucontext must be 304 bytes");
...
static_assert(
    sizeof(RtSigframe) == 440,
    "Linux amd64 rt_sigframe base must be 440 bytes");
static_assert(
    sizeof(Fpstate) == 512, "Linux amd64 fpstate must be 512 bytes");
static_assert(
    alignof(Fpstate) == 16, "Linux amd64 fpstate must be 16-byte aligned");

static_assert(
    __builtin_offsetof(Ucontext, mcontext) == 40,
    "Linux amd64 ucontext mcontext offset must be 40");
static_assert(
    __builtin_offsetof(RtSigframe, ucontext) == 8,
    "Linux amd64 rt_sigframe ucontext offset must be 8");
static_assert(
    __builtin_offsetof(RtSigframe, info) == 312,
    "Linux amd64 rt_sigframe siginfo offset must be 312");
static_assert(
    __builtin_offsetof(Sigcontext, fpstate) == 184,
