...
uint64_t MeminfoFile::writeBytewise(uint64_t location, uint64_t size, uintptr_t buffer,
                                    bool bCanBlock) {
  return 0;
}

PciDevicesFile::PciDevicesFile(size_t inode, Filesystem* pParentFS, File* pParent)
    : File(String("devices"), 0, 0, 0, inode, pParentFS, 0, pParent), m_Contents() {
  setPermissionsOnly(FILE_UR | FILE_UW | FILE_GR | FILE_GW | FILE_OR);
  setUidOnly(0);
  setGidOnly(0);
...
uint64_t PciDevicesFile::readBytewise(uint64_t location, uint64_t size, uintptr_t buffer,
                                      bool bCanBlock) {
  resync();
...
uint64_t PciDevicesFile::writeBytewise(uint64_t location, uint64_t size, uintptr_t buffer,
                                       bool bCanBlock) {
  return 0;
}
...
void PciDevicesFile::resync() {
  ...
  auto printer = [this](Device* p) -> Device* {
    String initial;
    initial.Format("%02x%02x\t%04x%04x\t%x", p->getPciBusPosition(),
...
uint64_t MountFile::readBytewise(uint64_t location, uint64_t size, uintptr_t buffer,
                                 bool bCanBlock) {
  String mounts;
...
uint64_t MountFile::writeBytewise(uint64_t location, uint64_t size, uintptr_t buffer,
                                  bool bCanBlock) {
  return 0;
}
...
uint64_t UptimeFile::readBytewise(uint64_t location, uint64_t size, uintptr_t buffer,
                                  bool bCanBlock) {
...
uint64_t UptimeFile::writeBytewise(uint64_t location, uint64_t size, uintptr_t buffer,
                                   bool bCanBlock) {
  return 0;
}
...
ConstantFile::ConstantFile(String name, const char* value, size_t size, size_t inode,
...
uint64_t ConstantFile::readBytewise(uint64_t location, uint64_t size, uintptr_t buffer,
                                    bool bCanBlock) {
  if (location >= m_Size) {
    return 0;  // EOF
...
uint64_t ConstantFile::writeBytewise(uint64_t location, uint64_t size, uintptr_t buffer,
                                     bool bCanBlock) {
  return 0;
}
...
bool ProcFs::initialise(Disk* pDisk) {
  ...
  ConstantFile* pCmdline = new ConstantFile(String("cmdline"), cmdline.cstr(), cmdline.length(),
                                            getNextInode(), this, m_pRoot);
  m_pRoot->addEntry(pCmdline->getName(), pCmdline);

  // /proc/version contains some extra version info (not same as uname)
  static String version;
  version.Format("Pedigree version %s (%s@%s) %s", g_pBuildRevision, g_pBuildUser, g_pBuildMachine,
                 g_pBuildTime);
  ConstantFile* pVersion = new ConstantFile(String("version"), version.cstr(), version.length(),
                                            getNextInode(), this, m_pRoot);
  m_pRoot->addEntry(pVersion->getName(), pVersion);

  ProcFsDirectory* pBusDir =
      new ProcFsDirectory(String("bus"), 0, 0, 0, getNextInode(), this, 0, m_pRoot);
  ProcFsDirectory* pPciDir =
      new ProcFsDirectory(String("pci"), 0, 0, 0, getNextInode(), this, 0, pBusDir);
  ...
  auto printer = [pPciDir, this](Device* p) -> Device* {
    ...
    if (!e) {
      ...
      ConstantFile* cf = new ConstantFile(fn, reinterpret_cast<const char*>(&space), sizeof(space),
