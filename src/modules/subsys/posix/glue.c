...
int select(int nfds, struct fd_set* readfds, struct fd_set* writefds, struct fd_set* errorfds,
           struct timeval* timeout) {
  return (long)syscall5(POSIX_SELECT, nfds, (long)readfds, (long)writefds, (long)errorfds,
                        (long)timeout);
}
...
ssize_t recvfrom(int sock, void* buff, size_t bufflen, int flags, struct sockaddr* remote_addr,
                 size_t* addrlen) {
...
ssize_t sendto(int sock, const void* buff, size_t bufflen, int flags,
...
int clock_getres(clockid_t clock_id, struct timespec* res) {
  return syscall2(POSIX_CLOCK_GETRES, clock_id, (long)res);
}
