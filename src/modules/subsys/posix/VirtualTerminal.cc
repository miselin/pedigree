...
bool VirtualTerminalManager::initialise() {
  ...
  for (size_t i = 1; i < 8; ++i) {
    ...
    if (tio->initialise(true)) {
      ConsolePhysicalFile* file = new ConsolePhysicalFile(i + 1, tio, ttyname, g_pDevFs);
...
void VirtualTerminalManager::reportPermission(SwitchPermission perm) {
  if (m_WantedTty == static_cast<size_t>(~0)) {
    ...
    NOTICE(
        "VirtualTerminalManager: can't acknowledge as no switch in "
        "progress");
    return;
  }
...
void VirtualTerminalManager::sendSignal(size_t n, bool acq) {
  ...
  if (!Scheduler::instance().acquireProcess(process, m_Terminals[n].owner)) {
    return;
  }
