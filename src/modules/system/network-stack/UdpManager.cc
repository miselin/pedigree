...
int UdpEndpoint::send(
    size_t nBytes, uintptr_t buffer, RemoteEndpoint remoteHost, bool broadcast,
    Network *pCard)
{
    bool success = Udp::instance().send(
        remoteHost.ip, getLocalPort(), remoteHost.remotePort, nBytes, buffer,
        broadcast, pCard);
    return success ? nBytes : -1;
};

int UdpEndpoint::recv(
    uintptr_t buffer, size_t maxSize, bool bBlock, RemoteEndpoint *remoteHost,
    int nTimeout)
{
    ...
    /// \todo we should replace this with proper condition checking and use the
    /// Mutex
    ...
    if (bDataReady)
    {
        ...
        if (ptr->magic != 0xdeadbeef)
        {
            ...
            ERROR("UDP packet information is corrupted - magic is not "
                  "0xdeadbeef!");
            ERROR("The packet cannot be forwarded to the application *or* "
                  "freed.");
            delete ptr;
            return 0;
        }
        ...
        MemoryCopy(
            ...
            reinterpret_cast<void *>(ptr->ptr + ptr->offset), nBytes);
...
size_t UdpEndpoint::depositPayload(
...
Endpoint *UdpManager::getEndpoint(
    IpAddress remoteHost, uint16_t localPort, uint16_t remotePort)
{
