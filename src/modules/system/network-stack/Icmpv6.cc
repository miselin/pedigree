    ...
    Network *pCard)
{
    ...
    uint16_t checksum =
        Ipv6::instance().ipChecksum(from, to, IP_ICMPV6, packet, nBytes);
    if (checksum)
    {
        WARNING(
            "ICMPv6: checksum incorrect on incoming packet from "
            << from.toString());
        pCard->badPacket();
        return;
    }
    ...
    switch (pHeader->type)
    {
        case ICMPV6_ECHOREQ:
            ...
            send(
                from, to, ICMPV6_ECHORESP, pHeader->code,
                packet + sizeof(icmpv6Header), nBytes - sizeof(icmpv6Header),
                pCard);
            break;
        ...
        case ICMPV6_NADVERT:
            Ndp::instance().receive(
                from, to, pHeader->type, pHeader->code,
                packet + sizeof(icmpv6Header), nBytes - sizeof(icmpv6Header),
                pCard);
            break;
    }
    ...
    uintptr_t payload, size_t nBytes, Network *pCard)
{
    ...
    {
        WARNING("ICMPv6: Packet had a multicast destination, but no given "
                "network interface. Can't figure out where to send this!");
        return;
    }
    ...
    if (dest.isUnicast() && !dest.isLinkLocal())
    {
        ...
        {
            WARNING(
                "ICMPv6: Couldn't find a route for destination '"
                << dest.toString() << "'.");
            return;
        }
    }
    ...
    if (nBytes)
        MemoryCopy(
            reinterpret_cast<void *>(packet + sizeof(icmpv6Header)),
            reinterpret_cast<void *>(payload), nBytes);

    header->checksum = 0;
    header->checksum = Ipv6::instance().ipChecksum(
        from, dest, IP_ICMPV6, packet, sizeof(icmpv6Header) + nBytes);

    Ipv6::instance().send(
        dest, from, IP_ICMPV6, nBytes + sizeof(icmpv6Header), packet, pCard);
