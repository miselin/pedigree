...
StateBlock::StateBlock() :
  ...
  lock(false), cond(), endpoint(0), connId(0),
  retransmitQueue(), nRemovedFromRetransmit(0),
  waitingForTimeout(false), didTimeout(false), useWaitSem(true),
...
void StateBlock::startCleanup()
{
    if (!waitingForTimeout)
    {
        Time::runAfter(performCleanupTrampoline, this, Time::Multiplier::MINUTE * 2);
