...
class StateBlock
{
  ...
  public:
    ...
    uint32_t rcv_wnd;  // receive window ----> How much we want to receive
                       // methinks...
    ...
    /// \note This will remove acked segments, however if there is only a
    /// partial ack on a segment
    ///       it will split it into two, remove the first, and leave a partial
    ///       segment on the queue. This behaviour does not affect anything
    ///       internally as long as this function is always used to acknowledge
    ///       segments.
    void ackSegment();
    ...
    /// Sends a segment over the network
    bool sendSegment(
        uint8_t flags, size_t nBytes, uintptr_t payload,
    ...
    /// \note This ensures that, if we end up releasing the timeout wait
    /// semaphore
    ///       via a non-timeout source (such as a data ack) we know where the
    ///       release actually came from.
    bool didTimeout;
