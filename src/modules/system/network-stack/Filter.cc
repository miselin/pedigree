...
NetworkFilter::NetworkFilter()
    : m_Callbacks(), m_Lock(), m_NextCallbackId(1),
      m_pActiveInvocations(nullptr)
{
}
...
NetworkFilter::~NetworkFilter()
{
    for (size_t level = 1; level <= 4; ++level)
    {
        while (true)
        {
            m_Lock.acquire();
            CallbackItem *item =
                m_Callbacks[level - 1].count()
                    ? *m_Callbacks[level - 1].begin()
                    : nullptr;
...
bool NetworkFilter::isSelfRemoval(
    CallbackItem *item, Thread *thread) const
{
