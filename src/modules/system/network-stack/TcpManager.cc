...
TcpManager::TcpManager()
    : m_NextTcpSequence(1), m_NextConnId(1), m_StateBlocks(),
      m_ListeningStateBlocks(), m_CurrentConnections(), m_Endpoints(),
      m_ListenPorts(), m_EphemeralPorts(), m_TcpMutex(false),
      m_SequenceMutex(false), m_Nanoseconds(0), m_ThreadHandle(nullptr),
      m_bAlive(false)
{
    manager = this;
    ...
    m_bAlive = true;
    m_ThreadHandle =
        pocketknife::runConcurrentlyAttached(sequenceIncrementer, this);
}
...
size_t TcpManager::Listen(Endpoint *e, uint16_t port, Network *pCard)
{
    ...
    // Allocate the port now - just about to register the connection.
    {
        ...
        {
            ERROR(
                "Can't listen on already-used port " << Dec << port << Hex
                                                     << "!");
            delete handle;
            delete stateBlock;
            return 0;
...
size_t TcpManager::Connect(
    Endpoint::RemoteEndpoint remoteHost, uint16_t localPort,
    TcpEndpoint *endpoint, bool bBlock)
{
    if (!endpoint)
        return 0;
    ...
    stateBlock->tcp_mss =
        1460;  /// \todo Base this on the MTU of the link, or PMTU Discovery.
    ...
    Tcp::send(
        stateBlock->remoteHost.ip, stateBlock->localPort,
        stateBlock->remoteHost.remotePort, stateBlock->iss, 0, Tcp::SYN,
    ...
    if ((stateBlock->currentState != Tcp::ESTABLISHED) || timedOut)
    {
        ...
        return 0;  /// \todo Keep track of an error number somewhere in
                   /// StateBlock
    }
    else
    {
...
void TcpManager::Disconnect(size_t connectionId)
{
    ...
    {
        NOTICE(
            "Connection Id " << Dec << connectionId << Hex
                             << " is trying to close but isn't valid state ["
    ...
    bool addToRetransmitQueue)
{
    ...
    if (stateBlock->currentState != Tcp::ESTABLISHED &&
        ...
        */
        return -1;  // When we SHUT_WR, we send FIN meaning no more data from
                    // us.

    stateBlock->sendSegment(
        Tcp::ACK | (push ? Tcp::PSH : 0), nBytes, payload,
        addToRetransmitQueue);
