...
void Ndp::receive(
    IpAddress from, IpAddress to, uint8_t icmpType, uint8_t icmpCode,
    uintptr_t payload, size_t nBytes, Network *pCard)
{
    ...
    switch (icmpType)
    {
        ...
        case NDP_RADVERT:
        {
            ...
            Option *pOption = reinterpret_cast<Option *>(
                payload + sizeof(RouterAdvertisement));
            if (nBytes > sizeof(RouterAdvertisement))
            {
                while (reinterpret_cast<uintptr_t>(pOption) <
                       (payload + nBytes))
                {
                    // Prefix information.
                    if (pOption->type == 0x3)
                    {
                        PrefixInformationOption *pPrefix =
                            reinterpret_cast<PrefixInformationOption *>(
                                pOption);

                        IpAddress prefix(pPrefix->prefix);

                        /// \todo Store the router address somewhere. We need it
                        /// to route packets!

                        // Have we already associated with this router?
                        /// \todo Cache this rather than recalculate on each
                        /// advertisement.
                        ...
                        // If we are allowed to allocate autoconfiguration
                        // addresses from this prefix, do so.
                        if ((!bAlreadyAssociated) &&
                            (pPrefix->rsvdFlags & 0x40))
                        {
                            // Create an IPv6-modified EUI64.
                            uint8_t eui[8];
                            Ipv6::getIpv6Eui64(me.mac, eui);

                            // Throw it into the base of the new IPv6 address we
                            // are creating here.
                            ...
                            // Update the NIC's information for the rest of the
                            // stack to use.
                            ...
                            RoutingTable::instance().Add(
                                RoutingTable::DestIpv6Sub, *pNewAddress,
                                IpAddress(localhost), String(""), pCard);
                            RoutingTable::instance().Add(
                                RoutingTable::DestPrefix, *pNewAddress,
                                IpAddress(), IpAddress(), String(""), pCard);
                            RoutingTable::instance().Add(
                                RoutingTable::DestPrefixComplement,
                                *pNewAddress, IpAddress(), from, String(""),
                                pCard);

                            // Add as a default route if none exists
                            if (!RoutingTable::instance().DefaultRouteV6())
                                RoutingTable::instance().Add(
                                    RoutingTable::NamedV6, IpAddress(),
                                    IpAddress(), String("default"), pCard);
                        }
                    }

                    pOption = reinterpret_cast<Option *>(
                        reinterpret_cast<uintptr_t>(pOption) +
                        (pOption->length * 8));
                }
            }
        }
        ...
        case NDP_SOLICIT:
        {
            NeighbourSolicitation *pMessage =
                reinterpret_cast<NeighbourSolicitation *>(payload);
            ...
            // Grab the link-layer address of the sender, if any, and add it to
            // our cache.
            Option *pOption = reinterpret_cast<Option *>(
                payload + sizeof(NeighbourSolicitation));
            if (nBytes > sizeof(NeighbourSolicitation))
            {
                       ...
                       (payload + nBytes))
                {
                    ...
                    if (pOption->type & 0x3)
                    {
                        LinkLayerAddressOption *p =
                            reinterpret_cast<LinkLayerAddressOption *>(pOption);
                        MacAddress mac;
            ...
            NeighbourAdvertisement *pReply =
                reinterpret_cast<NeighbourAdvertisement *>(packet);
            packetTarget.getIp(pReply->target);
            pReply->flags = NADVERT_FLAGS_SOLICIT << 5;

            // Add the options we need.
            LinkLayerAddressOption *pAddrOption =
                reinterpret_cast<LinkLayerAddressOption *>(
                    packet + sizeof(NeighbourAdvertisement));
            pAddrOption->type = 2;
            pAddrOption->length = sizeof(LinkLayerAddressOption) / 8;
            MemoryCopy(pAddrOption->address, me.mac.getMac(), 6);

            // Send the response.
            Icmpv6::instance().send(
                from, *pMatch, NDP_ADVERT, 0, packet,
                sizeof(NeighbourAdvertisement) + sizeof(LinkLayerAddressOption),
                pCard);
...
bool Ndp::routerSolicit(Network *pCard)
{
    ...
    uintptr_t packet = NetworkStack::instance().getMemPool().allocate();
    RouterSolicitation *solicit =
        reinterpret_cast<RouterSolicitation *>(packet);
    ...
    Icmpv6::instance().send(
        to, from, NDP_RSOLICIT, 0, packet, sizeof(RouterSolicitation), pCard);
    NetworkStack::instance().getMemPool().free(packet);
...
bool Ndp::neighbourSolicit(IpAddress addr, MacAddress *pMac, Network *pCard)
{
    ...
    uintptr_t packet = NetworkStack::instance().getMemPool().allocate();
    NeighbourSolicitation *solicit =
        reinterpret_cast<NeighbourSolicitation *>(packet);
    solicit->reserved = 0;
    ...
    LinkLayerAddressOption *pAddrOption =
        reinterpret_cast<LinkLayerAddressOption *>(
            packet + sizeof(NeighbourSolicitation));
    pAddrOption->type = 1;
    pAddrOption->length = sizeof(LinkLayerAddressOption) / 8;
    MemoryCopy(pAddrOption->address, me.mac.getMac(), 6);
    ...
    // Send the solicit packet.
    Icmpv6::instance().send(
        to, from, NDP_SOLICIT, 0, packet,
        sizeof(NeighbourSolicitation) + sizeof(LinkLayerAddressOption), pCard);
    NetworkStack::instance().getMemPool().free(packet);
