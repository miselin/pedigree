...
bool Udp::send(
    ...
    uintptr_t payload, bool broadcast, Network *pCard)
{
    // IP base for all operations here.
    /// \todo Abstract this out so that we don't have to have this specific code
    /// in the protocol.
    ...
    if (!pCard)
    {
        if (broadcast)
        {
            WARNING("UDP: No NIC given to send(), and attempting to send a "
                    "broadcast packet!");
            return false;
        }
        ...
        {
            WARNING("UDP: No NIC given to send(), and no routes available in "
                    "the routing table");
            return false;
        }
        ...
        {
            WARNING(
                "UDP: Couldn't find a route for destination '"
                << dest.toString() << "'.");
            return false;
        }
    }
    ...
    if (nBytes)
        MemoryCopy(
            reinterpret_cast<void *>(packet + sizeof(udpHeader)),
            reinterpret_cast<void *>(payload), nBytes);

    // Calculate the checksum
    header->checksum = pIp->ipChecksum(
        src, dest, IP_UDP, reinterpret_cast<uintptr_t>(header),
        sizeof(udpHeader) + nBytes);

    // Transmit
    bool success =
        pIp->send(dest, src, IP_UDP, nBytes + sizeof(udpHeader), packet, pCard);
    ...
    Network *pCard)
{
    ...
    {
        // not for us, TODO: check a flag to see if we'll accept these sorts of
    packets
    ...
    // Find the payload and its size - udpHeader::len is the size of the header
    ...
    if (header->checksum != 0)
    {
        uint16_t checksum = pIp->ipChecksum(
            from, to, IP_UDP, reinterpret_cast<uintptr_t>(header), header_len);
        if (checksum)
        {
            WARNING(
                "UDP Checksum failed on incoming packet ["
                << header->checksum << ", and " << checksum
                << " should be zero]!");
            pCard->badPacket();
            return;
        }
    ...
    UdpManager::instance().receive(
        from, to, BIG_TO_HOST16(header->src_port),
        BIG_TO_HOST16(header->dest_port), payload, payloadSize, pCard);
}
