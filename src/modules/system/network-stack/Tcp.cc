...
bool Tcp::send(
    IpAddress dest, uint16_t srcPort, uint16_t destPort, uint32_t seqNumber,
    uint32_t ackNumber, uint8_t flags, uint16_t window, size_t nBytes,
    uintptr_t payload)
{
    ...
    {
        WARNING(
            "TCP: Couldn't find a route for destination '" << dest.toString()
                                                           << "'.");
        return false;
    }
    ...
    if (flags & Tcp::SYN)
    {
        ...
        MemoryCopy(
            reinterpret_cast<void *>(tcpPacket + payloadOffset), mss,
            sizeof mss);
        payloadOffset += 4;
    }
    ...
    if (payload && nBytes)
        MemoryCopy(
            reinterpret_cast<void *>(tcpPacket + payloadOffset),
            reinterpret_cast<void *>(payload), nBytes);

    header->checksum = 0;
    header->checksum =
        pIp->ipChecksum(src, dest, IP_TCP, tcpPacket, nBytes + payloadOffset);

    // Transmit
    bool success =
        pIp->send(dest, src, IP_TCP, nBytes + payloadOffset, packet, pCard);
...
void Tcp::receive(
    IpAddress from, IpAddress to, uintptr_t packet, size_t nBytes, IpBase *pIp,
    Network *pCard)
{
    if (!packet || !nBytes)
        return;
    ...
    if (from.getType() == IpAddress::IPv4 && cardInfo.ipv4 != to &&
        to.getIp() != 0xffffffff && cardInfo.broadcast != to)
    {
        // not for us (depending on future requirements, we may need to
        // implement a "catch-all" flag in the same way as UDP)
        return;
    }
    ...
    // find the payload and its size - tcpHeader::len is the size of the header
    // + data we use it rather than calculating the size from offsets in order
    // to be able to handle packets that may have been padded (for whatever
    // reason)
    uintptr_t payload = reinterpret_cast<uintptr_t>(header) +
                        headerSize;  // offset is in DWORDs
    ...
    {
        uint16_t checksum = pIp->ipChecksum(
            from, to, IP_TCP, reinterpret_cast<uintptr_t>(header), nBytes);
        if (checksum)
        {
            WARNING(
                "TCP Checksum failed on incoming packet [dp="
                << Dec << BIG_TO_HOST16(header->dest_port) << Hex
                << "]. Header checksum is " << header->checksum
                << ", calculated should be zero but is " << checksum << "!");
            pCard->badPacket();
            return;
        }
    }
    ...
    // NOTICE("TCP: Packet has arrived! Dest port is " << Dec <<
    // BIG_TO_HOST16(header->dest_port) << Hex << " and flags are " <<
    // header->flags << ".");

    // either no checksum, or calculation was successful, either way go on to
    // handle it
    NOTICE(
        "packet is " << nBytes << " bytes, payload is " << payloadSize
                     << " bytes");
    TcpManager::instance().receive(
        from, BIG_TO_HOST16(header->src_port), BIG_TO_HOST16(header->dest_port),
        header, payload, payloadSize, pCard);
}
