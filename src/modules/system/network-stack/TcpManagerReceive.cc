    ...
    Network *pCard)
{
    ...
    if ((stateBlock = m_StateBlocks.lookup(handle)) == 0)
    {
        ...
        {
            // Port doesn't exist, so temporary stateBlock required for proper
            // RST handle
            ...
            WARNING(
                "TCP Packet arriving on port " << Dec << handle.localPort << Hex
                                               << " has no destination.");
    ...
    NOTICE(
        ...
        << " [remote port = " << Dec << stateBlock->remoteHost.remotePort << Hex
        << "] [connId = " << stateBlock->connId << "].");

    // dump some pretty information
    NOTICE(
        " + SEQ=" << Dec << stateBlock->seg_seq << " [+"
                  << (stateBlock->seg_seq - stateBlock->irs)
                  << "] ACK=" << stateBlock->seg_ack << " [+"
                  << (stateBlock->seg_ack - stateBlock->iss)
                  << "] LEN=" << stateBlock->seg_len
                  << " WND=" << stateBlock->seg_wnd << Hex);
    NOTICE(
        " + FLAGS: " << (header->flags & Tcp::FIN ? "FIN " : "")
    ...
    switch (stateBlock->currentState)
    {
        ...
        case Tcp::CLOSED:
            ...
            if (!(header->flags & Tcp::RST))
            {
                ...
                if (!Tcp::send(
                        from, handle.localPort, handle.remotePort, seq, ack,
                        flags, window, 0, 0))
                    WARNING("TCP: Sending RST due to incoming segment while in "
                            "CLOSED state failed.");
            }
        ...
        case Tcp::LISTEN:
            ...
            else if (header->flags & Tcp::ACK)
            {
                ...
                if (!Tcp::send(
                        ...
                        stateBlock->seg_ack, 0, Tcp::RST, 0, 0, 0))
                    WARNING("TCP: Sending RST due to ACK while in LISTEN state "
                            "failed.");
            }
            else if (header->flags & Tcp::SYN)
            {
                // Create a new server state block for this incoming connection,
                // and register the new client connection ID with the listening
                // endpoint
                ...
                if (!newStateBlock)
                {
                    // If we don't get this new block, pretend we're not
                    // listening
                    if (!Tcp::send(
                            from, handle.localPort, handle.remotePort, 0,
                            stateBlock->seg_ack, Tcp::RST | Tcp::ACK, 0, 0, 0))
                        WARNING("TCP: Couldn't ACK a SYN because no memory was "
                                "available for a state block.");
                    return;
                }
                ...
                if (!tmp)
                {
                    ...
                    if (!Tcp::send(
                            ...
                            stateBlock->seg_ack, Tcp::RST | Tcp::ACK, 0, 0, 0))
                        WARNING("TCP: Couldn't ACK a SYN because no memory was "
                                "available for a state block handle.");
                    return;
                }
                ...
                *tmp = handle;
                ...
                if (!Tcp::send(
                        dest, newStateBlock->localPort,
                        newStateBlock->remoteHost.remotePort,
                        newStateBlock->iss, newStateBlock->rcv_nxt,
                        Tcp::SYN | Tcp::ACK, newStateBlock->snd_wnd, 0, 0))
                    WARNING("TCP: Sending SYN/ACK failed");
            }
            else
            {
                WARNING(
                    "TCP Packet incoming on port "
                    << Dec << handle.localPort << Hex
                    << " during LISTEN without RST, ACK, or SYN set.");
            }
        ...
        case Tcp::SYN_SENT:
            ...
            if (header->flags & Tcp::ACK)
            {
                if ((stateBlock->seg_ack <= stateBlock->iss) ||
                    (stateBlock->seg_ack > stateBlock->snd_nxt))
                {
                    NOTICE(
                        "TCP Packet arriving on port "
                        << Dec << handle.localPort << Hex
                        << " during SYN-SENT has unacceptable ACK 1.");
                    ...
                    if (!(header->flags & Tcp::RST))
                    {
                        if (!Tcp::send(
                                from, handle.localPort, handle.remotePort,
                                stateBlock->seg_ack, 0, Tcp::RST, 0, 0, 0))
                            WARNING("TCP: Sending RST due to invalid ACK while "
                                    "in SYN_SENT state failed.");
                        break;
                    }
                }

                if (!((stateBlock->snd_una <= stateBlock->seg_ack) &&
                      (stateBlock->seg_ack <= stateBlock->snd_nxt)))
                {
                    // ACK unacceptable
                    NOTICE(
                        "TCP Packet arriving on port "
                        << Dec << handle.localPort << Hex
                        << " during SYN-SENT has unacceptable ACK 2.");
                    break;
                }
            }
            ...
            if (header->flags & Tcp::SYN)
            {
                if (header->flags & Tcp::ACK)
                {
                    ...
                    if (stateBlock->snd_una > stateBlock->iss)
                    {
                        ...
                        if (!Tcp::send(
                                from, handle.localPort, handle.remotePort,
                                stateBlock->snd_nxt, stateBlock->rcv_nxt,
                                Tcp::ACK, stateBlock->snd_wnd, 0, 0))
                            WARNING("TCP: Sending ACK due to SYN/ACK while in "
                                    "SYN_SENT state failed.");
                    ...
                    else
                    {
                        ...
                        if (!Tcp::send(
                                from, handle.localPort, handle.remotePort,
                                stateBlock->iss, stateBlock->rcv_nxt,
                                Tcp::SYN | Tcp::ACK, stateBlock->snd_wnd, 0, 0))
                            WARNING("TCP: Sending SYN/ACK due to incorrect "
                                    "SYN/ACK while in SYN_SENT state failed.");
        ...
        /* These all have the same style of handling (with special handling as
         * needed) */
        ...
        case Tcp::LAST_ACK:
            ...
            if (header->flags & Tcp::SYN)
            {
                ...
                if (!Tcp::send(
                        from, handle.localPort, handle.remotePort,
                        stateBlock->snd_nxt, stateBlock->rcv_nxt,
                        Tcp::ACK | Tcp::RST, stateBlock->snd_wnd, 0, 0))
                    WARNING("TCP: Sending RST due to SYN during non-SYN phase "
                            "failed.");
                break;
            }
            ...
            if ((stateBlock->seg_len == 0) && (stateBlock->rcv_wnd == 0))
            {
                ...
                if (!(stateBlock->seg_seq == stateBlock->rcv_nxt))
                {
                    NOTICE(
                        "TCP Packet arriving on port "
                        << Dec << handle.localPort << Hex << " during "
                        << Tcp::stateString(stateBlock->currentState)
                        << " is unacceptable 1.");
                    if (!Tcp::send(
                            from, handle.localPort, handle.remotePort,
                            stateBlock->snd_nxt, stateBlock->rcv_nxt, Tcp::ACK,
                            stateBlock->snd_wnd, 0, 0))
                        WARNING("TCP: Sending ACK due to unacceptable ACK (1) "
                                "while in post-SYN_SENT state failed.");
                    break;
                }
            }

            /*
              (NN) TCP Packet arriving on port 1234 during SYN-RECEIVED is
              unacceptable 2. (NN)     >> RCV_NXT = 0xd3c9b468 (NN)     >>
              SEG_SEQ = 0xd3c9b467 (NN)     >> RCV_NXT + RCV_WND = 0xd3c9cae8
              */

            if ((stateBlock->seg_len == 0) && (stateBlock->rcv_wnd > 0))
            {
                // Unacceptable
                if (!((stateBlock->rcv_nxt <= stateBlock->seg_seq) &&
                      (stateBlock->seg_seq <
                       (stateBlock->rcv_nxt + stateBlock->rcv_wnd))))
                {
                    NOTICE(
                        "TCP Packet arriving on port "
                        << Dec << handle.localPort << Hex << " during "
                    ...
                    if (!Tcp::send(
                            ...
                            stateBlock->snd_nxt, stateBlock->rcv_nxt, Tcp::ACK,
                            stateBlock->snd_wnd, 0, 0))
                        WARNING("TCP: Sending ACK due to unacceptable ACK (2) "
                                "while in post-SYN_SENT state failed.");
                    break;
                }
            }
            ...
            if ((stateBlock->seg_len > 0) && (stateBlock->rcv_wnd == 0))
            {
                ...
                if (!Tcp::send(
                        ...
                        stateBlock->snd_nxt, stateBlock->rcv_nxt, Tcp::ACK,
                        stateBlock->snd_wnd, 0, 0))
                    WARNING("TCP: Sending ACK due to unacceptable ACK (3) "
                            "while in post-SYN_SENT state failed.");
                break;
            }
            ...
            if ((stateBlock->seg_len > 0) && (stateBlock->rcv_wnd > 0))
            {
                if (!(((stateBlock->rcv_nxt <= stateBlock->seg_seq) &&
                       (stateBlock->seg_seq <
                        (stateBlock->rcv_nxt + stateBlock->rcv_wnd))) ||
                      ((stateBlock->rcv_nxt <=
                        (stateBlock->seg_seq + stateBlock->seg_len - 1)) &&
                       ((stateBlock->seg_seq + stateBlock->seg_len - 1) <
                        (stateBlock->rcv_nxt + stateBlock->rcv_wnd)))))
                {
                    NOTICE(
                    ...
                    if (!Tcp::send(
                            ...
                            stateBlock->snd_nxt, stateBlock->rcv_nxt, Tcp::ACK,
                            stateBlock->snd_wnd, 0, 0))
                        WARNING("TCP: Sending ACK due to unacceptable ACK (4) "
                                "while in post-SYN_SENT state failed.");
                    break;
                }
            }
            ...
            if (header->flags & Tcp::RST)
            {
                switch (stateBlock->currentState)
                {
                    ...
                    case Tcp::CLOSE_WAIT:
                        ...
                        /// \todo recv/send need to handle the connection being
                        /// reset
            ...
            if (header->flags & Tcp::ACK)
            {
                ...
                switch (stateBlock->currentState)
                {
                    case Tcp::SYN_RECEIVED:
                    {
                              ...
                              stateBlock->seg_ack <= stateBlock->snd_nxt))
                        {
                            NOTICE(
                                ...
                                << Dec << handle.localPort << Hex << " during "
                                << Tcp::stateString(stateBlock->currentState)
                                << " is an unacceptable segment ACK.");
                            if (!Tcp::send(
                                    from, handle.localPort, handle.remotePort,
                                    stateBlock->seg_ack, 0, Tcp::RST, 0, 0, 0))
                                WARNING("TCP: Sending ACK due to unacceptable "
                                        "segment ACK while in post-SYN_SENT "
                                        "state failed.");
                            break;
                        }
                        ...
                        if (!parent)
                        {
                            WARNING("TCP State Block is in SYN_RECEIVED but "
                                    "has no parent endpoint!");
                            return;
                        }

                        stateBlock->endpoint = new TcpEndpoint(
                            connId, from, stateBlock->localPort,
                            stateBlock->remoteHost.remotePort);
                        if (!stateBlock->endpoint)
                        {
                            removeConn(connId);
                            if (!Tcp::send(
                                    from, handle.localPort, handle.remotePort,
                                    stateBlock->seg_ack, 0, Tcp::RST, 0, 0, 0))
                                WARNING("TCP: Sending RST due to no memory for "
                                        "incoming connection's endpoint");
                            return;
                        }
                    ...
                    case Tcp::CLOSING:
                        ...
                        // Remove from retransmission queue any acknowledged
                        // packets...
                        // stateBlock->retransmitQueue.remove(stateBlock->snd_una
                        // - stateBlock->nRemovedFromRetransmit,
                        // stateBlock->seg_len);
                        // stateBlock->nRemovedFromRetransmit +=
                        // (stateBlock->seg_ack - stateBlock->snd_una);
                        ...
                        {
                            // Ack the ack with the proper sequence number,
                            // because the remote TCP has ack'd data that hasn't
                            // been sent
                            if (!Tcp::send(
                                    from, handle.localPort, handle.remotePort,
                                    stateBlock->snd_nxt, stateBlock->seg_seq,
                                    Tcp::ACK, stateBlock->snd_wnd, 0, 0))
                                WARNING("TCP: Sending ACK with proper sequence "
                                        "number (remote TCP ack'd data that we "
                                        "didn't send) failed.");
                            else
                                alreadyAck = true;
                            break;
                        ...
                        if (stateBlock->snd_una < stateBlock->seg_ack &&
                            stateBlock->seg_ack <= stateBlock->snd_nxt)
                        {
                            if ((stateBlock->snd_wl1 < stateBlock->seg_seq) ||
                        ...
                        if (stateBlock->currentState == Tcp::FIN_WAIT_1)
                        {
                            if (stateBlock->fin_seq <= stateBlock->seg_ack)
                            {
                                ...
                                stateBlock->fin_ack =
                                    true;  // FIN has been acked
                            }
                        }
                        else if (stateBlock->currentState == Tcp::FIN_WAIT_2)
                        {
                            // user's close can return now, but no deletion of
                            // the state block yet
                    ...
                    case Tcp::TIME_WAIT:
                        ...
                        // only a FIN can come in during this state, ACK will be
                        // performed later on
                        stateBlock->resetTimer(
                            120);  // 2 minute timeout for TIME_WAIT
                ...
                stateBlock->currentState == Tcp::FIN_WAIT_2)
            {
                ...
                else if (stateBlock->seg_seq > stateBlock->rcv_nxt)
                {
                    ...
                    if (!Tcp::send(
                            ...
                            stateBlock->snd_wnd, 0, 0))
                        WARNING(
                            "TCP: Sending ACK for out-of-order data failed!");
                    else
                        alreadyAck = true;
                }
                else if (stateBlock->seg_len)
                {
                    ...
                    if (stateBlock->endpoint)
                    {
                        ...
                        if (stateBlock->seg_len >
                            stateBlock->endpoint->m_ShadowDataStream
                                .getRemainingSize())
                        {
                            stateBlock->seg_len =
                                stateBlock->endpoint->m_ShadowDataStream
                        ...
                        /// \todo We should queue this ACK and transmit it when
                        /// the timer fires (200ms max wait)
                        size_t winChange =
                            stateBlock->endpoint->depositTcpPayload(
                                stateBlock->seg_len, payload,
                                stateBlock->seg_seq - stateBlock->irs - 1,
                                (header->flags & Tcp::PSH) == Tcp::PSH);
                        stateBlock->rcv_nxt += winChange;
                        if (winChange > stateBlock->rcv_wnd)
                        {
                            WARNING(
                                "TCP: incoming data was larger than rcv_wind");
                        ...
                        if (!Tcp::send(
                                ...
                                Tcp::ACK, stateBlock->snd_wnd, 0, 0))
                            WARNING(
                                "TCP: Sending ACK for incoming data failed!");
                        else
                            alreadyAck = true;
                    }
            ...
            if (header->flags & Tcp::FIN)
            {
                ...
                // FIN means the remote host has nothing more to send, so push
                // any remaining data to the application
                if (stateBlock->endpoint)
                ...
                switch (stateBlock->currentState)
                {
                    ...
                    case Tcp::FIN_WAIT_1:
                        ...
                        // already been acked previously? if so this needs to go
                        // to TIME_WAIT if NOT, closing
    ...
    if (oldState != stateBlock->currentState)
    {
        ...
        NOTICE(
            "TCP Packet arriving on port "
            << Dec << handle.localPort << Hex << " caused state change from "
            << Tcp::stateString(oldState) << " to "
            << Tcp::stateString(stateBlock->currentState) << ".");
#endif
        if (stateBlock->endpoint)
        {
    ...
    if (stateBlock->currentState == Tcp::CLOSED)
    {
        ...
        NOTICE(
            "TCP Packet arriving on port " << Dec << handle.localPort << Hex
                                           << " caused connection to close.");
#endif

        // If we are in a state that's not created by user intervention, we can
        // safely remove and close the connection both of these require the user
        // to go through a close() operation, which inherently calls disconnect.
        /// \note If you're working in kernel space, and you close a connection,
        /// *DO NOT* return the endpoint
