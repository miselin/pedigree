    ...
    bool broadcast, Network *pCard)
{
    ...
    if (!pCard)
    {
        ...
        {
            WARNING(
                "RAW: Couldn't find a route for destination '"
    ...
    {
        case RAW_ICMP:
            success = Ipv4::instance().send(
                remoteHost.ip, pCard->getStationInfo().ipv4, IP_ICMP, nBytes,
                buffer);
            if (success)
        ...
        case RAW_UDP:
            success = Ipv4::instance().send(
                remoteHost.ip, pCard->getStationInfo().ipv4, IP_UDP, nBytes,
                buffer);
            if (success)
                return static_cast<int>(nBytes);
            break;
        case RAW_TCP:
            success = Ipv4::instance().send(
                remoteHost.ip, pCard->getStationInfo().ipv4, IP_TCP, nBytes,
                buffer);
            if (success)
                return static_cast<int>(nBytes);
            break;

        // RAW_WIRE is for them PF_SOCKET things... it'll allow you direct
        // access to the very bottom level of the implementation.
        case RAW_WIRE:
        default:
...
int RawEndpoint::recv(
    uintptr_t buffer, size_t maxSize, bool bBlock,
    Endpoint::RemoteEndpoint *remoteHost, int nTimeout)
{
    // Check for data to be ready. dataReady (acquire) takes one from the
    // Semaphore, so using dataReady *again* to see if data is available is
    // *wrong* unless the queue is empty.
    ...
    if (bDataReady)
    {
        ...
        MemoryCopy(
            ...
            reinterpret_cast<void *>(ptr->ptr), nBytes);
...
size_t RawEndpoint::depositPacket(
...
void RawManager::receive(
    uintptr_t payload, size_t payloadSize, Endpoint::RemoteEndpoint *remoteHost,
    int type, Network *pCard)
{
    ...
    for (List<Endpoint *>::Iterator it = m_Endpoints.begin();
