...
Ipv4::Ipv4() : m_NextIdLock(), m_IpId(0), m_Fragments()
{
}
...
size_t Ipv4::injectHeader(
    uintptr_t packet, IpAddress dest, IpAddress from, uint8_t type)
{
    // Basic validation
    if (!packet)
        return 0;
...
void Ipv4::injectChecksumAndDataFields(
    uintptr_t ipv4HeaderStart, size_t payloadSize)
{
    // Basic validation
    ...
    pHeader->checksum = 0;
    pHeader->checksum =
        Network::calculateChecksum(ipv4HeaderStart, pHeader->header_len * 4);
}

uint16_t Ipv4::ipChecksum(
    ...
    uintptr_t packet, Network *pCard)
{
    ...
    if (!pCard)
    {
        ...
        {
            WARNING(
                "IPv4: Couldn't find a route for destination '"
                << dest.toString() << "'.");
            return false;
        }
    }
    ...
    MemoryCopy(
        reinterpret_cast<void *>(packet + sizeof(ipHeader)),
    ...
    /// \todo Perhaps flag this so if we don't want to automatically resolve the
    /// MAC
    ///       it doesn't happen?
    MacAddress destMac;
    bool macValid = true;
    if (dest == me.broadcast)
        ...
        macValid =
            Arp::instance().getFromCache(realDest, true, &destMac, pCard);

    if (macValid)
        Ethernet::send(
            nBytes + sizeof(ipHeader), packet, pCard, destMac, dest.getType());
    ...
    size_t nBytes, uintptr_t packet, Network *pCard, uint32_t offset)
{
    ...
    header->checksum = 0;
    uint16_t calcChecksum = Network::calculateChecksum(
        reinterpret_cast<uintptr_t>(header), sizeof(ipHeader));
    ...
    if (checksum == calcChecksum)
    {
        ...
        StationInfo me = pCard->getStationInfo();
        if ((!me.ipv4.getIp()) &&
            (!Arp::instance().isInCache(from)))  // Not configured yet?
        {
        ...
        if (to.isUnicast() && (to != me.ipv4) && (me.ipv4.getIp() != 0) &&
            (to != me.broadcast))
        {
            // Not for us!
            MacAddress e;
            DEBUG_LOG(
                "IPv4: forwarding packet from " << from.toString() << " to "
                                                << to.toString());
            ...
            {
                DEBUG_LOG(
                    "IPv4: no route to forwarding destination "
                    << to.toString());
                pCard->droppedPacket();
                return;
            }
            ...
            bool macValid =
                Arp::instance().getFromCache(realDest, true, &e, pCard);
            if (macValid)
                Ethernet::send(
                    nBytes - Ethernet::instance().ethHeaderSize(),
                    packet + Ethernet::instance().ethHeaderSize(), pCard, e,
                    to.getType());
            else
                pCard->droppedPacket();
            return;
        ...
        uint16_t frag_offset =
            (BIG_TO_HOST16(header->frag_offset) & ~0xF000) * 8;

        /// \note This implementation of IP fragmentation makes many
        /// assumptions,
        ///       the most important of which is that the fragments come in with
        ///       some sort of order. If the last fragment comes in too early,
        ///       the packet will not be reassembled properly.
        ...
        if ((frag_offset != 0) || (flags & IP_FLAG_MF))
        {
            ...
            if (dataLength)
            {
                ...
                MemoryCopy(
                    buff, reinterpret_cast<void *>(packet + dataOffset),
                    dataLength);
            ...
            if (header->frag_offset && !(flags & IP_FLAG_MF))
            {
                ...
                // Iterate through the fragment list, copying data into the
                // buffer as we go
                for (Tree<size_t, fragment *>::Iterator it =
                         p->fragments.begin();
                     it != p->fragments.end(); it++)
                {
                    // Copy this one in
                    ...
                    if (f)
                    {
                        ...
                        {
                            MemoryCopy(
                                buff + (copyOffset + fragment_offset), data,
                                f->length);
                            delete[] data;
                        }
                        delete f;
            ...
            {
                // Do not handle an incomplete packet, wait for another one to
                // arrive
                return;
            }
        }
...
// NOTICE("IP: ICMP packet");
                ...
                RawManager::instance().receive(
                    packetAddress, nBytes - offset, &remoteHost, IPPROTO_ICMP,
                    pCard);
#endif

                // icmp needs the ip header as well
                Icmp::instance().receive(
                    from, to, dataAddress, payloadSize, this, pCard);
                break;
...
#ifndef DISABLE_RAWNET
                RawManager::instance().receive(
                    packetAddress, nBytes - offset, &remoteHost, IPPROTO_UDP,
                    pCard);
#endif
...
#ifndef DISABLE_RAWNET
                RawManager::instance().receive(
                    packetAddress, nBytes - offset, &remoteHost, IPPROTO_TCP,
                    pCard);
#endif
