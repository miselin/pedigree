...
bool Arp::getFromCache(
    IpAddress ip, bool resolve, MacAddress *ent, Network *pCard)
{
    ...
    bool success = addRequest(
        0, reinterpret_cast<uint64_t>(req), reinterpret_cast<uint64_t>(pCard));
...
void Arp::send(IpAddress req, Network *pCard)
{
    ...
    NOTICE(
        "arp who-has " << req.toString() << " tell "
                       << cardInfo.ipv4.toString());
    ...
    size_t nBytes, uintptr_t packet, Network *pCard, uint32_t offset)
{
    ...
    {
        WARNING(
            "Arp: Request for either unknown MAC format or non-IPv4 address");
        return;
    }
    ...
    if (BIG_TO_HOST16(header->opcode) == ARP_OP_REQUEST)
    {
        ...
        {
            // allocate the reply
            uintptr_t replyPacket =
                NetworkStack::instance().getMemPool().allocate();
            arpHeader *reply = reinterpret_cast<arpHeader *>(replyPacket);
            ...
            // send it out
            Ethernet::send(
                sizeof(arpHeader), replyPacket, pCard, sourceMac, ETH_ARP);

            // and now that it's sent, destroy the reply
            NetworkStack::instance().getMemPool().free(replyPacket);
        }
    }
    // reply
    ...
    {
        NOTICE(
            "arp " << IpAddress(header->ipSrc).toString() << " is at "
                   << sourceMac.toString());
