...
static void progress(const char* text) {
  const bool bFinished =
      g_BootProgressTotal && ((g_BootProgressCurrent + 1) >= g_BootProgressTotal);
  if (bFinished) {
  ...
  if (g_NoGraphics) {
    ...
    if (g_Previous <= g_BootProgressCurrent)
      g_pFramebuffer->rect(g_ProgressX, g_ProgressY, w, g_ProgressH, g_ProgressColour,
                           g_ColorFormat);
    else
      g_pFramebuffer->rect(g_ProgressX + w, g_ProgressY, g_ProgressW - w, g_ProgressH,
    ...
    g_pFramebuffer->redraw(g_ProgressX, g_ProgressY - (FONT_HEIGHT * 2), g_ProgressW,
...
static bool handleSplash() {
  ...
  if (pFeatures && pFeatures->provides(ServiceFeatures::probe))
    if (pService)
      bSuccess = pService->serve(ServiceFeatures::probe, reinterpret_cast<void*>(&g_GraphicsParams),
                                 sizeof(g_GraphicsParams));

  if (!(bSuccess && g_GraphicsParams.providerFound)) {
    NOTICE(
        "splash: this system does not support graphics, using fallback "
        "log callback");
