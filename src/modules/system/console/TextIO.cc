...
TextIO::TextIO(String str, size_t inode, Filesystem* pParentFS, File* pParent)
  ...
  size_t backbufferPages = (backbufferSize + PhysicalMemoryManager::getPageSize() - 1) /
  ...
  InputManager::instance().installCallback(InputManager::MachineKey, inputCallback, this);
}
...
bool TextIO::initialise(bool bClear) {
  ...
  if (m_pVga) {
    ...
    if (m_pFramebuffer != 0) {
      if (bClear) {
        if (isPrimary()) {
          ByteSet(m_pFramebuffer, 0,
                  m_pVga->getNumRows() * m_pVga->getNumCols() * sizeof(uint16_t));
        }
  ...
  while ((*s) && (len--)) {
    ...
    if (m_bUtf8) {
      ...
      if ((m_nUtf8Handled == 0) || ((byte & 0xC0) != 0x80)) {
        if (m_nUtf8Handled > 0)
          ERROR(
              "TextIO: expected a continuation byte, but didn't "
              "get one");
      ...
      if ((thisByte & 0xF8) == 0xF0) {
        ...
        ERROR(
            "TextIO: invalid UTF8 leading byte (possible 5- or "
            "6-byte sequence?)");
        m_bUtf8 = false;
      }
    ...
    if (m_bControlSeq && m_bBracket) {
      switch (m_nCharacter) {
        ...
        case 'c':
          if (m_Params[0]) {
            ERROR(
                "TextIO: Device Attributes command with non-zero "
                "parameter.");
          } else {
        ...
        case 'n':
          switch (m_Params[0]) {
            ...
            case 6: {
              ...
              response.append("R");
              m_OutBuffer.write(const_cast<char*>(static_cast<const char*>(response)),
                                response.length());
            } break;
            default:
        ...
        case 'p':
          ...
          WARNING(
              "TextIO: dropping command after seeing 'p' command "
              "sequence terminator.");
          m_bControlSeq = false;
          break;
        ...
        case 'x':
          ...
          if (m_Params[0] > 1) {
            ERROR(
                "TextIO: invalid 'sol' parameter for 'Request "
                "Terminal Parameters'");
          } else {
        ...
        case '/': {
          ...
          if ((next >= '0' && next <= '2') || (next >= 'A' && next <= 'B')) {
            ...
            // Designate G1 character set.
              ...
              WARNING(
                  "TextIO: only 'ESC(C' and 'ESC)C' are "
...
void TextIO::checkScroll() {
  ...
  if (m_CursorY < m_ScrollStart) {
    ...
    MemoryCopy(&m_pBackbuffer[startOffset], &m_pBackbuffer[fromOffset],
...
uint64_t TextIO::writeBytewise(uint64_t location, uint64_t size, uintptr_t buffer, bool bCanBlock) {
