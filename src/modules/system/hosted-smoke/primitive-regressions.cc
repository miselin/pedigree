...
bool radixTreeExportedAbi()
{
    ...
    void *value = expected;
    const bool missing =
        !tree.lookup(key, value) && value == nullptr;

    tree.insert(key, expected);
    value = nullptr;
    const bool found =
        tree.lookup(key, value) && value == expected;
    ...
    const bool passed = check(
        missing && found && removed, "radix-tree-exported-abi",
...
struct OperationBarrierContext
{
    explicit OperationBarrierContext(OperationBarrier *barrier)
        : barrier(barrier), workEntered(0), workFinished(0), closeFinished(0),
          releaseWork(0)
    {
    }
...
int waitForCompletion(void *parameter)
{
    CompletionContext *context =
        reinterpret_cast<CompletionContext *>(parameter);
    context->entered += 1;
...
{
    TerminalCompletionContext *context =
        reinterpret_cast<TerminalCompletionContext *>(parameter);
    context->entered += 1;
...
{
    OperationBarrierContext *context =
        reinterpret_cast<OperationBarrierContext *>(parameter);
...
bool completionLifecycle()
{
    ...
    passed &= check(
        context.entered == 1 && queued, "completion-lifecycle",
        "waiter-before-complete did not publish its wait");
    passed &= check(
        completed && duplicateRejected, "completion-lifecycle",
        "delayed completion was not exactly-once");
    passed &= check(
        joined && context.completed == 1, "completion-lifecycle",
...
bool terminalCompletionBarrier()
{
    ...
    Thread *waiter = new Thread(
        ...
        waitForTerminalCompletion, &context, nullptr, false, true);
    ...
    const bool deferred =
        context.entered == 1 && context.returned == 0;

    completion.release();
    const bool joined = waiter->join();

    const bool passed = check(
        queued && deferred && joined && context.acquired == 1 &&
            context.returned == 1,
...
bool operationBarrierLifecycle()
{
    ...
    Thread *worker = new Thread(
        process, runAdmittedOperation, &context, nullptr, false, true);
    ...
    Thread *closer = new Thread(
        process, closeOperationBarrier, &context, nullptr, false, true);
    closer->setName("hosted operation barrier closer");

    const bool closeQueued =
        waitUntilQueued(closer, Thread::CallbackDrain);
    const bool closeBlocked =
        closeQueued && context.closeFinished == 0 &&
        context.workFinished == 0;
    ...
    const bool passed = check(
        admitted && closeBlocked && lateRejected && workerJoined &&
            closerJoined && context.workFinished == 1 &&
            context.closeFinished == 1 && drained,
...
{
    ConditionTimeoutContext(
        ConditionVariable *condition, Thread *waiter,
        Time::Timestamp signalAt)
        : condition(condition), waiter(waiter), signalAt(signalAt),
          published(0), signals(0)
    {
    }
...
{
    ConditionTimeoutContext *context =
        reinterpret_cast<ConditionTimeoutContext *>(parameter);
...
bool conditionVariableTimeoutAccounting(Thread *thread)
{
    ...
    constexpr Time::Timestamp InitialTimeout =
    ...
    Thread *signaler = new Thread(
        Scheduler::instance().getKernelProcess(),
        delayedConditionSignal, &context, nullptr, false, true);
    signaler->setName("hosted timed ConditionVariable signaler");

    passed &= check(
        mutex.acquire(), "condition-variable-timeout",
    ...
    passed &= check(
        !timedWait && error == ConditionVariable::TimedOut,
        "condition-variable-timeout",
        "an expired wait did not report TimedOut");
    passed &= check(
        expiring == 0, "condition-variable-timeout",
        "an expired wait retained a nonzero timeout");
    passed &= check(
        mutexHeldAfterTimeout, "condition-variable-timeout",
    ...
    error = ConditionVariable::NoError;
    const bool immediateWait =
        condition.wait(mutex, immediate, error);
    const bool mutexHeldAfterImmediate =
        mutex.isOwnedByCurrentThread();
    mutex.release();

    passed &= check(
        !immediateWait && error == ConditionVariable::TimedOut &&
            immediate == 0,
        "condition-variable-timeout",
        "zero did not request an immediate timeout");
    passed &= check(
        mutexHeldAfterImmediate, "condition-variable-timeout",
...
{
    explicit MemoryPoolContext(MemoryPool *pool)
        : pool(pool), entered(0), returned(0), result(0)
    {
    }
...
{
    MemoryPoolContext *context =
        reinterpret_cast<MemoryPoolContext *>(parameter);
    context->entered += 1;
...
bool memoryPoolBlockingAndStride()
{
    ...
    passed &= check(
        context.entered == 1 && queued, "memory-pool-lifecycle",
        "the exhausted-pool allocation did not block");
    passed &= check(
        joined && reused == freed, "memory-pool-lifecycle",
...
bool memoryPoolCloseAndDrain()
{
    constexpr size_t BufferSize = 512;
    MemoryPool *pool =
        new MemoryPool("hosted-memory-pool-close-regression");
    bool passed = true;
    if (!pool->initialise(1, BufferSize))
    {
        delete pool;
        return check(
            false, "memory-pool-close-drain",
            "a one-page hosted pool could not be initialised");
    }
    ...
    {
        passed &= check(
            pool->allocateNow() != 0, "memory-pool-close-drain",
    ...
    passed &= check(
        context.entered == 1 && queued, "memory-pool-close-drain",
        "the exhausted allocation was not active before close");
    passed &= check(
        joined && context.result == 0, "memory-pool-close-drain",
...
bool memoryPoolTerminalDrain()
{
    constexpr size_t BufferSize = 512;
    MemoryPool *pool =
        new MemoryPool("hosted-memory-pool-terminal-regression");
    bool passed = true;
    if (!pool->initialise(1, BufferSize))
    {
        delete pool;
        return check(
            false, "memory-pool-terminal-drain",
            "a one-page hosted pool could not be initialised");
    }
    ...
    passed &= check(
        context.entered == 1 && queued && joined &&
            context.returned == 1 && context.result == 0,
        "memory-pool-terminal-drain",
...
{
  public:
    BlockingPressureHandler(
        MemoryPressureManager *manager, MemoryPressureHandler *reentryPeer)
        : entered(0), releaseCallback(0), calls(0), reentries(0),
          manager(manager), reentryPeer(reentryPeer)
    {
    }
    ...
    bool compact() override
    {
        ...
        manager->registerHandler(
            MemoryPressureManager::LowPriority, reentryPeer);
        reentries += 1;
...
{
    PressureManagerContext(
        MemoryPressureManager *manager, MemoryPressureHandler *handler)
        : manager(manager), handler(handler), compactEntered(0),
          compactReturned(0), removeEntered(0), removeReturned(0)
    {
    }
...
{
    PressureManagerContext *context =
        reinterpret_cast<PressureManagerContext *>(parameter);
...
bool memoryPressureCallbackBarrier()
{
    ...
    manager.registerHandler(
        MemoryPressureManager::LowPriority, &reentryPeer);
    manager.registerHandler(
        MemoryPressureManager::HighestPriority, &handler);
    ...
    const bool callbackEntered =
        handler.entered.acquire(1, 0, 500000);
    Thread *remover = new Thread(
        Scheduler::instance().getKernelProcess(),
        removePressureHandler, &context, nullptr, false, true);
    remover->setName("hosted pressure remover");
    const Time::Timestamp removerDeadline =
        Time::getTicks() + (500 * Time::Multiplier::Millisecond);
    while (
        context.removeEntered != 1 &&
        Time::getTicks() < removerDeadline)
    {
        Scheduler::instance().yield();
    }

    Thread *followingCompactor = new Thread(
        Scheduler::instance().getKernelProcess(),
        compactPressureManager, &context, nullptr, false, true);
    followingCompactor->setName("hosted pressure following compactor");
    const Time::Timestamp followerDeadline =
        Time::getTicks() + (500 * Time::Multiplier::Millisecond);
    while (
        context.compactEntered < 2 && Time::getTicks() < followerDeadline)
    {
        Scheduler::instance().yield();
    }
    ...
    const Time::Timestamp publicationDeadline =
        Time::getTicks() + (500 * Time::Multiplier::Millisecond);
    while (
        !(removerWaitPublished && compactorWaitPublished) &&
        Time::getTicks() < publicationDeadline)
    {
        Thread::WaitDebugInfo waitInfo;
        removerWaitPublished =
            remover->getStatus() == Thread::Sleeping &&
            remover->getWaitDebugInfo(waitInfo) &&
            waitInfo.channelOwner == &handler;
        compactorWaitPublished =
            followingCompactor->getStatus() == Thread::Sleeping &&
    ...
    passed &= check(
        callbackEntered && context.removeEntered == 1 &&
            context.removeReturned == 0 &&
            handler.reentries == 1 && context.compactEntered == 2 &&
            context.compactReturned == 0 && removerWaitPublished &&
            compactorWaitPublished,
    ...
    passed &= check(
        compactJoined && followingCompactJoined && removeJoined &&
            context.compactReturned == 2 &&
    ...
    passed &= check(
        !manager.compact() && handler.calls == 1 &&
            reentryPeer.calls == peerCallsBefore + 1,
        "memory-pressure-callback-barrier",
    ...
    passed &= check(
        !atomicCompactResult &&
            reentryPeer.calls == callsBeforeAtomicAttempt,
        "memory-pressure-callback-barrier",
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        ...
        passed &= check(
            !compactResult && !emptyCompactResult &&
                callsAfterRemoval == callsBefore + 1 &&
...
struct RingBufferCloseContext
{
    ...
    RingBufferCloseContext(RingBuffer<char> *buffer, Operation operation)
        : buffer(buffer), operation(operation), entered(0), returned(0),
          succeeded(1), error(RingBuffer<char>::NoError)
    {
    }
...
{
    RingBufferCloseContext *context =
        reinterpret_cast<RingBufferCloseContext *>(parameter);
    context->entered += 1;
    ...
    if (context->operation == RingBufferCloseContext::Read)
    {
        ...
        context->succeeded =
            context->buffer->read(value, timeout, error) ? 1 : 0;
        context->error = error;
    }
    else
    {
        ...
        context->succeeded =
            context->error == RingBuffer<char>::NoError ? 1 : 0;
    }
...
bool runRingBufferCloseCase(
    RingBufferCloseContext::Operation operation, bool fill)
{
    ...
    delete buffer;
    const bool joined = waiter->join();
    return passed && context.entered == 1 && queued && joined &&
           context.returned == 1 && context.succeeded == 0 &&
           context.error == RingBuffer<char>::Closed;
}
...
bool ringBufferCloseAndDrain()
{
    bool passed = true;
    passed &= check(
        runRingBufferCloseCase(RingBufferCloseContext::Read, false),
        "ringbuffer-close-drain",
        "close did not wake and drain a blocked reader with Closed");
    passed &= check(
        runRingBufferCloseCase(RingBufferCloseContext::Write, true),
        "ringbuffer-close-drain",
    ...
    delete buffer;
    passed &= check(
        monitor.tryAcquire(), "ringbuffer-close-drain",
...
{
    BufferCloseContext *context =
        reinterpret_cast<BufferCloseContext *>(parameter);
    context->entered += 1;
...
bool runBufferCloseCase(
    BufferCloseContext::Operation operation, bool fill)
{
    ...
    Thread *waiter = new Thread(
        Scheduler::instance().getKernelProcess(),
        runBlockingBufferOperation, &context, nullptr, false, true);
...
bool bufferCloseAndDrain()
{
    bool passed = true;
    passed &= check(
        runBufferCloseCase(BufferCloseContext::Read, false),
        "buffer-close-drain",
        "close did not drain an already-entered blocking read");
    passed &= check(
        runBufferCloseCase(BufferCloseContext::Write, true),
        "buffer-close-drain",
        "close did not drain an already-entered blocking write");
    passed &= check(
        runBufferCloseCase(BufferCloseContext::CanRead, false),
        "buffer-close-drain",
        "close did not drain an already-entered canRead");
    passed &= check(
        runBufferCloseCase(BufferCloseContext::CanWrite, true),
        "buffer-close-drain",
...
bool bufferTerminalDrain()
{
    bool passed = true;
    passed &= check(
        runBufferTerminalCase(BufferCloseContext::Read, false),
        "buffer-terminal-drain",
        "a terminated blocking read retained an active operation");
    passed &= check(
        runBufferTerminalCase(BufferCloseContext::Write, true),
        "buffer-terminal-drain",
        "a terminated blocking write retained an active operation");
    passed &= check(
        runBufferTerminalCase(BufferCloseContext::CanRead, false),
        "buffer-terminal-drain",
        "a terminated canRead retained an active operation");
    passed &= check(
        runBufferTerminalCase(BufferCloseContext::CanWrite, true),
        "buffer-terminal-drain",
...
bool terminalOperationAdmissionScope()
{
    ...
    Buffer<char> *buffer = new Buffer<char>(1);
    BufferCloseContext bufferContext(
        buffer, BufferCloseContext::Read);
    buffer->acquireHostedOperationLock();
    Thread *bufferWaiter = new Thread(
        Scheduler::instance().getKernelProcess(),
        runBlockingBufferOperation, &bufferContext, nullptr, false, true);
    bufferWaiter->setName("hosted Buffer operation-admission waiter");
    const bool bufferQueued =
        waitUntilQueued(bufferWaiter, Thread::SemWait);
    ...
    const bool bufferRetired =
        buffer->getHostedActiveOperationCount() == 0;
    delete buffer;

    passed &= check(
        bufferQueued && bufferStayedBlocked && bufferJoined &&
            bufferContext.returned == 1 && bufferContext.result == 0 &&
            bufferRetired,
    ...
    constexpr size_t BufferSize = 512;
    MemoryPool *pool =
        new MemoryPool("hosted-terminal-operation-admission");
    if (!pool->initialise(1, BufferSize))
    {
        delete pool;
        return check(
            false, "terminal-operation-admission",
    ...
    const bool poolRetired =
        pool->getHostedActiveOperationCount() == 0;
    delete pool;

    passed &= check(
        poolQueued && poolStayedBlocked && poolJoined &&
            poolContext.returned == 1 && poolContext.result == 0 &&
            poolRetired,
        "terminal-operation-admission",
...
{
    TerminalTimeoutContext(
        Semaphore *semaphore, ConditionVariable *condition, Mutex *mutex)
        : semaphore(semaphore), condition(condition), mutex(mutex),
          semaphoreEntered(0), semaphoreReturned(0),
          conditionEntered(0), conditionReturned(0), delayEntered(0),
          delayReturned(0)
    {
    }
...
int terminalTimedSemaphoreWait(void *parameter)
{
    TerminalTimeoutContext *context =
        reinterpret_cast<TerminalTimeoutContext *>(parameter);
    context->semaphoreEntered += 1;
    Semaphore::SemaphoreError error = Semaphore::NoError;
    const bool acquired =
        context->semaphore->acquireWithError(1, 0, 500000, error);
    (void) acquired;
...
bool terminalTimeoutCleanup()
{
    ...
    const size_t semaphoreCreates =
        Semaphore::getHostedTimeoutCreateCount();
    const size_t semaphoreDestroys =
        Semaphore::getHostedTimeoutDestroyCount();
    const size_t alarmCreates = Time::getHostedAlarmCreateCount();
    const size_t alarmDestroys = Time::getHostedAlarmDestroyCount();
    const Time::Timestamp deadline =
        Time::getTicks() + (550 * Time::Multiplier::Millisecond);

    Thread *semaphoreWaiter = new Thread(
        process, terminalTimedSemaphoreWait, &context, nullptr, false, true);
    Thread *conditionWaiter = new Thread(
        process, terminalTimedConditionWait, &context, nullptr, false, true);
    Thread *delayWaiter = new Thread(
        process, terminalDelay, &context, nullptr, false, true);

    const bool semaphoreQueued =
        waitUntilQueued(semaphoreWaiter, Thread::SemWait);
    const bool conditionQueued =
        waitUntilQueued(conditionWaiter, Thread::CondWait);
    const bool delayQueued =
        waitUntilQueued(delayWaiter, Thread::EventWait);
    ...
    bool passed = true;
    passed &= check(
        semaphoreQueued && conditionQueued && delayQueued,
        "terminal-timeout-cleanup",
        "not every timed waiter published its wait");
    passed &= check(
        semaphoreJoined && conditionJoined && delayJoined &&
            context.semaphoreEntered == 1 &&
            context.conditionEntered == 1 && context.delayEntered == 1 &&
            context.semaphoreReturned == 0 &&
            context.conditionReturned == 0 && context.delayReturned == 0,
        "terminal-timeout-cleanup",
        "a terminal timed wait returned through ordinary cleanup");
    passed &= check(
        Semaphore::getHostedTimeoutCreateCount() ==
                semaphoreCreates + 1 &&
            Semaphore::getHostedTimeoutDestroyCount() ==
...
bool runHostedPrimitiveRegressions(Thread *thread)
{
    return radixTreeExportedAbi() &&
           ...
           memoryPoolBlockingAndStride() &&
           memoryPoolCloseAndDrain() &&
           memoryPoolTerminalDrain() &&
           memoryPressureCallbackBarrier() &&
           bufferCloseAndDrain() && bufferTerminalDrain() &&
           terminalOperationAdmissionScope() &&
           terminalTimeoutCleanup();
}
