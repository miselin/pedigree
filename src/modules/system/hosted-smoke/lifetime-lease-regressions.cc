...
class ObservedThread : public Thread
{
  public:
    ObservedThread(
        Process *parent, ThreadStartFunc entry, Atomic<size_t> *destroyed,
        bool delayedStart = false)
        : Thread(
              parent, entry, nullptr, nullptr, false, true, delayedStart),
...
{
    OwnedWorkerContext()
        : gate(0), worker(nullptr), entered(0), returnedPastWait(0),
          joins(0)
    {
    }
...
{
    if (
        g_OwnedWorkerContext &&
        target == g_OwnedWorkerContext->worker)
    {
...
int blockedOwnedWorker(void *parameter)
{
    OwnedWorkerContext *context =
        reinterpret_cast<OwnedWorkerContext *>(parameter);
    context->entered += 1;
...
{
    ProcessDeleteContext *context =
        reinterpret_cast<ProcessDeleteContext *>(parameter);
...
bool waitForLeaseDrain(Thread *waiter, const void *owner)
{
    for (size_t attempt = 0; attempt < Attempts; ++attempt)
    {
        ...
        if (
            waiter->getWaitDebugInfo(info) && info.queued &&
            info.channelOwner == owner &&
...
bool ownedThreadTerminalJoin(Process *kernelProcess)
{
    ...
    bool waiting = false;
    {
        OwnedThread worker(new Thread(
            kernelProcess, blockedOwnedWorker, &context, nullptr, false,
            true));
    ...
    const bool passed = check(
        waiting && context.entered == 1 &&
            context.returnedPastWait == 0 && context.joins == 1,
...
bool processLeaseBarrier(Process *kernelProcess)
{
    ...
    const bool leaseAcquired =
        Scheduler::instance().acquireProcess(lease, process);
    bool passed = check(
        leaseAcquired && static_cast<bool>(lease),
        "could not acquire the process lease");

    ProcessDeleteContext context(process, &destroyed);
    Thread *deleter = new Thread(
        kernelProcess, deleteLeasedProcess, &context, nullptr, false, true);
    deleter->setName("hosted ProcessLease deleter");

    const bool draining = waitForLeaseDrain(deleter, process);
    passed &= check(
        draining && context.entered == 1 && destroyed == 0,
        "a held ProcessLease did not stop derived destruction");

    Scheduler::ProcessLease lateLease;
    const bool lateLeaseAcquired =
        Scheduler::instance().acquireProcess(lateLease, process);
    passed &= check(
        !lateLeaseAcquired && !lateLease,
    ...
    passed &= check(
        context.finished == 0 && destroyed == 0,
...
bool threadLeaseBarrier(Process *kernelProcess)
{
    ...
    Thread *target =
        new ObservedThread(
            kernelProcess, immediateExit, &destroyed, true);
    ...
    const bool leaseAcquired =
        kernelProcess->acquireThread(lease, target);
    bool passed = check(
        leaseAcquired && static_cast<bool>(lease),
        "could not acquire the thread lease");
    if (!lease)
    {
        target->start();
    ...
    passed &= check(
        target->detach(), "the ThreadLease target could not detach");
    passed &= check(
        target->start(), "the delayed ThreadLease target did not start");
...
bool runHostedLifetimeLeaseRegressions()
{
    ...
    const bool passed =
        ...
        processLeaseBarrier(kernelProcess) &&
        threadLeaseBarrier(kernelProcess);
    if (passed)
    {
