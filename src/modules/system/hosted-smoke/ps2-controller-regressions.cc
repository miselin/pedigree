...
bool oneShotAdmission() {
  ...
  const bool passed =
      check(first && contended && remainedOwned && reacquired && !gate.owned(), Test,
...
bool debuggerTransitionLockIndependent() {
  ...
  bool passed = check(gate.tryAcquire() && gate.owned() && !debugState.active(), Test,
                      "the test could not freeze an ordinary 8042 transaction");
  debugState.set(true);
  passed &= check(debugState.active() && gate.owned() && !gate.tryAcquire(), Test,
                  "debugger entry waited on or changed 8042 admission");
  debugState.set(false);
  passed &= check(!debugState.active() && gate.owned(), Test,
...
bool captureQueueFidelity() {
  ...
  passed &= check(queue.tryPush(Ps2CapturedByte(0x1E, false)) &&
  ...
  passed &=
      check(queue.pop(record) && record.value == 0x1E && !record.secondPort && queue.pop(record) &&
                record.value == 0xA5 && record.secondPort && !queue.pop(record) && !queue.pending(),
            Test, "capture order or status-based destination identity changed");

  for (size_t i = 0; i < Ps2CaptureQueue::Capacity; ++i) {
    passed &= queue.tryPush(Ps2CapturedByte(static_cast<uint8_t>(i), (i & 1) != 0));
  }
  passed &= check(!queue.hasCapacity() && !queue.tryPush(Ps2CapturedByte(0xFF, false)) &&
                      queue.pending() == Ps2CaptureQueue::Capacity,
                  Test, "a full hard-stage queue overwrote an unread byte");

  for (size_t i = 0; i < Ps2CaptureQueue::Capacity; ++i) {
    passed &= queue.pop(record) && record.value == static_cast<uint8_t>(i) &&
              record.secondPort == ((i & 1) != 0);
  }
  passed &= check(!queue.pending() && queue.hasCapacity() &&
...
bool runHostedPs2ControllerRegressions() {
  return oneShotAdmission() && debuggerTransitionLockIndependent() && captureQueueFidelity();
}
