...
bool removerWaiting(Thread* thread, Log::LogCallback* callback) {
  ...
  return thread && thread->getWaitDebugInfo(info) && info.queue && info.channelOwner &&
         info.queued && thread->getDebugState(debugAddress) == Thread::CallbackDrain &&
...
void callbackPinHook(Log::LogCallback* callback) {
  ...
  if (!context || callback != &context->logger || !context->phase.compareAndSwap(0, 1)) {
    return;
  }

  for (size_t attempt = 0; attempt < Attempts; ++attempt) {
    if (context->removerStarted == static_cast<size_t>(2) &&
...
int removePinnedCallback(void* parameter) {
  ...
  for (size_t attempt = 0; attempt < Attempts && context->phase == static_cast<size_t>(0);
       ++attempt) {
    Scheduler::instance().yield();
  }
...
bool callbackLifetime() {
  ...
  context.removerA =
      new Thread(process, removePinnedCallback, &context.workerA, nullptr, false, true);
  context.removerB =
      new Thread(process, removePinnedCallback, &context.workerB, nullptr, false, true);
  ...
  const bool selfInstalled = Log::instance().installCallback(&selfRemoving, true);
  NOTICE("hosted-log-self-removal");
  NOTICE("hosted-log-after-self-removal");
  Log::instance().removeCallback(&selfRemoving);

  bool passed = true;
  passed &= check(joinedA && joinedB && context.failures == 0,
                  "concurrent callback removers did not complete cleanly");
  passed &= check(context.removersObservedWaiting == 1 && context.removerReturned == 2,
...
struct SnapshotContext {
  SnapshotContext() : logger(*this), phase(0), sawA(0), sawB(0), unknown(0), worker(nullptr) {}
...
void entrySnapshotHook(const Log::LogEntry& entry) {
  ...
  if (!context || !(entry.str == SnapshotA) || !context->phase.compareAndSwap(0, 1)) {
    return;
  }

  for (size_t attempt = 0; attempt < Attempts && context->phase != static_cast<size_t>(2);
       ++attempt) {
    Scheduler::instance().yield();
  }
}
...
bool entrySnapshotIsolation() {
  ...
  context.worker = new Thread(Scheduler::instance().getKernelProcess(), writeSecondSnapshot,
  ...
  const bool passed = check(joined && context.phase == static_cast<size_t>(2) &&
                                context.sawA == 1 && context.sawB == 1 && context.unknown == 0,
