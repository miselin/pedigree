...
struct PendingStealContext
{
    PendingStealContext(DeliveryQueue *queue, const DeliveryQueue::Key &key)
        : queue(queue), pendingKey(key), sequence(0), firstOrder(0),
          stolenOrder(0), releaseOrder(0), resumedOrder(0), firstCalls(0),
          stolenCalls(0), drainSucceeded(0), failures(0)
    {
    }
...
bool pendingRecordCanBeStolen()
{
    ...
    DeliveryQueue::Record *first = queue.create(
        firstKey, firstCallback, reinterpret_cast<uintptr_t>(&context), 11,
        nullptr, nullptr, countDestruction, &destroyed);
    DeliveryQueue::Record *pending = queue.create(
        pendingKey, stolenCallback, reinterpret_cast<uintptr_t>(&context), 22,
    ...
    const bool passed = check(
        context.firstCalls == 1 && context.stolenCalls == 1 &&
            context.drainSucceeded == 1 && context.failures == 0 &&
            context.firstOrder == 1 && context.stolenOrder == 2 &&
            context.releaseOrder == 3 && context.resumedOrder == 4 &&
            destroyed == 2 && queue.empty(),
...
bool runningRecordCanDrainItself()
{
    ...
    DeliveryQueue::Record *record = queue.create(
        key, selfDrainingCallback, reinterpret_cast<uintptr_t>(&context), 0,
        nullptr, nullptr, countDestruction, &destroyed);
    List<DeliveryQueue::Record *> records;
    ...
    const bool passed = check(
        context.calls == 1 && context.drained == 1 && destroyed == 1 &&
            queue.empty(),
        "usb-callback-self-drain",
        "a callback draining its own generation blocked or ran twice");
    if (passed)
...
bool generationIsPartOfIdentity()
{
    ...
    DeliveryQueue::Record *record = queue.create(
        key, countCallback, reinterpret_cast<uintptr_t>(&context), 0, nullptr,
    ...
    const bool passed = check(
        !wrongDrained && context.calls == 1 && destroyed == 1 && queue.empty(),
