...
void lifetimeCallback(InputManager::InputNotification& notification) {
  CallbackLifetimeContext* context = reinterpret_cast<CallbackLifetimeContext*>(notification.meta);
...
void callbackPinHook(InputManager::callback_t callback, void* meta) {
  ...
  if (!context || callback != lifetimeCallback || meta != context ||
  ...
  for (size_t attempt = 0; attempt < Attempts; ++attempt) {
    ...
    for (size_t i = 0; allDraining && i < 2; ++i) {
      ...
      allDraining &= context->removers[i]->getWaitDebugInfo(info) && info.queue &&
                     info.channelOwner && info.queued &&
                     context->removers[i]->getDebugState(debugAddress) == Thread::CallbackDrain &&
                     debugAddress == reinterpret_cast<uintptr_t>(lifetimeCallback);
    }
    if (allDraining) {
...
void selfRemovingCallback(InputManager::InputNotification& notification) {
  SelfRemovalContext* context = reinterpret_cast<SelfRemovalContext*>(notification.meta);
  context->calls += 1;
...
bool callbackLifetimeBarrier() {
  ...
  for (size_t i = 0; i < 2; ++i) {
    context.removers[i] = new Thread(Scheduler::instance().getKernelProcess(), removePinnedCallback,
  ...
  manager.installCallback(InputManager::Key, selfRemovingCallback, &selfRemoval);
  manager.keyPressed('k');
  for (size_t attempt = 0; attempt < Attempts && !selfRemoval.calls; ++attempt) {
    Scheduler::instance().yield();
  }
  ...
  passed &= check(context.callbackCalls == 1 && context.callbackCalls == callsAfterRemoval &&
                      context.callbackAfterRemoval == 0 && context.removalReturned == 2,
                  "a callback began after removeCallback returned");
  passed &= check(callsAfterSelfRemoval == 1 && selfRemoval.calls == 1,
