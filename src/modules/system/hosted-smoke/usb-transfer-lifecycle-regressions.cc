...
class TransferLifecycleModel {
 public:
  ...
  struct Slot {
    Slot()
        : completion(), accepted(), linked(false), dmaOwned(false), reclaimed(false), cleanup() {}
  ...
  bool submit(size_t slotIndex, Completion::Callback callback, uintptr_t parameter,
  ...
  bool cancel(size_t slotIndex, Completion::Callback callback, uintptr_t parameter) {
    ...
    bool drain = false;
    {
      ...
      const Completion::CancellationDisposition disposition =
          slot.completion.claimCancellation(callback, parameter, -TransactionError, claim);
      if (disposition == Completion::CancellationDisposition::NoMatch)
        return false;
      if (disposition == Completion::CancellationDisposition::DrainPublished) {
  ...
  bool shutdown() {
    ...
    const bool passed = m_Deliveries.empty() && m_Accepted.isClosedAndDrained() && !m_DmaViolations;
    m_Shutdown = true;
    return passed;
  }
 ...
 private:
  ...
  DeliveryQueue::Record* createRecordLocked(size_t slotIndex, const Completion::Claim& claim) {
    Slot& slot = m_Slots[slotIndex];
    assert(!slot.dmaOwned);
    return m_Deliveries.create({slotIndex, claim.generation}, claim.callback, claim.parameter,
...
int submitPausedTransfer(void* parameter) {
  ...
  if (context->controller->submit(0, recordResult, reinterpret_cast<uintptr_t>(&context->callback),
...
int cancelPausedTransfer(void* parameter) {
  auto* context = reinterpret_cast<SubmissionRaceContext*>(parameter);
  if (context->controller->cancel(0, recordResult,
                                  reinterpret_cast<uintptr_t>(&context->callback))) {
    context->cancelled += 1;
  }
  return 0;
}
...
bool cancellationCannotObserveHalfPublishedTransfer() {
  ...
  Thread* submission =
      new Thread(kernelProcess, submitPausedTransfer, &context, nullptr, false, true);
  const bool commitEntered = context.commitEntered.acquireForCompletion();
  Thread* cancellation =
      new Thread(kernelProcess, cancelPausedTransfer, &context, nullptr, false, true);
  const bool cancellationBlocked = waitUntilSleeping(cancellation);
  const bool callbackRanEarly = static_cast<size_t>(context.callback.calls) != 0;
  ...
  const bool passed =
      check(commitEntered && cancellationBlocked && !callbackRanEarly && submissionJoined &&
                cancellationJoined && context.submitted == 1 && context.cancelled == 1 &&
                context.callback.calls == 1 && context.callback.result == -TransactionError &&
...
bool teardownPreservesNaturalAndFailsActive() {
  ...
  const bool naturalSubmitted =
      controller.submit(0, recordResult, reinterpret_cast<uintptr_t>(&natural));
  const bool activeSubmitted =
      controller.submit(1, recordResult, reinterpret_cast<uintptr_t>(&active));
  const bool captured = controller.captureNatural(0, 2718);
  const bool shutdown = controller.shutdown();

  const bool passed =
      check(naturalSubmitted && activeSubmitted && captured && shutdown && natural.calls == 1 &&
                natural.result == 2718 && active.calls == 1 && active.result == -TransactionError &&
                controller.reclaimed(0) && controller.reclaimed(1),
...
struct NestedCancelContext {
  NestedCancelContext(TransferLifecycleModel* controller, ResultContext* target)
...
void cancelSecondFromFirst(uintptr_t parameter, ssize_t result) {
  ...
  if (result != -TransactionError)
    return;
  context->calls += 1;
  if (context->controller->cancel(1, recordResult, reinterpret_cast<uintptr_t>(context->target))) {
...
bool teardownBatchSupportsNestedCancellation() {
  ...
  const bool firstSubmitted =
      controller.submit(0, cancelSecondFromFirst, reinterpret_cast<uintptr_t>(&first));
  const bool secondSubmitted =
      controller.submit(1, recordResult, reinterpret_cast<uintptr_t>(&second));
  const bool shutdown = controller.shutdown();

  const bool passed = check(firstSubmitted && secondSubmitted && shutdown && first.calls == 1 &&
...
bool runHostedUsbTransferLifecycleRegressions() {
  return cancellationCannotObserveHalfPublishedTransfer() &&
         teardownPreservesNaturalAndFailsActive() && teardownBatchSupportsNestedCancellation();
}
