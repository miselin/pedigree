...
bool check(bool condition, const char* detail) {
  ...
  ERROR("HOSTED-WAIT-TEST: FAIL interrupt-manager-lock-independent: " << detail);
  return false;
}
 ...
 public:
  explicit CountingHandler(size_t identity)
      : m_Identity(identity),
        ...
        observedIdentity(0),
        observedSignal(0),
        observedContext(static_cast<size_t>(ExecutionContext::AtomicThread)) {}
...
bool runHostedInterruptManagerRegressions() {
  ...
  const bool firstInstalled = manager.registerInterruptHandler(TestSignal, &first);
  const bool duplicateRejected = !manager.registerInterruptHandler(TestSignal, &second);
  if (firstInstalled) {
    HostedInterruptManager::withMutationLockForTest(dispatchWhileMutationLocked);
  }

  const bool firstRemoved = firstInstalled && manager.registerInterruptHandler(TestSignal, nullptr);
  ...
  const bool secondRemoved =
      secondInstalled && manager.registerInterruptHandler(TestSignal, nullptr);
  const bool restoredToWaitable = Processor::executionContext() == ExecutionContext::WaitableThread;

  bool passed = true;
  passed &= check(firstInstalled && duplicateRejected && firstRemoved && emptyRemovalRejected &&
                      secondInstalled && secondRemoved,
                  "install/remove exclusivity changed");
  passed &= check(g_MutationHookCalls == 2 && g_RaiseSucceeded == 2,
                  "production dispatch did not return while the mutation lock was held");
  passed &= check(first.calls == 1 && first.observedIdentity == 1 &&
  ...
  passed &= check(
      first.observedContext == static_cast<size_t>(ExecutionContext::HostedSyntheticIrq) &&
          second.observedContext == static_cast<size_t>(ExecutionContext::HostedSyntheticIrq) &&
          restoredToWaitable,
