...
class TrackingDisk final : public Disk
{
  public:
    ...
    TrackingDisk()
        : m_ReadCount(0), m_WriteCount(0), m_UnpinCount(0),
          m_Data(new uint8_t[DataSize]), m_BalanceError(false)
    {
    ...
    bool balanced() const
    {
        return !m_BalanceError && !m_PageReferences[0] &&
               !m_PageReferences[1];
    }
...
class FatFilesystemHarness : public FatFilesystem
{
  public:
    ...
    bool writeSectors(
        uint32_t sector, size_t size, uintptr_t source)
    {
...
bool fatSectorPageBoundary()
{
    ...
    const bool wrote = filesystem.writeSectors(
        1, TransferSize, reinterpret_cast<uintptr_t>(source));
    const bool bytesMatch =
        !MemoryCompare(disk.data() + 512, source, TransferSize);
    delete[] source;

    const bool splitAtPage =
        disk.m_ReadCount == 2 && disk.m_WriteCount == 2 &&
        disk.m_ReadLocations[0] == 512 &&
        disk.m_ReadLocations[1] == 4096 &&
        disk.m_WriteLocations[0] == 512 &&
        disk.m_WriteLocations[1] == 4096;
    const bool balanced =
        disk.m_UnpinCount == 2 && disk.m_UnpinLocations[0] == 512 &&
...
bool fatShortReadPublication()
{
    ...
    FatFile file(
        String("short-read"), 0, 0, 0, 1, &filesystem, 4096);
    ...
    const bool passed =
        !failed && retried && filesystem.m_ReadCalls == 2;
    if (passed)
    {
