...
bool schedulerContains(Process* process) {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (Scheduler::instance().acquireProcess(candidate, i) && candidate.get() == process) {
      return true;
    }
  }
  return false;
...
struct ProcessExitContext {
  explicit ProcessExitContext(Mutex* mutex)
      : mutex(mutex), peerEntered(0), peerAcquiredMutex(0), unstartedPeerEntered(0) {}
...
bool check(bool condition, const char* detail) {
  ...
  ERROR("HOSTED-WAIT-TEST: FAIL process-exit-rendezvous: " << detail);
  return false;
}
...
bool checkJoinLease(bool condition, const char* detail) {
  ...
  ERROR("HOSTED-WAIT-TEST: FAIL join-reaper-lease: " << detail);
  return false;
}

int blockedProcessPeer(void* parameter) {
  ProcessExitContext* context = reinterpret_cast<ProcessExitContext*>(parameter);
...
void joinOperationHook(Thread* target, Process* parent) {
  JoinReaperContext* context = __atomic_load_n(&g_JoinReaperContext, __ATOMIC_ACQUIRE);
  if (!context) {
    return;
  }
...
int joinRaceTarget(void* parameter) {
  JoinReaperContext* context = reinterpret_cast<JoinReaperContext*>(parameter);
...
bool joinReaperLease(Process* kernelProcess) {
  ...
  Thread* target = new Thread(process, immediateThreadExit, nullptr, nullptr, false, true, true);
  target->setName("hosted join/reaper target");

  Thread* retained = new Thread(process, immediateThreadExit, nullptr, nullptr, false, true, true);
  ...
  bool passed = checkJoinLease(targetsStarted && retainedReapable,
                               "the retained admission target did not become reapable");
  if (targetsStarted && retainedReapable) {
    NOTICE("HOSTED-WAIT-TEST: PASS scheduler-same-priority-progress");
  }
  if (!retainedReapable) {
    return false;
  }
  ...
  Thread* joiner = new Thread(kernelProcess, joinRaceTarget, &context, nullptr, false, true, true);
  joiner->setName("hosted join/reaper joiner");

  __atomic_store_n(&g_JoinReaperContext, &context, __ATOMIC_RELEASE);
  Thread::setJoinOperationHook(joinOperationHook);

  passed &= checkJoinLease(joiner->start(), "the delayed joiner did not start");
  ...
  passed &= checkJoinLease(joinerPinned && context.hookFailures == 0,
                           "the joiner did not reach the leased pre-delete window");

  Thread::setJoinOperationHook(nullptr);
  __atomic_store_n(&g_JoinReaperContext, static_cast<JoinReaperContext*>(nullptr),
                   __ATOMIC_RELEASE);

  if (!joinerPinned) {
    context.releaseJoiner.release();
    passed &= checkJoinLease(joiner->join(), "the failed-window joiner did not retire");
    passed &= checkJoinLease(retained->join(), "the retained target did not retire");
    delete process;
    return false;
  }

  Thread* reaper = new Thread(kernelProcess, reapJoinProcess, &context, nullptr, false, true, true);
  reaper->setName("hosted join/reaper process reaper");
  passed &= checkJoinLease(reaper->start(), "the delayed process reaper did not start");

  bool reaperBlocked = false;
  for (size_t attempt = 0; attempt < Attempts; ++attempt) {
    Thread::WaitDebugInfo info = {};
    if (context.reaperEntered && reaper->getWaitDebugInfo(info) && info.queue && info.queued &&
        info.channelOwner == process && reaper->getStatus() == Thread::Sleeping) {
      reaperBlocked = true;
      break;
    }
    Scheduler::instance().yield();
  ...
  passed &= checkJoinLease(reaperBlocked && context.processDeleted == 0,
                           "Process destruction did not wait for the active join lease");

  const bool lateJoin = retained->join();
  passed &= checkJoinLease(!lateJoin, "Process destruction admitted a new join after closing");
  ...
  passed &= checkJoinLease(completed && context.joinSucceeded == 1,
                           "the join lease did not release into orderly Process destruction");
  passed &= checkJoinLease(joiner->join(), "the joiner thread did not retire");
  passed &= checkJoinLease(reaper->join(), "the process reaper thread did not retire");
...
struct ExitElectionContext {
  explicit ExitElectionContext(ExitElectionProcess* process)
      : process(process),
        releaseCompetitor(0, false),
        hookCalls(0),
        hookFailures(0),
        ownerElected(0),
        competitorEntered(0),
        competitorElected(0),
        resumeSawTerminating(0),
        owner(nullptr) {}
...
void exitElectionHook(Process* process, Thread*) {
  ExitElectionContext* context = __atomic_load_n(&g_ExitElectionContext, __ATOMIC_ACQUIRE);
...
int competingProcessExit(void* parameter) {
  ExitElectionContext* context = reinterpret_cast<ExitElectionContext*>(parameter);
  ...
  context->resumeSawTerminating = context->process->getState() == Process::Terminating ? 1 : 0;
...
bool exitElectionQuiescence(Process* kernelProcess) {
  ...
  Thread* competitor =
      new Thread(process, competingProcessExit, &context, nullptr, false, true, true);
  competitor->setName("hosted process-exit competitor");
  process->competitor = competitor;
  Thread* owner = new Thread(process, owningProcessExit, &context, nullptr, false, true, true);
  owner->setName("hosted process-exit owner");
  context.owner = owner;

  bool passed = check(competitor->start(), "the competing exit thread did not start");

  constexpr size_t Attempts = 10000;
  bool competitorBlocked = false;
  for (size_t attempt = 0; attempt < Attempts; ++attempt) {
    Thread::WaitDebugInfo info = {};
    if (competitor->getWaitDebugInfo(info) && info.queue && info.queued &&
  ...
  passed &= check(competitorBlocked, "the competing exit thread did not publish its election gate");

  __atomic_store_n(&g_ExitElectionContext, &context, __ATOMIC_RELEASE);
  ...
  for (size_t attempt = 0; attempt < Attempts; ++attempt) {
    Thread::WaitDebugInfo info = {};
    if (context.hookCalls == 1 && owner->getWaitDebugInfo(info) && info.queue && info.queued &&
  ...
  passed &= check(ownerBlocked, "the elected exit owner did not publish its peer rendezvous");
  ...
  __atomic_store_n(&g_ExitElectionContext, static_cast<ExitElectionContext*>(nullptr),
                   __ATOMIC_RELEASE);

  passed &= check(reapable && process->getState() == Process::Terminated,
                  "the competing exit process never became reapable");
  passed &= check(context.hookCalls == 1 && context.hookFailures == 0,
                  "the deterministic election hook did not complete cleanly");
  passed &= check(
      context.ownerElected == 1 && context.competitorEntered == 1 && context.competitorElected == 0,
      "more than one exiting thread won the process election");
  passed &= check(context.resumeSawTerminating == 1,
                  "a concurrent resume downgraded a terminating process");
  passed &= check(process->cleanupCalls == 1 && process->cleanupBeforePeerReapable == 0,
                  "shared process cleanup was repeated or ran before peer quiescence");
  passed &= check(
      owner->getStatus() == Thread::AwaitingJoin && competitor->getStatus() == Thread::AwaitingJoin,
...
struct OrphanExitContext {
  OrphanExitContext()
      : process(nullptr),
        ...
        publishedCalls(0),
        workerEntered(0),
        workerEnteredBeforeOwnerExit(0),
        reapableCalls(0),
        destructorCalls(0),
        cleanupCalls(0),
        hookFailures(0),
...
class OrphanExitProcess : public Process {
 public:
  ...
  ~OrphanExitProcess() override {
    prepareForDestruction();
    if (static_cast<size_t>(m_Context->reapableCalls) != 1 || getState() != Process::Terminated) {
...
void orphanPublicationHook(Process* process, Process::OrphanPublicationPhase phase,
                           bool interruptsEnabled, bool processLockHeld) {
  OrphanExitContext* context = __atomic_load_n(&g_OrphanExitContext, __ATOMIC_ACQUIRE);
...
bool orphanPublicationInterleaving(Process* kernelProcess) {
  ...
  OrphanExitProcess* process = new OrphanExitProcess(kernelProcess, context);
  ...
  Thread* owner = new Thread(process, terminateChildProcess, process, nullptr, false, true, true);
  owner->setName("hosted orphan-exit owner");
  if (!owner->start()) {
    Process::setOrphanPublicationHook(nullptr);
    ZombieProcess::setReapHook(nullptr);
    __atomic_store_n(&g_OrphanExitContext, static_cast<OrphanExitContext*>(nullptr),
                     __ATOMIC_RELEASE);
    delete process;
    delete context;
  ...
  bool passed = true;
  passed &= check(context->preparingCalls == 1 && context->publishedCalls == 1,
                  "orphan publication did not cross both unlocked checkpoints");
  passed &= check(context->workerEntered == 1 && context->workerEnteredBeforeOwnerExit == 1,
                  "the ZombieQueue worker did not enter before owner stack retirement");
  passed &= check(context->reapableCalls == 1 && context->destructorCalls == 1 &&
...
struct ZombieBacklogContext {
  ZombieBacklogContext() : release(0, false), entered(0), destroyed(0), failures(0) {}
...
class HostedBacklogZombie : public ZombieObject {
 public:
  explicit HostedBacklogZombie(ZombieBacklogContext* context) : m_Context(context) {}
...
bool mandatoryZombieBacklog() {
  ...
  bool passed = check(context.destroyed == 0,
                      "mandatory ZombieQueue work executed through its closed test gate");
  context.release.release(Backlog);
  passed &= check(ZombieQueue::instance().drain(), "mandatory ZombieQueue backlog did not drain");
  passed &=
      check(context.entered == Backlog && context.destroyed == Backlog && context.failures == 0,
...
bool runHostedProcessExitRegressions() {
  ...
  passed &= check(!schedulerContains(publishedChild),
                  "an incomplete derived Process was visible after its constructor");
  publishedChild->finishAssembly();
  passed &=
      check(schedulerContains(publishedChild), "a complete derived Process was not published");
  delete publishedChild;

  const size_t processCountBeforeAbandon = Scheduler::instance().getNumProcesses();
  DeferredHostedProcess* abandonedChild = new DeferredHostedProcess(kernelProcess);
  delete abandonedChild;
  passed &= check(Scheduler::instance().getNumProcesses() == processCountBeforeAbandon,
  ...
  DeferredHostedProcess* freshChild = new DeferredHostedProcess(terminatingParent);
  freshChild->finishAssembly();
  passed &= check(freshChild->getState() == Process::Active,
                  "a new child inherited its parent's terminal state");
  passed &= check(freshChild->getParent() != terminatingParent,
                  "a terminating parent retained a newly published child");
  delete freshChild;
  delete terminatingParent;

  passed &= check(mutex.acquire(), "the supervisor could not hold the peer mutex");

  Process* child = new Process(kernelProcess);
  Process* orphan = new Process(child);
  Thread* peer = new Thread(child, blockedProcessPeer, &context, nullptr, false, true, true);
  peer->setName("hosted process-exit blocked peer");
  Thread* unstartedPeer =
      new Thread(child, unstartedProcessPeer, &context, nullptr, false, true, true);
  unstartedPeer->setName("hosted process-exit unstarted peer");
  Thread* terminator = new Thread(child, terminateChildProcess, child, nullptr, false, true, true);
  terminator->setName("hosted process-exit terminator");

  passed &= check(peer->start(), "the delayed blocked peer did not start");

  bool peerEnrolled = false;
  constexpr size_t EnrolmentAttempts = 10000;
  for (size_t attempt = 0; attempt < EnrolmentAttempts; ++attempt) {
    Thread::WaitDebugInfo info = {};
    if (peer->getWaitDebugInfo(info) && info.queue && info.queued &&
  ...
  passed &=
      check(context.peerEntered == 1 && peerEnrolled, "the peer did not publish its mutex wait");

  passed &= check(terminator->start(), "the delayed terminating thread did not start");
  const bool reapable = child->waitUntilTerminationReapable();

  passed &= check(reapable && child->getState() == Process::Terminated,
                  "the child did not publish off-stack termination");
  passed &= check(peer->getStatus() == Thread::AwaitingJoin &&
  ...
  passed &=
      check(context.peerAcquiredMutex == 0, "the blocked peer escaped terminal wait cancellation");
  passed &= check(context.unstartedPeerEntered == 0,
                  "the delayed unstarted peer executed during process exit");
  passed &=
      check(orphan->getParent() != child, "process exit left a child parented to the dead process");
