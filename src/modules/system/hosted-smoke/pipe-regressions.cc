...
{
    explicit PipeWaitContext(Pipe *pipe)
        : pipe(pipe), entered(0), returned(0), observedReader(0)
    {
    }
...
{
    PipeWaitContext *context =
        reinterpret_cast<PipeWaitContext *>(parameter);
    context->entered += 1;
...
Thread *startPipeWaiter(PipeWaitContext &context, const char *name)
{
    Thread *thread = new Thread(
        Scheduler::instance().getKernelProcess(), waitForPipeReader, &context,
        nullptr, false, true);
...
{
    Pipe pipe(
        String("hosted-stale-reader-fifo"), 0, 0, 0, 1, nullptr, 0,
        nullptr, false);
    ...
    Thread *waiter = startPipeWaiter(
        context, "hosted stale FIFO reader waiter");
...
{
    Pipe pipe(
        String("hosted-broadcast-reader-fifo"), 0, 0, 0, 2, nullptr, 0,
        nullptr, false);
    PipeWaitContext firstContext(&pipe);
    PipeWaitContext secondContext(&pipe);
    Thread *first = startPipeWaiter(
        firstContext, "hosted FIFO writer waiter one");
    Thread *second = startPipeWaiter(
        secondContext, "hosted FIFO writer waiter two");

    const bool bothBlocked =
        waitUntilBlocked(first) && waitUntilBlocked(second) &&
    ...
    const Time::Timestamp deadline =
        Time::getTicks() + (500 * Time::Multiplier::Millisecond);
    while (
        (firstContext.returned != 1 || secondContext.returned != 1) &&
    ...
    return bothBlocked && bothReturned && firstJoined && secondJoined &&
           firstContext.observedReader == 1 &&
           secondContext.observedReader == 1;
}
}  // namespace
...
{
    const bool passed =
        staleReaderDoesNotSatisfyOpen() && oneReaderWakesEveryWriter();
    if (passed)
    {
