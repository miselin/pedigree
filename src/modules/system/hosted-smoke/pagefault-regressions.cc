...
struct RegistryDispatchContext {
  explicit RegistryDispatchContext(PageFaultHandler* registry)
      : registry(registry),
        ...
        writerHookCalls(0),
        writerDispatches(0),
        deferredScopeRequested(0),
        deferredScopeHookCalls(0),
        deferredScopeDispatches(0),
        prePinHookCalls(0),
...
bool RegistryDispatchHandler::trap(InterruptState&, uintptr_t, bool) {
  ...
  if (m_Context.deferredScopeRequested.compareAndSwap(1, 2)) {
    Processor::information().getCurrentThread()->withDeferredScopeLockForTest(
...
bool lockIndependentDispatch() {
  ...
  const bool writerDispatched = registry->dispatchHandlerForTest(&context.handler);
  context.deferredScopeRequested = 1;
  const bool deferredScopeDispatched = registry->dispatchHandlerForTest(&context.handler);
  g_RegistryDispatchContext = nullptr;

  const bool cleaned = registered && registry->unregisterHandler(&context.handler);
  bool passed = true;
  passed &= check(registered, "the test handler could not be registered", Test);
  passed &= check(writerDispatched && context.writerHookCalls == 1 && context.writerDispatches == 1,
                  "dispatch or callback return waited for the registry writer lock", Test);
  passed &= check(deferredScopeDispatched && context.deferredScopeHookCalls == 1 &&
                      context.deferredScopeDispatches == 1 && context.calls == 4,
                  "dispatch entered the interrupted Thread's cleanup publication lock", Test);
  passed &= check(registry->activeDispatchCountForTest(&context.handler) == 0,
...
void unregisterBeforePin(MemoryTrapHandler* handler) {
  RegistryDispatchContext* context = g_RegistryDispatchContext;
  if (!context || handler != &context->handler || !context->prePinHookCalls.compareAndSwap(0, 1)) {
    return;
  }
...
bool admissionCloseRevalidation() {
  ...
  const bool staleDispatch = registry->dispatchHandlerForTest(&context.handler);
  ...
  bool passed = true;
  passed &= check(registered, "the test handler could not be registered", Test);
  passed &= check(context.prePinHookCalls == 1 && context.unregisterSucceeded == 1,
                  "unregister did not retire the pre-pin publication", Test);
  passed &= check(!staleDispatch && context.calls == callsBeforeDispatch,
                  "a stale pre-pin snapshot entered the retired callback", Test);
  passed &= check(registry->activeDispatchCountForTest(&context.handler) == 0,
                  "rejected admission leaked a callback hazard", Test);
  passed &= check(reused && cleaned, "the revalidated slot could not be reused and removed", Test);
  if (passed) {
    NOTICE("HOSTED-WAIT-TEST: PASS pagefault-admission-close-race");
  }
  return passed;
}
...
bool AtomicReopenHandler::trap(InterruptState&, uintptr_t, bool) {
  ...
  while (!m_Context.atomicRemovalReturned && Time::getTicks() < deadline) {
...
void atomicReopenPinHook(MemoryTrapHandler* handler) {
  ...
  const Time::Timestamp deadline = Time::getTicks() + (500 * Time::Multiplier::Millisecond);
  while (context->phase != static_cast<size_t>(2) && Time::getTicks() < deadline) {
    Scheduler::instance().yield();
  }
...
void atomicReopenDrainHook(MemoryTrapHandler* handler) {
  ...
  context->phase = 2;
  const Time::Timestamp deadline = Time::getTicks() + (500 * Time::Multiplier::Millisecond);
  while (context->phase != static_cast<size_t>(3) && Time::getTicks() < deadline) {
    Scheduler::instance().yield();
  }
...
int atomicReopenRemover(void* parameter) {
  AtomicReopenContext* context = reinterpret_cast<AtomicReopenContext*>(parameter);
  const Time::Timestamp deadline = Time::getTicks() + (500 * Time::Multiplier::Millisecond);
...
bool atomicReopenPreservesSelfRemoval() {
  constexpr const char* Test = "pagefault-atomic-reopen-self-removal";
  PageFaultHandler* registry = &PageFaultHandler::instance();
  AtomicReopenContext context(registry);
  context.remover = new Thread(Scheduler::instance().getKernelProcess(), atomicReopenRemover,
  ...
  const bool registered = registry->registerHandler(&context.handler);
  const bool dispatched = registry->dispatchHandlerForTest(&context.handler);
  ...
  const bool lateDispatchRejected = !registry->dispatchHandlerForTest(&context.handler);
  const bool hazardsDrained = registry->activeDispatchCountForTest(&context.handler) == 0;
  const bool reused = registry->registerHandler(&context.handler);
  const bool cleaned = reused && registry->unregisterHandler(&context.handler);

  bool passed = true;
  passed &= check(registered && dispatched && removerJoined,
                  "the atomic-removal workers did not complete", Test);
  passed &= check(context.failures == 0 && context.pinHookCalls == 1 &&
                      context.drainHookCalls == 1 && context.handlerCalls == 1,
                  "the atomic reopen interleaving was not reached", Test);
  passed &= check(context.selfRemovalRejected == 1 && context.atomicRemovalReturned == 1 &&
                      context.atomicRemovalRejected == 1,
                  "atomic reopen erased or replaced callback self-removal", Test);
  passed &= check(lateDispatchRejected && hazardsDrained,
                  "the self-removal publication remained enabled after callback return", Test);
  passed &= check(reused && cleaned, "the retired slot was not reusable", Test);
  if (passed) {
    NOTICE(
...
bool nestedDispatchRemoval() {
  ...
  const bool dispatched = registry->dispatchHandlerForTest(&context.outer);
  const bool innerLateDispatchRejected = !registry->dispatchHandlerForTest(&context.inner);
  const bool hazardsDrained = registry->activeDispatchCountForTest(&context.outer) == 0 &&
                              registry->activeDispatchCountForTest(&context.inner) == 0;
  const bool outerCleaned = outerRegistered && registry->unregisterHandler(&context.outer);
  const bool innerReused = registry->registerHandler(&context.inner);
  const bool innerCleaned = innerReused && registry->unregisterHandler(&context.inner);

  bool passed = true;
  passed &= check(outerRegistered && innerRegistered && dispatched,
                  "the nested handlers could not be registered and dispatched", Test);
  passed &= check(context.innerDispatches == 1 && context.selfRemovalRejected == 1 &&
                      context.orderCount == 4 && context.order[0] == 1 && context.order[1] == 2 &&
                      context.order[2] == 3 && context.order[3] == 4,
                  "nested dispatch did not unwind in strict LIFO order", Test);
  passed &= check(innerLateDispatchRejected && hazardsDrained,
                  "nested self-removal did not retire after the final hazard", Test);
  passed &= check(outerCleaned && innerReused && innerCleaned,
...
struct HandlerLifetimeContext {
  explicit HandlerLifetimeContext(PageFaultHandler* registry)
      : registry(registry),
        ...
        unregisterSucceeded(0),
        competitorReturned(0),
        competitorRejected(0),
        selfRemovalDuringDrain(0),
        failures(0) {}
...
bool LifetimeHandler::trap(InterruptState&, uintptr_t, bool) {
  ...
  if (m_Context.phase == static_cast<size_t>(3) && !m_Context.registry->unregisterHandler(this)) {
...
void handlerPinHook(MemoryTrapHandler* handler) {
  ...
  if (context->phase == static_cast<size_t>(2) &&
      hasCallbackDrainWait(context->remover, &context->handler) && !context->unregisterReturned) {
    context->hookObservedDrain += 1;
    context->competitorStart = 1;
    while (!context->competitorReturned && Time::getTicks() < deadline) {
      Scheduler::instance().yield();
    }
...
bool handlerLifetimeBarrier() {
  ...
  context.competitor = new Thread(Scheduler::instance().getKernelProcess(), competingUnregister,
                                  &context, nullptr, false, true);
  ...
  const bool registered = registry->registerHandler(&context.handler);
  const bool duplicateRejected = !registry->registerHandler(&context.handler);
  const bool dispatched = registry->dispatchHandlerForTest(&context.handler);
  ...
  const bool lateDispatchRejected = !registry->dispatchHandlerForTest(&context.handler);
  const bool unregisterBoundaryHeld = lateDispatchRejected &&
  ...
  const bool redispatched = registry->dispatchHandlerForTest(&context.handler);
  const bool recleaned = reregistered && registry->unregisterHandler(&context.handler);

  SelfRemovingHandler selfRemoving(registry);
  const bool selfRegistered = registry->registerHandler(&selfRemoving);
  const bool selfDispatched = registry->dispatchHandlerForTest(&selfRemoving);
  const bool selfLateDispatchRejected = !registry->dispatchHandlerForTest(&selfRemoving);
  const bool selfReregistered = registry->registerHandler(&selfRemoving);
  const bool selfCleanup = selfReregistered && registry->unregisterHandler(&selfRemoving);

  bool passed = true;
  passed &= check(registered && duplicateRejected && dispatched,
                  "registration, duplicate rejection, or initial dispatch failed");
  passed &= check(joined && competitorJoined && context.failures == 0,
                  "the concurrent unregister workers did not finish cleanly");
  passed &= check(context.hookCalls == 1 && context.hookObservedDrain == 1,
                  "unregister did not wait for the callback pin");
  passed &= check(context.unregisterSucceeded == 1 && context.unregisterReturned == 1 &&
                      context.competitorReturned == 1 && context.competitorRejected == 1 &&
                      context.selfRemovalDuringDrain == 1,
                  "draining self-removal was lost or a second drainer was admitted");
  passed &= check(unregisterBoundaryHeld, "a callback began after unregisterHandler returned");
  passed &= check(reregistered && redispatched && recleaned &&
                      context.handlerCalls == callsAtUnregisterReturn + 1,
                  "a drained handler could not be registered and dispatched again");
  passed &= check(selfRegistered && selfDispatched && selfRemoving.rejectionSeen == 1 &&
                      selfRemoving.calls == 1 && selfLateDispatchRejected,
                  "self-unregister did not reject its own drain and retire on return");
  passed &= check(selfReregistered && selfCleanup,
                  "deferred self-removal did not release its registry slot");

  if (passed) {
    NOTICE("HOSTED-WAIT-TEST: PASS pagefault-handler-waitqueue-drain");
    NOTICE("HOSTED-WAIT-TEST: PASS pagefault-handler-lifetime");
  }
  return passed;
}
...
int unregisterAbandonedDispatch(void* parameter) {
  AbandonedDispatchContext* context = reinterpret_cast<AbandonedDispatchContext*>(parameter);
  const Time::Timestamp deadline = Time::getTicks() + (500 * Time::Multiplier::Millisecond);
  while (context->phase != static_cast<size_t>(1) && Time::getTicks() < deadline) {
    Scheduler::instance().yield();
  }
...
bool abandonedDispatchCleanup() {
  ...
  context.remover = new Thread(Scheduler::instance().getKernelProcess(),
                               unregisterAbandonedDispatch, &context, nullptr, false, true);
  context.remover->setName("hosted abandoned pagefault remover");
  context.dispatcher = new Thread(Scheduler::instance().getKernelProcess(), dispatchThenAbandon,
  ...
  const size_t activeDispatches = registry->activeDispatchCountForTest(&context.handler);
  const bool lateDispatchRejected = !registry->dispatchHandlerForTest(&context.handler);

  const bool passed =
      check(registered && started && dispatcherJoined && removerJoined,
            "the abandoned-dispatch workers did not complete") &&
      check(context.failures == 0 && context.hookCalls == 1 && context.hookObservedDrain == 1,
            "the forced abandonment window was not reached") &&
      check(context.handlerCalls == 0 && context.dispatchReturned == 0,
            "the abandoned fault stack returned through normal dispatch") &&
      check(activeDispatches == 0, "stack abandonment leaked an active callback hazard") &&
      check(context.unregisterSucceeded == 1 && context.unregisterReturned == 1 &&
                lateDispatchRejected,
            "state cleanup did not release the pinned callback exactly once");
  if (passed) {
    NOTICE("HOSTED-WAIT-TEST: PASS pagefault-abandoned-dispatch");
  }
  return passed;
}
}  // namespace
