...
class TrackingDisk final : public Disk
{
  public:
    ...
    TrackingDisk()
        : m_ReadCount(0), m_UnpinCount(0),
          m_DataAllocation(new uint8_t[DataSize + PageSize - 1]),
          m_Data(reinterpret_cast<uint8_t *>(
              (reinterpret_cast<uintptr_t>(m_DataAllocation) + PageSize - 1) &
              ~(PageSize - 1))),
          m_PageAllocation(
              new uint8_t[(PageSlots * PageSize) + PageSize - 1]),
          m_PageData(reinterpret_cast<uint8_t *>(
              (reinterpret_cast<uintptr_t>(m_PageAllocation) + PageSize - 1) &
              ~(PageSize - 1))),
          m_AlignmentPoint(0), m_BalanceError(false)
    {
        for (size_t i = 0; i < DataSize; ++i)
        {
            m_Data[i] =
                static_cast<uint8_t>((i * 29) ^ (i >> 2));
        }
    ...
    uintptr_t read(uint64_t location) override
    {
        ...
        return reinterpret_cast<uintptr_t>(
            m_PageData + (slot * PageSize) + (location - page));
    }
  ...
  private:
    ...
    {
        const uint64_t alignPoint =
            m_AlignmentPoint <= location ? m_AlignmentPoint : 0;
    ...
    size_t findPage(uint64_t page, bool create)
    {
        ...
        MemoryCopy(
            m_PageData + (freeSlot * PageSize), m_Data + page,
            PageSize);
        return freeSlot;
    }
...
bool rawFsNativePageOwnership()
{
    ...
    RawFs filesystem;
    RawFsFile file(
        String("raw-disk"), &filesystem, nullptr, &disk);
    uint8_t *destination = new uint8_t[TransferSize];

    const uint64_t bytesRead = file.read(
        Start, TransferSize,
        reinterpret_cast<uintptr_t>(destination));
    const bool bytesMatch =
        !MemoryCompare(
            destination, disk.data() + Start, TransferSize);
    delete[] destination;

    const bool splitAtPage =
        disk.m_ReadCount == 2 && disk.m_ReadLocations[0] == 0 &&
        disk.m_ReadLocations[1] == 4096;
    const bool balanced =
        disk.m_UnpinCount == 2 && disk.m_UnpinLocations[0] == 0 &&
        disk.m_UnpinLocations[1] == 4096 && disk.balanced();

    const bool passed =
        bytesRead == TransferSize && bytesMatch && splitAtPage && balanced;
    if (passed)
    {
...
bool rawFsParentAlignmentIsolation()
{
    ...
    RawFs filesystem;
    RawFsFile file(
        String("aligned-raw-disk"), &filesystem, nullptr, &disk);
    uint8_t *destination = new uint8_t[TransferSize];
    ...
    const bool splitAtParentPage =
        disk.m_ReadCount == 2 && disk.m_ReadLocations[0] == Start &&
        disk.m_ReadLocations[1] == 4608;
    const bool balanced =
        disk.m_UnpinCount == 2 &&
        disk.m_UnpinLocations[0] == Start &&
        disk.m_UnpinLocations[1] == 4608 && disk.balanced();

    const bool passed =
        bytesRead == TransferSize && bytesMatch && splitAtParentPage &&
        balanced;
    if (passed)
    {
...
bool filePastEofDoesNotRead()
{
    ...
    const uint64_t bytesRead = file.read(
        TrackingDisk::DataSize + 1, 1,
        reinterpret_cast<uintptr_t>(&destination));
    const bool passed =
        !bytesRead && !disk.m_ReadCount && !disk.m_UnpinCount &&
        disk.balanced();
    if (passed)
    {
...
{
    return rawFsNativePageOwnership() &&
           rawFsParentAlignmentIsolation() &&
           filePastEofDoesNotRead();
}
