...
struct FilterLifetimeContext
{
    FilterLifetimeContext()
        : remover(nullptr), callbackId(static_cast<size_t>(-1)), phase(0),
          callbackCalls(0), callbackAfterRemoval(0), removalReturned(0),
          observedDrain(0), failures(0)
    {
    }
...
void filterPinHook(bool (*callback)(uintptr_t, size_t), size_t id)
{
    FilterLifetimeContext *context = g_FilterLifetimeContext;
    if (
        !context || callback != lifetimeFilter || id != context->callbackId ||
...
int removePinnedFilter(void *parameter)
{
    FilterLifetimeContext *context =
        reinterpret_cast<FilterLifetimeContext *>(parameter);
    for (
        size_t attempt = 0;
...
bool check(bool condition, const char *detail)
{
    ...
    ERROR("HOSTED-WAIT-TEST: FAIL network-filter-callback-lifetime: "
          << detail);
    return false;
}
...
bool filterCallbackLifetimeBarrier()
{
    ...
    context.remover = new Thread(
        Scheduler::instance().getKernelProcess(), removePinnedFilter, &context,
        nullptr, false, true);
    ...
    bool passed = true;
    passed &= check(
        context.callbackId != static_cast<size_t>(-1) && joined &&
            !context.failures,
        "the concurrent removal worker did not complete cleanly");
    passed &= check(
        context.observedDrain == 1 && context.removalReturned == 1,
    ...
    passed &= check(
        selfRemoval.callbackId != static_cast<size_t>(-1) &&
