...
Iso9660Filesystem::Iso9660Filesystem()
    : m_pDisk(0), m_PrimaryVolDesc(), m_SuppVolDesc(), m_VolDesc(),
      m_RootDir(0), m_JolietLevel(0), m_BlockSize(0), m_BlockNumber(0),
      m_VolumeLabel(""), m_pRoot(0)
{
}
...
bool Iso9660Filesystem::initialise(Disk *pDisk)
{
    ...
    for (size_t i = 16; i < 256; i++)
    {
        ...
        Iso9660VolumeDescriptor *vDesc =
            reinterpret_cast<Iso9660VolumeDescriptor *>(buff);
        if (StringCompareN(
        ...
        else if (vDesc->Type == SUPP_VOL_DESC)
        {
            ...
            if (m_SuppVolDesc.Unused3_EscSequences[0] == 0x25 &&
                m_SuppVolDesc.Unused3_EscSequences[1] == 0x2F)
            {
    ...
    if (!bFound)
    {
        WARNING("ISO9660: Neither a primary or supplementary volume descriptor "
                "was found");
        return false;
    }
    ...
    if (m_JolietLevel)
    {
        String volLabelString =
            WideToMultiByteStr(reinterpret_cast<uint8_t *>(volLabel), 32, 32);
    ...
    m_RootDir =
        reinterpret_cast<Iso9660DirRecord *>(m_PrimaryVolDesc.RootDirRecord);
...
bool Iso9660Filesystem::createDirectory(
    File *parent, String filename, uint32_t mask)
{
    return false;
}

bool Iso9660Filesystem::createSymlink(
    File *parent, String filename, String value)
{
    return false;
}
...
String Iso9660Filesystem::parseName(Iso9660DirRecord &dirRecord)
{
    ...
    uint8_t *fileIdent = reinterpret_cast<uint8_t *>(
        adjust_pointer(&dirRecord, sizeof(dirRecord)));
...
{
    uint8_t *fileIdent =
        reinterpret_cast<uint8_t *>(adjust_pointer(&name, sizeof(name)));
...
File *Iso9660Filesystem::fileFromDirRecord(
    Iso9660DirRecord &dir, size_t inodeNum, File *parent, bool bDirectory)
{
    String fileName = parseName(dir);
    Time::Timestamp t = timeToUnix(dir.Time);
    if (bDirectory)
    {
        Iso9660Directory *ret = new Iso9660Directory(
            fileName, inodeNum, this, parent, dir, t, t, t);
        return ret;
    }
    else
    {
        Iso9660File *ret = new Iso9660File(
            fileName, t, t, t, inodeNum, this, LITTLE_TO_HOST32(dir.DataLen_LE),
            dir, parent);
        return ret;
    }
}
