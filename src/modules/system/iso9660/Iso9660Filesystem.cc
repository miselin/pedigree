...
bool Iso9660Filesystem::initialise(Disk* pDisk) {
  ...
  if (!bFound) {
    WARNING(
        "ISO9660: Neither a primary or supplementary volume descriptor "
        "was found");
    return false;
  }
...
bool Iso9660Filesystem::createDirectory(File* parent, const String& filename, uint32_t mask) {
  return false;
}

bool Iso9660Filesystem::createSymlink(File* parent, const String& filename, const String& value) {
  return false;
}
...
File* Iso9660Filesystem::fileFromDirRecord(Iso9660DirRecord& dir, size_t inodeNum, File* parent,
                                           bool bDirectory) {
  ...
  if (bDirectory) {
    ...
    Iso9660File* ret = new Iso9660File(fileName, t, t, t, inodeNum, this,
                                       LITTLE_TO_HOST32(dir.DataLen_LE), dir, parent);
    return ret;
  }
}
