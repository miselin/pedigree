...
class Rtl8139 : public Network, public IrqHandler {
 ...
 private:
  ...
  bool drainReceive(size_t& descriptorCount, size_t& packetCount, size_t& stagingBytes);

  /** Copies bytes from the wrapping 64K receive ring. */
  void copyFromReceiveRing(void* destination, size_t offset, size_t length) const;
