...
constexpr uint16_t EnabledInterrupts =
    RTL_ISR_RXFOVW | RTL_ISR_RXOVW | RTL_ISR_TXERR | RTL_ISR_RXERR | RTL_ISR_RXOK;
constexpr uint16_t ReceiveInterrupts = RTL_ISR_RXERR | RTL_ISR_RXOK;
constexpr uint16_t ReceiveOverflowInterrupts = RTL_ISR_RXFOVW | RTL_ISR_RXOVW;
constexpr uint16_t InvalidReceiveStatus =
    RTL_RXSTS_ISE | RTL_RXSTS_RUNT | RTL_RXSTS_LONG | RTL_RXSTS_CRC | RTL_RXSTS_FAE;
...
Rtl8139::Rtl8139(Network* pDev)
    : Network(pDev),
      ...
      m_TxCurr(0),
      m_DeviceLock(),
      m_pRxBuffVirt(nullptr),
      m_pRxBuffPhys(0),
      m_pTxBuffers(),
      m_TxBufferPhysical(),
      m_RxBuffMR("rtl8139-rxbuffer"),
      m_TxBuffMR("rtl8139-txbuffer"),
      m_IrqId(0),
      m_Stopping(false),
  ...
  const uint32_t pciCommand = PciBus::instance().readConfigSpace(this, 1);
  ...
  const size_t dmaConstraints = PhysicalMemoryManager::continuous | PhysicalMemoryManager::below4GB;
  ...
  WARNING("RTL8139: MAC is " << m_StationInfo.mac[0] << ":" << m_StationInfo.mac[1] << ":"
...
void Rtl8139::resetController() {
  ...
  m_pBase->write16(static_cast<uint16_t>((m_RxCurr - RTL_CAPR_BIAS) & 0xFFFF), RTL_CAPR);
  for (size_t i = 0; i < RTL_TX_DESCRIPTOR_COUNT; ++i) {
    m_pBase->write32(static_cast<uint32_t>(m_TxBufferPhysical[i]), RTL_TXADDR0 + (i * 4));
  }
  ...
  m_pBase->write32(RTL_RXCFG_FTH_NONE | RTL_RXCFG_RBLN_64K | RTL_RXCFG_MDMA_UNLM | RTL_RXCFG_AB |
                       RTL_RXCFG_AM | RTL_RXCFG_APM,
                   RTL_RXCFG);
  m_pBase->write32(RTL_TXCFG_IFG96 | RTL_TXCFG_MDMA_2K | RTL_TXCFG_RR_48, RTL_TXCFG);
  m_pBase->write32(0xFFFFFFFF, RTL_MAR);
  m_pBase->write32(0xFFFFFFFF, RTL_MAR + 4);
  m_pBase->write8(RTL_CFG9346_LOCK, RTL_CFG9346);

  size_t startPolls = StartPollLimit;
  while (startPolls-- && ((m_pBase->read8(RTL_CMD) & (RTL_CMD_RXEN | RTL_CMD_TXEN)) !=
                          (RTL_CMD_RXEN | RTL_CMD_TXEN))) {
    Time::delay(1 * Time::Multiplier::Millisecond);
  }
  if ((m_pBase->read8(RTL_CMD) & (RTL_CMD_RXEN | RTL_CMD_TXEN)) != (RTL_CMD_RXEN | RTL_CMD_TXEN)) {
...
void Rtl8139::haltController() {
  ...
  while (haltPolls-- && (m_pBase->read8(RTL_CMD) & (RTL_CMD_RXEN | RTL_CMD_TXEN))) {
    Time::delay(1 * Time::Multiplier::Millisecond);
  }
...
bool Rtl8139::send(size_t nBytes, uintptr_t buffer) {
  if (!buffer || nBytes < RTL_ETHERNET_HEADER_SIZE || nBytes > RTL_ETHERNET_FRAME_MAX) {
    ERROR("RTL8139: invalid transmit packet length " << Dec << nBytes << Hex);
    return false;
  }
  ...
  const uint32_t status = m_pBase->read32(RTL_TXSTS0 + (descriptor * 4));
  if (!(status & RTL_TXSTS_OWN))
    return false;
  FENCE();

  const size_t transmitLength =
      nBytes < RTL_ETHERNET_FRAME_MIN ? static_cast<size_t>(RTL_ETHERNET_FRAME_MIN) : nBytes;
  MemoryCopy(m_pTxBuffers[descriptor], reinterpret_cast<void*>(buffer), nBytes);
  if (transmitLength > nBytes) {
    ByteSet(m_pTxBuffers[descriptor] + nBytes, 0, transmitLength - nBytes);
  }

  FENCE();
  m_pBase->write32(static_cast<uint32_t>(m_TxBufferPhysical[descriptor]),
                   RTL_TXADDR0 + (descriptor * 4));
  m_pBase->write32(RTL_TXSTS_EARLY_THRESHOLD | static_cast<uint32_t>(transmitLength),
...
void Rtl8139::copyFromReceiveRing(void* destination, size_t offset, size_t length) const {
...
bool Rtl8139::drainReceive(size_t& descriptorCount, size_t& packetCount, size_t& stagingBytes) {
  while (descriptorCount < ReceivePacketBudget) {
    ...
    const bool validLength = rawLength >= (RTL_ETHERNET_FRAME_MIN + RTL_ETHERNET_CRC_SIZE) &&
                             rawLength <= (RTL_ETHERNET_FRAME_MAX + RTL_ETHERNET_CRC_SIZE);

    if (!validLength) {
      ERROR("RTL8139: corrupt receive length " << Dec << rawLength << Hex);
      badPacket();
      resetController();
      return false;
    }

    const bool validStatus = (status & RTL_RXSTS_RXOK) && !(status & InvalidReceiveStatus);
    if (validStatus) {
      ...
      copyFromReceiveRing(packet.buffer, m_RxCurr + sizeof(header), packetLength);
      stagingBytes += packetLength;
      gotPacket();
    } else {
      WARNING("RTL8139: dropping receive entry with status " << Hex << status);
      badPacket();
    }

    m_RxCurr = (m_RxCurr + sizeof(header) + rawLength + 3) & ~static_cast<size_t>(3);
...
bool Rtl8139::isConnected() {
  LockGuard<Mutex> deviceGuard(m_DeviceLock);
  return m_Initialised && !m_Stopping && !(m_pBase->read8(RTL_MSR) & RTL_MSR_LINK);
}
...
IrqDisposition Rtl8139::irq(irq_id_t number) {
...

  {
    ...
    for (size_t pass = 0; pass < InterruptPassLimit; ++pass) {
      const uint16_t irqStatus = m_pBase->read16(RTL_ISR) & EnabledInterrupts;
      const bool receivePending = !(m_pBase->read8(RTL_CMD) & RTL_CMD_BUFE);
      ...
      if (receivePending || (irqStatus & ReceiveInterrupts)) {
        if (!drainReceive(descriptorCount, packetCount, stagingBytes))
          break;
      }
    }
  ...
  for (size_t i = 0; i < packetCount; ++i) {
    ...
    NetworkStack::instance().receive(packet.length, reinterpret_cast<uintptr_t>(packet.buffer),
                                     this, 0);
  }
