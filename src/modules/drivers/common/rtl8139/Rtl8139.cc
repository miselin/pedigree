...
Rtl8139::Rtl8139(Network *pDev)
    : Network(pDev), m_pBase(0), m_StationInfo(), m_RxCurr(0), m_TxCurr(0),
      m_RxLock(false), m_TxLock(), m_pRxBuffVirt(0), m_pTxBuffVirt(0),
      m_pRxBuffPhys(0), m_pTxBuffPhys(0), m_RxBuffMR("rtl8139-rxbuffer"),
      m_TxBuffMR("rtl8139-txbuffer")
{
    ...
    if (!PhysicalMemoryManager::instance().allocateRegion(
            m_RxBuffMR,
            RTL_BUFF_SIZE / PAGE_SIZE + (RTL_BUFF_SIZE % PAGE_SIZE ? 1 : 0),
            PhysicalMemoryManager::continuous, 0, -1))
    {
...
void Rtl8139::reset()
{
    ...
    m_pBase->write32(
        RTL_RXCFG_FTH_NONE | RTL_RXCFG_RBLN_64K | RTL_RXCFG_MDMA_UNLM |
            RTL_RXCFG_AR | RTL_RXCFG_AB | RTL_RXCFG_AM | RTL_RXCFG_APM |
            RTL_RXCFG_AAP,
        RTL_RXCFG);
...
bool Rtl8139::send(size_t nBytes, uintptr_t buffer)
{
    ...
    m_pBase->write32(
        static_cast<uint32_t>(m_pTxBuffPhys), RTL_TXADDR0 + m_TxCurr * 4);
...
void Rtl8139::recv()
{
    ...
    // if bad packet, reset
    if (!(status & RTL_RXSTS_RXOK) ||
        (status & (RTL_RXSTS_ISE | RTL_RXSTS_CRC | RTL_RXSTS_FAE)) ||
        (length >= RTL_PACK_MAX) || (length < RTL_PACK_MIN))
    {
        WARNING(
            "RTL8139: Bad packet: len: " << length << ", status: " << status
                                         << "!");
        reset();
        return;
    }
    ...
    {
        // copy first the part of the packet until the end of the buffer and
        // then the rest of it, at the beginning of the buffer
    ...
    else
    {
        // copy the packet
        MemoryCopy(
            packBuff, reinterpret_cast<void *>(rxPacket + 4), length - 4);
...
bool Rtl8139::setStationInfo(StationInfo info)
{
    ...
    NOTICE(
        "RTL8139: Setting DNS servers [" << Dec << m_StationInfo.nDnsServers
...
bool Rtl8139::irq(irq_id_t number, InterruptState &state)
{
    while (true)
    {
        ...
        // no irq left, break
        if ((irqStatus & (RTL_ISR_RXOK | RTL_ISR_TXOK | RTL_ISR_RXERR |
                          RTL_ISR_TXERR)) == 0)
            break;
