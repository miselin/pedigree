...
void Dma::init() {
  ...
  if (m_Card >= NV40A) {
    ...
    pRegs->write32(((m_nRamSize - 1) & 0xffff8000) | 0x00000002,
    ...
    // (first set)
    pRegs->write32(0x80000000 | NV4_SURFACE,
                   NVACC_HT_HANDL_00);              // 32bit handle (not used)
...
void Dma::dmaCmd(uint32_t cmd, uint32_t offset, uint16_t size) {
  if (m_Card >= NV40A)
    m_pFramebuffer->write32((size << 18) | ((m_pFifoPtrs[cmd] + offset) & 0x0000fffc),
...
void Dma::screenToScreenBlit(uint16_t src_x, uint16_t src_y, uint16_t dest_x, uint16_t dest_y,
  ...
  switch (m_Card) {
    ...
    case NV30A:
      ...
      for (int i = 0; i < 32; i++) {
        ...
        m_pRegs->write32(m_pRegs->read32(NVACC_NV10_FBTIL0AD + (i << 2)),
