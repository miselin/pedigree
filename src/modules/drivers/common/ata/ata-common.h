...
inline AtaStatus ataWait(IoBase* pBase, IoBase* pControl) {
  ...
  while (status & 0x80) {
    if (++polls >= maximumPolls || (Time::getTicks() - started) >= timeout) {
      ERROR("ATA: timed out waiting for BSY to clear, status=" << status);
      status |= 0x01;
      break;
    }
    Processor::pause();
  ...
  if (!(status & 0x8)) {
    while (!(status & 0x41)) {
      if (++polls >= maximumPolls || (Time::getTicks() - started) >= timeout) {
        ERROR("ATA: timed out waiting for DRQ, DRDY, or ERR, status=" << status);
        status |= 0x01;
        break;
      }
      Processor::pause();
