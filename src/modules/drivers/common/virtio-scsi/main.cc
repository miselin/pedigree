...
List<VirtioScsiController*> controllers;

void collect(Device* pci) {
  for (size_t i = 0; i < pci->getNumChildren(); ++i) {
    if (pci->getChild(i)->getSpecificType() == String("virtio-scsi-controller")) {
      return;
    }
  }
...
bool entry() {
  Device::searchByVendorIdAndDeviceId(0x1af4, 0x1048, collect);
  Device::searchByVendorIdAndDeviceId(0x1af4, 0x1004, collect);
  while (candidates.count()) {
    Device* pci = candidates.popFront();
    auto* controller = new VirtioScsiController(pci);
...
void exit() {
  while (controllers.count()) {
    VirtioScsiController* controller = controllers.popFront();
    controller->shutdown();
    {
      Device::TreeLockGuard guard;
...
MODULE_INFO("virtio-scsi", &entry, &exit, "pci", "virtio-pci", "scsi");
