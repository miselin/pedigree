...
class Inquiry : public ScsiCommand {
 public:
  Inquiry(uint16_t len = 0, bool enableVitalData = false, uint8_t pageCode = 0, uint8_t ctl = 0);
...
class SendDiagnostic : public ScsiCommand {
 public:
  SendDiagnostic(bool selfTest, uint8_t selfTestCode = 0, uintptr_t params = 0, size_t paramLen = 0,
                 bool deviceOffline = false, bool unitOffline = false, uint8_t ctl = 0);
