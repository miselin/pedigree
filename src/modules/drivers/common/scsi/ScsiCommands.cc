...
StartStop::StartStop(bool imm, uint8_t newpower, bool eject_load, bool start, uint8_t ctl) {
  ByteSet(&command, 0, sizeof(command));
  command.opcode = 0x1b;
  command.imm = imm ? 1 : 0;
...
SendDiagnostic::SendDiagnostic(bool selfTest, uint8_t selfTestCode, uintptr_t params,
                               size_t paramLen, bool deviceOffline, bool unitOffline, uint8_t ctl) {
  ByteSet(&command, 0, sizeof(command));
  command.opcode = 0x1d;
