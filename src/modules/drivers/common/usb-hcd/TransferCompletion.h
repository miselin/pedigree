...
class TransferCompletion
{
  public:
    ...
    {
        Claim()
            : callback(nullptr), parameter(0), generation(0), result(0),
    ...
    TransferCompletion()
        : m_State(static_cast<size_t>(State::Idle)), m_Callback(nullptr),
          m_Parameter(0), m_Generation(0), m_Result(0)
    {
    }
     ...
     */
    MUST_USE_RESULT CancellationDisposition claimCancellation(
        Callback callback, uintptr_t parameter, ssize_t cancellationResult,
        Claim &claim)
    {
     ...
     */
    MUST_USE_RESULT bool
    claimForTeardown(ssize_t cancellationResult, Claim &claim)
    {
        while (true)
        {
            const State observed = state();
  ...
  private:
    ...
    {
        return state() != State::Idle && m_Callback == callback &&
               m_Parameter == parameter;
    }

    bool transition(State from, State to)
    {
        return m_State.compareAndSwap(
