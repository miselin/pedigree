...
class CallbackDeliveryQueue
{
  public:
    ...
    class Record
    {
      private:
        ...
        Record(
            ...
            ssize_t result, AfterDelivery afterDelivery,
            void *afterDeliveryContext, OnDestroy onDestroy,
            void *destroyContext)
            : m_Key(key), m_Callback(callback), m_Parameter(parameter),
              m_Result(result), m_AfterDelivery(afterDelivery),
              m_AfterDeliveryContext(afterDeliveryContext),
              m_OnDestroy(onDestroy), m_DestroyContext(destroyContext),
              m_References(1), m_State(State::Pending), m_Runner(nullptr),
              m_CompletionWaiters(), m_Completed(false)
        {
        }
    ...
    MUST_USE_RESULT Record *create(
        const Key &key, Callback callback, uintptr_t parameter, ssize_t result,
        AfterDelivery afterDelivery = nullptr,
        void *afterDeliveryContext = nullptr, OnDestroy onDestroy = nullptr,
        void *destroyContext = nullptr)
    {
        return new Record(
            key, callback, parameter, result, afterDelivery,
            afterDeliveryContext, onDestroy, destroyContext);
    }
