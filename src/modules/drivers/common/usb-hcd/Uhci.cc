...
#define INDEX_FROM_TD_VIRT(ptr) \
  (((reinterpret_cast<uintptr_t>((ptr)) - reinterpret_cast<uintptr_t>(m_pTDList)) / sizeof(TD)))
...
Uhci::Uhci(Device* pDev)
    : UsbHub(pDev),
      RequestQueue(MakeConstantString("UHCI")),
      m_pBase(0),
      m_SubmissionOperations(),
      m_CancelOperations(),
      m_TransferOperations(),
      m_CallbackOperations(),
      m_IrqId(0),
      m_TimerRegistered(false),
      m_InterruptsClosing(false),
      m_nPorts(0),
      m_PortChangesClosing(false),
      m_PortChangeLock(),
      m_Mutex(),
      m_IrqProcessingLock(),
  ...
  for (size_t i = 0; i < m_nPorts; ++i) {
    ...
    const size_t portRegister = UHCI_PORTSC + (i * 2);
    {
      ...
      if (portStatus & UHCI_PORTSC_CSCH) {
        ...
        m_pBase->write16(UsbHcd::selectiveW1cValue(portStatus, ChangeMask,
...
void Uhci::detachQueueHeadLocked(QH* pQH) {
  ...
  for (List<QH*>::Iterator it = m_AsyncSchedule.begin(); it != m_AsyncSchedule.end();) {
    if (*it == pQH) {
    ...
    if (m_pQHList) {
      ...
      while (m_AsyncSchedule.count()) {
        ...
        if (pQH->pMetaData->bPeriodic) {
          const bool dequeueAdmitted = m_DequeueOperations.tryEnter();
          ...
          const bool claimed =
              pQH->pMetaData->completion.claimForTeardown(-TransactionError, claim);
          assert(claimed);
          if (claimed) {
            const bool dequeueAdmitted = m_DequeueOperations.tryEnter();
            assert(dequeueAdmitted);
            if (dequeueAdmitted) {
              teardownCompletions.pushBack(m_CompletionDeliveries.create(
                  {pQH->pMetaData->id, claim.generation}, claim.callback, claim.parameter,
                  claim.result, enqueueCompletedTransfer, pQH));
            }
          }
        }
...
IrqDisposition Uhci::irq(irq_id_t number) {
  ...
  List<UsbHcd::CallbackDeliveryQueue::Record*> completions;
  {
...

        {
          ...
          while (pQH->pMetaData->tdList.count() && tdBudget) {
              ...
              if (pTD->nEndpoint) {
                if ((nResult >= 0) && (pTD->nPid == UsbPidIn)) {
                  ...
                  if (nResult < pTD->nBufferSize) {
                    DEBUG_LOG_NOLOCK("UHCI: Short read - got " << nResult << " bytes, wanted "
            ...
            if (bEndOfTransfer) {
              ...
              if (!bPeriodic) {
                const bool captured = pQH->pMetaData->completion.captureNatural(completionResult);
                assert(captured);
                if (captured) {
                  ...
                  UsbHcd::TransferCompletion::Claim claim;
                  const bool claimed = pQH->pMetaData->completion.claimCaptured(claim);
                  const bool dequeueAdmitted = m_DequeueOperations.tryEnter();
          ...
          if (!tdBudget && pQH->pMetaData->tdList.count()) {
            ERROR_NOLOCK("UHCI: QH #" << Dec << pQH->pMetaData->id << Hex
    ...
    if (persistList.count()) {
      LockGuard<Mutex> guard(m_AsyncQueueListChangeLock);
      for (List<QH*>::Iterator it = persistList.begin(); it != persistList.end();) {
...
void Uhci::addTransferToTransaction(uintptr_t pTransaction, bool bToggle, UsbPid pid,
...
bool Uhci::doAsyncOwned(uintptr_t pTransaction, void (*pCallback)(uintptr_t, ssize_t),
  ...
  if (pQH->pMetaData->pPrev || pQH->pMetaData->pNext ||
      (!pQH->pMetaData->bPeriodic &&
       pQH->pMetaData->completion.state() != UsbHcd::TransferCompletion::State::Idle)) {
    ERROR("UHCI: doAsync: transaction is already owned [" << pTransaction << "].");
    return false;
  }
...
void Uhci::cancelAsyncAndDrain(uintptr_t pTransaction, void (*pCallback)(uintptr_t, ssize_t),
                               uintptr_t pParam) {
...

  {
    ...
    if ((pTransaction != static_cast<uintptr_t>(-1)) && (pTransaction < QhListCount) &&
      ...
      if (pQH->pMetaData && !pQH->pMetaData->bPeriodic) {
        LockGuard<Mutex> irqGuard(m_IrqProcessingLock);
        UsbHcd::TransferCompletion::Claim claim;
        const auto disposition = pQH->pMetaData->completion.claimCancellation(
...
void Uhci::modifyPortControl(size_t portRegister, uint16_t clearMask, uint16_t setMask) {
  constexpr uint16_t ChangeMask = UHCI_PORTSC_CSCH | UHCI_PORTSC_EDCH;
  LockGuard<Spinlock> portChangeGuard(m_PortChangeLock);
  uint16_t portControl = UsbHcd::selectiveW1cValue(m_pBase->read16(portRegister), ChangeMask,
...
bool Uhci::portReset(uint8_t nPort, bool bErrorResponse) {
  ...
  if (bErrorResponse) {
    ...
    while (disablePolls-- && (m_pBase->read16(portRegister) & UHCI_PORTSC_ENABLE)) {
      Time::delay(1 * Time::Multiplier::Millisecond);
    }
    if (m_pBase->read16(portRegister) & UHCI_PORTSC_ENABLE) {
      ERROR("USB: UHCI: Port " << Dec << nPort << Hex << " did not disable within 100 ms");
      return false;
    }
  }
  ...
  if (!(m_pBase->read16(portRegister) & UHCI_PORTSC_ENABLE)) {
    // #ifdef USB_VERBOSE_DEBUG
    DEBUG_LOG("USB: UHCI: During reset, port "
...
uint64_t Uhci::executeRequest(uint64_t p1, uint64_t p2, uint64_t p3, uint64_t p4, uint64_t p5,
  ...
  if (portStatus & UHCI_PORTSC_CONN) {
    ...
    if (portStatus & UHCI_PORTSC_LOSPEED) {
      DEBUG_LOG("USB: UHCI [" << this << "]: Port " << Dec << p1 << Hex
                              << " has a low-speed device connected to it");
      if (!deviceConnected(p1, LowSpeed))
        WARNING("USB: UHCI [" << this << "]: Port " << Dec << p1 << Hex
    ...
    DEBUG_LOG("USB: UHCI [" << this << "]: Device on port " << Dec << p1 << Hex
                            << " disconnected.");
