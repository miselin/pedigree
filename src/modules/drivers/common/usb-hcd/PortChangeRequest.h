...
inline T selectiveW1cValue(T status, T changeMask, T acknowledgeMask)
{
    return static_cast<T>(
        (status & ~changeMask) | (acknowledgeMask & changeMask));
}
...
class PortChangeRequest
{
  public:
    ...
    struct Observation
    {
        Observation(Result result, size_t generation)
    ...
    class Completion
    {
      public:
        Completion(PortChangeRequest &request, size_t generation)
            : m_Request(request), m_Generation(generation),
    ...
    bool configure(
        RequestQueue &queue, size_t priority, uint64_t p1 = 0,
        uint64_t p2 = 0, uint64_t p3 = 0, uint64_t p4 = 0,
        uint64_t p5 = 0, uint64_t p6 = 0, uint64_t p7 = 0)
    {
        if (
            m_Configured || priority >= REQUEST_QUEUE_NUM_PRIORITIES ||
            !m_Request.isAvailable())
        {
            return false;
        }
    ...
    bool isIdle() const
    {
        return m_Request.isAvailable() &&
               (m_Stopping || m_Consumed >= m_Observed);
    }
