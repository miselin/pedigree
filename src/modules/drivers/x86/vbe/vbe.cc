...
static bool entry() {
  ...
  vbeModeInfo* mode =
      reinterpret_cast<vbeModeInfo*>(Bios::instance().malloc(/*sizeof(vbeModeInfo)*/ 256));
  ...
  for (List<Display::ScreenMode*>::Iterator it = modeList.begin(); it != modeList.end(); it++) {
    Display::ScreenMode* pSm = *it;
    NOTICE(Hex << pSm->id << "\t " << Dec << pSm->width << "x" << pSm->height << "x" << pSm->pf.nBpp
               << "\t " << Hex << pSm->framebuffer);
    if (!pSm->textMode) {
      NOTICE("    " << pSm->pf.mRed << "<<" << pSm->pf.pRed << "    " << pSm->pf.mGreen << "<<"
                    << pSm->pf.pGreen << "    " << pSm->pf.mBlue << "<<" << pSm->pf.pBlue);
    } else {
      NOTICE("    text mode");
    }
  }
  ...
  auto searchNode = [&pDevice, fbAddr](Device* pDev) {
    ...
    for (unsigned int j = 0; j < pDev->addresses().count(); j++) {
      if (pDev->getPciClassCode() == 0x03 && pDev->addresses()[j]->m_Address <= fbAddr &&
          (pDev->addresses()[j]->m_Address + pDev->addresses()[j]->m_Size) > fbAddr) {
        pDevice = pDev;
        break;
      }
    }
  ...
  if (!pResult) {
    ...
    delete pResult;
    str.Format("UPDATE displays SET id=%d WHERE pointer=%d", g_nDisplays,
  ...
  if (pFeatures->provides(ServiceFeatures::touch))
    if (pService)
      pService->serve(ServiceFeatures::touch, reinterpret_cast<void*>(pProvider),
                      sizeof(*pProvider));
...
void VbeFramebuffer::setFramebuffer(uintptr_t p) {
  ...
  if (m_nBackbufferBytes) {
    ...
    size_t nPages = (m_nBackbufferBytes + PhysicalMemoryManager::instance().getPageSize()) /
                    PhysicalMemoryManager::instance().getPageSize();

    m_pFramebufferRegion = new MemoryRegion("VBE Backbuffer");
    if (!PhysicalMemoryManager::instance().allocateRegion(*m_pFramebufferRegion, nPages,
                                                          PhysicalMemoryManager::continuous,
                                                          VirtualAddressSpace::Write)) {
      delete m_pFramebufferRegion;
      m_pFramebufferRegion = 0;
    } else {
      NOTICE("VBE backbuffer is at "
             << reinterpret_cast<uintptr_t>(m_pFramebufferRegion->virtualAddress()));
