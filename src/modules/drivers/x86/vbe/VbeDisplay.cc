...
VbeDisplay::VbeDisplay(Device* p, VbeVersion version, List<Display::ScreenMode*>& sms,
                       size_t vidMemSz, size_t displayNum)
    : Display(p),
      m_VbeVersion(version),
      m_ModeList(sms),
      m_Mode(),
      m_pFramebuffer(0),
      m_Buffers(),
  ...
  for (List<Display::ScreenMode*>::Iterator it = m_ModeList.begin(); it != m_ModeList.end(); it++) {
    str.Format("INSERT INTO 'display_modes' VALUES (NULL, %d,%d,%d,%d,%d,%d)", (*it)->id,
               displayNum, (*it)->width, (*it)->height, (*it)->pf.nBpp, (*it)->refresh);
  ...
  for (Vector<Device::Address*>::Iterator it = m_Addresses.begin(); it != m_Addresses.end(); it++) {
...
bool VbeDisplay::setScreenMode(Display::ScreenMode sm) {
  ...
  if (m_Mode.pf.nBpp == 16 && m_Mode.pf.pRed == 11 && m_Mode.pf.mRed == 5 &&
      m_Mode.pf.pGreen == 5 && m_Mode.pf.mGreen == 6 && m_Mode.pf.pBlue == 0 &&
      m_Mode.pf.mBlue == 5) {
    m_Mode.pf2 = Graphics::Bits16_Rgb565;
    m_SpecialisedMode = Mode_16bpp_5r6g5b;
  } else if (m_Mode.pf.nBpp == 16 && m_Mode.pf.pRed == 10 && m_Mode.pf.mRed == 5 &&
             m_Mode.pf.pGreen == 5 && m_Mode.pf.mGreen == 5 && m_Mode.pf.pBlue == 0 &&
             m_Mode.pf.mBlue == 5) {
  ...
  for (Tree<rgb_t*, Buffer*>::Iterator it = m_Buffers.begin(); it != m_Buffers.end(); it++) {
  ...
  m_pFramebufferRawAddress->map((m_Mode.height * m_Mode.bytesPerLine) + m_Mode.width, true, true);
...
void VbeDisplay::setCurrentBuffer(rgb_t* pBuffer) {
  ...
  MemoryCopy(getFramebuffer(), pBuf->pFbBackbuffer,
             m_Mode.width * m_Mode.height * (m_Mode.pf.nBpp / 8));
}

void VbeDisplay::updateBuffer(rgb_t* pBuffer, size_t x1, size_t y1, size_t x2, size_t y2) {
  Buffer* pBuf = m_Buffers.lookup(pBuffer);
  if (!pBuf || !pBuf->valid) {
  ...
  for (size_t y = y1; y <= y2; y++) {
    ...
    MemoryCopy(
        reinterpret_cast<uint8_t*>(getFramebuffer()) + y * m_Mode.pf.nPitch + x1 * bytesPerPixel,
        pBuf->pFbBackbuffer + y * m_Mode.pf.nPitch + x1 * bytesPerPixel, (x2 - x1) * bytesPerPixel);
  }
}
...
void VbeDisplay::bitBlit(rgb_t* pBuffer, size_t fromX, size_t fromY, size_t toX, size_t toY,
  ...
  if (toX == 0 && fromX == 0 && width == m_Mode.width) {
    ...
    MemoryCopy(&pFb[to * bytesPerPixel], &pFb[from * bytesPerPixel], sz * bytesPerPixel);
    MemoryCopy(reinterpret_cast<uint8_t*>(getFramebuffer()) + extent_s * bytesPerPixel,
               pBuf->pFbBackbuffer + extent_s * bytesPerPixel, extent_e * bytesPerPixel);
  } else {
    ...
    for (size_t y = min; y < max; y += increment) {
      ...
      MemoryCopy(&pFb[to * bytesPerPixel], &pFb[from * bytesPerPixel], width * bytesPerPixel);
      MemoryCopy(reinterpret_cast<uint8_t*>(getFramebuffer()) + to * bytesPerPixel,
                 pBuf->pFbBackbuffer + to * bytesPerPixel, width * bytesPerPixel);
    }
  }
}

void VbeDisplay::fillRectangle(rgb_t* pBuffer, size_t x, size_t y, size_t width, size_t height,
                               rgb_t colour) {
  Buffer* pBuf = m_Buffers.lookup(pBuffer);
  if (!pBuf || !pBuf->valid) {
  ...
  if (m_SpecialisedMode == Mode_16bpp_5r6g5b) {
    compiledColour = ((colour.r >> 3) << 11) | ((colour.g >> 2) << 5) | ((colour.b >> 3) << 0);
  ...
  for (size_t i = y; i < y + height; i++) {
    for (size_t j = x; j < x + width; j++) {
      ...
      switch (m_Mode.pf.nBpp) {
        ...
        default:
          WARNING(
              "VbeDisplay: Pixel format not handled in "
              "fillRectangle.");
      }
    }
  }
...
void VbeDisplay::packColour(rgb_t colour, size_t idx, uintptr_t pFb) {
  ...
  // Assemble the colour.
  uint32_t c = 0 | (static_cast<uint32_t>(r) << pf.pRed) | (static_cast<uint32_t>(g) << pf.pGreen) |
