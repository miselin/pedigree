...
class Ps2Mouse : public Device {
 public:
  ...
  EXPORTED_PUBLIC MUST_USE_RESULT bool subscribe(MouseHandlerFunction handler, void* param,
                                                 Registration& registration);

 private:
  Ps2Controller* m_pController;

  enum WaitType { Data, Signal };

  enum Ps2Ports { KbdStat = 0x64, KbdCommand = 0x60 };
