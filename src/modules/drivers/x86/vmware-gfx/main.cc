...
class VmwareGraphics : public Display {
 public:
  explicit VmwareGraphics(Device* pDev)
      : Display(pDev),
        ...
        m_MaxHeight(0),
        m_FramebufferSize(0),
        m_CommandRegionSize(0),
        m_FifoHeaderBytes(0),
        m_HardwareAccepted(false),
        m_Online(false),
        m_FifoRunning(false),
        m_Stopping(false),
  ...
  bool redraw(size_t x, size_t y, size_t width, size_t height) {
    ...
    const uint32_t command[] = {SVGA_CMD_UPDATE, static_cast<uint32_t>(x), static_cast<uint32_t>(y),
                                static_cast<uint32_t>(width), static_cast<uint32_t>(height)};
    return submitFifoLocked(command, sizeof(command) / sizeof(command[0]));
  }

  bool copy(size_t sourceX, size_t sourceY, size_t destinationX, size_t destinationY, size_t width,
            size_t height, bool softwareFallback) {
    if (!Processor::getInterrupts())
      return false;
    LockGuard<Mutex> guard(m_DeviceLock);
    if (!availableLocked() || !m_pFramebuffer || !m_pFramebuffer->getActive())
      return false;

    const size_t screenWidth = m_pFramebuffer->getWidth();
    const size_t screenHeight = m_pFramebuffer->getHeight();
    if ((sourceX >= screenWidth) || (destinationX >= screenWidth) || (sourceY >= screenHeight) ||
    ...
    if (m_FifoRunning && (m_Capabilities & SVGA_CAP_RECT_COPY)) {
      const uint32_t command[] = {SVGA_CMD_RECT_COPY,
    ...
    if (!softwareFallback)
      return false;
    m_pFramebuffer->softwareCopy(sourceX, sourceY, destinationX, destinationY, width, height);
    return true;
  }
  ...
  class VmwareFramebuffer : public Framebuffer {
   public:
    ...
    virtual void hwRedraw(size_t x = ~0UL, size_t y = ~0UL, size_t width = ~0UL,
                          size_t height = ~0UL) {
      if (!Processor::getInterrupts())
        return;
    ...
    virtual void copy(size_t sourceX, size_t sourceY, size_t destinationX, size_t destinationY,
                      size_t width, size_t height, bool bLowestCall = true) {
      if (!Processor::getInterrupts())
        return;
      OperationBarrier::Lease callback;
      if (!m_CallbackOperations.tryAcquire(callback))
        return;
      if (m_pDisplay)
        m_pDisplay->copy(sourceX, sourceY, destinationX, destinationY, width, height, bLowestCall);
    }

    void softwareCopy(size_t sourceX, size_t sourceY, size_t destinationX, size_t destinationY,
 ...
 private:
  ...
  static Graphics::PixelFormat pixelFormat(size_t bpp, uint32_t redMask, uint32_t greenMask,
                                           uint32_t blueMask) {
    switch (bpp) {
      case 24:
        return redMask > blueMask ? Graphics::Bits24_Rgb : Graphics::Bits24_Bgr;
      case 16:
        if ((redMask == greenMask) && (greenMask == blueMask)) {
          return blueMask == 0xF ? Graphics::Bits16_Argb : Graphics::Bits16_Rgb555;
        }
  ...
  bool initialiseHardwareLocked() {
    ...
    if (!pFramebufferAddress || !pCommandAddress || (pFramebufferAddress == pCommandAddress) ||
        !pFramebufferAddress->m_Io || !pCommandAddress->m_Io || !m_MaxWidth || !m_MaxHeight ||
        !commandSize || (commandSize > pCommandAddress->m_Size) ||
        (commandSize > static_cast<size_t>(~static_cast<uint32_t>(0)))) {
      WARNING("vmware-gfx reported an invalid framebuffer/FIFO layout");
      return false;
    }

    m_Framebuffer = static_cast<MemoryMappedIo*>(pFramebufferAddress->m_Io);
    ...
    if (!m_Framebuffer->virtualAddress() || (m_Framebuffer->size() < m_FramebufferSize)) {
      WARNING("vmware-gfx could not map its framebuffer aperture");
      return false;
    }

    pCommandAddress->map(m_CommandRegionSize);
    if (!m_CommandRegion->virtualAddress() || (m_CommandRegion->size() < m_CommandRegionSize)) {
      WARNING("vmware-gfx could not map its FIFO aperture");
      return false;
    }

    uint32_t fifoMin = 4 * sizeof(uint32_t);
    const uint32_t fifoMax = static_cast<uint32_t>(commandSize) & ~static_cast<uint32_t>(3);
    if (m_Capabilities & SVGA_CAP_EXTENDED_FIFO) {
      ...
      if ((fifoRegisters < SVGA_FIFO_NUM_REGS) ||
          (fifoRegisters > (static_cast<size_t>(~static_cast<uint32_t>(0)) / sizeof(uint32_t)))) {
    ...
    NOTICE("vmware-gfx found, caps=" << Hex << m_Capabilities << ", maximum resolution is " << Dec
                                     << m_MaxWidth << "x" << m_MaxHeight << Hex);
    NOTICE("vmware-gfx framebuffer at " << Hex << fbBase << " - " << (fbBase + vramSize)
                                        << ", command FIFO at " << commandBase);
    if (m_Capabilities & SVGA_CAP_EXTENDED_FIFO) {
      NOTICE("vmware-gfx using extended fifo, caps=" << fifo[SVGA_FIFO_CAPABILITIES]
  ...
  bool registerProvider() {
    ...
    if (!pFeatures || !pService || !pFeatures->provides(ServiceFeatures::touch) ||
        !pService->serve(ServiceFeatures::touch, static_cast<void*>(m_pProvider),
                         sizeof(*m_pProvider)))
      return false;
  ...
  void unregisterProvider() {
    ...
    ServiceFeatures* pFeatures = ServiceManager::instance().enumerateOperations(String("graphics"));
    Service* pService = ServiceManager::instance().getService(String("graphics"));
    if (!pFeatures || !pService || !pFeatures->provides(ServiceFeatures::withdraw) ||
        !pService->serve(ServiceFeatures::withdraw, static_cast<void*>(pProvider),
                         sizeof(*pProvider))) {
  ...
  bool setModeLocked(size_t width, size_t height, size_t bpp) {
    if (!availableLocked() || !width || !height || !bpp || (width > m_MaxWidth) ||
        (height > m_MaxHeight) || (bpp > 32))
      return false;
    ...
    writeRegisterLocked(SVGA_REG_BITS_PER_PIXEL, static_cast<uint32_t>(bpp));
    ...
    if (!actualWidth || !actualHeight || !bytesPerLine || (actualWidth > m_MaxWidth) ||
        (actualHeight > m_MaxHeight) ||
        (actualWidth > ((~static_cast<size_t>(0) - 7) / static_cast<size_t>(bpp))) ||
    ...
    if ((fbOffset > m_FramebufferSize) || (framebufferBytes > (m_FramebufferSize - fbOffset))) {
      markOfflineLocked("mode exceeds the framebuffer aperture");
      return false;
    }

    const size_t requiredMapping = fbOffset + framebufferBytes;
    if (!m_Framebuffer->virtualAddress() || (m_Framebuffer->size() < requiredMapping)) {
    ...
    m_pFramebuffer->setFormat(pixelFormat(bpp, redMask, greenMask, blueMask));
    ...
    m_FifoRunning = true;
    NOTICE("vmware-gfx entered mode " << Dec << actualWidth << "x" << actualHeight << "x"
                                      << actualDepth << Hex << ", mode framebuffer is "
                                      << (fbBase + fbOffset));
    return true;
  }

  bool submitFifoLocked(const uint32_t* pCommand, size_t words) {
    if (!availableLocked() || !m_FifoRunning || !pCommand || (words > LargestCommandWords))
      return false;
  ...
  bool prepareFifoLocked(size_t words, VmwareSvgaState::FifoLayout& layout) {
    ...
    if (!VmwareSvgaState::valid(layout, m_FifoHeaderBytes, m_CommandRegionSize)) {
  ...
  VmwareSvgaState::FifoLayout readFifoLayoutLocked() const {
    volatile uint32_t* fifo = fifoLocked();
    return {fifo[SVGA_FIFO_MIN], fifo[SVGA_FIFO_MAX], fifo[SVGA_FIFO_NEXT_CMD],
            fifo[SVGA_FIFO_STOP]};
  }

  volatile uint32_t* fifoLocked() const {
    return reinterpret_cast<volatile uint32_t*>(m_CommandRegion->virtualAddress());
  }
...
static bool entry() {
  g_Found = false;
  Device::searchByVendorIdAndDeviceId(PCI_VENDOR_ID_VMWARE, PCI_DEVICE_ID_VMWARE_SVGA2,
                                      probeDevice);
  return g_Found;
}

static void exit() {
  auto restoreDisplay = [](Device* pDevice, Device* pTarget, bool* pRestored) -> Device* {
    if (pDevice != pTarget)
      return pDevice;
    *pRestored = true;
