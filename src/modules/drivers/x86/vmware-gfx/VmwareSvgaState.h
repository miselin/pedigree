...
class PollBudget {
 public:
  explicit PollBudget(uint64_t started, uint64_t timeout = FifoSyncTimeout,
                      size_t pollLimit = FifoSyncPollLimit)
      : m_Started(started), m_Timeout(timeout), m_PollLimit(pollLimit), m_Polls(0) {}
...
inline bool valid(const FifoLayout& layout) {
  ...
  return !(layout.min & alignmentMask) && !(layout.max & alignmentMask) &&
         ...
         (layout.max > layout.min) && ((layout.max - layout.min) >= (2 * sizeof(uint32_t))) &&
         (layout.next >= layout.min) && (layout.next < layout.max) && (layout.stop >= layout.min) &&
         (layout.stop < layout.max);
}

inline bool valid(const FifoLayout& layout, size_t headerFloor, size_t apertureBytes) {
  return valid(layout) && (layout.min >= headerFloor) && (layout.max <= apertureBytes);
}
...
inline size_t freeBytes(const FifoLayout& layout) {
  ...
  if (layout.next >= layout.stop) {
    return (layout.max - layout.next) + (layout.stop - layout.min) - sizeof(uint32_t);
  }
