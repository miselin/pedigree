...
Window::Window(uint64_t handle, int sock, struct sockaddr* sa, size_t sa_len, ::Container* pParent)
    : m_Handle(handle),
      m_pParent(pParent),
      m_Framebuffer(0),
      m_Dirty(),
...
void Window::refreshContext() {
  ...
  if ((m_Socket >= 0) && m_Framebuffer) {
    size_t totalSize =
        sizeof(LibUiProtocol::WindowManagerMessage) + sizeof(LibUiProtocol::RepositionMessage);
...
void Window::render(cairo_t* cr) {
  ...
  if (pBuffer && (isDirty() && m_bRefresh)) {
    ...
    cairo_surface_t* surface = cairo_image_surface_create_for_data(
        (uint8_t*)pBuffer, CAIRO_FORMAT_ARGB32, regionWidth, regionHeight, stride);
    ...
    cairo_set_source_surface(cr, surface, me.getX() + WINDOW_CLIENT_START_X,
                             me.getY() + WINDOW_CLIENT_START_Y);

    // Clip to the dirty rectangle (don't update anything more)
    cairo_rectangle(cr, me.getX() + realDirty.getX(), me.getY() + realDirty.getY(),
                    realDirty.getW(), realDirty.getH());
    cairo_clip(cr);

    // Clip to the window only (fixes rendering glitches during resize)
    cairo_rectangle(cr, me.getX() + WINDOW_CLIENT_START_X, me.getY() + WINDOW_CLIENT_START_Y,
                    me.getW() - WINDOW_CLIENT_LOST_W, me.getH() - WINDOW_CLIENT_LOST_H);
    cairo_clip(cr);
...
void Window::focus() {
  ...
  LibUiProtocol::WindowManagerMessage* pHeader = new LibUiProtocol::WindowManagerMessage;
...
void Container::retile() {
  ...
  WObjectList_t::iterator it = m_Children.begin();
  for (; it != m_Children.end(); ++it) {
    if ((*it)->getType() == WObject::Container || (*it)->getType() == WObject::Root) {
...
void Container::resize(ssize_t horizDistance, ssize_t vertDistance, WObject* pChild) {
  ...
  // Resize siblings.
  if (m_pParent &&
      ((m_pParent->getType() == WObject::Container) || m_pParent->getType() == WObject::Root)) {
