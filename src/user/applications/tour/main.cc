...
int main(int argc, char *argv[])
{
    ...
    dialog_msgbox(
        ...
        gettext("This tour is designed to help you understand how Pedigree "
                "differs from other UNIX-like systems. It's interactive, so you "
                "can practice along the way."), 0, 0, 1);

    dlg_clear();

    dialog_prgbox(
        gettext("Pedigree Tour"),
        gettext("Let's run the `ls' command for you:"),
        "ls root»/", 20, 52, 1);
    ...
    dialog_msgbox(
        ...
        gettext("As you can see, the typical /bin, /lib, /var (and so on) are "
                ...
                "but it can cause problems with some software."), 0, 0, 1);
    ...
    dialog_msgbox(
        ...
        gettext("Another significant difference in Pedigree is the path "
                ...
                "[mount]»/path/to/file."), 0, 0, 1);

    dlg_clear();

    dialog_msgbox(
        gettext("Pedigree Tour"),
        gettext("We've switched directory to root»/ if you were elsewhere. "
    ...
    while (true)
    {
        ...
        dialog_inputbox(
            gettext("Pedigree Tour"),
            gettext("Before we dig into what other mounts may exist, it's "
    ...
    dialog_msgbox(
        gettext("Pedigree Tour"),
        gettext("Now that you know how to type the paths, here are a "
                ...
                "* unix» provides a location for named UNIX sockets."), 0, 0, 1);

    dlg_clear();

    dialog_msgbox(
        gettext("Pedigree Tour"),
        gettext("Note that there is a significant caveat with respect to the "
                ...
                "different mount point can cause you to be unable to run any commands."), 0, 0,
        1);
    ...
    dialog_msgbox(
        ...
        gettext("This image has been configured such that the default PATH "
                ...
                "`cd root»/` to return to the root mount."), 0, 0, 1);
    ...
    dialog_msgbox(
        ...
        gettext("If something goes wrong, you may find yourself in the "
                ...
                "pressing F12 at any time."), 0, 0, 1);
    ...
    dialog_msgbox(
        ...
        gettext("In the debugger, you can read the kernel log, view "
                ...
                "wrong or inspect kernel state."), 0, 0, 1);
    ...
    dialog_msgbox(
        gettext("Pedigree Tour"),
        gettext("The tour is now complete, and  you are now better-equipped to handle Pedigree! "
                "Join us in #pedigree on Freenode IRC, and raise any issues you find "
                "at https://pedigree-project.org.\n\n"
                "Thank you for trying out Pedigree!"), 0, 0, 1);
