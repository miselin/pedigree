...
int sparse(int idx, int bisect_size) {
  ...
  if (all_clear(&table[idx - bisect_size], &table[idx]) > 0) {
    ...
    me->left = data_add(&table[idx - bisect_size], &table[idx]) |
  ...
  if (all_clear(&table[idx], &table[idx + bisect_size]) > 0) {
    ...
    //         printf("DATA: %x -> %x (R)\n", idx, idx+bisect_size);
    // All set, add to data section and set ptr.
    me->right = data_add(&table[idx], &table[idx + bisect_size]) |
                SPARSE_DATA_FLAG /* This is actual data, not another node. */;
  } else {
    uint16_t tmp = sparse(idx + bisect_size / 2, bisect_size / 2);
    // Buffer location may have changed with the me->left call.
...
void install(char* filename) {
  ...
  if (len < 4 || filename[len - 3] != 'k' || filename[len - 2] != 'm' || filename[len - 1] != 'c') {
    printf(
        "Warning: Filename does not end in '.kmc' - are you sure this "
        "is a compiled keymap file?\n");
    printf(
        "*** If you load an invalid keymap your system will be rendered "
