...
typedef struct table_entry
{
    ...
    uint32_t
        val;  // If flags & SPECIAL, 4 char string for special key, else U+val.
} table_entry_t;

#define TABLE_IDX(combinator, modifiers, scancode) \
...
int sparse_add()
{
    ...
    if (sparse_pos + sz > sparse_buffsz)
    {
        ...
        {
            fprintf(
                stderr, "Couldn't allocate buffer for new sparse buffer: %s.\n",
                strerror(errno));
            exit(1);
        }
...
int data_add(table_entry_t *start, table_entry_t *end)
{
    ...
    if (data_pos + sz > data_buffsz)
    {
        ...
        {
            fprintf(
                stderr, "Couldn't allocate buffer for new data buffer: %s.\n",
                strerror(errno));
            exit(1);
        }
...
int sparse(int idx, int bisect_size)
{
    ...
    if (all_clear(&table[idx - bisect_size], &table[idx]) > 0)
    {
        ...
        (all_set(&table[idx - bisect_size], &table[idx]) > 0) ||
        (bisect_size % 2))
    {
        //         printf("DATA: %x -> %x (L)\n", idx-bisect_size, idx);
        // All set, add to data section and set ptr.
        me->left =
            data_add(&table[idx - bisect_size], &table[idx]) |
            SPARSE_DATA_FLAG /* This is actual data, not another node. */;
    }
    else
    {
    ...
    if (all_clear(&table[idx], &table[idx + bisect_size]) > 0)
    {
        ...
        (all_set(&table[idx], &table[idx + bisect_size]) > 0) ||
        (bisect_size % 2))
    {
        //         printf("DATA: %x -> %x (R)\n", idx, idx+bisect_size);
        // All set, add to data section and set ptr.
        me->right =
            data_add(&table[idx], &table[idx + bisect_size]) |
            SPARSE_DATA_FLAG /* This is actual data, not another node. */;
    }
    else
    {
...
void install(char *filename)
{
    ...
    if (len < 4 || filename[len - 3] != 'k' || filename[len - 2] != 'm' ||
        filename[len - 1] != 'c')
    {
        printf("Warning: Filename does not end in '.kmc' - are you sure this "
               "is a compiled keymap file?\n");
        printf("*** If you load an invalid keymap your system will be rendered "
               "unusable without a manual reboot. ***\n");
        printf("\nContinue? [no] ");
