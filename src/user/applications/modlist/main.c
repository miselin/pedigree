...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "aluh")) != -1)
    {
        switch (c)
        {
            ...
            case 'h':
                ...
                fprintf(
                    stderr, "-a%-16slist all modules and show their status\n",
                    "");
                fprintf(stderr, "-l%-16slist all already-loaded modules\n", "");
                fprintf(
                    stderr, "-u%-16slist all modules that are not loaded\n",
                    "");
                fprintf(stderr, "-h%-16sshow this usage information\n", "");
                fprintf(stderr, "\n");
                fprintf(
                    stderr, "modlist without arguments defaults to listing all "
                            "modules with their status\n");
                err = 3;
                break;
            default:
    ...
    if (err)
    {
        if (err == 1)
            fprintf(
                stderr, "Only one action (-a, -l, or -u) can be specified\n");
        else if (err == 3)
            return 0;
        return err;
    }
    ...
    {
        printf(
            "Couldn't open the directory %s: %s.\n", MODULE_DIR,
            strerror(errno));
        return -1;
    }
        ...
        if (lastPeriod && !strcasecmp(suffix, "o"))
        {
            *lastPeriod = '\0';
            if (listAll)
            {
                printf(
                    "%-32s%s\n", filename,
                    pedigree_module_is_loaded(filename) ? "[loaded]" : "");
            }
            else if (listLoaded)
            {
