...
enum LookupPolicy { LocalFirst, NotThisObject, LocalLast };
...
extern "C" int _libload_main(const char* argv[], const char* env[]) {
  ...
  mmap(&__start_bss, bssEnd - bssStart, PROT_READ | PROT_WRITE,
...
extern "C" int main(int argc, const char* argv[]) {
  ...
  g_LibLoadSymbols.insert(
      std::pair<std::string, uintptr_t>("_libload_dlopen", (uintptr_t)_libload_dlopen));
  g_LibLoadSymbols.insert(
      std::pair<std::string, uintptr_t>("_libload_dlsym", (uintptr_t)_libload_dlsym));
  g_LibLoadSymbols.insert(
      std::pair<std::string, uintptr_t>("_libload_dlopen", (uintptr_t)_libload_dlclose));
  ...
  if (ld_debug) {
    ...
    for (std::list<std::string>::iterator it = g_lSearchPaths.begin(); it != g_lSearchPaths.end();
         ++it) {
      printf(" -> %s\n", it->c_str());
    }
  }
  ...
  if (meta->needed.size()) {
    for (std::list<std::string>::iterator it = meta->needed.begin(); it != meta->needed.end();
         ++it) {
...
bool loadSharedObjectHelper(const char* filename, object_meta_t* parent, object_meta_t** out,
  ...
  if (!loadObject(filename, object)) {
    ...
    if (object->needed.size()) {
      for (std::list<std::string>::iterator it = object->needed.begin(); it != object->needed.end();
           ++it) {
        if (g_LoadedObjects.find(*it) == g_LoadedObjects.end()) {
...
bool loadObject(const char* filename, object_meta_t* meta, bool envpath) {
  ...
  if (fd < 0) {
    fprintf(stderr, "libload.so: couldn't load object '%s' (%s) (%s)\n", filename,
            meta->path.c_str(), strerror(errno));
    return false;
  }
  ...
  if (header.ident[1] != 'E' || header.ident[2] != 'L' || header.ident[3] != 'F' ||
      header.ident[0] != 127) {
#ifdef DEBUG_LIBLOAD
  ...
  const char* pBuffer =
      (const char*)mmap(0, meta->mapped_file_sz, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
  ...
  if (header.phnum) {
      ...
      if (meta->phdrs[i].type == PT_DYNAMIC) {
        ...
        if ((msync((void*)phdr_base, mapsz, MS_SYNC) != 0) && (errno == ENOMEM)) {
          ...
          if (meta->phdrs[i].memsz > meta->phdrs[i].filesz) {
            ...
            if ((vaddr_start & ~(getpagesize() - 1)) != (vaddr_end & ~(getpagesize() - 1))) {
              ...
              void* p = mmap((void*)vaddr_start, totalLength, PROT_READ | PROT_WRITE,
          ...
          void* p = mmap((void*)phdr_base, mapsz, PROT_READ | PROT_WRITE, mapflags, map_fd,
                         map_fd ? offset : 0);
          if (p == MAP_FAILED) {
            /// \todo cleanup.
            errno = ENOEXEC;
    ...
    if (meta->dyn_strtab) {
      for (std::list<uintptr_t>::iterator it = tmp_needed.begin(); it != tmp_needed.end(); ++it) {
  ...
  for (size_t i = 0; i < header.shnum; i++) {
    if (meta->shdrs[i].type == SHT_HASH) {
      ...
      if (((uintptr_t)meta->dyn_symtab) == vaddr) {
        ...
        meta->hash_chains = (const Elf_Word*)&pBuffer[meta->shdrs[i].offset + sizeof(ElfHash_t) +
...
bool lookupSymbol(const char* symbol, object_meta_t* meta, ElfSymbol_t& sym, bool bWeak,
  ...
  // others.
  for (std::list<object_meta_t*>::iterator it = meta->preloads.begin(); it != meta->preloads.end();
       ++it) {
