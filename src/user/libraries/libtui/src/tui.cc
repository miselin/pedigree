...
bool Tui::initialise(size_t width, size_t height) {
  ...
  m_LocalData->pTerminal = new Terminal(newTermName, m_LocalData->nWidth, m_LocalData->nHeight, 0,
                                        0, 0, m_LocalData->pCairo, m_pWidget, this,
                                        m_LocalData->pNormalFont, m_LocalData->pBoldFont);
  ...
  if (!m_LocalData->pTerminal) {
    ...
    m_LocalData->pBoldFont->render("Press any key to close this window.", 5,
                                   m_LocalData->pBoldFont->getHeight() + 5, 0xFFFFFF, 0x000000,
                                   false);
...
void Tui::recreateSurfaces(void* fb) {
  ...
  m_LocalData->pSurface = cairo_image_surface_create_for_data(
      (uint8_t*)fb, CAIRO_FORMAT_ARGB32, m_LocalData->nWidth, m_LocalData->nHeight, stride);
