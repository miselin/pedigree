...
bool Widget::construct(const char* endpoint, const char* title, widgetCallback_t cb,
  ...
  CreateMessage* pCreate =
      reinterpret_cast<CreateMessage*>(messageData + sizeof(WindowManagerMessage));
...
bool Widget::setTitle(const std::string& newTitle) {
  ...
  memset(messageData, 0, totalSize);
  WindowManagerMessage* pWinMan = reinterpret_cast<WindowManagerMessage*>(messageData);
  SetTitleMessage* pMessage =
      reinterpret_cast<SetTitleMessage*>(messageData + sizeof(WindowManagerMessage));
...
bool Widget::redraw(PedigreeGraphics::Rect& rt) {
  ...
  memset(messageData, 0, totalSize);
  WindowManagerMessage* pWinMan = reinterpret_cast<WindowManagerMessage*>(messageData);
  RequestRedrawMessage* pMessage =
      reinterpret_cast<RequestRedrawMessage*>(messageData + sizeof(WindowManagerMessage));
...
bool Widget::visibility(bool vis) {
  ...
  memset(messageData, 0, totalSize);
  WindowManagerMessage* pWinMan = reinterpret_cast<WindowManagerMessage*>(messageData);
  SetVisibilityMessage* pMessage =
      reinterpret_cast<SetVisibilityMessage*>(messageData + sizeof(WindowManagerMessage));
    ...
    case LibUiProtocol::Reposition: {
      const LibUiProtocol::RepositionMessage* pReposition =
          reinterpret_cast<const LibUiProtocol::RepositionMessage*>(
              pMessageBuffer + sizeof(LibUiProtocol::WindowManagerMessage));
      delete pWidget->m_SharedFramebuffer;
#ifdef TARGET_LINUX
      pWidget->m_SharedFramebuffer =
          new SharedBuffer(pReposition->shmem_size, pReposition->shmem_handle);
#else
      pWidget->m_SharedFramebuffer =
