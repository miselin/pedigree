...
struct BufferHandle {
  ...
  bool operator==(const BufferHandle& other) const { return bytes == other.bytes; }
};
...
class SharedBuffer {
 public:
  ...
  bool valid() const { return m_message != nullptr && m_data != nullptr; }
  const BufferHandle& handle() const { return m_handle; }
  Size size() const { return m_size; }
  PixelFormat format() const { return m_format; }
  std::size_t stride() const { return m_stride; }
  std::size_t byteSize() const { return m_byteSize; }
  std::uint8_t* data() { return static_cast<std::uint8_t*>(m_data); }
  const std::uint8_t* data() const { return static_cast<const std::uint8_t*>(m_data); }
 ...
 private:
  SharedBuffer(std::unique_ptr<PedigreeIpc::SharedIpcMessage> message, BufferHandle handle,
               Size size, PixelFormat format, std::size_t stride, std::size_t byteSize,
               void* data)
