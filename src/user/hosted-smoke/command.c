...
static void* run_compute_preemption_worker(void* parameter) {
  ...
  __atomic_store_n(&probe->child_self[worker->index], (uintptr_t)pthread_self(), __ATOMIC_RELEASE);
  const uint64_t started = monotonic_nanoseconds();
  __atomic_store_n(&probe->child_syscall_ok[worker->index], started != 0, __ATOMIC_RELEASE);
  const uint64_t deadline = started + compute_probe_fallback_ns;

  while (!__atomic_load_n(&probe->stop, __ATOMIC_ACQUIRE)) {
    const uint64_t count = __atomic_add_fetch(&probe->counters[worker->index], 1, __ATOMIC_RELAXED);
    if ((count & 0x3fff) == 0) {
...
static int run_compute_preemption_test(void) {
  ...
  if (pthread_create(&threads[0], 0, run_compute_preemption_worker, &workers[0])) {
    return 1;
  }
  if (pthread_create(&threads[1], 0, run_compute_preemption_worker, &workers[1])) {
  ...
  const uintptr_t first_self = __atomic_load_n(&probe.child_self[0], __ATOMIC_ACQUIRE);
  const uintptr_t second_self = __atomic_load_n(&probe.child_self[1], __ATOMIC_ACQUIRE);
  const int child_contract = first_self && second_self && first_self != second_self &&
...
static int run_detached_clear_tid_test(void) {
  ...
  pthread_t thread;
  const int create_result = pthread_create(&thread, &attributes, run_detached_exit_worker, &probe);
  pthread_attr_destroy(&attributes);
  if (create_result) {
    return 3;
  }
  ...
  for (int attempt = 0; attempt < 3; ++attempt) {
    const int observed = __atomic_load_n(&probe.tid_word, __ATOMIC_ACQUIRE);
    if (!observed) {
      break;
    }
  ...
  if (__atomic_load_n(&probe.tid_word, __ATOMIC_ACQUIRE) ||
      __atomic_load_n(&probe.failure, __ATOMIC_ACQUIRE)) {
    return 5;
  }
...
static void* run_loopback_server(void* parameter) {
  ...
  __atomic_store_n(&server->phase, loopback_accept_ready, __ATOMIC_RELEASE);
  ...
  __atomic_store_n(&server->phase, loopback_receive_ready, __ATOMIC_RELEASE);
...
static int run_loopback_test(void) {
  ...
  int thread_error = pthread_create(&server_thread, 0, run_loopback_server, &server);
  if (thread_error) {
    close(client);
    close(listener);
    return 15;
  ...
  if (send(client, &expected, sizeof(expected), 0) != (ssize_t)sizeof(expected) ||
  ...
  if (server.failure || server.received != expected || !server.saw_eof) {
