...
TEST(RtcTimeAccounting, CarriesAggregatedTimeAcrossYear) {
  RtcTimeAccounting::CivilTime time = {2023, 12, 30, 22, 58, 59, 750000000ULL};
  constexpr uint64_t Delta =
      (2ULL * 24 * 60 * 60 + 61ULL * 60 + 2) * RtcTimeAccounting::NanosecondsPerSecond +
      500000000ULL;

  EXPECT_EQ(RtcTimeAccounting::advanceCivilTime(time, Delta), 2ULL * 24 * 60 * 60 + 61ULL * 60 + 3);
  EXPECT_EQ(time.year, 2024U);
...
TEST(RtcTimeAccounting, CarriesOneAggregateAcrossMultipleYears) {
  ...
  constexpr uint64_t Delta = 731ULL * 24 * 60 * 60 * RtcTimeAccounting::NanosecondsPerSecond;

  EXPECT_EQ(RtcTimeAccounting::advanceCivilTime(time, Delta), 731ULL * 24 * 60 * 60);
...
TEST(RtcTimeAccounting, CenturyLeapRuleIsExact) {
  ...
  RtcTimeAccounting::advanceCivilTime(nonLeap, RtcTimeAccounting::NanosecondsPerSecond);
  RtcTimeAccounting::advanceCivilTime(leap, RtcTimeAccounting::NanosecondsPerSecond);
