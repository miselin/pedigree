...
TEST(LocalApicLint0Policy, BootstrapProcessorOwnsVirtualWire) {
  ...
  constexpr uint32_t Hostile = Preserved | LocalApicLint0Policy::Masked | 0x8000 | 0x2000 | 0x45;
  const uint32_t configured =
      LocalApicLint0Policy::configuredValue(Hostile, LocalApicLint0Policy::BootstrapProcessor);

  EXPECT_EQ(configured & LocalApicLint0Policy::ProgrammableFields,
            LocalApicLint0Policy::BootstrapValue);
  EXPECT_EQ(configured & ~LocalApicLint0Policy::ProgrammableFields, Preserved);
  EXPECT_TRUE(LocalApicLint0Policy::routesLegacyPic(configured));
  EXPECT_TRUE(
      LocalApicLint0Policy::matchesRole(configured, LocalApicLint0Policy::BootstrapProcessor));
}

TEST(LocalApicLint0Policy, ApplicationProcessorCannotOwnVirtualWire) {
  constexpr uint32_t Preserved = 0x5A000000;
  const uint32_t configured =
      LocalApicLint0Policy::configuredValue(Preserved | LocalApicLint0Policy::ExtInt, 0);
...
TEST(LocalApicLint0Policy, ExactlyOneProcessorRoutesLegacyPic) {
  ...
  for (size_t processor = 0; processor < 4; ++processor) {
    const uint64_t apicBase = processor == 0 ? LocalApicLint0Policy::BootstrapProcessor : 0;
    const uint32_t configured = LocalApicLint0Policy::configuredValue(0, apicBase);
...
TEST(LocalApicLint0Policy, ReadbackMustMatchTheProcessorRole) {
  EXPECT_FALSE(LocalApicLint0Policy::matchesRole(
      LocalApicLint0Policy::BootstrapValue | LocalApicLint0Policy::Masked,
      LocalApicLint0Policy::BootstrapProcessor));
  EXPECT_FALSE(LocalApicLint0Policy::matchesRole(LocalApicLint0Policy::ExtInt, 0));
}
