...
TEST(RtcAlarmQueue, RemoteRemovalDrainsCommittedDispatch)
{
    ...
    const RtcAlarmQueue::Removal removal =
        queue.removeFirst(event, removalOwner);
    EXPECT_EQ(
        removal.disposition, RtcAlarmQueue::RemovalDisposition::RemoteInFlight);
...
TEST(RtcAlarmQueue, SameContextRemovalDefersWithoutSelfDrain)
{
    ...
    EXPECT_EQ(
        removal.disposition, RtcAlarmQueue::RemovalDisposition::SelfDeferred);
...
TEST(RtcAlarmQueue, CancellationRecyclesRecordsForLaterAlarms)
{
    ...
    const RtcAlarmQueue::Removal removal =
        queue.removeFirst(pointer(0x1001), owner);
