...
TEST(ExecutionContextState, RestoresNestedScopesInLifoOrder) {
  ...
  const ExecutionContext thread = state.enter(ExecutionContext::HostedSyntheticIrq);
  const ExecutionContext signal = state.enter(ExecutionContext::HardDeviceIrq);
...
TEST(ExecutionContextState, ThreadHandoffCannotLeakAnIrqClassification) {
  ...
  const ExecutionContext previous = interrupted.enter(ExecutionContext::HardDeviceIrq);
