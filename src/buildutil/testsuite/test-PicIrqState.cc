...
void expectWrite(const PicWriteRecorder& recorder, size_t index, PicControllerWriteTarget target,
                 uint8_t value) {
...
TEST(PicIrqState, SchedulerRouteExcludesGenericRegistration) {
  ...
  EXPECT_FALSE(state.canRegister(0, IrqPolicy::edgeThreaded(), IrqDelivery::Threaded));
  EXPECT_FALSE(state.canRegister(0, IrqPolicy::levelHard(), IrqDelivery::Hard));
  EXPECT_FALSE(state.canRegister(0, IrqPolicy::levelThreaded(), IrqDelivery::Threaded));
...
TEST(PicIrqState, CascadeInputIsReservedFromHandlers) {
  ...
  EXPECT_FALSE(
      state.canRegister(PicIrqState::CascadeIrq, IrqPolicy::edgeHard(), IrqDelivery::Hard));
  EXPECT_FALSE(
      state.canRegister(PicIrqState::CascadeIrq, IrqPolicy::edgeThreaded(), IrqDelivery::Threaded));
  EXPECT_FALSE(
      state.canRegister(PicIrqState::CascadeIrq, IrqPolicy::levelHard(), IrqDelivery::Hard));
  EXPECT_FALSE(state.canRegister(PicIrqState::CascadeIrq, IrqPolicy::levelThreaded(),
                                 IrqDelivery::Threaded));

  state.setAllEnabled(false);
  EXPECT_EQ(state.masterMask() & (1U << PicIrqState::CascadeIrq), static_cast<uint8_t>(0));
}
...
TEST(PicIrqState, ReplacementLifetimeWaitsForDeliveryProcessorRelease) {
  ...
  const uint16_t deferredTemporary = preserveUnsafePicUnmasks(applied, canonical, 0, Irq4, false);
  EXPECT_NE(deferredTemporary & Irq4, static_cast<uint16_t>(0));
  EXPECT_NE(effectivePicMask(canonical, deferredTemporary) & Irq4, static_cast<uint16_t>(0));

  const uint16_t releaseTemporary = preserveUnsafePicUnmasks(applied, canonical, 0, Irq4, true);
  EXPECT_EQ(releaseTemporary & Irq4, static_cast<uint16_t>(0));
  EXPECT_EQ(effectivePicMask(canonical, releaseTemporary) & Irq4, static_cast<uint16_t>(0));
}
...
TEST(PicIrqState, StableLifetimeCanRestoreWhileRemoteOwnerIsHeld) {
  ...
  EXPECT_EQ(restorablePicTemporaryMask(temporary, Irq4), Irq0);
  EXPECT_EQ(preserveUnsafePicUnmasks(0xFFFF, 0xFFEE, 0, Irq4, false), Irq4);
}
...
TEST(PicHardTailQueue, FinalHardSelfRemovalRetainsThreadedTerminalRecord) {
  ...
  state.setAllEnabled(false);
  state.handlerRegistered(Irq, IrqPolicy::levelThreaded(), IrqDelivery::Threaded);
  ...
  ASSERT_NE(gate.finishLineTransition(Irq), published.controllerLifetime);
...
TEST(PicHardTailQueue, LifetimeMutationDoesNotDemoteHardOutcome) {
  ...
  EXPECT_EQ(plan.effectiveHardDisposition, HardIrqDisposition::KeepMasked);
...
TEST(PicHardTailQueue, MixedPeerMutationRetainsBothStageOutcomes) {
  ...
  EXPECT_TRUE(plan.hardStageLifetimeCurrent);
  EXPECT_TRUE(plan.threadedLifetimeCurrent);
  EXPECT_EQ(plan.effectiveHardDisposition, HardIrqDisposition::Handled);
  EXPECT_EQ(plan.threadedAction, PicHardTailThreadedAction::Publish);
  tails.complete(Irq);
}
...
TEST(PicHardTailFinalizer, StaleMixedTailMasksBeforeDoorbellAndEoiAndThenReleasesSlot) {
  constexpr uint8_t Irq = 5;
  enum Step : size_t {
    TemporaryMask = 1,
    ThreadedDoorbell,
  ...
  EXPECT_TRUE(resolvePicHardTail(record, sameLifetime).controllerLifetimeCurrent);

  const PicHardTailDoorbellResult missing = resolvePicHardTailDoorbell(false, false);
  ...
  for (size_t attempt = 0; attempt < 2; ++attempt) {
    ...
    size_t steps[4] = {};
    size_t stepCount = 0;
    ASSERT_TRUE(tails.consume(Irq, [&](const PicHardTailRecord& consumed) {
      ...
      PicHardTailTerminalSequence terminal(!plan.controllerLifetimeCurrent, consumed.controllerAck);
      terminal.applyTemporaryMask([&]() { steps[stepCount++] = TemporaryMask; });
      steps[stepCount++] = ThreadedDoorbell;
      const PicHardTailDoorbellResult doorbell =
          resolvePicHardTailDoorbell(consumed.threadedPublished, doorbellAccepted);
      if (doorbell.quarantine) {
        steps[stepCount++] = PublicationFailureMask;
      }
      terminal.acknowledge([&]() { steps[stepCount++] = ControllerEoi; });
    }));
...
TEST(PicHardTailQueue, ConcurrentHandoffsPublishCompleteRecords) {
  ...
  std::thread consumer([&]() {
    for (size_t sequence = 1; sequence <= Iterations; ++sequence) {
      ...
      if (record.controllerLifetime != sequence || record.dispatchGeneration != sequence * 3 ||
          record.hardStageGeneration != sequence * 5 || record.threadedCookie != sequence * 7 ||
          record.controllerAck != IrqControllerAck::AfterHardStage ||
          record.hardDisposition != HardIrqDisposition::Handled || !record.hasThreadedStage ||
          !record.threadedPublished || !record.admitted) {
...
TEST(PicControllerStateGate, RemoteTransitionStaysStaleUntilSafeRelease) {
  ...
  gate.beginLineTransition(Irq);
  state.finishLineTransition(Irq);
  uint16_t temporary = preserveUnsafePicUnmasks(static_cast<uint16_t>(1U << Irq), state.mask(), 0,
                                                static_cast<uint16_t>(1U << Irq), false);

  EXPECT_EQ(gate.currentLifetime(Irq), PicControllerStateGate::TransitionLifetime);
  EXPECT_NE(effectivePicMask(state.mask(), temporary) & (1U << Irq), static_cast<uint16_t>(0));
  EXPECT_FALSE(gate.queueEntry(Irq, gate.currentLifetime(Irq)));
  ASSERT_TRUE(gate.takePending(actions));
  EXPECT_EQ(actions.entry[Irq], static_cast<size_t>(0));
  EXPECT_EQ(actions.staleEntry[Irq], static_cast<size_t>(1));
  ...
  EXPECT_EQ(effectivePicMask(state.mask(), temporary) & (1U << Irq), static_cast<uint16_t>(0));
  EXPECT_TRUE(gate.releaseIfIdle());
}
...
TEST(PicControllerStateGate, ConcurrentLifetimeChangesLoseNoPublications) {
  ...
  ASSERT_TRUE(gate.takePending(actions));
  EXPECT_EQ(actions.entry[Irq] + actions.staleEntry[Irq], ProducerCount * PublicationsPerProducer);
  EXPECT_TRUE(gate.releaseIfIdle());
}
...
TEST(PicControllerStateGate, SchedulerDropRestoresBeforeContinuationHandoff) {
  ...
  const PicContentionLineResult first =
      resolvePicContentionLine(state, 0, realEntries[0], actions.tail[0], actions.tailEoi[0]);
  ASSERT_TRUE(first.schedulerDrop);
  uint16_t temporary = temporaryPicMaskForDeferredWork(0, realEntries, actions, spuriousEntries);
...
TEST(PicContentionActions, DeviceEntryMasksBeforeMasterSpecificEoi) {
  ...
  emitPicContentionWrites(state, result.maskChanged, realEntries, pending, 0, recorder);
  ...
  expectWrite(recorder, 0, PicControllerWriteTarget::MasterMask, state.masterMask());
  expectWrite(recorder, 1, PicControllerWriteTarget::SlaveMask, state.slaveMask());
  expectWrite(recorder, 2, PicControllerWriteTarget::MasterCommand, 0x64);
}
...
TEST(PicContentionActions, CurrentRtcThreadedEntrySurvivesControllerOwner) {
  ...
  PicWriteRecorder recorder;
  emitPicContentionWrites(state, false, remainingEntries, pending, 0, recorder);
...
TEST(PicContentionActions, DeferredWorkUsesOnlyPhysicalOverride) {
  ...
  const uint16_t temporary =
      temporaryPicMaskForDeferredWork(existing, realEntries, stale, spuriousEntries);
...
TEST(PicContentionActions, SpuriousSlaveMasksBeforeCascadeEoiAndRestore) {
  ...
  const uint16_t temporary =
      temporaryPicMaskForDeferredWork(0, realEntries, pending, spuriousEntries);
  ...
  expectWrite(recorder, 0, PicControllerWriteTarget::MasterMask,
              static_cast<uint8_t>(effectivePicMask(canonical, temporary) & 0xFF));
  expectWrite(recorder, 1, PicControllerWriteTarget::SlaveMask,
              static_cast<uint8_t>(effectivePicMask(canonical, temporary) >> 8));
  expectWrite(recorder, 2, PicControllerWriteTarget::MasterCommand, 0x62);
  expectWrite(recorder, 3, PicControllerWriteTarget::MasterMask,
              static_cast<uint8_t>(canonical & 0xFF));
  expectWrite(recorder, 4, PicControllerWriteTarget::SlaveMask,
...
TEST(PicContentionActions, StaleWorkDoesNotMutateReplacementState) {
  ...
  stale.tailEoi[Irq] = 1;
  const uint16_t temporary =
      temporaryPicMaskForDeferredWork(0, realEntries, stale, spuriousEntries);
  PicWriteRecorder recorder;
  emitPicMaskWrites(effectivePicMask(canonical, temporary), recorder);
...
TEST(PicContentionActions, SlaveEntryMasksBeforeSlaveAndCascadeEois) {
  ...
  expectWrite(recorder, 0, PicControllerWriteTarget::MasterMask, state.masterMask());
  expectWrite(recorder, 1, PicControllerWriteTarget::SlaveMask, state.slaveMask());
  expectWrite(recorder, 2, PicControllerWriteTarget::SlaveCommand, 0x64);
...
TEST(PicContentionActions, SchedulerEntryDropsWithoutStickyMask) {
  ...
  const PicContentionLineResult result =
      resolvePicContentionLine(state, 0, realEntries[0], pending.tail[0], pending.tailEoi[0]);
  ...
  expectWrite(recorder, 0, PicControllerWriteTarget::MasterCommand, 0x60);
}
...
TEST(PicContentionActions, SchedulerTailStillFailsClosed) {
  ...
  expectWrite(recorder, 0, PicControllerWriteTarget::MasterMask, state.masterMask());
  expectWrite(recorder, 1, PicControllerWriteTarget::SlaveMask, state.slaveMask());
  expectWrite(recorder, 2, PicControllerWriteTarget::MasterCommand, 0x60);
}
...
TEST(PicContentionActions, TailEoiOnlyBatchMasksBeforeEoi) {
  ...
  expectWrite(recorder, 0, PicControllerWriteTarget::MasterMask, state.masterMask());
  expectWrite(recorder, 1, PicControllerWriteTarget::SlaveMask, state.slaveMask());
  expectWrite(recorder, 2, PicControllerWriteTarget::MasterCommand, 0x63);
}
...
TEST(PicContentionActions, SpuriousIrq15EoisCascadeOnly) {
  ...
  ASSERT_FALSE(recorder.overflow);
  ASSERT_EQ(recorder.count, static_cast<size_t>(1));
  expectWrite(recorder, 0, PicControllerWriteTarget::MasterCommand, 0x62);
}
...
TEST(PicContentionActions, RealSlaveEoiPrecedesSpuriousCascadeEoi) {
  ...
  PicWriteRecorder recorder;
  emitPicContentionWrites(state, result.maskChanged, realEntries, pending, 1, recorder);
