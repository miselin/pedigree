...
TEST(IntervalTimerState, LeavesDisarmedAndZeroElapsedTimersAlone) {
  const Consumption disarmed = PosixIntervalTimerState::consume(10, 20, false, 50);
  EXPECT_EQ(disarmed.value, 10U);
  EXPECT_FALSE(disarmed.armed);
  EXPECT_FALSE(disarmed.expired);

  const Consumption zero = PosixIntervalTimerState::consume(10, 20, true, 0);
...
TEST(IntervalTimerState, CountsDownWithoutExpiring) {
  const Consumption result = PosixIntervalTimerState::consume(11, 7, true, 6);
...
TEST(IntervalTimerState, ExpiresOneShotOnExactBoundary) {
  const Consumption result = PosixIntervalTimerState::consume(11, 0, true, 11);
...
TEST(IntervalTimerState, ReloadsPeriodicTimerOnExactBoundary) {
  const Consumption result = PosixIntervalTimerState::consume(11, 7, true, 11);
...
TEST(IntervalTimerState, RetainsPhaseAcrossCoalescedPeriods) {
  const Consumption result = PosixIntervalTimerState::consume(5, 7, true, 30);
...
TEST(IntervalTimerState, StaleAbsoluteSnapshotCannotDebitRearmedTimer) {
  const AbsoluteConsumption result =
      PosixIntervalTimerState::consumeAbsolute(20, 0, true, 200, 150);
...
TEST(IntervalTimerState, AbsoluteSnapshotRetainsPeriodicPhase) {
  const AbsoluteConsumption result = PosixIntervalTimerState::consumeAbsolute(5, 7, true, 100, 130);
