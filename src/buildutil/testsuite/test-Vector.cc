...
struct VectorConstructionTracker {
  ...
  VectorConstructionTracker(const VectorConstructionTracker& other) : value(other.value) {
    ++live;
  }
...
TEST(PedigreeVector, ConstIndexingReturnsConstReference) {
  ...
  static_assert(pedigree_std::is_same<decltype(readOnly[0]), const int&>::value,
