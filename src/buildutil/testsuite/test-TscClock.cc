...
TEST(TscClockConversion, GuardsUnderflowAndSaturatesOverflow)
{
    ...
    EXPECT_EQ(
        PcTscClock::fromAnchor(
            110, 100, PcTscClock::MaximumTimestamp - 4, identity),
...
TEST(TscClockConversion, PerCpuAnchorsRemoveAbsoluteTscOffsets)
{
    ...
    const uint64_t first =
        PcTscClock::fromAnchor(1000240, 1000000, 5000, calibration);
    const uint64_t second =
        PcTscClock::fromAnchor(1240, 1000, 5000, calibration);
