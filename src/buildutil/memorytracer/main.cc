...
typedef std::unordered_map<uint64_t, std::shared_ptr<AllocationTraceEntry>> dataset_t;
typedef std::pair<uint64_t, std::shared_ptr<AllocationTraceEntry>> dataset_pair_t;
...
bool processRecord(AllocationTraceEntry& record, dataset_t& dataset,
  ...
  if (record.data.type == Free) {
    ...
    if (it != dataset.end()) {
      /// \todo add more info
      std::cerr << "A pointer was allocated twice [" << std::hex << extendPointer(record.data.ptr)
                << "]." << std::endl;
      performBacktrace(record);
      return false;
    }
...
int processRecords(FILE* fp, int max_records, bool reverse, bool common_callsites, bool seen) {
  ...
  std::cout << "This data file contains " << totalAllocs << " allocations in total ("
            << totalRecords << " total records)." << std::endl;
  ...
  if (common_callsites) {
    ...
    std::vector<std::pair<uint32_t, std::shared_ptr<CallerCountEntry>>> entries_vec(entries.begin(),
                                                                                    entries.end());
    if (reverse) {
    ...
    for (auto it = entries_vec.begin(); it != entries_vec.end() && i < max_records; ++it, ++i) {
      std::cout << " With " << it->second->count << " entries, a total of " << it->second->size
                << " bytes:" << std::endl;
    ...
    if (reverse) {
      std::sort(vec.begin(), vec.end(),
                [](const dataset_pair_t& left, const dataset_pair_t& right) -> bool {
    ...
    for (auto it = vec.begin(); it != vec.end() && i < max_records; ++it, ++i) {
      std::cout << it->second->data.sz << " bytes left unfreed @" << std::hex
                << extendPointer(it->first) << std::dec << "." << std::endl;
...
void usage() {
  ...
  std::cerr << "  --max-rows, -m   Maximum rows to output (default is 10, "
               "pass no value to show all rows)."
            << std::endl;
  std::cerr << "  --callers, -c    Show most common callers with unfreed "
               "allocations, rather than the unfreed allocations themselves."
            << std::endl;
      ...
      case 'm': {
        if (!optarg) {
          ...
          if (end != optarg) {
            ...
            std::cerr << "Could not convert maximum row '" << optarg << "' to a number."
                      << std::endl;
            return 1;
          }
        }
