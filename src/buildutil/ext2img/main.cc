...
bool writeFile(const std::string& source, const std::string& dest) {
  ...
  if (stat(source.c_str(), &st) < 0) {
    std::cerr << "Could not open source file '" << source << "': " << strerror(errno) << "."
              << std::endl;
    return false;
  }
  ...
  while (!ifs.eof()) {
    ...
    if (readCount) {
      ...
      if (memcmp(bufferA, bufferB, count) != 0) {
        std::cerr << "Files do not match at block starting at offset " << offset << "."
                  << std::endl;
...
bool changeOwner(const std::string& filename, const std::string& uid, const std::string& gid) {
  ...
  try {
    ...
    std::cerr << "Bad uid/gid value '" << uid << "' or '" << gid << "' passed: " << e.what()
              << std::endl;
    return false;
  }
...
bool setDefaultPermissions(const std::string& file_perms, const std::string& dir_perms,
  ...
  for (auto it = cmdlist.begin(); it != cmdlist.end() && rc == 0; ++it, ++nth) {
    switch (it->what) {
      ...
      case SetDefaultPermissions:
        if ((!setDefaultPermissions(it->params[0], it->params[1], it->params[2])) &&
            !ignoreErrors) {
          rc = 1;
        }
        break;
    ...
    if ((!rc) && (nth % 10) == 0) {
      ...
      std::cout << "Progress: " << std::setprecision(4) << (progress * 100.0) << "%      \r"
                << std::flush;
    }
  }
...
bool parseCommandFile(const char* cmdFile, std::vector<Command>& output) {
  ...
  while (std::getline(f, line)) {
    ...
    if (cmd == "write") {
      ...
      std::cerr << "Unknown command '" << cmd << "' at line " << lineno << ": '" << line << "'"
                << std::endl;
      ok = false;
    }

    if (c.params.size() < requiredParamCount) {
      std::cerr << "Not enough parameters for '" << cmd << "' at line " << lineno << ": '" << line
                << "'" << std::endl;
      ok = false;
    }
