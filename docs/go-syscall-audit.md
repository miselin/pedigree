...
| `futex` (202) | Atomic compare-and-sleep, wake counts, and optional relative timeouts | `WAIT`, `WAKE` and `REQUEUE` serialize comparison and queue publication. Private keys use address space plus address; shared file mappings use stable backing identity. `WAIT_BITSET` with `FUTEX_BITSET_MATCH_ANY` accepts an absolute monotonic deadline, or an absolute realtime deadline with `FUTEX_CLOCK_REALTIME`; a null timeout waits indefinitely. Clock steps trigger realtime deadline reevaluation. Partial bitsets, `WAKE_BITSET`, PI operations and realtime flags on other operations remain unsupported | Partial | Go normally uses relative `WAIT`/`WAKE`. Eight absolute-wait families and legacy futex/robust contracts passed in one/four-CPU guests; see [bounded clock and futex support](musl-syscall-implementation.md#raw-monotonic-reads-and-absolute-futex-waits). |
...
| `clock_gettime` (228) | A monotonic clock for `nanotime` and realtime for wall time | The syscall and vDSO distinguish realtime from monotonic time. `CLOCK_MONOTONIC_RAW` reads the same currently unadjusted nanosecond ticks as `CLOCK_MONOTONIC`; `clock_getres` also accepts RAW. Realtime steps do not change these ticks. RAW remains invalid for sleep, clock setting and timers | Ready | Go uses realtime and monotonic clocks. RAW libc, explicit vDSO and direct-syscall checks passed in one/four-CPU guests; the alias does not implement clock discipline or establish suspend behavior. |
| `nanosleep` (35) | Sleep until the interval elapses or return an accurate remainder | Pedigree validates the timespec, waits against a monotonic deadline, and returns `EINTR` with the remaining interval when interrupted | Ready | Musl guest checks cover interruption with `SA_RESTART`; nanosleep deliberately remains non-restarting. |
...
| `rt_sigaction` (13) | Linux amd64's 32-byte kernel structure and Linux delivery frame | The local adapter stores the handler, flags, restorer, and full 64-bit mask for signals 1 through 31 and bundled musl's private signals 32 through 34. Those private signals have real dispositions and thread-directed `tkill`/`tgkill` delivery; unsupported signals 35 through 64 return `EINVAL` | Partial | Synchronous and asynchronous custom dispositions line up for the supported range. Linux amd64 now resets `SA_RESETHAND` atomically and restarts eligible interrupted calls with `SA_RESTART`. Realtime signals 35 through 64 remain unsupported; see `posix-supported-syscall-spike.md` for the executed contract scope. |
...
| `epoll_create1` (291), `epoll_ctl` (233), `epoll_pwait` (281), `eventfd2` (290) | Go's Linux network poller and its wakeup descriptor | The epoll calls are translated with level-triggered and `EPOLLONESHOT` readiness over pipes and sockets, plus `poll`/`select` interoperability for epoll descriptors. Device files must explicitly opt into readiness notifications; regular files and directories return `EPERM`, matching Linux. Edge-triggered and nested epoll are rejected, and `eventfd2` remains absent. The local Go target still selects its Pedigree-specific backend | Partial | Level-triggered epoll consumers can begin runtime validation, but Go's Linux epoll backend cannot replace the local poll backend until edge-triggering and a wakeup descriptor are available. |
