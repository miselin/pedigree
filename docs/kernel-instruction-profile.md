...
objects still iterate the list. The temporary reverse-lookup ablation has been
removed; the indexed production lookup remains.

## Historical results

These measurements describe the frozen revisions below, not the final integrated
source. See [current syscall evidence](syscall-framework-performance.md) and
[kernel build profiles](kernel-build-profiles.md) for newer accounting and `-O3`
results. Full experimental history is preserved on
...
`PEDIGREE_TRACK_LOCKS`. Runtime ablations were zero and disk writes were
