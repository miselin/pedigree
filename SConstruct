...
autogen_opts.AddVariables(
    ...
    ('QEMU_IMG', 'Cached path to "qemu-img"'),
    ('LOSETUP', 'Cached path to "losetup"'),
    ('MKE2FS', 'Cached path to "mke2fs"'),
    ('DEBUGFS', 'Cached path to "debugfs"'),
    ('CCACHE', 'Cached path to "ccache"'),
    ('DISTCC', 'Cached path to "distcc"'),
    ('PYFLAKES', 'Cached path to "pyflakes"'),
    ('MTOOLS_MMD', 'Cached path to "mmd"'),
    ('MTOOLS_MCOPY', 'Cached path to "mcopy"'),
    ('MTOOLS_MDEL', 'Cached path to "mdel"'),
    ('MKISOFS', 'Cached path to "mkisofs" (or a compatible tool)'),
    ('SQLITE', 'Cached path to "sqlite3"'),
    ('MKIMAGE', 'Cached path to "mkimage"'),
    ('GIT', 'Cached path to "git"'),
)
...
version_cc = env['PEDIGREE_BUILD_BASE'].File('Version.cc')
# Avoid building Version.cc if nothing has changed.
if env['genversion'] or (not version_cc.exists()):
    env.Command(version_cc, None, Action(create_version_cc, None))
